...
*Section id 1 · status `draft` · v1 · 365 words · author `claude_ai`*
...
## Section types not yet populated

_Stubs — `prose_section_type` rows with `chapter_no=NULL` or no `prose_section` content. `id` is the value to use as `section_type_id` in a PROSE patch insert._

| id | code | label | chapter | sort | description |
|---:|---|---|---:|---:|---|
| 27 | `prog_anchor_verse` | Programme — Anchor Verse Definition | — | 101 | Canonical definition of anchor verses, dual purpose (analytical anchor + verse-context seed), minimum requirements. Authoritative home for the concept (currently mirrored in wa-reference §16 and wa-dimensionreview-instruction §4.2). |
| 28 | `prog_xref_architecture` | Programme — XREF Architecture | — | 102 | OWNER / XREF term semantics, verse_context inheritance, canonical-row rule, cross-registry link pattern (currently scattered across wa-reference §17, wa-registry-management-guide). |
| 29 | `prog_validation_standard` | Programme — Document Validation Standard | — | 103 | Inflection point completeness; gap status discipline; cross-doc consistency; dry-run gate assessment (currently wa-reference §18). |
| 30 | `prog_delete_discipline` | Programme — Soft-Delete Discipline | — | 104 | Delete_flagged semantics; cascade rules; no physical deletes in automated flows; audit-trail retention. |
| 31 | `prog_field_authority` | Programme — Field Authority Rules | — | 105 | mti_term_flags canonical for somatic / god_as_subject; deprecation notes; which field wins on conflict. |
| 32 | `prog_backup_discipline` | Programme — Backup Discipline | — | 106 | Per-migration backup; per-patch backup; retention period; restoration procedure. |
| 33 | `prog_patch_failure_protocol` | Programme — Patch Failure / REPAIR Protocol | — | 107 | What to do when a patch is rejected; failure patches; mid-pool recovery. |

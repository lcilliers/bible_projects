"""Build VCB-030 batch extract — Registry 15 (boastfulness) repair batch.

Pipeline gap fix: mti_terms.status was 'delete' for all OWNER terms,
making them invisible to VC batch builder. Now corrected to extracted/extracted_thin.
"""
import sqlite3
import os
import json
...
def build_term_json(mti_id):
    ...
    existing_groups = [{
        ...
        "context_description": g["context_description"], "notes": g["notes"],
        "delete_flagged": g["delete_flagged"],
        "anchor_count": g["anchor_count"], "related_count": g["related_count"],
    } for g in groups]
    ...
    for vr in verses_rows:
        ...
        if vc:
            vc_obj = {
                "id": vc["id"], "group_id": vc["group_id"], "group_code": vc["group_code"],
            ...
            if vc["delete_flagged"] == 0:
                prev_classified += 1
                if vc["is_relevant"] == 0: set_aside += 1
                if vc["is_anchor"] == 1: anchors_existing += 1

        verses_json.append({
            "verse_record_id": vr["verse_record_id"], "reference": vr["reference"],
    ...
    return {
        "term": {
            "mti_term_id": mti_id, "strongs_number": mt["strongs_number"],
            "transliteration": mt["transliteration"], "gloss": mt["gloss"],
            "language": mt["language"], "mti_status": mt["status"],
        ...
        "stats": {
            "total": term_total, "unclassified": term_unclassified,
            "previously_classified": prev_classified, "set_aside": set_aside,
...
for mti_id in all_mti_ids:
    ...
    for k in totals:
        totals[k] += result["stats"].get({"verses": "total", "classified": "previously_classified"}.get(k, k), 0)

terms_json.sort(key=lambda t: (t["owning_registry_id"], t["strongs_number"]))

batch_json = {
    "batch_id": BATCH_ID, "produced_date": TODAY,
...
outpath = os.path.join(os.path.dirname(__file__), "..", "data", "exports", "verse_context",
                       f"wa-vcb-030-extract-{TODAY}.json")

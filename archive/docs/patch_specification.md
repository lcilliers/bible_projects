...
The `word_registry.session_b_status` column tracks each word's progress through the analysis pipeline. Status transitions are enforced by the tools that set them.

```
NULL
  │
  ▼  (audit_word completes successfully → A10)
"Ready for Analysis"
  │
...
| `Ready for Analysis` | `audit_word.py` (A10) | Successful audit completion (PASS or REVIEW) |
...
**Rules:**
- `audit_word` uses `COALESCE` — it sets `Ready for Analysis` only if the current status is NULL. It never downgrades a more advanced status.
- `apply_session_patch.py` always overwrites the status with the value from `_patch_meta.session_b_status`.
- Every patch MUST include `session_b_status` in `_patch_meta`. Patches without it are rejected.
...
|-------|--------------------------|-------------|
| `PATCH-YYYYMMDD-NNN-PREANALYSIS-V1` | `Pre-Analysis Complete` | Term classifications (extracted/delete/xref), bleed removal, anchor verses |
...
| Value | Meaning |
|-------|---------|
| `NULL` | Not yet extracted/audited |
| `Ready for Analysis` | Extraction + audit complete, awaiting pre-analysis |
...
| Patch type | session_b_status value |
|-----------|----------------------|
| PREANALYSIS | `Pre-Analysis Complete` |
| SESSIONB | `Analysis Complete` or `Session B Complete` |
| REPAIR | Current status (do not change) |

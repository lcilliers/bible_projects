# WA-DimensionReview-Instruction-v1_6-2026-04-08

**Framework B — Soul Word Analysis Programme**
**Dimension Review Instruction — Claude AI and Claude Code**
**Version 1.6 | April 2026 | Status: Active governing instruction**

| **Document** | **Value** |
|---|---|
| Filename | WA-DimensionReview-Instruction-v1.6-2026-04-08.md |
| Supersedes | WA-DimensionReview-Instruction-v1.5-2026-04-08.md |
| Change note | v1.6 — Dimension vocabulary extended by one. Section 5.7 updated: dimension 11 (Divine-Human Correspondence) added. This dimension names inner-being characteristics that operate across the God-human boundary — where the same quality, act, or state appears in both divine and human subjects within the same verse evidence, or where God's inner-being characteristic is the direct ground, mirror, or counterpart of the human characteristic. Confirmed by researcher decision during C18 Phase B flag review (2026-04-08). The vocabulary remains data-derived; this dimension was prompted by verse evidence in C18 groups spanning both divine and human subjects. Section 1.3 note updated to reflect 11 dimensions. No other sections changed. |
...
```json
{
  ...
  "record": {
    ...
    "session_b_instruction": "WA-DimensionReview-Instruction-v1.6-2026-04-08"
  },
...
```json
{
  ...
  "record": {
    ...
    "session_raised": "WA-DimensionReview-Instruction-v1.6-2026-04-08",
    "raised_date": "{ISO date}",
    "resolved": 0
  },
...
*Derived from data. Working vocabulary as of v1.6. Will be revised as additional data is reviewed.*
...
Assign when the group's verse evidence shows the same inner-being characteristic present in both GOD and HUMAN subjects — not merely God acting on a human (which is Agency/Power or Transformation), but the same quality, act, or state appearing across the divine-human boundary within the same group. This includes cases where God's inner-being characteristic is the direct ground, mirror, or counterpart of the human characteristic, and where holding both poles together is necessary to understand either.

Examples from the data: rov (abundance) applied to both God's steadfast love and human iniquity within the same verse set; covenantal rejection that is simultaneously God's sovereign act and the human community's experienced loss; portion and belonging operating as both divine grant and human inner orientation.

Note: This dimension does not replace the other ten. It is assigned where the God-human correspondence is itself the analytically significant feature — where separating the divine and human subjects would lose the essential inner-being meaning the group carries. The full significance of these correspondences is Session B and Session D work; the dimension assignment here names and preserves them for that analysis.

**dominant_subject for this dimension:** Assign `NONE` where the subject is genuinely dual across the verse evidence. Assign `GOD` or `HUMAN` where one pole dominates the anchor verse but the correspondence with the other pole is present in related verses and analytically significant. Record the God-human tension explicitly in the notes field.
...
```json
{
  "_patch_meta": {
    ...
    "produced_by": "WA-DimensionReview-Instruction-v1.6-2026-04-08",
...
|---|---|---|---|
| `DIMREVIEW` | WA-DimensionReview-Instruction-v1.6 | `null` | `wa_dimension_index` updates (dimension + dominant_subject); `wa_session_b_findings` inserts; `wa_session_research_flags` inserts; `verse_context_group` description corrections (when arising within a live session) |
| `DIMREVIEW-GRPDESC` | WA-DimensionReview-Instruction-v1.6 | `null` | `verse_context_group` description updates; `wa_dimension_index` context_description sync — **out-of-session use only** |
  ...
  "groups": [
    {
      ...
      "group_code": "...",
      "...(all dimension index fields — see field mapping below)...": "...",
      "anchor_verses": [
        {
          "verse_record_id": 0,
          "book": "...",
...
**⚠ Verse arrays are mandatory.** Every group in the extract must carry its `anchor_verses` array populated with at least one entry. A group with an empty `anchor_verses` array is a data integrity problem — Claude Code must report it explicitly rather than omitting the array silently. The `related_verses` array must be present but may be empty (`[]`) if no related verses exist for a group. Omitting either array is not permitted.

**Field mapping — `wa_dimension_index` fields and verse arrays:**
...
| `group.anchor_verses[].verse_record_id` | `verse_context` | `verse_record_id` where `is_anchor = 1` and `group_id = group.verse_context_group_id` and `delete_flagged = 0` |
| `group.anchor_verses[].book` | `wa_verse_records` joined via `verse_record_id` | book name (via `books` table) |
| `group.anchor_verses[].chapter` | `wa_verse_records` | `chapter` |
| `group.anchor_verses[].verse` | `wa_verse_records` | `verse` |
| `group.anchor_verses[].verse_text` | `wa_verse_records` | `verse_text` |
| `group.related_verses[].verse_record_id` | `verse_context` | `verse_record_id` where `is_related = 1` and `is_anchor = 0` and `group_id = group.verse_context_group_id` and `delete_flagged = 0` |
| `group.related_verses[].book` | `wa_verse_records` joined via `verse_record_id` | book name (via `books` table) |
| `group.related_verses[].chapter` | `wa_verse_records` | `chapter` |
| `group.related_verses[].verse` | `wa_verse_records` | `verse` |
| `group.related_verses[].verse_text` | `wa_verse_records` | `verse_text` |
...
*WA-DimensionReview-Instruction-v1.6-2026-04-08 | Supersedes v1.5-2026-04-08 | v1.6: Dimension vocabulary extended — dimension 11 (Divine-Human Correspondence) added to Section 5.7; Section 1.3 updated to reflect 11 dimensions; vocabulary remains data-derived | v1.5: Anchor verses and related verses added as mandatory fields in cluster extract (Section 9.1)*

...
**Version:** v1_6
**Date:** 2026-04-20
**Status:** Active — supersedes wa-global-flags-v1_5-20260418.md
...
**Change from v1_5:** FLAG-008 moved Open → **Obsolete** and FLAG-010 moved Open → **Obsolete** per researcher decision 2026-04-20. Rationale: the flag system was ineffective as a mechanism to force the updating of instruction documents; treating those updates as a flag-gated activity created friction without driving resolution. The instruction-update work itself does not disappear — it is removed from the flag system and will be handled through a different mechanism to be defined outside this file. The blocking gate on new word analysis that FLAG-010 imposed is explicitly lifted; no Open flag now gates programme operations.

**Count reconciliation note (v1_5 → v1_6):** The v1_5 summary stated Open = 7 and total = 14, but the v1_5 register body carried 8 Open entries (FLAG-001, FLAG-006, FLAG-007, FLAG-008, FLAG-010, FLAG-011, FLAG-013, FLAG-014). The v1_4 → v1_5 change note reasoned "7 Open (was 8)" on the basis of one Open → Resolved move (FLAG-015), which — starting from a v1_4 Open count of 8 — leaves 8 Open in v1_5, not 7. The summary figure in v1_5 was therefore off by one against its own register. v1_6 resolves this by starting from the register-accurate v1_5 figure of 8 Open, then moving FLAG-008 and FLAG-010 to Obsolete to yield 6 Open. Totals also reconciled: v1_5 stated 14 rows against a register of 15 (8 Open + 6 Resolved + 1 Obsolete + 0 Standing) — v1_6 totals 15.

**Change from v1_4:** FLAG-015 moved Open → **Resolved** (registry management guide confirmed present in Project Files; equivalent FLAG-012 sweep applied; produced `wa-registry-management-guide-v5_10-20260418.md` with operational references migrated to `[current]` per GR-REF-002, dangling `GR-DATA-008` citation removed, Document Scope section added per GR-REF-001 Discipline 5, GR-DATA-001 and GR-FILE-001 through GR-FILE-009 citations added). FLAG-012 description extended to record the 11th document now included in the sweep (retroactive close-out). FLAG-014 description updated — the registry management guide has been checked and cleared; the "DataPrep" persistence in that document is localised to historical change notes and one retirement marker (acceptable); `WA-PipelineStatusReview-v2-20260330` reference is not present in the guide. FLAG-014 remains Open for the other documents referenced in the original flag.

**Change from v1_3:** FLAG-012 moved Open → **Resolved** (cross-instruction reference cleanup sweep executed in session 2026-04-18 per researcher direction, ahead of FLAG-013 in the original sequencing). FLAG-014 added Open (legacy references found during sweep — `WA-PipelineStatusReview-v2-20260330` in versecontext body L1331; "DataPrep" terminology persistence in versecontext body prose). FLAG-015 added Open (registry management guide not present in Project Files despite researcher-reported upload). FLAG-013 remains Open — the classification review of the document_discipline category has not been performed in this session; its sequencing relationship to FLAG-012 is now historical since FLAG-012 was executed first.

**Change from v1_2:** v1_2 register had 6 Open, 4 Resolved, 1 Obsolete (total 11) — the v1_2 header summary said 7 Open / 3 Resolved which was already inconsistent with the register. Corrected in v1_3: register-derived counts used. In v1_3: FLAG-012 added Open (cross-instruction reference cleanup sweep under GR-REF-002); FLAG-013 added Open (document_discipline category classification review).

**Change from v1_1:** v1_1 had 6 Open, 3 Resolved, 1 Obsolete (total 10). In v1_2: FLAG-011 added Open (retirement of wa-sessionb-cc-instructions-v3_6).
...
### FLAG-008 — Researcher decision discipline — programme-wide rules added — OBSOLETE

- **Status:** Obsolete
- **Raised:** 2026-04-15
- **Marked obsolete:** 2026-04-20
- **Description:** GR-RD-001 through GR-RD-006 added in v2.2 governing the format, gate conditions, presentation, and resolution requirements for researcher decision items. GR-DB-001 added prohibiting DB state assumptions. GR-DIR-001 expanded; GR-DIR-002 through GR-DIR-005 added governing directive and patch distinction, format, and confirmation requirements. All instruction documents that reference the former GR-DIR-001 single-sentence rule should be updated to reflect the expanded directive/patch governance and researcher decision rules. Originally tracked under FLAG-010 audit scope.
- **Reason marked obsolete:** Researcher decision 2026-04-20. The flag system was ineffective as a mechanism to force the updating of instruction documents. Treating instruction updates as flag-gated activity created friction without driving resolution. The underlying work — that instruction documents citing superseded rule IDs should be updated — does not disappear; it is removed from the flag system and will be handled through a different mechanism defined outside this file. Set aside without outcome; preserved for audit. Not counted in programme state.
...
### FLAG-010 — Post-GR-v2_8 instruction audit — OBSOLETE
...
- **Instructions originally in scope:** Verse Context instruction, Dimension Review instruction, Analysis_readiness instruction, Analysis_output instruction, plus several utility instructions.
- **Reason marked obsolete:** Researcher decision 2026-04-20. The flag system was ineffective as a mechanism to force the updating of instruction documents. Using a blocking-gate flag to force the audit produced friction without driving resolution — the flag got in the way of the work rather than enabling it. **The blocking gate on new word analysis that this flag imposed is lifted.** Programme operations are no longer gated by instruction-audit completion. The audit work itself does not disappear — it is removed from the flag system and will be handled through a different mechanism defined outside this file. Set aside without outcome; preserved for audit. Not counted in programme state.
...
- **Relationship to FLAG-010 (now Obsolete):** FLAG-010 was a programme-wide audit of instructions against GR v2_8 before being marked Obsolete on 2026-04-20. FLAG-011 is a specific retirement-and-replacement which was originally scoped inside that audit. With FLAG-010 obsolete, FLAG-011 stands on its own. The instruction-update mechanism that replaces FLAG-010 will determine whether FLAG-011 remains in the flag system or moves to that mechanism.
...
- **Gate:** Not a gate. No pipeline operations blocked.
...
**v1_6 (2026-04-20):** FLAG-008 moved Open → **Obsolete** and FLAG-010 moved Open → **Obsolete** per researcher decision 2026-04-20. Rationale: the flag system was ineffective as a mechanism to force the updating of instruction documents; flags got in the way of resolving the outstanding work. Instruction-update work is removed from the flag system and will be handled through a different mechanism defined outside this file. The blocking gate on new word analysis that FLAG-010 imposed is explicitly lifted. v1_6 also reconciles prior count-summary inconsistencies: v1_5 stated Open = 7 against a register of 8 Open, and total 14 against a register of 15; v1_6 counts directly from the register. v1_6 totals: 6 Open + 6 Resolved + 3 Obsolete + 0 Standing = 15. Session record: `wa-global-flags-validation-obslog-v1-20260420.md` (O-008 decision; O-009 production of this file).

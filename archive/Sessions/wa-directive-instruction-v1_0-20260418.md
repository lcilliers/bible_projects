# wa-directive-instruction-v1_0-20260418

> Framework B Soul Word Analysis Programme — Directive Preparation and Execution
> Version: v1_0 | Date: 20260418
> New document — no supersession
...
- Patch format and execution → wa-patch-instruction v2_0
- Controlled vocabulary → wa-reference v5_6
- CC operational routines outside directive handling → wa-claudecode-instruction v4_0
...
Per GR-PROG-005 in global rules v2_11, Claude AI determines what should be done and why; Claude Code executes. There are two and only two mechanisms for instructing Claude Code to make database changes — patches (wa-patch-instruction v2_0) and directives (this document). Conversational instructions to CC without either format are not valid database change requests.
...
Use a patch (wa-patch-instruction v2_0 §1.1) when Claude AI is certain of the field names, FK keys, table structure, and exact operations. Patches are machine-readable, idempotent, and auditable.
...
> *No physical deletes per wa-patch-instruction v2_0 §5.4 (deletion discipline).*
...
Every directive specifies the completion confirmation that Claude Code must return. This parallels wa-patch-instruction v2_0 §6 for patches; the principle is the same.
...
3. Executes the operations — applying the no-physical-deletion rule per wa-patch-instruction v2_0 §5.4
...
- Claude AI diagnoses; if the directive caused DB state corruption, produce a REPAIR patch per wa-patch-instruction v2_0 §9 before proceeding
...
- A **REPAIR patch** (wa-patch-instruction v2_0 §9.3) to reset status and delete_flag the affected records — structured, all fields known in advance
...
*wa-directive-instruction-v1_0-20260418 | New document | Consolidates global rules addenda ADD-PATCHDIR-002, -003, -005, -006, -008, -009 | Resolves addendum ADD-PATCHDIR-002 (directive peer specification now exists)*

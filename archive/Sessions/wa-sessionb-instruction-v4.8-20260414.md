...
v4.8 (20260414): Global rules integration. (1) Governing Rules header added — mandatory load of wa-global-general-rules-v2-20260414.json before session start. (2) Governing Disciplines section revised: write-on-discovery paragraph replaced with reference to GR-OBS-001; step-by-step replaced with GR-PROC-001; data-is-authoritative replaced with GR-PROC-002; all-changes-through-patches replaced with GR-PROC-003; session-logs-at-breakpoints replaced with GR-PROC-006. (3) Integrity rules SB-3 (no patch without researcher review → GR-PROC-004), SB-5 (session log must record stage/step/next → GR-PROC-006), SB-6 (observations log governs → GR-PROC-005) removed from inline table and replaced with references to global rules. (4) Pass count corrected throughout: "five-pass" and "five passes" updated to "six-pass" and "six passes" (Pass 6 was added in v4.5; descriptions were not consistently updated). (5) Analytical Principles section: "The data is authoritative" replaced with reference to GR-PROC-002; "Emergence, not imposition" replaced with reference to GR-PROG-008; "Inferential is not confirmed" replaced with reference to GR-PROG-009. (6) god_as_subject note updated: WA-Reference Section 13.3 citation replaced with GR-DATA-005. (7) somatic_link note updated: WA-Reference Section 13.3 citation replaced with GR-DATA-005. (8) Naming conventions table: all example dates updated to compact format per GR-FILE-009; "All filenames lowercase" note replaced with reference to GR-FILE-007. (9) What to Attach section: file references updated to compact date format. (10) Pipeline diagram: "five passes" corrected to "six passes".

v4.7 (2026-04-12): Three additions addressing the gap identified in the C17 SD pointer audit (2026-04-12): Reg 23 (compassion) had 27 SD pointers in the observations log and zero in the database at session close. Root cause: v4.6 contained no step requiring database persistence of SD pointers, and no validation gate that would prevent Stage 3b from completing without that persistence. (1) **Section 2.2 — SD Pointer Persistence (mandatory):** A new mandatory step inserted between the Analytical Brief and Stage 3. All SD pointers raised during Stage 2 must be compiled, structured as `SD_POINTER` flag records in `wa_session_research_flags`, and submitted to Claude Code as a dedicated SD pointer patch (`PATCH-{date}-{nnn}-SDPOINTERS-V1.json`). Stage 3 does not begin until Claude Code confirms the patch is applied. (2) **Section 3.0 — Pre-Stage 3b Validation Gate (hard gate):** A new mandatory check inserted immediately before Stage 3b begins. Claude AI must state the count of SD_POINTER records in the current extract and compare it against the count in the observations log. If the counts do not match, Stage 3b is blocked until the discrepancy is resolved. This is a hard gate — it cannot be bypassed. (3) **Section 4 — Mandatory Closing Patch:** A new mandatory final step after Stage 3b. A closing patch (`PATCH-{date}-{nnn}-ANALYSIS-V1.json`) setting `session_b_status = Analysis Complete` must be produced and submitted to Claude Code before the session ends. This step was absent from v4.6, causing grace (Reg 68), compassion (Reg 23), and mercy (Reg 111) to complete full analytical work without their status advancing. Four integrity rules added: SB-14 (SD pointer patch is a mandatory output), SB-15 (Stage 3b blocked without SD pointer count match), SB-16 (closing patch is mandatory final output), SB-17 (session does not close without all six outputs confirmed). Pool architecture confirmed retired; this instruction governs single-word per-session work. Companion documents list updated.

v4.6 (2026-04-10): Stage 3b (Publication Review) added as the final step of Session B, after the Session B Analytical Brief. This stage produces a clean reader-facing version of the word study by removing all internal programme language (database field names, registry numbers, cluster codes, group codes, patch references, signal type codes, flag codes, instruction version references, completion notes). The Internal Session C Completion Note is separated into a standalone internal file. Two output files result from Stage 3: the internal record (previous version) and the reader-facing publication (new version). Naming conventions table updated with sessionC-note file type.
...
2. **Analysis complete.** A rigorous six-pass analysis has been conducted against the clean data, producing verified findings on meaning, divine dimension, verse annotations, somatic evidence, language accuracy, and correlation audit.
...
Session B  (this session — multi-session operation)
    ...
    ├── Stage 2: Analytical passes (six passes against clean JSON)
...
- Global rules file: `wa-global-general-rules-v2-20260414.json`
- The complete word data export: `wa-[nnn]-[word]-complete-[date].json`
- The Session C word study: `wa-[nnn]-[word]-word-study-v[n]-[date].md`
- For Stage 1 Dimension Review sub-process: `WA-DimensionReview-Instruction-v1.9-20260409.md`
...
These disciplines apply without exception across all stages and all sessions. All are governed by the global rules file.

**Step-by-step.** Per GR-PROC-001 (non-waivable).

**Write on discovery.** Per GR-OBS-001 (non-waivable). Every finding, decision, gap, patch consequence, and open question is written to the observations log at the moment it is determined. Nothing is accumulated in memory.
...
**No assumptions.** Per GR-PROC-002. If a field is null, it is null. If a statement in the word study is not supported by the data, it is unsupported — flag it.

**All changes through patches.** Per GR-PROC-003 (non-waivable). Claude AI produces directives and patches. Claude Code executes them.
...
| `god_as_subject` | 0 or 1 — note value; check against verse evidence summary. This field is redundant (superseded by `mti_term_flags` flag_id 1 GOD_AS_SUBJECT per WA-Reference 13.3). If the value appears wrong, do not update this field — instead issue a directive to add the correct `mti_term_flags` record in Pass 2. |
| `somatic_link` | 0 or 1 — note value; check against known somatic evidence. This field is redundant (superseded by `mti_term_flags` SOMATIC_INNER_LINK/BODY_INNER_EXPRESSION per WA-Reference 13.3). If the value appears wrong, do not update this field — instead issue a directive to add the correct flag record in Pass 4. |
...
**Database write-back:** Do NOT update `wa_term_inventory.god_as_subject` — this is a redundant field per GR-DATA-005. The authoritative record is `mti_term_flags` flag_id 1 (GOD_AS_SUBJECT). Issue a directive to Claude Code to insert the flag record where the pass analysis reveals the flag should be set. Add `FRAMEWORK_SIGNAL` phase2 flag to `wa_term_phase2_flags` where the divine-human relationship has direct implications for the spirit-soul-body classification.
...
**Database write-back:** Do NOT update `wa_term_inventory.somatic_link` — redundant field per GR-DATA-005; not written by the Session B pipeline. The authoritative somatic records are in the flag mechanism. To record somatic evidence for a term, issue a directive to Claude Code to insert into `mti_term_flags` (keyed on `mti_terms.id`) or `wa_term_phase2_flags` (keyed on `wa_term_inventory.id`) using `phase2_flag_types` flag_id for `SOMATIC_INNER_LINK` (flag_id 3) or `BODY_INNER_EXPRESSION` (flag_id 4) — confirm flag_ids from the database before issuing. Similarly do NOT update `wa_term_inventory.god_as_subject` — use `mti_term_flags` flag_id 1 (GOD_AS_SUBJECT) instead (per GR-DATA-005). Add `SOMATIC_FLAG_RECOMMENDED`, `SPIRIT_SOUL_BOUNDARY`, or `BODY_SOUL_BOUNDARY` phase2 flags to `wa_term_phase2_flags` as warranted. Write provisional classification to `sb_classification` and reasoning to `sb_classification_reasoning` on `word_registry`. Issue directives to Claude Code rather than patch files for all flag insertions.
...
### 2.2 SD Pointer Persistence — Mandatory Database Write-Back

**This step is mandatory. Stage 3 does not begin until it is complete and confirmed by Claude Code.**

All Session D pointers raised during Stage 2 (across all six passes and the Analytical Brief) must be persisted to the database as `SD_POINTER` flag records in `wa_session_research_flags`. Raising pointers only in the observations log is insufficient — they must be in the database to be visible to the SD pointer audit, to Session D analysis, and to the programme-wide pointer record.

#### 2.2a — Compile the pointer list

From the observations log, extract every Session D pointer. For each pointer record the following fields:

- `flag_code`: `SD_POINTER`
- `flag_label`: `DIM-[nnn]-SD[nnn]` — sequential and unique per registry (e.g. DIM-023-SD001)
- `strongs_reference`: the primary Strong's number implicated, if any (or null)
- `cross_registry_id`: the primary partner registry_id implicated, if any (or null)
- `priority`: HIGH / MEDIUM / LOW — based on the strength of the observation
- `session_target`: `D`
- `description`: full analytical text of the pointer — specific verse or data point, registries implicated, the question for Session D, why it cannot be resolved within this registry
- `session_raised`: `Session B v4.7`
- `raised_date`: today's date
- `resolved`: `0`

Count the pointers. State the count aloud in the session: *"SD pointer count: [n]. Writing to observations log."* Write the count to the observations log as the authoritative record.

#### 2.2b — Produce the SD pointer patch

File naming: `PATCH-{YYYYMMDD}-{nnn}-SDPOINTERS-V1.json`

Patch type: `SESSIONB` with `session_b_status: null`. Contains one `insert` operation per pointer targeting `wa_session_research_flags`. Format follows patch_specification.

Deliver the patch to Claude Code. Wait for confirmation that the patch was applied and a fresh extract produced.

#### 2.2c — Spot-check after patch

On receiving the fresh extract:
- Count `SD_POINTER` records in `session_research_flags`
- Confirm count equals the pointer count from 2.2a
- Write confirmation to the observations log: *"SD pointer persistence confirmed: [n] records in database. Observations log count: [n]. Match: YES."*

If the counts do not match, resolve the discrepancy before proceeding to Stage 3.
...
### 3.5 Pre-Stage 3b Validation Gate — SD Pointer Count Check

**This check is a hard gate. Stage 3b does not begin until the check passes.**

Before beginning Stage 3b, Claude AI must:

1. State the SD pointer count from the observations log (recorded at Section 2.2a): *"Observations log SD pointer count: [n]."*

2. State the SD_POINTER record count from the current extract's `session_research_flags` array: *"Database SD_POINTER count: [n]."*

3. Compare the two counts:
   - If they match → state *"SD pointer count verified. Proceeding to Stage 3b."*
   - If they do not match → **STOP. Do not begin Stage 3b.** Identify the discrepancy: which pointers are in the observations log but not in the database? Produce a supplementary SD pointer patch for the missing records. Wait for Claude Code confirmation. Re-check before proceeding.

This gate exists because Stage 3b produces the reader-facing publication and the session C completion note — both of which must reference the correct SD pointer count. A word study published before SD pointers are in the database is analytically incomplete.

**The gate cannot be waived under any circumstances, including session time pressure or approaching context limits.**

---

## Stage 3b — Publication Review

### 3b.1 Purpose

To produce a clean, reader-facing version of the word study from which all internal programme language has been removed. The analytical brief and the word study serve different purposes: the brief is an internal Session D input; the word study is a reader-facing document. A word study that contains database field names, registry numbers, patch references, internal process notes, and programme-specific signal codes is not a published document.

Stage 3b is a final editorial pass over the completed word study. It does not change the analysis — it changes the register.

### 3b.2 What to remove or translate

Read the completed word study as a reader who knows nothing about the programme's internal structures. Identify and address every instance of:

**Internal field and process language:**
- Database field names: `somatic_link`, `god_as_subject`, `meaning_numbered`, `dimension_confidence`, `CLAUDE_AI`, `delete_flagged` — remove entirely or replace with plain description
- Stage references: "confirmed from Stage 2", "per Session C Instruction v1.2", "as noted in Pass 4" — remove entirely
- Patch and directive references: "directive issued", "patch applied", "DR-002" — remove entirely
- Flag codes: "RELATIONAL_DIRECTION", "SEMANTIC_RANGE_BREADTH", "ESCHATOLOGICAL_USAGE", "phase 2 flags" — remove or translate into plain language where the observation is worth keeping

**Registry and programme structure:**
- Registry numbers: "Reg 140", "Reg 103", "Reg 023" — replace with the word: "seeking", "love", "compassion"
- Cluster codes: "C17", "C20", "C05" — remove or replace with the words the cluster contains
- Group codes: "group 888-001", "group 889-002" — remove entirely
- Signal type codes: "coo", "xref", "dim" — translate or remove from prose; if a summary table is used, write out the full words
- Cross-reference codes: "DIM-187-SD001", "PH2-068-001" — remove entirely
- "this registry's active verse set" — replace with plain language
- "owner terms" — replace with "primary terms" or name the terms directly

**Footnotes and completion notes:**
- The Internal Session C Completion Note section is not for readers — remove entirely from the publication version, or place it in a separate internal document
- Footnotes referencing instruction versions and XREF handling procedures — remove from reader-facing text; the relevant substance can be retained as plain prose if it adds meaning

**Occurrence count language:**
- "88 times in this registry's active verse set, with 241 total NT occurrences" — translate to plain language: "appears 241 times in the New Testament" or note only what is meaningful for a reader
- Strong's numbers (G5483, H2580 etc.) — retain in vocabulary section headers and parenthetical identifications where helpful, but do not use as standalone navigation labels

### 3b.3 What to preserve

- All analytical observations and verse readings
- All connections and their key questions — stated in plain language
- The verse annotations in Section 3
- The vocabulary analysis in Section 4 — including transliterations, root families, and semantic observations
- All cross-word observations that are framed as questions for further study

### 3b.4 Style check

After removing internal language, read each section for consistency and flow:
- Sections 1 and 2 should read as continuous prose essays — no internal asides, no hedging phrases that reference programme process
- Section 3 should read as a series of verse readings that build a picture — transitions between verses should be natural, not formulaic
- Section 4 should read as a vocabulary essay — the technical linguistic content is appropriate; the programme machinery is not
- Section 5 should read as a series of short studies in connections — each connection as a self-contained mini-discussion, not a data record

### 3b.5 Output

Produce the reader-facing version as a new version:
- Filename: `wa-[nnn]-[word]-word-study-v[n+1]-[date].md`
- This version is the published document
- The previous version (containing the Internal Completion Note and programme references) is retained as the internal record

**The Internal Session C Completion Note** is separated into a standalone internal file:
- Filename: `wa-[nnn]-[word]-sessionC-note-v[n]-[date].md`
- This file contains the programme record of what changed, what was patched, what flags were raised, and what open items remain

---

## Stage 4 — Mandatory Closing Patch

**This step is mandatory. The session does not close without it.**

After Stage 3b is complete, produce the Session B closing patch. This patch does one thing: it advances `session_b_status` to `Analysis Complete`. Without it, the registry overview will not reflect that Session B analytical work is done — regardless of how much work was completed.

### 4.1 When this step runs

After all of the following are confirmed:
- Stage 1 complete (fresh extract obtained)
- Stage 2 complete (all six passes, Analytical Brief produced)
- Section 2.2 complete (SD pointer patch applied and confirmed)
- Stage 3 complete (word study updated)
- Stage 3b complete (publication version and completion note produced)

### 4.2 Patch format

File naming: `PATCH-{YYYYMMDD}-{nnn}-ANALYSIS-V1.json`
...
```json
{
  "_patch_meta": {
    ...
    "produced_by": "WA-SessionB-Instruction-v4.7 — Stage 4",
    "session_b_status": "Analysis Complete",
    "description": "Session B complete for registry {nnn} ({word}). All stages done. Advancing session_b_status to Analysis Complete."
  },
  "operations": [
    {
      "op_id": "OP-001",
      ...
      "description": "Session B Analysis Complete — registry {nnn} {word}"
    }
  ],
  "_patch_summary": {
    "total_operations": 1,
    "registry_status_updates": 1
  }
}
```

Deliver to Claude Code. Wait for confirmation.

### 4.3 Session close confirmation

After the closing patch is confirmed, state the following to the observations log:

```
SESSION CLOSE — Registry [nnn] ([word])
Date: [date]
Stage 1: COMPLETE
Stage 2: COMPLETE — [n] SD pointers raised
Section 2.2: COMPLETE — [n] SD_POINTER records confirmed in database
Stage 3: COMPLETE — word study v[n] produced
Stage 3b: COMPLETE — publication version v[n+1] produced; completion note produced
Stage 4: COMPLETE — closing patch applied; session_b_status = Analysis Complete

All six mandatory outputs confirmed:
  [ ] Observations log: wa-[nnn]-[word]-sessionB-observations-v[n]-[date].md
  [ ] Analytical brief: wa-[nnn]-[word]-sessionB-brief-v[n]-[date].md
  [ ] SD pointer patch: PATCH-{YYYYMMDD}-{nnn}-SDPOINTERS-V1.json (applied)
  [ ] Word study (internal): wa-[nnn]-[word]-word-study-v[n]-[date].md
  [ ] Word study (publication): wa-[nnn]-[word]-word-study-v[n+1]-[date].md
  [ ] Session C completion note: wa-[nnn]-[word]-sessionC-note-v[n]-[date].md
...
**The data is authoritative.** Per GR-PROC-002. Every finding must be traceable to a specific verse record, term entry, lexical source, or correlation signal in the JSON.

**Emergence, not imposition.** Per GR-PROG-008. Classifications must emerge from the evidence.
...
**Inferential is not confirmed.** Per GR-PROG-009. Label inferential connections as such. Do not upgrade them.
...
The following rules apply in addition to programme-wide rules GR-PROC-003 (all changes through patches), GR-PROC-004 (no patch without researcher review), GR-PROC-005 (observations log governs), and GR-PROC-006 (session logs at every breakpoint), which are not repeated here.
...
| SB-11 | Session D pointers must be raised at the moment of discovery during any pass — not accumulated for Pass 6 (per GR-OBS-001) |
...
| SB-14 | The SD pointer patch (Section 2.2) is a mandatory output — Stage 3 does not begin without it confirmed by Claude Code |
| SB-15 | Stage 3b does not begin without the SD pointer count check (Section 3.5) passing — database count must equal observations log count |
| SB-16 | The closing patch (Stage 4) is a mandatory final output — the session does not close without it confirmed by Claude Code |
| SB-17 | The session does not close without all seven outputs confirmed: observations log, analytical brief, SD pointer patch (applied), word study internal, word study publication, Session C completion note, closing patch (applied) |
...
|---|---|---|
| Session log | `wa-[nnn]-[word]-sessionb-log-v[n]-[date].md` | `wa-068-grace-sessionb-log-v1-20260410.md` |
| Observations log | `wa-[nnn]-[word]-sessionb-observations-v[n]-[date].md` | `wa-068-grace-sessionb-observations-v1-20260410.md` |
| Field-level patch | `wa-[nnn]-[word]-sessionb-patch-v[n]-[date].json` | `wa-068-grace-sessionb-patch-v1-20260410.json` |
| Dimension Review patch (from sub-process) | `wa-dim-[cluster]-[nnn]-patch-v[n]-[date].json` | `wa-dim-c17-068-patch-v1-20260410.json` |
| CC directive | `wa-[nnn]-[word]-sessionb-cc-directive-v[n]-[date].md` | `wa-068-grace-sessionb-cc-directive-v1-20260410.md` |
| Analytical brief | `wa-[nnn]-[word]-sessionb-brief-v[n]-[date].md` | `wa-068-grace-sessionb-brief-v1-20260410.md` |
| Updated word study (internal) | `wa-[nnn]-[word]-word-study-v[n]-[date].md` | `wa-068-grace-word-study-v3-20260410.md` |
| Updated word study (publication) | `wa-[nnn]-[word]-word-study-v[n+1]-[date].md` | `wa-068-grace-word-study-v4-20260410.md` |
| Session C completion note (internal) | `wa-[nnn]-[word]-sessionc-note-v[n]-[date].md` | `wa-068-grace-sessionc-note-v3-20260410.md` |

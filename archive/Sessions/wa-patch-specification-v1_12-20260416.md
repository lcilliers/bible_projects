...
This document is governed by **wa-global-general-rules-v2_3-20260416.json**.

Claude AI must confirm the global rules file has been loaded before producing or applying any patch. Per GR-PROC-003: all database changes go through patches. Per GR-PROC-004: no patch is applied without researcher review. Per GR-DIR-006: all patches must pass the six-point format self-check before submission.
...
|-------|--------------------------|-------------|
| `wa-vcb001-versecontext-v1-{YYYYMMDD}.json` | `null` | Verse context classification — relevance filter, grouping, anchor designation |
| `wa-{nnn}-preanalysis-v1-{YYYYMMDD}.json` | `Pre-Analysis Complete` | Term classifications (extracted/delete/xref), bleed removal |
| `wa-{nnn}-analysis-v1-{YYYYMMDD}.json` | `Analysis Complete` | Status advance — registry_note only |
| `wa-{nnn}-sessionb-v1-{YYYYMMDD}.json` | `Analysis Complete` | Full analytical data — evidential status, dimensions, findings, SD pointers |
| `wa-{nnn}-sessionb-complete-v1-{YYYYMMDD}.json` | `Session B Complete` | Confirms all four outputs produced |
...
```
wa-{registry_no}-{type}-v{version}-{YYYYMMDD}.json
```

Examples:
- `wa-061-preanalysis-v1-20260325.json`
- `wa-182-sessionb-v1-20260325.json`
...
```
wa-vcb{batch_id}-versecontext-v{n}-{YYYYMMDD}.json    (batch classification)
wa-vcgroup{group_id}-v{n}-{YYYYMMDD}.json              (targeted group update)
wa-vcverse{verse_record_id}-v{n}-{YYYYMMDD}.json       (targeted verse update)
```

Examples:
- `wa-vcb001-versecontext-v1-20260329.json`
- `wa-vcgroup47-v1-20260329.json`
- `wa-vcverse4821-v1-20260329.json`
...
|---|---|
| STEP extraction re-run reset | `wa-{nnn}-repair-step-rerun-v1-{YYYYMMDD}.json` |
| audit_word re-run reset | `wa-{nnn}-repair-auditword-rerun-v1-{YYYYMMDD}.json` |
| Verse Context re-run reset | `wa-{nnn}-repair-vc-rerun-v1-{YYYYMMDD}.json` |
| Analysis re-run reset | `wa-{nnn}-repair-analysis-rerun-v1-{YYYYMMDD}.json` |
| Mid-pool interruption reset | `wa-{nnn}-repair-midpool-v1-{YYYYMMDD}.json` |
| Failure recording | `wa-{nnn}-repair-failure-v1-{YYYYMMDD}.json` |

# WA-SessionB-ClaudeCode-Instructions-v3.3-20260414

> Claude Code consolidated instruction document for Framework B.
> Version 3.3 — 20260414.
> Supersedes: WA-SessionB-ClaudeCode-Instructions-v3.2-20260330.md.
> Change note v3.3: (1) Governing Rules block added — Claude Code must load wa-global-general-rules-v2.1-20260414.json at session start; GR-DIR-001 referenced for directive format; GR-OBS-005 referenced for no-physical-delete rule; GR-DATA-001 referenced for mti_terms status filter. (2) Section 0 document sources table updated: stale references corrected — WA-Reference-v5 → v5.5; WA-Registry-Management-Guide-v5 → v5.9; WA-SessionB-DataPrep-Instruction-v5 and WA-SessionB-Extraction-Instruction-v5 marked retired; WA-SessionB-Analysis-Instruction-v5 → WA-SessionB-Instruction-v4.8. (3) Section 2.5: DataPrep reference removed. (4) Section 6: Registry Management Guide reference updated to v5.9. (5) Section 7: filename convention updated to reference global rules GR-FILE-001 through GR-FILE-009. (6) Section 8.2 session_b_status description: "pool-based Session B" language removed (pool architecture retired). (7) Section 8.5 patch types note: VerseContext reference updated to v2.6. (8) Section 9 anomalies source reference corrected. (9) Section 14: VerseContext instruction reference updated to v2.6-20260414. (10) Failure patch template: produced_date format corrected to compact YYYYMMDD per GR-FILE-009. (11) Footer date updated to compact format.
...
- **GR-DIR-001**: All directives from Claude AI to Claude Code are written in plain language specifying motivation, exact operations, and expected outcomes. Claude Code executes; Claude AI determines what should be done.
...
Note: VERSECONTEXT, VCGROUP, VCVERSE carry `session_b_status: null` in _patch_meta. The applicator must not reject these. See WA-VerseContext-Instruction-v2.6-20260414 Section 14 of this document.
...
*WA-SessionB-ClaudeCode-Instructions-v3.3  |  20260414  |  Consolidation of Claude Code responsibilities from v5 document suite*
...
**Failure patch naming:** `PATCH-{YYYYMMDD}-{nnn}-REPAIR-FAILURE-V1`
...
```json
{
  "_patch_meta": {
    ...
    "produced_by": "WA-SessionB-ClaudeCode-Instructions-v3.3-20260414",
    "patch_type": "REPAIR",
...
*WA-SessionB-ClaudeCode-Instructions-v3.3 | 20260414 | Supersedes v3.2-20260330 | Global rules integration; stale references corrected; pool architecture references removed; GR-OBS-005/GR-DATA-001/GR-DIR-001/GR-PROC-003/GR-PROC-004 referenced*

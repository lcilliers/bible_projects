...
**Version 5.4 | March 2026 | Reference guide — not an operational instruction**

| **Document** | **Value** |
| --- | --- |
| Filename | WA-Registry-Management-Guide-v5.4-20260329.md |
| Supersedes | WA-Registry-Management-Guide-v5.3-20260328.md |
...
| Not covered | Transactional operations — see WA-VerseContext-Instruction-v1 and WA-SessionB-DataPrep-Instruction-v5 |

**Change Control — v5.4**

| **Change** | **Detail** |
| --- | --- |
| Section 2 | Added `verse_context_status` field; removed `anchor_verses` field (dropped M17); updated `session_b_status` to include `Verse Context Reset` |
| Section 3 | Registry Status Lifecycle updated — full pipeline including Verse Context stage; DataPrep gate now requires `verse_context_status = Complete` |
| Section 6 | Queries 6.2 updated; new query 6.8 added (Verse Context stage monitoring); query 6.9 added (pool processing readiness) |
| Section 7 | Pool-based processing sequence documented |
| Section 8 | Terminology additions: Verse Context, anchor verse, contextual meaning group, pool analysis dataset, pool/cluster batch |
| Section 9 | File naming additions: vcb-extract, vcb-patch, pool-analysis tokens |
...
| dimensions | Multi-value semantic category field (formerly source_category, renamed M17). Comma-delimited. Populated at Session B extraction. |
...
| unique_term_count | Terms unique to this registry. Engine-derived. |
| shared_term_count | Terms shared with other registries. Engine-derived. |
| term_sharing_ratio | 0.0 (all unique) to 1.0 (all shared). Engine-derived. |
...
*WA-Registry-Management-Guide-v5.4 | 20260329 | Supersedes v5.3-20260328*

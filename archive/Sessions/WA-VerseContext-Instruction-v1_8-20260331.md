...
| Outputs | Verse Context batch JSON — wa-vcb-{batch_id}-extract-{date}.json (Claude Code → Claude AI) │ Verse Context patch — wa-vcb-{batch_id}-patch-v{n}-{date}.json (Claude AI → Claude Code) │ Observations file — wa-vcb-{batch_id}-term-observations-v{major}.{minor}-{date}.md (Claude AI, progressive) │ Re-exported full word JSON per registry (Claude Code → DataPrep) |
...
**Observations file naming convention:**
`wa-vcb-{batch_id}-term-observations-v{major}.{minor}-{date}.md`

- `{date}` is the creation date of the file — it does not change when the file is updated across sessions
- `v{major}.{minor}` is the version number — this increments on every save that adds or changes content
  - Start at `v1` when the file is first created for a batch
  - Increment the minor version on each progressive save within a batch: `v1.1`, `v1.2`, etc.
  - The version number is the only reliable differentiator between saves — do not rely on file size or timestamp
- When multiple versions of a file exist (e.g. following an extract from an archive), the version number in the filename identifies the current version unambiguously

**Example:** A batch observations file created on 2026-03-30 and updated across three sessions:
- Initial creation: `wa-vcb-003-term-observations-v1-20260330.md`
- After first update: `wa-vcb-003-term-observations-v1.1-20260330.md`
- After second update: `wa-vcb-003-term-observations-v1.2-20260330.md`

This convention applies from VCB-003 onward. Files produced before this version of the instruction are not retroactively renamed.
...
*WA-VerseContext-Instruction-v1.8-20260331 | Supersedes v1.7-20260330 | v1.8: Section 6.4 — observations file versioning rule added; filename convention updated to include v[major].[minor] version number*

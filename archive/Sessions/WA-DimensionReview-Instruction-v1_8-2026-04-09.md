# WA-DimensionReview-Instruction-v1.8-2026-04-09

**Framework B — Soul Word Analysis Programme**
**Dimension Review Instruction — Claude AI and Claude Code**
**Version 1.8 | April 2026 | Status: Active governing instruction**

| **Document** | **Value** |
|---|---|
| Filename | WA-DimensionReview-Instruction-v1.8-2026-04-09.md |
| Supersedes | WA-DimensionReview-Instruction-v1.7-2026-04-09.md |
| Change note | v1.8 — Three substantive changes. (1) **Two-stage session model introduced** (Section 6): the analytical session (Phases A–C) and the patch generation session are now separate. The observations log (Section 6.2) is the bridge between them — all analytical decisions, field updates, description rewrites, Session B/D pointers, and any instruction amendment notes are written to disk immediately on discovery, in series. The patch session reads the observations log and constructs the patch mechanically from its contents. This eliminates the need to hold the full analytical context in memory during patch construction. (2) **Observations log introduced as primary analytical output** (Section 6.2, Section 7.2): replaces the refinement log as the in-session write-to-disk artefact. The observations log is written continuously throughout Phases A–C — not accumulated and written at the end. Every observation is written on discovery. Format specified in Section 7.2. (3) **File naming convention revised** (Section 13): reference identifier (cluster, registry, batch, group, etc.) moved to immediately after the prefix, before the short description. Pattern: `[Prefix]-[reference]-[short description]-[Version]-[date]`. Allows all related files to sort together by reference. All naming examples updated throughout. Companion document reference and instruction version strings updated throughout to v1.8. No other analytical or process changes. |
...
Date: {date} | Instruction version: WA-DimensionReview-v1.8-2026-04-09
...
7. Write all findings to observations log before moving to the next group
...
**Purpose:** Verify that the verse evidence supporting each group is complete and coherent with the group description and dimension assignment.

**When to trigger:** Claude AI requests an anchor verse vertical pass extract (Section 9.5) when:
...
- A group's anchor verse is in a key theological passage and the vertical pass data would confirm or challenge the dimension assignment
- A group's anchor verse was previously flagged as analytically complex
...
**Do not over-request.** Request only when trigger conditions above are met.

### 5.7 The Dimension Vocabulary

*Derived from data. Working vocabulary as of v1.8. Will be revised as additional data is reviewed.*
...
```json
{
  "_patch_meta": {
    ...
    "produced_by": "WA-DimensionReview-Instruction-v1.8-2026-04-09",
...
```json
{
  ...
  "description": "Registry {nnn} ({word}): Dimension Review complete under v1.8"
}
```
...
```json
{
  ...
  "record": {
    ...
    "instruction_version": "WA-DimensionReview-Instruction-v1.8-2026-04-09",
    "registry_count": {n},
    "group_count": {n},
    "anchored_count": {n},
...
|---|---|---|---|
| `DIMREVIEW` | WA-DimensionReview-Instruction-v1.8 | `null` | `wa_dimension_index` updates; `verse_context_group` corrections; `wa_session_b_findings` inserts; `wa_session_research_flags` inserts; `word_registry` stamp updates; `wa_dim_review_cluster_log` inserts |
| `DIMREVIEW-GRPDESC` | WA-DimensionReview-Instruction-v1.8 | `null` | Out-of-session group description corrections only |
...
**`produced_by`:** `"Claude Code — WA-DimensionReview-Instruction-v1.8-2026-04-09"`
...
| `dim_review_version` | `word_registry` | `WA-DimensionReview-Instruction-v1.8-2026-04-09` |
...
*WA-DimensionReview-Instruction-v1.8-2026-04-09 | Supersedes v1.7-2026-04-09 | v1.8: Two-stage session model (analytical session + separate patch session) introduced (Section 6); observations log introduced as primary analytical artefact replacing refinement log (Sections 6.2, 7.2); write-on-discovery principle stated and encoded in DR-19 and DR-20; patch session startup protocol added (Section 6.5); observations_log field added to patch meta; file naming convention revised — reference identifier moved to immediately after prefix (Section 13); all naming examples updated throughout; instruction amendment note format added to observations log (Section 7.2); pattern note DR-20 added; companion document and version strings updated throughout to v1.8*

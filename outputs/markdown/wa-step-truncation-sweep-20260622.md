# STEP 60-cap truncation sweep + confirmation — 2396 terms, 62 non-T2 suspects

_Read-only. STEP true total (oracle) vs DB; suspects confirmed by the chapter-cutoff test in each term's own densest book. Run 2026-06-22 on the fixed client._

- **Confirmed TRUNCATION** (clean canonical cutoff, recoverable like rāšāʿ): **6**
- Scope / span-filter gaps (scattered — legitimate, NOT cap-truncation): 56

> A high STEP−DB gap on a *polysemous* term is mostly legitimate span-filter (the cluster keeps only the inner-being sense). Only a **clean chapter cutoff** in the term's own densest book proves the cap silently dropped recoverable verses. Even there, the recoverable inner-being count is a subset of the raw gap — re-ingest via the fixed client (engine audit_word per registry) lets the span-filter re-select.

## Confirmed truncation (fix candidates, worst first)

| Strong's | term | cluster | STEP | DB | gap | cutoff evidence |
|---|---|---|---|---|---|---|
| H5414G | na.tan (to give: give) | M12 | 1187 | 242 | 945 | Gen: DB to ch34, STEP to ch49 (10 later chapters present in STEP) |
| H3820A | lev (heart) | M47 | 550 | 331 | 219 | Pro: DB to ch19, STEP to ch31 (11 later chapters present in STEP) |
| G4100 | pisteuō (to trust (in)) | M31 | 218 | 120 | 98 | Joh: DB to ch6, STEP to ch20 (12 later chapters present in STEP) |
| H5315G | ne.phesh (soul) | M47 | 230 | 179 | 51 | Psa: DB to ch62, STEP to ch146 (25 later chapters present in STEP) |
| H0341 | o.yev (enemy) | M44 | 276 | 244 | 32 | Psa: DB to ch89, STEP to ch143 (10 later chapters present in STEP) |
| G2564G | kaleō (to call: call) | M37 | 133 | 110 | 23 | Luk: DB to ch15, STEP to ch23 (5 later chapters present in STEP) |

## Scope / span-filter (high gap but scattered — not cap-truncation)

| Strong's | term | cluster | STEP | DB | gap | densest-book pattern |
|---|---|---|---|---|---|---|
| H8085G | sha.ma (to hear: hear) | M41 | 904 | 403 | 501 | Isa: DB ch48 vs STEP ch66 (no clean cutoff) |
| G2192 | echō (to have/be) | M23 | 615 | 120 | 495 | Mat: DB ch26 vs STEP ch27 (no clean cutoff) |
| H3045 | ya.da (to know) | M15 | 874 | 421 | 453 | Isa: DB ch59 vs STEP ch66 (no clean cutoff) |
| G3056 | logos (word) | M15 | 317 | 43 | 274 | 1Cor: DB ch15 vs STEP ch15 (no clean cutoff) |
| G4151G | pneuma (spirit/breath: breath) | M25 | 340 | 120 | 220 | Act: DB ch28 vs STEP ch28 (no clean cutoff) |
| G6063 | oida (to know) | M15 | 245 | 56 | 189 | 1Cor: DB ch16 vs STEP ch16 (no clean cutoff) |
| G4102G | pistis (faith) | M31 | 224 | 97 | 127 | Rom: DB ch16 vs STEP ch16 (no clean cutoff) |
| H6635B | tsa.va (Hosts) | M23 | 274 | 156 | 118 | Isa: DB ch54 vs STEP ch54 (no clean cutoff) |
| H7230 | rov (abundance) | M46 | 144 | 41 | 103 | Isa: DB ch63 vs STEP ch63 (no clean cutoff) |
| G1391 | doxa (glory) | M22 | 149 | 49 | 100 | Rom: DB ch16 vs STEP ch16 (no clean cutoff) |
| H3201 | ya.khol (be able) | M23 | 183 | 86 | 97 | Gen: DB ch48 vs STEP ch48 (no clean cutoff) |
| H7665 | sha.var (to break) | M24 | 142 | 61 | 81 | Jer: DB ch52 vs STEP ch52 (no clean cutoff) |
| G2309 | thelō (to will/desire) | M29 | 196 | 120 | 76 | Mat: DB ch27 vs STEP ch27 (no clean cutoff) |
| H2896A | tov (pleasant) | M04 | 381 | 306 | 75 | Pro: DB ch31 vs STEP ch31 (no clean cutoff) |
| H8334 | sha.rat (to minister) | M36 | 92 | 21 | 71 | Num: DB ch18 vs STEP ch18 (no clean cutoff) |
| H2617A | che.sed (kindness) | M05 | 239 | 169 | 70 | Psa: DB ch147 vs STEP ch147 (no clean cutoff) |
| G0932 | basileia (kingdom) | M23 | 154 | 88 | 66 | Mat: DB ch26 vs STEP ch26 (no clean cutoff) |
| H6944G | qo.desh (holiness) | M22 | 346 | 286 | 60 | Num: DB ch35 vs STEP ch35 (no clean cutoff) |
| H7121G | qa.ra (to call: call to) | M37 | 344 | 286 | 58 | Psa: DB ch145 vs STEP ch145 (no clean cutoff) |
| G5485 | charis (grace) | M39 | 146 | 88 | 58 | Rom: DB ch16 vs STEP ch16 (no clean cutoff) |
| H3444 | ye.shu.ah (salvation) | M38 | 77 | 22 | 55 | Isa: DB ch62 vs STEP ch62 (no clean cutoff) |
| H6041 | a.ni (afflicted) | M24 | 72 | 23 | 49 | Psa: DB ch140 vs STEP ch140 (no clean cutoff) |
| G0266 | hamartia (sin) | M10 | 150 | 105 | 45 | Rom: DB ch14 vs STEP ch14 (no clean cutoff) |
| G0040G | hagios (holy) | M22 | 161 | 120 | 41 | Act: DB ch28 vs STEP ch28 (no clean cutoff) |
| G4561 | sarx (flesh) | M47 | 126 | 85 | 41 | Rom: DB ch13 vs STEP ch13 (no clean cutoff) |
| H1288 | ba.rakh (to bless) | M39 | 289 | 249 | 40 | Gen: DB ch48 vs STEP ch49 (no clean cutoff) |
| G0026 | agapē (love) | M05 | 106 | 68 | 38 |  |
| H1320 | ba.sar (flesh) | M47 | 241 | 205 | 36 | Lev: DB ch26 vs STEP ch26 (no clean cutoff) |
| G2212 | zēteō (to seek) | M41 | 113 | 83 | 30 | Luk: DB ch24 vs STEP ch24 (no clean cutoff) |
| G2588 | kardia (heart) | M47 | 149 | 120 | 29 | Luk: DB ch24 vs STEP ch24 (no clean cutoff) |
| H1285 | be.rit (covenant) | M44 | 264 | 236 | 28 | Deu: DB ch33 vs STEP ch33 (no clean cutoff) |
| G1577 | ekklēsia (assembly) | M05 | 111 | 85 | 26 | Act: DB ch20 vs STEP ch20 (no clean cutoff) |
| H3824 | le.vav (heart) | M47 | 230 | 207 | 23 | Deu: DB ch32 vs STEP ch32 (no clean cutoff) |
| G0757 | archō (be first) | M08 | 86 | 63 | 23 | Mar: DB ch15 vs STEP ch15 (no clean cutoff) |
| H7592 | sha.al (to ask) | M21 | 157 | 136 | 21 | 1Sa: DB ch30 vs STEP ch30 (no clean cutoff) |
| H2388G | cha.zaq (to strengthen: strengthen) | M23 | 179 | 163 | 16 | Neh: DB ch10 vs STEP ch10 (no clean cutoff) |
| H7121H | qa.ra (to call: call by) | M37 | 208 | 194 | 14 | Gen: DB ch38 vs STEP ch50 (no clean cutoff) |
| H2930A | ta.me (to defile) | M10c | 142 | 128 | 14 | Lev: DB ch18 vs STEP ch22 (no clean cutoff) |
| H2416A | chay (alive) | M25 | 220 | 207 | 13 | Gen: DB ch46 vs STEP ch46 (no clean cutoff) |
| H6662 | tsad.diq (righteous) | M26 | 197 | 184 | 13 | Pro: DB ch28 vs STEP ch29 (no clean cutoff) |
---

## Recovery executed (2026-06-22)

**M47 seat-words — RECOVERED cleanly** (separate additional JSON `Sessions-v2/M47-Constitution/Data/wa-ve-lexical-extract-M47-additional-truncation-recovery-20260622-b{1,2}of2.json`, gitignored):
- **ne.phesh H5315G (soul)**: +50 Psalms-tail verses (1 within-range excluded) → active 179→**229**.
- **lev H3820A (heart)**: +219 across 12 books, **0 within-range = pure truncation** → active 331→**550**.
- Added 1 active row per recovered ref (no new duplicates; OT-DBR-009 flagged dupes untouched); measure layer + ve_lexical generated; 272 soul/heart nodes all carry lexicals. integrity ok.
- These were safe because the seat-words are studied in **every** book — every absent verse is genuine truncation (within-range≈0).

**Remaining 4 — NOT clean targeted-recoveries (scope-entangled).** The "beyond-DB-reach = truncation" rule only holds for comprehensively-studied terms. For polysemous, scope-limited terms the absent set is mostly legitimate **scope/span-filter**, not truncation:
- **na.tan H5414G (give, M12)**: tail=943 across *every* book — overwhelmingly scope (M12 studies only inner-being giving, 242). Blind recovery would wrongly add ~900 literal-"give" verses. **Do NOT targeted-recover.**
- **pisteuō G4100 (believe, M31)**: scope-entangled (gap mixes faith-in-God vs general belief).
- **o.yev H0341 (enemy, M44)**: +30 (Psa/Chron tail) — small, plausibly truncation, needs scope check.
- **kaleō G2564G (call, M37)**: +23 (Acts/Luke, within-range 0) — small, plausibly truncation, needs scope check.

**Proper mechanism for the remaining 4** = engine `audit_word` per registry (re-pull via the fixed client + re-apply the span-filter to select only authoritative in-scope occurrences) — NOT the blind targeted-ingest used for the seat-words. na.tan's truncation portion is small relative to its scope gap; o.yev/kaleō are small. Recommend doing these via the engine, cluster-by-cluster, with researcher scope review — or deferring (lower value than the M47 seat-words).

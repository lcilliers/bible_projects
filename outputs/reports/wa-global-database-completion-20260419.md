...
- File: `data/file_manifest.json` — refreshed
...
The sweep design `outputs/investigations/wa-global-readiness-sweep-design-v1-20260419.md` is in DRAFT state (awaiting researcher markup of Q1–Q14). Post-migration review required:
...
Prior Session B narratives (`.md` files in `data/imports/WA/Session_B_Analysis/` and `outputs/markdown/`) remain as historical artefacts — not deleted, not imported into the prose store at this time (deferred per prose store design Q2 = "new work only").

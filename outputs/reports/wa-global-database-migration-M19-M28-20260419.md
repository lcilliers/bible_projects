...
5. **`data/schema/create_tables.sql`** — regenerated from dry-run post-migration DB. Prior file preserved as `create_tables.sql.pre_DBR_bak`.
...
3. Restore pre-migration backup: copy `backups/bible_research_pre_M{N}_{ts}.db` → `data/bible_research.db`
...
Restore the baseline backup `backups/bible_research_pre_DBR_20260419_122435.db` to `data/bible_research.db`. This reverts to the pre-Phase-A state.

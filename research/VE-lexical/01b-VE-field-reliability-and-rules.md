...
- **valence** · {righteous·sinful·commanded·forbidden·neutral·UNRESOLVED} · term-inherent valence + context morph · **RULE:** term-inherent valence (a per-term moral tag for inherently-moral lemmas, e.g. *resha* "wickedness"=sinful) ∪ context (imperative→commanded, prohibition/negation→forbidden); **[Q11 RESOLVED — keep valence; expectation test, P5]** clear → value; verse **silent** → NONE (do not impute); verse **expects valence but it can't be determined** → UNRESOLVED. (Same logic as cause.) Built from a term-inherent-valence list + context (imperative→commanded, prohibition→forbidden).

...
Current scenario - observations
 ...
 - current folders have evolved over time, some good, some bad
 - current file naming evolved over time, naming convensions changing all the time
 - a large number of files are no longer current. Superceded by new version; the approach or methods used to generate them has changed; files are short lived, experimental or working files.
 - most files, including archived ones, may contain important information that may need to be extracted in future.  

there is a need to for improved file handling. Let me first try to think through it.

 - categories of files:
   - system files (folder is determined by the sub system requirements e.g. claude, git, engine,scripts, logs (general system logs), backups, analytics, archive)
   - standing instructions 
   - Sessions
     - Session Type (phase a (STEP), session a, dimensions, verse_context,phases, session B analytics, Session D, Session C )
       - Session inputs
       - Session outputs
       - Session prose
       - logs

# M10 Phase 9 — Characteristic 10 (Specialised sinful mechanisms) — brief

**Cluster:** M10 — Sin, Guilt and Transgression
**Characteristic:** 10 — Specialised sinful mechanisms
**Verses in scope:** ~5 across 1 sub-group(s)
...
| # | Document | Purpose |
|---|---|---|
| 1 | **This brief** — `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char10-Specialised-sinful-mechanisms-brief-v1-20260523.md` | Primary task instructions |
| 2 | **Structural input** — `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char10-Specialised-sinful-mechanisms-input-v1-20260523.md` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts + carry-forward observations |
...
M10 has 22 inner-being characteristics. You are processing **Characteristic 10 (Specialised sinful mechanisms)**. Per researcher direction 2026-05-18 (carried forward to M10 under v2_8 §8.0):
...
## Characteristic 10 — Specialised sinful mechanisms: definition

> Sin operating through a specific inner mechanism — desire exploited by enticement (deleazō; M14 cross-register), identity falsified through hypocrisy (sunupokrinomai; M08 cross-register), covenantal identity abandoned through apostasy (apostasia; M31 cross-register).

---

## Your task

For each of the **189 catalogue prompts** (8 tiers, T0–T7), author a finding at characteristic scope for Characteristic 10 (Specialised sinful mechanisms). Use the verse evidence in the structural input.
...
**[CHAR-10]** E — Finding text. Cite specific verses / VCGs / sub-groups from the evidence in §3 of the structural input. Quote the specific verse phrases that evidence the answer. The finding must be self-contained for a Session C reader.
...
Scope marker is `**[CHAR-10]**` (CC's loader maps this to characteristic_id=34 for this characteristic).
...
6. **CONFIRM + PROCEED (or ALERT)** —
   - If self-evaluation passes: announce *"Tier T{N} written: {file_path}, {n}/{n} prompts, [CHAR-10] markers verified, self-evaluation PASS. Proceeding to Tier T{N+1}."* — then immediately begin Tier T{N+1} in the SAME response (re-read §3 fresh; start from step 1).
...
| 2 | **Scope marker** | Every `[CHAR-N]` marker in this tier is `[CHAR-10]` | ALERT — name the wrong-marker prompts |
...
### INTER_RELATIONSHIP — M10 specialised-mechanisms ↔ M14 / M08 / M31

> CHAR-10 covers three small-corpus mechanisms with distinct cross-registers: deleazō (enticement → M14 Deceit), sunupokrinomai (hypocrisy → M08 Pride/performed self-elevation), apostasia (apostasy → M31 Faith/Unbelief covenantal defection). Downstream T6 findings should articulate each cross-register polarity.
...
Target filename: `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char10-Specialised-sinful-mechanisms-findings-v1-20260523.md`
...
```markdown
# M10 Phase 9 — Characteristic 10 (Specialised sinful mechanisms) — findings
...
**[CHAR-10]** E — finding text with verse citations…
...
2. Ping CC: "M10 Char 10 (Specialised sinful mechanisms) Phase 9 findings ready"

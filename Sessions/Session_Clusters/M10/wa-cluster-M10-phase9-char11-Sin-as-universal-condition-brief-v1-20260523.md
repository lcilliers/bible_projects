# M10 Phase 9 — Characteristic 11 (Sin as universal condition) — brief

**Cluster:** M10 — Sin, Guilt and Transgression
**Characteristic:** 11 — Sin as universal condition
**Verses in scope:** ~112 across 1 sub-group(s)
...
| # | Document | Purpose |
|---|---|---|
| 1 | **This brief** — `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char11-Sin-as-universal-condition-brief-v1-20260523.md` | Primary task instructions |
| 2 | **Structural input** — `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char11-Sin-as-universal-condition-input-v1-20260523.md` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts + carry-forward observations |
...
M10 has 22 inner-being characteristics. You are processing **Characteristic 11 (Sin as universal condition)**. Per researcher direction 2026-05-18 (carried forward to M10 under v2_8 §8.0):
...
## Characteristic 11 — Sin as universal condition: definition

> Sin as a structural condition of the inner person — not primarily an act but a state under which every person stands. Universal human condition, fall-short-of-glory register.

---

## Your task

For each of the **189 catalogue prompts** (8 tiers, T0–T7), author a finding at characteristic scope for Characteristic 11 (Sin as universal condition). Use the verse evidence in the structural input.
...
**[CHAR-11]** E — Finding text. Cite specific verses / VCGs / sub-groups from the evidence in §3 of the structural input. Quote the specific verse phrases that evidence the answer. The finding must be self-contained for a Session C reader.
...
Scope marker is `**[CHAR-11]**` (CC's loader maps this to characteristic_id=35 for this characteristic).
...
6. **CONFIRM + PROCEED (or ALERT)** —
   - If self-evaluation passes: announce *"Tier T{N} written: {file_path}, {n}/{n} prompts, [CHAR-11] markers verified, self-evaluation PASS. Proceeding to Tier T{N+1}."* — then immediately begin Tier T{N+1} in the SAME response (re-read §3 fresh; start from step 1).
...
| 2 | **Scope marker** | Every `[CHAR-N]` marker in this tier is `[CHAR-11]` | ALERT — name the wrong-marker prompts |
...
Target filename: `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char11-Sin-as-universal-condition-findings-v1-20260523.md`
...
```markdown
# M10 Phase 9 — Characteristic 11 (Sin as universal condition) — findings
...
**[CHAR-11]** E — finding text with verse citations…
...
2. Ping CC: "M10 Char 11 (Sin as universal condition) Phase 9 findings ready"

# M10 Phase 9 — Characteristic 3 (Confession) — brief
...
| # | Document | Purpose |
|---|---|---|
| 1 | **This brief** — `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char3-Confession-brief-v1-20260523.md` | Primary task instructions |
| 2 | **Structural input** — `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char3-Confession-input-v1-20260523.md` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts + carry-forward observations |
...
M10 has 22 inner-being characteristics. You are processing **Characteristic 3 (Confession)**. Per researcher direction 2026-05-18 (carried forward to M10 under v2_8 §8.0):
...
## Characteristic 3 — Confession: definition

> The moment the inner person names a committed sin as sin — first-person confession, corporate declaration, or the conscience voicing an act already done. Conscience in confessional mode.

---

## Your task

For each of the **189 catalogue prompts** (8 tiers, T0–T7), author a finding at characteristic scope for Characteristic 3 (Confession). Use the verse evidence in the structural input.
...
**[CHAR-3]** E — Finding text. Cite specific verses / VCGs / sub-groups from the evidence in §3 of the structural input. Quote the specific verse phrases that evidence the answer. The finding must be self-contained for a Session C reader.
...
Scope marker is `**[CHAR-3]**` (CC's loader maps this to characteristic_id=27 for this characteristic).
...
6. **CONFIRM + PROCEED (or ALERT)** —
   - If self-evaluation passes: announce *"Tier T{N} written: {file_path}, {n}/{n} prompts, [CHAR-3] markers verified, self-evaluation PASS. Proceeding to Tier T{N+1}."* — then immediately begin Tier T{N+1} in the SAME response (re-read §3 fresh; start from step 1).
...
| 2 | **Scope marker** | Every `[CHAR-N]` marker in this tier is `[CHAR-3]` | ALERT — name the wrong-marker prompts |
...
Target filename: `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char3-Confession-findings-v1-20260523.md`
...
```markdown
# M10 Phase 9 — Characteristic 3 (Confession) — findings
...
**[CHAR-3]** E — finding text with verse citations…

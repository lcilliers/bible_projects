# M10 Phase 9 — Characteristic 20 (Faithlessness as covenant-breaking sin) — brief

**Cluster:** M10 — Sin, Guilt and Transgression
**Characteristic:** 20 — Faithlessness as covenant-breaking sin
**Verses in scope:** ~101 across 1 sub-group(s)
...
| # | Document | Purpose |
|---|---|---|
| 1 | **This brief** — `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char20-Faithlessness-as-covenant-breaking-sin-brief-v1-20260523.md` | Primary task instructions |
| 2 | **Structural input** — `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char20-Faithlessness-as-covenant-breaking-sin-input-v1-20260523.md` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts + carry-forward observations |
...
M10 has 22 inner-being characteristics. You are processing **Characteristic 20 (Faithlessness as covenant-breaking sin)**. Per researcher direction 2026-05-18 (carried forward to M10 under v2_8 §8.0):
...
## Characteristic 20 — Faithlessness as covenant-breaking sin: definition

> The inner disposition of deliberate unfaithfulness — the will that breaks covenantal loyalty, betrays a trust, acts treacherously against God or persons. Cross-register flag: M13 (Truth/Faithfulness) + M31 (Faith/Unbelief).

---

## Your task

For each of the **189 catalogue prompts** (8 tiers, T0–T7), author a finding at characteristic scope for Characteristic 20 (Faithlessness as covenant-breaking sin). Use the verse evidence in the structural input.
...
**[CHAR-20]** E — Finding text. Cite specific verses / VCGs / sub-groups from the evidence in §3 of the structural input. Quote the specific verse phrases that evidence the answer. The finding must be self-contained for a Session C reader.
...
Scope marker is `**[CHAR-20]**` (CC's loader maps this to characteristic_id=44 for this characteristic).
...
6. **CONFIRM + PROCEED (or ALERT)** —
   - If self-evaluation passes: announce *"Tier T{N} written: {file_path}, {n}/{n} prompts, [CHAR-20] markers verified, self-evaluation PASS. Proceeding to Tier T{N+1}."* — then immediately begin Tier T{N+1} in the SAME response (re-read §3 fresh; start from step 1).
...
| 2 | **Scope marker** | Every `[CHAR-N]` marker in this tier is `[CHAR-20]` | ALERT — name the wrong-marker prompts |
...
### INTER_RELATIONSHIP — M10 faithlessness ↔ M13 truth/faithfulness + M31 faith/unbelief

> CHAR-20 (Faithlessness as covenant-breaking sin) — all 3 terms (ba.gad, ma.al verb, ma.al noun = 101V) carry the M13/M31 cross-register. Faithlessness is the negative of covenant-faithfulness (M13) and of faith (M31). Phase 9 T6 should articulate the polarity.
...
Target filename: `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char20-Faithlessness-as-covenant-breaking-sin-findings-v1-20260523.md`
...
```markdown
# M10 Phase 9 — Characteristic 20 (Faithlessness as covenant-breaking sin) — findings
...
**[CHAR-20]** E — finding text with verse citations…
...
2. Ping CC: "M10 Char 20 (Faithlessness as covenant-breaking sin) Phase 9 findings ready"

# M10 Phase 9 — Characteristic 9 (Sinful speech) — brief

**Cluster:** M10 — Sin, Guilt and Transgression
**Characteristic:** 9 — Sinful speech
**Verses in scope:** ~22 across 1 sub-group(s)
...
| # | Document | Purpose |
|---|---|---|
| 1 | **This brief** — `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char9-Sinful-speech-brief-v1-20260523.md` | Primary task instructions |
| 2 | **Structural input** — `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char9-Sinful-speech-input-v1-20260523.md` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts + carry-forward observations |
...
M10 has 22 inner-being characteristics. You are processing **Characteristic 9 (Sinful speech)**. Per researcher direction 2026-05-18 (carried forward to M10 under v2_8 §8.0):
...
## Characteristic 9 — Sinful speech: definition

> Sin committed through the mouth: blasphemy and slander originating in inner moral corruption, directed against God or persons. Cross-register flag: M06 (Hate/Contempt).

---

## Your task

For each of the **189 catalogue prompts** (8 tiers, T0–T7), author a finding at characteristic scope for Characteristic 9 (Sinful speech). Use the verse evidence in the structural input.
...
**[CHAR-9]** E — Finding text. Cite specific verses / VCGs / sub-groups from the evidence in §3 of the structural input. Quote the specific verse phrases that evidence the answer. The finding must be self-contained for a Session C reader.
...
Scope marker is `**[CHAR-9]**` (CC's loader maps this to characteristic_id=33 for this characteristic).
...
6. **CONFIRM + PROCEED (or ALERT)** —
   - If self-evaluation passes: announce *"Tier T{N} written: {file_path}, {n}/{n} prompts, [CHAR-9] markers verified, self-evaluation PASS. Proceeding to Tier T{N+1}."* — then immediately begin Tier T{N+1} in the SAME response (re-read §3 fresh; start from step 1).
...
| 2 | **Scope marker** | Every `[CHAR-N]` marker in this tier is `[CHAR-9]` | ALERT — name the wrong-marker prompts |
...
### INTER_RELATIONSHIP — M10 sinful-speech ↔ M06 contempt cross-register

> CHAR-9 (Sinful speech, blasfēmia/blasfēmos) is register-adjacent with M06 (Hate, Contempt and Hostility). Slander/blasphemy originates in inner moral corruption (Mat 15:19 — "from the heart"; Mar 7:22) but also expresses contempt and hostility (Rev 13:5/6; Col 3:8). When M06 opens or is re-examined, these verses inform its T6 cross-cluster findings.
...
Target filename: `Sessions/Session_Clusters/M10/wa-cluster-M10-phase9-char9-Sinful-speech-findings-v1-20260523.md`
...
```markdown
# M10 Phase 9 — Characteristic 9 (Sinful speech) — findings
...
**[CHAR-9]** E — finding text with verse citations…
...
2. Ping CC: "M10 Char 9 (Sinful speech) Phase 9 findings ready"

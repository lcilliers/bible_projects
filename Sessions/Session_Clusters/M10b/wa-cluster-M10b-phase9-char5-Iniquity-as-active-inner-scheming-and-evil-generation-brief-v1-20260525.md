# M10b Phase 9 — Characteristic 5 (Iniquity as active inner scheming and evil generation) — brief

**Cluster:** M10b — Wickedness, Evil and Abomination
**Characteristic:** 5 — Iniquity as active inner scheming and evil generation
**Verses in scope:** ~79 across 1 sub-group(s)
...
| # | Document | Purpose |
|---|---|---|
| 1 | **This brief** — `Sessions/Session_Clusters/M10b/wa-cluster-M10b-phase9-char5-Iniquity-as-active-inner-scheming-and-evil-generation-brief-v1-20260525.md` | Primary task instructions |
| 2 | **Structural input** — `Sessions/Session_Clusters/M10b/wa-cluster-M10b-phase9-char5-Iniquity-as-active-inner-scheming-and-evil-generation-input-v1-20260525.md` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts + carry-forward observations |
...
| 4 | **Science extract** — `Workflow/Sciences/wa-m10-guilt-scienceextract-v1_0-20260513.md` (Section 3 only) | Programme-curated scientific lens for T7.3. **Note:** the doc was authored pre-split and covers M10/M10b/M10c siblings; for M10b only Section 3 (Evil/Wickedness) is in scope. Sections 1–2 (Guilt + Transgression) belong to M10. |
...
M10b has 6 inner-being characteristics. You are processing **Characteristic 5 (Iniquity as active inner scheming and evil generation)**. Per researcher direction 2026-05-18 (carried forward to M10b under v2_8 §8.0):
...
## Characteristic 5 — Iniquity as active inner scheming and evil generation: definition

> Evil as an ongoing active inner process — devised, cultivated, generated, and enacted from within. Iniquity is plotted on the bed at night, conceived in the heart, schemed by the mind, and born into harmful deeds. The evildoer is a person who works iniquity as a habitual pursuit, exploiting others, lurking with predatory intent. Also encompasses the evil-deeds register — deeds whose inner origin is wickedness, and whose quality marks the one who commits them.

---

## Your task

For each of the **189 catalogue prompts** (8 tiers, T0–T7), author a finding at characteristic scope for Characteristic 5 (Iniquity as active inner scheming and evil generation). Use the verse evidence in the structural input.
...
**[CHAR-5]** E — Finding text. Cite specific verses / VCGs / sub-groups from the evidence in §3 of the structural input. Quote the specific verse phrases that evidence the answer. The finding must be self-contained for a Session C reader.
...
Scope marker is `**[CHAR-5]**` (CC's loader maps this to characteristic_id=51 for this characteristic).
...
6. **CONFIRM + PROCEED (or ALERT)** —
   - If self-evaluation passes: announce *"Tier T{N} written: {file_path}, {n}/{n} prompts, [CHAR-5] markers verified, self-evaluation PASS. Proceeding to Tier T{N+1}."* — then immediately begin Tier T{N+1} in the SAME response (re-read §3 fresh; start from step 1).
...
| 2 | **Scope marker** | Every `[CHAR-N]` marker in this tier is `[CHAR-5]` | ALERT — name the wrong-marker prompts |
| 3 | **Citation discipline** | Every E finding cites at least one verse-reference or VCG code (e.g. `Pro 16:5`, `M10b-A-VCG-01`) | ALERT — name the uncited E prompts; do NOT proceed |
...
### INTEGRATION_NOTE — a.ven trouble-suffered sub-corpus — VCG-level cross-register signal

> M10b-E-VCG-01 (7V) carries a.ven in the trouble/distress/affliction-suffered register — the harm that befalls the victim rather than the iniquity-scheming of the agent. Phase 9 T6 findings should attend to the M03 (grief/sorrow) and M27 (calamity/ruin) cross-register relationships. The VCG is structurally separated from the iniquity-scheming and evil-deeds VCGs within M10b-E. (The brief estimated ~18V for this sub-corpus; the AI's per-verse read identified 7 cleaner trouble-register cases.)
...
Target filename: `Sessions/Session_Clusters/M10b/wa-cluster-M10b-phase9-char5-Iniquity-as-active-inner-scheming-and-evil-generation-findings-v1-20260525.md`
...
```markdown
# M10b Phase 9 — Characteristic 5 (Iniquity as active inner scheming and evil generation) — findings
...
**[CHAR-5]** E — finding text with verse citations…
...
2. Ping CC: "M10b Char 5 (Iniquity as active inner scheming and evil generation) Phase 9 findings ready"
3. CC parses, validates evidence-grounding + completeness, applies to cluster_finding with characteristic_id set.
4. Move to next characteristic (Char 6).

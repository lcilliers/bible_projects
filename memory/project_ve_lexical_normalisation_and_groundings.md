...
**TEMPLATED NARRATION built** (`scripts/_produce_ve_narration_v1.py`, read-only **deterministic view** per 01b §1f, NOT stored prose): composes sense→type→mode→location→origin→faculty→attributed→compound→relational; **all elements included, multiples as multiples, UNRESOLVED as a `[field: UNRESOLVED]` placeholder, NONE/SILENT omitted**. First output: `research/VE-lexical/wa-ve-templated-narration-first-20260615.md`.

**NEXT:** resolve the UNRESOLVED worklists (separately); refine iter-1 signal-lists (homographs); decide whether to PERSIST the narration as the single `l2_meaning` finding per term-in-verse. Still out of scope: VE4 (mode=column), VE9/10/11/12/14 (read/deferred), type sense-supersede, sense coarse-ceiling refinement. T2 got the mechanical lexical facts but stays excluded from standalone analysis.

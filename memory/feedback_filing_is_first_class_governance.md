...
**How to apply:** before writing any file, set its location/name from the rules; archive superseded versions (silent supersession = a filing-rule breach); rebuild the manifest after moves; **locate files via the manifest, not the folders** (folders are unreliable). **Filing audit DONE 2026-06-14** (`docs/filing-audit-20260614.md`): the rules doc had drifted (§3.7 instructions, §3.9 schema, §3.15 analytics pointed at dead `data/...` paths) — corrected in place. **Residual (researcher):** the `data/` vs `Sessions/`/`Workflow/` duality, a schema-snapshot refresh to v3.31.0, and archiving 15 `_tmp_` scripts + ~12 silently-superseded docs. See [[reference-core-memory-orientation-map]] · [[project_reconstruction_baseline_20260614]] · [[source-of-truth-is-written-record]].

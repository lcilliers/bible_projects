...
## G · Researcher notes (2026-06-08)

*Researcher directions for M01's V3_2 run. (Manual annotation — not regenerated by the script; kept here as
part of the cluster record.)*

1. **Pre-start (Step 1):** **reset the cluster `status` to `In Progress`** before the run begins.
2. **Set-aside re-evaluation is OPTIONAL.** The 81 set-aside verses *may* be re-evaluated — but this is an
   **optional step requiring explicit researcher approval**, not an automatic part of the run.
3. **VCG comparison & re-evaluation (per sub-group cycle).** The existing VCGs are **compared and
   re-evaluated** against the new read. **Provide a side-by-side surface for the researcher to evaluate
   new / changed / set-aside VCGs.** This evaluation is done **within each sub-group cycle** (L3–L5).
4. **⚑ Possible roll-up-logic adjustment — characteristic timing.** **Characteristic assessment must be done
   AFTER all sub-groups have been processed, not during each sub-group cycle.** *(Design implication: this
   moves characteristic emergence out of the per-sub-group L5 step into a post-all-sub-groups step — to be
   decided against `wa-cluster-rollup-design.md` (L4/L5/L6) before L1 proceeds. See response.)*
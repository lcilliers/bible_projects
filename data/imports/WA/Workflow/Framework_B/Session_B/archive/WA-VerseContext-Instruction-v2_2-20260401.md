# WA-VerseContext-Instruction-v2_2-20260401
...
**All-verses-fail rule:** If every verse for a term fails the relevance filter, this must be handled carefully, as follows:

**(a) Individual inspection is mandatory and non-waivable.** Every verse must be individually read and assessed before an all-verses-fail finding is reached. Sampling — reviewing a subset and inferring the remainder — is not permitted regardless of corpus size, apparent uniformity of use, or the grammatical/functional character of the term. The finding is only valid when every verse has been individually inspected.

**(b) After full individual inspection, two scenarios apply:**

- **If the term has 10 or fewer verses:** all-verses-fail is an error condition. Stop. Flag the term to the researcher with: the term's strongs_number, transliteration, gloss, verse count, and a brief reason why each verse was assessed as non-relevant. Await researcher instruction. Do not submit a patch for this term until the researcher has reviewed.

- **If the term has more than 10 verses AND individual inspection is complete for all verses:** all-verses-fail may be confirmed and submitted without researcher confirmation when: (i) the term is a grammatical or functional particle (pronoun, quantifier, conjunction, adverb, particle) whose non-passage is consistent across the entire corpus; or (ii) all verses clearly represent a single non-inner-being sense (e.g. a homograph where all instances are the physical/concrete sense). In all other cases, researcher confirmation is still required even after full inspection.

In all cases, the all-verses-fail finding must be documented in the session log with: term identity, verse count, the basis for the finding (grammatical particle / homograph / other), and a statement confirming that individual inspection was completed for all verses.

...
**Version:** 1.5 | **Date:** 2026-03-30
**Applicator:** `scripts/apply_session_patch.py`
**Supersedes:** patch_specification v1.4 (2026-03-29)

**Change note v1.5:**
- Section 0: Status workflow updated — 'Ready for Analysis' removed from active pipeline path for existing registries. Direct path shown: Verse Context Reset → Pre-Analysis Complete. Ready for Analysis documented as legacy state (new registries only).
- Section 0: Who sets each status table updated to reflect audit_word COALESCE applies to NULL status only.
- Section 3.12: REPAIR patch type added — cascade reset and failure patch operations.
- Section 5: Validation rule added — REPAIR patches with session_b_status matching current value are always accepted (idempotent write).
...
|---|---|---|---|---|
| VERSECONTEXT | WA-VerseContext-Instruction | v1.5 | `null` — applicator must not reject | verse_context_group inserts; verse_context inserts/updates; full batch |
| VCGROUP | WA-VerseContext-Instruction | v1.5 | `null` | Targeted verse_context_group update |
| VCVERSE | WA-VerseContext-Instruction | v1.5 | `null` | Targeted verse_context insert or update |
...
*patch_specification_v1.5 | 20260330 | Supersedes v1.4-20260329 | Section 0: Ready for Analysis removed from active path; REPAIR patch rules added; Section 3.12: REPAIR patch catalogue reference; Section 5: idempotency rule for REPAIR status writes*

...
```json
{
  ...
  "produced_by": "Claude Code — WA-VerseContext-Instruction v1.7",
  "governing_instruction": "WA-VerseContext-Instruction-v1.7-20260330.md",
  "total_verse_count": 0,
  "total_term_count": 0,
...
The per-batch observations file (`wa-vcb-{batch_id}-term-observations-{date}.md`) must be written progressively to disc during the session, not accumulated in memory and written at the end. After completing each term's classification (Step 6 of Section 6.2), write that term's entry to the observations file immediately. Do not defer. If the session is interrupted, the observations file must contain a complete record of every term classified up to that point.
...
```json
{
  "_patch_meta": {
    ...
    "batch_id": "VCB-{nnn}",
    "produced_date": "{yyyy-mm-dd}",
    "produced_by": "WA-VerseContext-Instruction-v1.7",
...
```
Verse Context v1.7 startup complete.
Batch: {batch_id}
...
*WA-VerseContext-Instruction-v1.7-20260330 | Supersedes v1.6-20260330 | v1.7: VCB-001 lessons — registry split documentation, session discipline and file writing rules, deferred patch construction workflow, dual_context_verses clarification, programmatic validation requirements, completion report extended*

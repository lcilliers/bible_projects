...
**Version 5.9 | 20260414 | Reference guide — not an operational instruction**

| **Document** | **Value** |
| --- | --- |
| Filename | wa-registry-management-guide-v5.9-20260414.md |
| Supersedes | WA-Registry-Management-Guide-v5.8-20260412.md |
...
| Not covered | Transactional operations — see WA-VerseContext-Instruction-v2.6-20260414 and WA-SessionB-Instruction-v4.8-20260414 |

**Change Control — v5.9**

| **Change** | **Detail** |
| --- | --- |
| Header | Governing Rules block added — mandatory load of wa-global-general-rules-v2-20260414.json. Filename updated to wa- prefix and compact date format per GR-FILE-009. Not covered references updated to v2.6 and v4.8. |
| Section 2 | Engine-derived Phase 1 fields note cross-referenced to GR-DATA-008. |
...
This guide is governed by **wa-global-general-rules-v2-20260414.json**.
...
Note on engine-derived fields: phase1_term_count, phase1_verse_count, unique_term_count, shared_term_count, and term_sharing_ratio reflect Phase 1 state only and are not updated by later pipeline stages. Per GR-DATA-008: use live queries against wa_term_inventory for current counts.
...
Session B is performed one word at a time under WA-SessionB-Instruction-v4.7. The input is the per-registry complete extract (`wa-[nnn]-[word]-complete-[date].json`). Processing proceeds in cluster order; within a cluster, any sequence is valid. Words with high `term_sharing_ratio` do not require simultaneous analysis — their XREF profiles are accessible in the per-registry extract.
...
*WA-Registry-Management-Guide-v5.8 | 20260412 | Supersedes v5.7 | Pool architecture retired; DataPrep removed; individual-word processing model documented*

# wa-patch-instruction-v2_5-20260424

> Framework B Soul Word Analysis Programme — Patch Preparation and Execution
> Version: v2_5 | Date: 20260424
> Supersedes: wa-patch-instruction-v2_4-20260422.md (v2_5 adds four new patch types — VCNEW, VCREVISE, VCSBFLAGS, VCSDPOINTERS — carrying the VC session four-output model per VC-instruction v3_3 and the four-patch design brief `vc-four-patch-design-v1-20260424.md`. Each covers one output class; all four are session_b_status-exempt; existing VERSECONTEXT type retained for backwards compatibility. Changes: §3.3 table gains four rows; §3.4 row for "All other types" updated to include the four; §3.6 pipeline sequence table shows the four in the VC stage. v2_4 content otherwise unchanged.)
...
**Required `_patch_meta` fields** (in addition to §3.2 standard): `batch_id`, `terms_covered` (array of `mti_term_id`s receiving first-time classification), `governing_instruction` (filename of the governing VC instruction version).
...
**Applicator behaviour (per VC-2 extension, post-v3_3):** after operations apply, the applicator runs per-term R1-R4 + orphan-group + coverage validation for each `mti_term_id` in `terms_covered`. On success: writes `mti_terms.vc_status = 'complete'`, `vc_instruction_version = {governing_instruction}`, `vc_status_updated_at = now()`, `vc_status_note = NULL`. Then derives affected registries and runs the aggregation check (see §13 of the VC instruction).
...
**Required `_patch_meta` fields:** `batch_id`, `terms_covered` (array of `mti_term_id`s being revised), `governing_instruction`.
...
**Applicator behaviour:** validates the same per-term R1-R4 + orphan-group + coverage for each term in `terms_covered`. For terms whose `vc_status` is already `'complete'` or `'approved'`, the revise bumps `vc_status_updated_at` without downgrading the state. Term-state downgrade (e.g. to `'to_revise'`) is not a VCREVISE effect; that is a separate directive.
...
**Required `_patch_meta` fields:** `batch_id`, `governing_instruction`. `terms_covered` optional (each flag row carries `strongs_reference` for term attribution).
...
3. `batch_id` is present and matches the VCB-{nnn} session id carried by all session artefacts.
4. For `VCNEW` and `VCREVISE`: `terms_covered` is a non-empty array; every `mti_term_id` referenced in operations is present in `terms_covered`.
...
7. For `VCSBFLAGS`: every operation is `insert` on `wa_session_research_flags`; every record has `session_target = 'Session B'`; `flag_code` from the VCSBFLAGS allowed list (§15.4).
8. For `VCSDPOINTERS`: same but `session_target = 'Session D'`; `cross_registry_id` present and non-null on every row; `flag_code` from the VCSDPOINTERS allowed list (§15.5).
9. No mixing — a patch of one of these four types contains only operations appropriate for that type.

---

*wa-patch-instruction-v2_5-20260424 | Supersedes wa-patch-instruction-v2_4-20260422 | v2_5 adds §15 VC Session Four-Patch Catalogue (VCNEW, VCREVISE, VCSBFLAGS, VCSDPOINTERS) supporting the per-term VC session output model; §3.3 and §3.4 updated; §3.6 pipeline sequence extended with the four new rows.*

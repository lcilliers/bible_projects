...
**Registry-level signal is CC-side derived.** The programme still needs a registry-level Complete signal — DataPrep (and, thereafter, Dimension Review) reads `word_registry.verse_context_status`. But that signal is no longer produced by VC itself. VC operates per term, writing `mti_terms.vc_status = 'complete'` on each successful per-term apply. Claude Code then aggregates: once all OWNER terms (with verses) and all XREF terms' OWNERs for a registry are at `complete` or `approved`, Claude Code advances the registry's `verse_context_status` to Complete. See §13 and `scripts/apply_session_patch.py` (VC-2 extension) for the derivation logic.

**Legacy batch JSON — deprecated:** the `wa-vcb-{batch_id}-extract-{date}.json` format that preceded the `.md` renderer is retained in Annexure C for historical continuity but is no longer a primary or fallback input form. The per-term `.md` handles both small-session and large-term cases natively; there is no split-registry use case the `.md` cannot serve directly.
...
4. **Prior-state posture declaration — mandatory, once per term.** For each per-term `.md` in the session, read its "Prior-state posture (this term)" block in the About section and state verbatim one of the two templates below.
...
5. State the startup summary (Annexure A template) — now term-scoped: list the terms in the session with their `mti_term_id`, Strong's, registry, and posture.
6. Note any term whose active verse_context state is fully in place and internally consistent (R1–R4) — review without re-classifying unless a revision is clearly warranted (see revision rule in Section 6.2 Step 2).
7. Note any term with existing `verse_context` records but incomplete coverage (some verses classified, others not) — flag to researcher before continuing (partial completion rule in Section 6.2 Step 2).
8. **Processing order:** Terms are processed in the order they appear in the session's input list, which is the researcher's session-composition order. Registry-ascending order is no longer an imposed discipline at the input layer — the researcher composes the session, the classifier follows its order.
...
```json
{
  "_patch_meta": {
    ...
    "governing_instruction": "wa-versecontext-instruction-v3_1-20260424.md",
    "produced_date": "{yyyy-mm-dd}",
    "produced_by": "wa-versecontext-instruction-v3_1",
...
* `batch_id` — the session identifier (VCB-{nnn}); names the output artefact family.
* **`terms_covered`** — array of integer `mti_term_id`s — **every term the session classified** (including terms with zero operations where the classification was "confirm existing, no change"). The applicator cross-checks this against the `mti_term_id`s referenced by operations: missing term_ids (operations reference a term not declared) fail the patch; extra declared ids (terms covered but no operations emitted for them) are accepted. The applicator uses `terms_covered` to know which terms to flip to `vc_status = 'complete'` after the patch applies. Required for VERSECONTEXT patches under v3_1 onward.
...
Action required (per v3_1 per-term model — scripts/apply_session_patch.py VC-2 extension):
  ...
  7. For each affected registry, run the derived completion aggregation:
       - All active OWNER terms (with verses) at vc_status IN ('complete','approved')?
       - All XREF terms' OWNER at vc_status IN ('complete','approved')?
...
The applicator logs each advancement in its final summary (`registries_advanced_to_complete` counter). Re-renders of the per-term `.md`s and any per-registry hybrid view are researcher-discretionary — they regenerate the same data from the canonical DB state; the state is already written.

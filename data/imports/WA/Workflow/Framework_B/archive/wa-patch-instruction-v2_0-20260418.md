# wa-patch-instruction-v2_0-20260418

> Framework B Soul Word Analysis Programme — Patch Preparation and Execution
> Version: v2_0 | Date: 20260418
> Supersedes: wa-patch-specification-v1_14-20260416.md + patch-related sections of wa-sessionb-cc-instructions-v3_6-20260416.md (§4, §15, §16)
...
- Controlled vocabulary values → wa-reference v5_6 §2 onwards
- Schema field definitions → wa-reference v5_6 §13
- File naming rules → GR-FILE-001 through GR-FILE-009; wa-reference v5_6 §1
- Directive method → wa-directive-instruction v1_0
- CC operational routines outside patch application → wa-claudecode-instruction v4_0
...
Per wa-global-general-rules-v2_11 GR-PROG-005, Claude AI determines what should be done and why; Claude Code executes.

There are two and only two mechanisms for instructing Claude Code to make database changes — **patches** (this document) and **directives** (wa-directive-instruction v1_0). Conversational instructions to CC without either format are not valid database change requests.
...
Use a directive (per wa-directive-instruction v1_0) when Claude AI knows the outcome required but is not certain of the exact execution path. Claude AI describes in plain language what needs to happen and why; Claude Code inspects the database, determines the correct operations, and executes.
...
Per wa-reference v5_6 §1.5. Uppercase `patch_id` is an applicator-compatibility convention and is not changed by GR-FILE-007 which governs filenames, not internal field values.
...
Valid `status` values per wa-reference v5_6 §2.
...
`flag_label` convention: `PH2-{registry_no}-{3-digit-sequence}` per wa-reference v5_6 §5.4.
...
Valid `evidential_status` values per wa-reference v5_6 §10.
...
JSON templates for both files are maintained in wa-reference v5_6 §14.
...
Governed by wa-versecontext-instruction v2_7. This section contains the patch-format rules specific to Verse Context patches.
...
Lookup values used in patches. Vocabulary sources are authoritative in wa-reference v5_6; this appendix shows values used in patch construction.
...
|---|---|---|
| PREANALYSIS | wa-sessionb-analysis-readiness (when finalised — see FLAG-001) | Pre-analysis data corrections; phase2 flag dispositions |
| SESSIONB | wa-sessionb-analysis-output (when finalised — see FLAG-003) | Evidential status, dimensions, findings, SD pointer flags |
| REPAIR | This document §9 | Cascade reset operations; failure recording |
| SESSIOND | wa-sessiond-orientation v3_0 | Session D discovery and synthesis |
| CLUSTERING | wa-registry-management-guide v5_9 | Cluster assignment updates |
| VERSECONTEXT | wa-versecontext-instruction v2_7 | Verse relevance filter, grouping, anchor designation |
| VCGROUP | wa-versecontext-instruction v2_7 | Targeted group update |
| VCVERSE | wa-versecontext-instruction v2_7 | Targeted verse update |

---

*wa-patch-instruction-v2_0-20260418 | Supersedes wa-patch-specification-v1_14-20260416 + wa-sessionb-cc-instructions-v3_6 §4, §15, §16 | Absorbs addenda ADD-PATCHDIR-001, -004, -005, -007, -008 from global rules v2_10*

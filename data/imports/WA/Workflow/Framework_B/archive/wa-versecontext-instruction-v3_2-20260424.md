...
3. Include this coverage in the registry completion aggregation (§13.1 and §13.2): a registry is complete when all its OWNER terms are at `vc_status IN ('complete', 'approved')` AND every XREF term's OWNER is at the same
...
**When to produce:** If any Session B flags are raised during the end-of-session flag resolution exchange, produce the Session B flags document before the patch construction session. If no Session B flags were raised, the document is not required for that session.

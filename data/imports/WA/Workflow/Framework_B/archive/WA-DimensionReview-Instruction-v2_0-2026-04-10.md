# WA-DimensionReview-Instruction-v2.0-2026-04-10

**Framework B — Soul Word Analysis Programme**
**Dimension Review Instruction — Claude AI and Claude Code**
**Version 2.0 | April 2026 | Status: Active governing instruction**

| **Document** | **Value** |
|---|---|
| Filename | WA-DimensionReview-Instruction-v2.0-2026-04-10.md |
| Supersedes | WA-DimensionReview-Instruction-v1.9-2026-04-09.md |
| Change note | v2.0 — Major structural addition: registry-scoped operation mode. The instruction previously assumed cluster-as-unit-of-work throughout, which blocked single-registry Dimension Review when run as a Session B Stage 1 sub-process. This version adds Section 0.2 (Operation Modes), amends Section 4.6 (Coverage Verification), amends Section 5 (Phase C scope), amends Section 6.1 (Unit of Work), amends Section 6.4 (Startup Protocol), amends Section 10.2–10.4 (Stamps and Gates), and adds Section 10.5a (Registry-Scoped Stamp). The analytical requirements are preserved in both modes: Phase B always spans the full cluster; Phase C may be scoped to the target registry in registry-scoped mode. The Session B gate (SB-10: dominant_subject assigned) is satisfied by registry-level completion; the DataPrep gate (DR-16: cluster stamp) is only required for programme-wide DataPrep sequencing and is not set in registry-scoped mode. No analytical or vocabulary changes. v1.9 — Two substantive changes. (1) **Mandatory anchor verse reading added to Phase C step-by-step** (Section 5.3): re-reading each anchor verse against the group description is now a required step in the standard Phase C workflow for every group. Step 3 revised to require: read each anchor verse text; verify that the description captures all inner-being aspects the verse shows; note any aspects present in the verse but absent from the description; note any aspects claimed in the description that are not grounded in the anchor verse. This was previously absent from Section 5.3, causing the anchor verse to be bypassed during dimension discernment. (2) **Section 5.6 reframed** (Anchor Verse Verification Sub-Process → Anchor Verse Vertical Pass Extract): the section now describes what happens when the mandatory in-session anchor verse reading (Section 5.3 Step 3) reveals a problem that cannot be resolved from the anchor verse text alone and requires full term-link context via a vertical pass extract. The four trigger conditions are retained but repositioned as conditions under which a vertical pass extract is requested — not as the only conditions under which anchor verses are read. The observations log format for AV verification findings is added to Section 7.2. No other analytical or process changes. |
| Companion documents | WA-VerseContext-Instruction-v2.5-20260409 │ WA-SessionB-Analysis-Instruction-v5.7 │ WA-SessionB-Instruction-v4.4-2026-04-10 │ WA-Reference-v5.5 │ WA-Registry-Management-Guide-v5.7 │ patch_specification-v1.6 │ WA-SessionD-Orientation-v2.1 │ WA-dim-session-log-methodology-v2.1-2026-04-08.md |

...
**Filename:** wa-global-flags-v1_3-20260418.md
**Version:** v1_3
**Date:** 2026-04-18
**Status:** Active — supersedes wa-global-flags-v1_2-20260418.md
...
**Change from v1_2:** v1_2 register had 6 Open, 4 Resolved, 1 Obsolete (total 11) — the v1_2 header summary said 7 Open / 3 Resolved which was already inconsistent with the register. Corrected in v1_3: register-derived counts used. In v1_3: FLAG-012 added Open (cross-instruction reference cleanup sweep under GR-REF-002); FLAG-013 added Open (document_discipline category classification review). v1_3 totals: 6 + 2 = 8 Open; 4 Resolved; 1 Obsolete; total 13. No other flag state changes in this revision.
...
### FLAG-012 — Cross-instruction reference cleanup sweep (GR-REF-002)

- **Status:** Open — sequenced
- **Raised:** 2026-04-18
- **Description:** GR-REF-002 (introduced in wa-global-general-rules v2_11-20260418) establishes the `[current]` reference convention for operational cross-references in the instruction corpus. Existing instructions contain versioned cross-references that pre-date this rule. A programme-wide sweep is required to replace operational cross-references with the `[current]` form. Provenance references (Supersedes, observation log entries, patch `_patch_meta.produced_by`) remain with specific versions and are NOT touched by this sweep.
- **Scope:** All instruction documents in Project Files. The five documents produced in the 2026-04-18 four-document consolidation (wa-reference v5_6, wa-patch-instruction v2_0, wa-directive-instruction v1_0, wa-claudecode-instruction v4_0, and the questions document) are within this scope but have not yet been swept — they were produced under GR-REF-001 discipline with specific versions, before GR-REF-002 was introduced. Other instructions in the corpus (Verse Context, Dimension Review, Session D orientation, Registry Management Guide, utility instructions) are also in scope.
- **Sequencing:** This sweep depends on the prior completion of FLAG-013 (document_discipline category classification review). Classification review may surface additional rules that affect reference discipline; running the sweep before classification is complete risks having to re-sweep. Sequence: FLAG-013 → FLAG-012.
- **Sweep mechanism:** For each instruction document: (1) identify operational cross-references — running text pointing the reader to another instruction for content or procedure, governing rules tables, inline "see" references, pointers in scope statements; (2) replace the version string with `[current]`; (3) leave provenance references untouched; (4) re-version the document with a minor bump recording the sweep in the change note.
- **Action needed:** (1) Wait for FLAG-013 resolution. (2) Execute the sweep across all instruction documents. (3) Record each instruction filename, old reference count, and new reference count as the sweep progresses. (4) Resolve this flag once the corpus is compliant.
- **Gate:** No pipeline operations are blocked by this flag.

### FLAG-013 — Document_discipline category classification review

- **Status:** Open — sequenced first
- **Raised:** 2026-04-18
- **Description:** The `document_discipline` category was introduced in wa-global-general-rules v2_10-20260418 to house GR-REF-001 (single-authority content referencing). v2_11 adds GR-REF-002 (current-version reference convention). The category currently has two rules. Additional rules covering document structure, authorship discipline, scope policing, and related authorship patterns are likely to be identified during ongoing programme work. Before the cross-instruction reference cleanup sweep (FLAG-012) is executed, the `document_discipline` category should be reviewed for classification completeness — new rules added if needed, existing rules refined if categorisation drifted, the category's scope statement sharpened.
- **Review scope:** (a) Does the `document_discipline` category as currently composed adequately cover the authorship patterns the programme has encountered? (b) Are there rules currently in other categories (process_discipline, document_governance, traceability) that properly belong in `document_discipline`? (c) Are there rules that should exist but have not yet been written? (d) Is the category's scope clear enough that future rule-placement decisions are unambiguous?
- **Sequencing:** Runs **before** FLAG-012. If classification review produces new rules that change reference discipline, those new rules must be in place before the sweep begins.
- **Action needed:** (1) Review the category in context of the programme's accumulated authorship experience. (2) Produce a next global rules revision (v2_12 or later) incorporating any new or refined document_discipline rules. (3) Confirm the category's scope statement is clear. (4) Resolve this flag, at which point FLAG-012 can proceed.
- **Gate:** Blocks FLAG-012 but does not block any pipeline operation.
- **Relationship to FLAG-012:** FLAG-013 is the structural prerequisite — the category must be right before sweeping the corpus against it. Running the sweep against an incomplete category would require a second sweep once the category is completed.

...
Version 5.4 | March 2026 | Schema v3.8.0 | Referenced by all instruction documents

| **Document** | **Value** |
| --- | --- |
| Filename | WA-Reference-v5.5-20260330.md |
| Supersedes | WA-Reference-v5.4-20260330.md |
...
**Change Control Note — v5.5**

| **Change** | **Detail** |
| --- | --- |
| Section 18 | New section added: Document Preparation and Validation Standard — governing criteria for instruction readiness, gap discipline, cross-document consistency, failure path requirements, dry run gate assessment, and version control rules |
...
**Engine-derived fields — Phase 1 state only:** The following fields are populated by the Session A extraction engine and reflect the state at Phase 1 completion. They are NOT updated by the Session B pipeline. Live counts must be derived from queries against mti_terms and wa_term_inventory, not from these fields.
- `phase1_term_count` — number of terms extracted in Phase 1
- `phase1_verse_count` — number of verses in the Phase 1 extract
- `unique_term_count` — terms whose Strong's number appears only in this registry (engine-derived)
- `shared_term_count` — terms shared with other registries (engine-derived)
- `term_sharing_ratio` — shared/(unique+shared), 0.0 to 1.0 (engine-derived)

**Redundant field — inference_note:** This field exists in word_registry but no pipeline stage writes to it. It may contain researcher-set pre-existing content (conceptual gap notes set at programme intake) which must be preserved. Pipeline operations must never overwrite it. It is retained in exports for researcher reference.
...
**Field note — status_note:** This field is written by the Session A engine only (audit_word step A10). The Session B pipeline does not write to mti_terms.status_note. The Analysis closing patch (ANALYSIS type) previously contained a bulk_update writing this field — this was removed in Analysis Instruction v5.4. All term-level analytical results go to wa_term_inventory.evidential_status via the Extraction patch.
...
**Redundant fields — do not write in Session B pipeline:**
- `god_as_subject`: superseded by the mti_term_flags mechanism (flag_id 1 = GOD_AS_SUBJECT). The flag is the authoritative record. This column is not updated by any pipeline stage.
- `somatic_link`: superseded by mti_term_flags (flag_id 3 = SOMATIC_INNER_LINK; flag_id 4 = BODY_INNER_EXPRESSION). The flags are the authoritative record. This column is not updated by any pipeline stage.
- `status_note`: NULL across all active records. No documented pipeline purpose. Not written by Session B. Retained in schema for historical compatibility.
...
This section governs how instruction documents and guidance are prepared, reviewed, and declared ready for use. It applies to all instruction documents in the Framework B programme. It must be read and applied before any instruction document is declared complete, before any gap is marked closed, and before any dry run or live pipeline execution is recommended.
...
Before declaring any instruction document ready, identify every inflection point (IP) governed by that instruction. An IP is any point where data moves between the pipeline and the database — either a DB read (data extracted to construct a JSON or inform a decision) or a DB write (patch applied, status updated, record inserted or updated).

Each IP must be assessed against four criteria:
...
| JSON structure | Where JSON is the carrier — the structure must be pre-defined in the instruction or referenced to an Annexure |

A gap exists if any criterion is not met in the current instruction text. The instruction is not ready until all IPs are fully specified or explicitly assigned to Claude Code with the necessary criteria provided.

**SQL policy:** SQL queries and scripts are Claude Code's responsibility to construct. Claude AI instructions must provide: field names, table names, join logic, derivation rules, expected outcomes, and a clear handoff statement to Claude Code. Where these are present and a handoff statement exists, the gap is closed regardless of whether explicit SQL is written in the Claude AI instruction.

---

## **18.2 Gap Status Discipline**

A gap is closed only when the instruction document itself contains the correct and complete information. The following do not close a gap:

- A researcher decision recorded in a session log
- A verbal agreement in a chat session
- A note in a gap register saying "decision made"
- A prior version of the instruction that has not been updated

A researcher decision creates a remediation action. The action is to update the instruction. Until the instruction is updated, the gap remains open. The gap register must distinguish between OPEN, PENDING UPDATE (decision made; instruction not yet updated), and CLOSED.

When an instruction is updated to address a gap, the gap register entry must be updated to CLOSED with a reference to the specific document version and section that now contains the correct information.

---

## **18.3 Instruction Update Verification**

After producing an updated instruction document:

1. Verify the key changes are present in the output file using grep or direct inspection. Do not rely on having made an edit — confirm it is in the file.
2. Check that the change note accurately describes what changed and why.
3. Check that the Supersedes reference points to the correct prior version.
4. Check that any cross-references within the document (section references, document references) point to the correct current versions.

If a verification step fails: correct the document before presenting it. Do not present a document that has not been verified.

---

## **18.4 Cross-Document Consistency**

Where two or more documents govern overlapping content, verify consistency before any document is declared ready. Overlapping content includes: controlled vocabulary, status transition rules, patch operation types and templates, field names and table names, version references to companion documents.

A change in one document must be reflected in all documents that reference or depend on that content. When a document is updated, check the full companion document list for that instruction and verify each companion does not contradict the change.

The patch_specification is the authoritative source for operation types and applicator behaviour. The WA-Reference is the authoritative source for controlled vocabulary and schema. No instruction document may define terms or operations that conflict with these authorities.

---

## **18.5 Failure and Recovery Paths**

Every instruction that produces a patch must specify what happens when that patch is rejected. The minimum required content is:

- The condition that indicates rejection (applicator returns error, patch_id conflict, validation failure)
- The immediate action required (produce failure patch before any other action)
- The failure patch format (see WA-SessionB-ClaudeCode-Instructions Section 16)
- The recovery procedure (diagnose cause, produce corrected patch with new patch_id, await researcher confirmation)

Every pipeline re-run scenario must have a defined REPAIR patch with:
- A named patch_id convention
- The specific fields to reset on word_registry
- The specific tables whose records must be delete_flagged or reset
- The pipeline stage to restart from after the patch is applied

These are not optional additions. A pipeline without documented failure paths is incomplete and must not be declared ready.

---

## **18.6 Dry Run Gate Assessment**

Before recommending a dry run or live pipeline execution, produce an explicit gate assessment. The assessment must:

1. Identify all gaps that are BLOCKING or MISSING for the specific word or pool being run.
2. For each such gap, confirm it is CLOSED in the current instruction text — not just decided.
3. State explicitly which gaps do not apply to this specific run (e.g. pool membership gaps do not apply to a not-shared single word).
4. Confirm the document versions in use match the current instruction suite.

If any BLOCKING gap remains open in the instruction text: the dry run recommendation must not be made. Record the outstanding items and the actions required before the gate can clear.

**The dry run gate is a statement about instruction text, not about decisions made.** It cannot be cleared by session log entries alone.

---

## **18.7 Version Control**

Every instruction document update must carry:

| Field | Requirement |
| --- | --- |
| Version number | [major].[minor] format. Minor increment for targeted updates to existing content. Major increment for substantive rewrites or new instructions. |
| Supersedes | Exact filename and version of the document being replaced |
| Change note | Specific description of what changed and why — not a summary of the document. Each change listed separately with the section affected. |
| Filename | Must include the version number: e.g. WA-VerseContext-Instruction-v1.5-20260330.md |

Version numbers in the document header, filename, and footer must all be consistent. A document that shows v1.3 in the header and v1.2 in the footer is not a valid output.

Documents are produced in a single session where possible. Where a document requires multiple sessions to complete, the version number is not incremented until the document is finalised and all verifications pass.

---

## **18.8 Application**

This standard applies to Claude AI in all sessions where instruction documents are being prepared, reviewed, or updated. It also applies when reviewing whether a pipeline stage is ready to proceed.

When Claude AI is asked whether instructions are ready, or whether a dry run can begin, the response must be grounded in this standard — not in confidence about the work done, and not in the absence of known problems. The answer is found by checking the instruction text against the criteria above.

If a prior session declared instructions ready without applying this standard, that declaration should be treated as provisional. The instructions should be re-assessed before execution proceeds.

---

*WA-Reference-v5.5 | 20260330 | Schema v3.8.0 | Supersedes WA-Reference-v5.4-20260330.md | Section 18 added: Document Preparation and Validation Standard*

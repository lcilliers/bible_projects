...
**Version 3.1 | 20260414 | Status: Active governing instruction**

| | |
|-|-|
| **Filename** | wa-dimensionreview-instruction-v3_1-20260414.md |
| **Supersedes** | wa-dimensionreview-instruction-v3_0-20260414.md |
| **Governing rules** | wa-global-general-rules-v1-2026-04-13.json — must be loaded and confirmed before beginning any session. Rules are referenced by ID throughout; the global file is authoritative in any conflict. |
| **Companion documents** | WA-VerseContext-Instruction-v3.1-20260414 · WA-SessionB-Instruction-v4.8-20260414 · WA-Reference-v5.5-20260330 · WA-Registry-Management-Guide-v5.8-20260412 · patch_specification-v1.10-20260412 · WA-SessionD-Orientation-v3.0-20260412 |
...
**Step 5 — Re-run trigger.** If VCB re-run triggered: governed by WA-VerseContext-Instruction-v3.1. Dimension Review for affected registry resumes after re-run completes.
...
**Purpose:** Realign the group description from term-centric to characteristic-perspective, following the model established in WA-VerseContext-Instruction-v3.1 Section 4. The fix-or-stop principle applies throughout.
...
- **Grouping is wrong — verses need re-reading and re-grouping:** trigger a VCB re-run under WA-VerseContext-Instruction-v3.1. Issue a return instruction (Section 6.7). The Dimension Review for this registry pauses.
...
Date: {YYYYMMDD} | Instruction version: WA-DimensionReview-Instruction-v3.1-20260414
...
## Return destination
{One of:}
- VCB re-run: Re-run Verse Context for the listed terms under WA-VerseContext-Instruction-v3.1.
...
2. **Global rules confirmed.** State: *"Global rules wa-global-general-rules-v2-20260414.json loaded."*
...
1. **Increment observations log version.** Before constructing the patch, write the current registry's STAMP entry and SESSION-END marker to the observations log. Produce a new version file (increment by 0.1). This ensures the patch reads from a stable, complete snapshot.
...
Governing instruction: WA-DimensionReview-Instruction-v3.1-20260414
...
**`session_b_status: null` — formal note:** The Dimension Review does not advance the `session_b_status` workflow. The correct value is always `null`. The patch applicator must not reject DIMREVIEW patches for having `null` in this field — this is a registered exception in patch_specification-v1.10.
...
|-|-|-|-|
| `DIMREVIEW` | WA-DimensionReview-Instruction-v3.1 | `null` | `wa_dimension_index` updates; `verse_context_group` corrections; `wa_session_b_findings` inserts; `wa_session_research_flags` inserts; `word_registry` stamp update (one registry per patch); `wa_dim_review_cluster_log` insert (final registry patch only) |
| `DIMREVIEW-GRPDESC` | WA-DimensionReview-Instruction-v3.1 | `null` | Out-of-session group description corrections only |
...
**Observations log versioning:** version increments by 0.1 on every write. The filename is never reused. Previous versions remain on disk.

---

*WA-DimensionReview-Instruction-v3.1-20260414 | Supersedes wa-dimensionreview-instruction-v3_0-20260414.md | v3.1: global rules coverage corrections — see Change note above*

...
6. **(v3_5 A-06) Operation type matches row presence.** For every `verse_context` op: if the header's Existing-vc-rows signal indicates no active row for the verse_record_id, the op MUST be `insert`; if it indicates an active row, the op MUST be `update`. A mismatch will be rejected by the applicator's rowcount gate at apply. Classifier must verify before submission. Same rule applies to `verse_context_group` ops (insert for new group_code; update for existing group_code).
7. **(v3_5 A-08) Every term in the session appears in exactly one VCNEW or VCREVISE `terms_covered` entry** (or both, for MIXED terms per §6.3 Step 3). Specifically:

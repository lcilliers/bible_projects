# wa-sessionb-cc-instructions-v3_4-20260416

> Claude Code consolidated instruction document for Framework B.
> Version 3.4 — 20260416.
> Supersedes: wa-sessionb-cc-instructions-v3_3-20260414.md.
...
This instruction is governed by **wa-global-general-rules-v2_4-20260416.json**.

Claude Code must load and apply this file at the start of every session. Key rules that apply to Claude Code's operations:

- **GR-LOAD-001**: Load global rules before beginning any operation.
- **GR-DIR-001**: There are two and only two mechanisms for instructing Claude Code to make database changes: patches (JSON format) and directives (plain language). Claude Code executes; Claude AI determines what should be done.
- **GR-DIR-002**: Every directive Claude AI sends must contain all five required elements: DIRECTIVE ID, MOTIVATION, SCOPE, OUTCOME REQUIRED, and COMPLETION CONFIRMATION. Claude Code must reject any directive missing one or more of these elements and report the missing elements to Claude AI before proceeding.
- **GR-DIR-005**: Every patch and directive must specify the completion confirmation that Claude Code must return. Claude Code returns the confirmation output to Claude AI and the researcher. The operation is not complete without a returned confirmation.
- **GR-DIR-008**: Claude AI verifies all five GR-DIR-002 elements are present before submitting any directive. If Claude Code receives a directive that fails this check, it reports the missing elements rather than attempting to execute.
- **GR-OBS-005**: No database record is physically deleted. Flagging with `delete_flagged = 1` is the authoritative mechanism for all record retirement. Claude Code must never execute DELETE statements against analytical records.
- **GR-DATA-001**: All SQL queries against mti_terms that are intended to return active terms must include `AND mt.status IN ('extracted', 'extracted_thin')`.
- **GR-PROC-003**: All database changes go through patches reviewed by the researcher. Claude Code applies patches; it does not initiate database changes independently.
- **GR-PROC-004**: No patch is applied without researcher review and confirmation.

Rules stated in the global file are not repeated in full here — the rule IDs above are the authoritative references.
...
**Patch filename:** `wa-{nnn}-analysis-v1-{YYYYMMDD}.json` | **patch_id:** `PATCH-{YYYYMMDD}-{nnn}-ANALYSIS-V1`
...
All files follow the programme-wide naming rules GR-FILE-001 through GR-FILE-009 (lowercase, compact date YYYYMMDD, version numbered, no overwrites). The CC-specific scope tokens below extend the general pattern.
...
|---|---|---|---|
| Patch filename | `wa-{nnn}-{type}-v{n}-{YYYYMMDD}.json` | Human-readable file on disk — lowercase, GR-FILE-001/007/009 compliant | Filesystem |
...
- `wa-097-preanalysis-v1-20260327.json` — first pre-analysis patch
- `wa-097-preanalysis-v2-20260327.json` — correction to first patch
- `wa-097-sessionb-v1-20260327.json` — session B patch
- `wa-097-repair-vc-rerun-v1-20260327.json` — repair patch
...
**VERSECONTEXT** (filename: `wa-vcb{nnn}-versecontext-v1-{YYYYMMDD}.json` | patch_id: `PATCH-{YYYYMMDD}-VCB{nnn}-VERSECONTEXT-V1`):
...
**VCGROUP** (filename: `wa-vcgroup{group_id}-v1-{YYYYMMDD}.json` | patch_id: `PATCH-{YYYYMMDD}-VCGROUP{group_id}-V1`):
...
**VCVERSE** (filename: `wa-vcverse{verse_record_id}-v1-{YYYYMMDD}.json` | patch_id: `PATCH-{YYYYMMDD}-VCVERSE{verse_record_id}-V1`):
...
**Patch filename:** `wa-{nnn}-repair-step-rerun-v1-{YYYYMMDD}.json` | **patch_id:** `PATCH-{YYYYMMDD}-{nnn}-REPAIR-STEP-RERUN-V1`
...
**Patch filename:** `wa-{nnn}-repair-auditword-rerun-v1-{YYYYMMDD}.json` | **patch_id:** `PATCH-{YYYYMMDD}-{nnn}-REPAIR-AUDITWORD-RERUN-V1`
...
**Patch filename:** `wa-{nnn}-repair-vc-rerun-v1-{YYYYMMDD}.json` | **patch_id:** `PATCH-{YYYYMMDD}-{nnn}-REPAIR-VC-RERUN-V1`
...
**Patch filename:** `wa-{nnn}-repair-analysis-rerun-v1-{YYYYMMDD}.json` | **patch_id:** `PATCH-{YYYYMMDD}-{nnn}-REPAIR-ANALYSIS-RERUN-V1`
...
**Failure patch filename:** `wa-{nnn}-repair-failure-v1-{YYYYMMDD}.json` | **patch_id:** `PATCH-{YYYYMMDD}-{nnn}-REPAIR-FAILURE-V1`

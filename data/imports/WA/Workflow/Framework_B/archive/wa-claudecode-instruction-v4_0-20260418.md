# wa-claudecode-instruction-v4_0-20260418

> Framework B Soul Word Analysis Programme — Claude Code Operating Guide
> Version: v4_0 | Date: 20260418
> Supersedes: wa-sessionb-cc-instructions-v3_6-20260416.md (and lineage v3_2 through v3_6)
...
- Patch format, patch application, REPAIR catalogue, failure patches → wa-patch-instruction v2_0
- Directive format, directive execution → wa-directive-instruction v1_0
- Controlled vocabulary → wa-reference v5_6 §2–§11
- Schema field definitions → wa-reference v5_6 §13
- File naming rules → GR-FILE-001 through GR-FILE-009; wa-reference v5_6 §1
- Programme-wide binding rules → wa-global-general-rules v2_11
...
| GR-DATA-001 (per wa-reference v5_6 §13.2) | All CC queries against `mti_terms` for active terms include `AND mt.status IN ('extracted', 'extracted_thin')` |
| GR-DATA-003 (per wa-reference v5_6 §13.2) | `mti_term_flags` is the authoritative field for somatic classification, not `wa_term_inventory.somatic_link` |
| No physical deletion (per wa-patch-instruction v2_0 §5.4) | CC never executes DELETE statements against analytical records; `delete_flagged = 1` only |
| Patch format self-check (per wa-patch-instruction v2_0 §7) | CC may reject patches that fail the self-check on receipt — reports which of the six points failed |
| Directive five elements (per wa-directive-instruction v1_0 §3) | CC may reject directives missing any of the five required elements — reports which are missing |
| Completion confirmation (per wa-patch-instruction v2_0 §6 and wa-directive-instruction v1_0 §6) | CC returns the specified confirmation before the operation is considered closed |
...
- Patch application workflow → wa-patch-instruction v2_0 §3–§7
- REPAIR patch catalogue → wa-patch-instruction v2_0 §9
- Failure patch → wa-patch-instruction v2_0 §10
- Controlled vocabulary tables → wa-reference v5_6 §2–§11
- Schema additions and housekeeping rules → wa-reference v5_6 §13, §15
- File naming convention table → wa-reference v5_6 §1
...
All classification, scope judgement, and analytical work belongs to **Claude AI**. Claude Code executes database operations based on validated instructions — patch files (wa-patch-instruction v2_0) and directive files (wa-directive-instruction v1_0).
...
- No physical deletes (per wa-patch-instruction v2_0 §5.4) — deletions flagged with `delete_flagged=1`
...
Any future schema changes are produced as migration scripts (`migrate.py`) and executed by CC with researcher approval. New tables or columns introduced after this document's date should be added to wa-reference v5_6 §13.
...
After an analysis completion patch is applied and confirmed, two additional files are produced per wa-patch-instruction v2_0 §8:
...
CC runs these queries to assess programme state. Vocabulary values per wa-reference v5_6 §3, §3a.
...
Full integrated instruction for both Claude AI and Claude Code: wa-versecontext-instruction v2_7. This section contains CC's operational rules. Patch format for all three VC patch types per wa-patch-instruction v2_0 §12.
...
- Output filename: `wa-vcb{nnn}-extract-v{n}-{YYYYMMDD}.json` per wa-reference v5_6 §1.2
...
Run after every Verse Context patch. Per wa-patch-instruction v2_0 §12.6.
...
Produced on researcher instruction after cluster registries reach Pre-Analysis Complete. Pool architecture has been retired (see wa-registry-management-guide v5_9 §5.2); Session B operates on individual words in cluster order. Retained here for dataset construction mechanics.

**Filename:** `wa-{cluster}-analysis-v{n}-{YYYYMMDD}.json` per wa-reference v5_6 §1.3.
...
- Pool membership: `word_registry` (cluster_assignment + processing sequence from wa-registry-management-guide v5_9)
...
Per wa-patch-instruction v2_0 §11.6:
...
A re-run without the preceding REPAIR patch risks leaving analytical outputs in an inconsistent state relative to the new verse corpus. REPAIR patch specifications are in wa-patch-instruction v2_0 §9.
...
- Patch files (wa-patch-instruction v2_0 format) — complete, validated before submission
- Directive files (wa-directive-instruction v1_0 format) — complete, validated before submission
...
Per wa-patch-instruction v2_0 §6 and wa-directive-instruction v1_0 §6. CC returns the completion confirmation as specified in the patch or directive. Not a general acknowledgement — the specific query result or state check.
...
*wa-claudecode-instruction-v4_0-20260418 | Supersedes wa-sessionb-cc-instructions-v3_6-20260416 and its lineage | Patch and directive content moved to dedicated instructions; vocabulary and schema duplication removed via GR-REF-001 pointers | Major version bump reflects document-scope rewrite*

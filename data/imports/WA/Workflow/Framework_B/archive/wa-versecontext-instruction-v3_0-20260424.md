...
|Inputs|**Primary:** Session A `.md` per registry — `wa-{nnn}-{word}-session_a-{date}.md` produced by `scripts/build_session_a_prose.py` from `data/exports/session_a/`. Self-contained, human- and AI-readable, carries all STEP-sourced data + prior verse_context state. │ **Fallback (split-registry only):** Verse Context batch JSON — `wa-vcb-{batch\_id}-extract-{date}.json` for large registries where classification must be split across sessions. │ **Source material (Claude Code internal):** engine `--export-word` JSONs in `data/exports/STEP Extracts/`; live database.|
...
|Claude Code role|Session A `.md` rendering from database (primary); batch JSON construction for split-registry cases (fallback); patch application; group_code resolution; XREF status propagation; registry status management; orphan-group validation; integrity validation; completion reporting; Session A `.md` re-render on registry completion|
|Interaction model|Session A `.md` rendered from current DB state → programme-prose + instruction loaded; prior-state posture declared → Claude AI classifies; per-term self-check + orphan-group check on re-evaluation terms → Claude Code applies patch, validates, advances status, re-renders Session A `.md` → Dimension Review gate opens|

**File-naming note:** All VC session outputs use the `wa-vcb-{batch\_id}-...` pattern. For per-registry classification, a session's `batch_id` is a sequential VCB-{nnn} identifier assigned by Claude Code; it does not imply multi-registry scope. Per-registry work is the default; multi-registry batching is an exception when many small registries are processed together. The word-based alternative naming scheme considered in the v2 alignment analysis was dropped in favour of consistency.
...
**Classifier input — the Session A `.md`:** Claude Code renders the per-registry Session A `.md` via `scripts/build_session_a_prose.py` from the current database state. The `.md` is the classifier's sole input for a per-registry session. It is self-contained — it carries all STEP-sourced data (terms, meanings, verse text), any existing `verse_context_group` rows (active and dissolved), and per-verse prior classification state where records exist. It states the **prior-state posture** (FRESH or RE-EVALUATION) in its header based on counts of active prior groups across the registry's OWNER terms. See §5 for input preparation details and `docs/prose-store-architecture.md` for the underlying prose-store model.

**Legacy batch JSON — split-registry fallback only:** For registries too large for a single classification session, Claude Code may split the work across multiple `.md` slices or fall back to the legacy batch JSON (`wa-vcb-{batch_id}-extract-{date}.json`) that pre-dated the `.md` renderer. The batch JSON schema is retained in Annexure C for this case. The batch JSON is not used for per-registry sessions.

**Database sources Claude Code reads to produce the `.md` (or the batch JSON fallback):**
...
|Claude Code|Renders the per-registry Session A `.md` from the live database (primary input for the classifier). Produces a legacy batch JSON where a split-registry session requires one. Applies verse context patches. Resolves group\_code to integer id. Handles XREF coverage. Updates registry verse\_context\_status. Re-renders the Session A `.md` on completion. Validates consistency rules including orphan-group validation. Runs integrity checks. Reports completion. Does not assess verse relevance or produce contextual descriptions.|
...
The classifier's primary input is the **Session A `.md` per registry**. Claude Code renders it from the live database via `scripts/build_session_a_prose.py`. The `.md` is self-contained, carries all STEP-sourced data and any prior `verse_context` state, and is human- and AI-readable. For very large registries where a single session cannot carry the full classification load, Claude Code falls back to term-level slicing across multiple `.md` documents or — for historical continuity — the legacy batch JSON described in Annexure C.

### 5.1 Trigger

Claude Code renders (or refreshes) a registry's Session A `.md`:

* At the start of every classification session for that registry
* After a VERSECONTEXT patch is applied (to reflect new DB state)
* When researcher instructs a fresh render (e.g. after an `audit_word` re-run for the registry)
* Before a Dimension Review hand-off (the re-rendered `.md` is the DimReview input substrate)

### 5.2 Input selection criteria

For a per-registry session (the default):
...
* **All such terms in the registry** — the `.md` carries every qualifying OWNER term, not a subset
* **Existing `verse_context` state rendered** — `verse_context_group` (active and dissolved) and `verse_context` (per verse) included so the classifier can review and revise; drives the prior-state posture declaration at §6.1
* **Batch ID assigned by Claude Code** — sequential VCB-{nnn} identifier for the session's output files (patch, observations, session log, flags register). Per-registry work is the default; `batch_id` is a session counter, not a multi-registry aggregator

For split-registry / multi-registry sessions (exception path):

* **Split only when a single session's `.md` would exceed practical session-context budget** (rough guide: >3,000–3,500 active verses, or the classifier explicitly requests a split due to term count)
* **Never split a term across input documents** — all verses for a single term must appear in one input
* **Split policy documented per case** — Claude Code records the split rationale and the registry's session sequence in the first session log produced for the registry
...
### 5.3 Input document structure — the Session A `.md`

The Session A `.md` is organised under six seeded `prose_section_type` handles (`sa_s1_d1` … `sa_s1_d6`): Word Summary, Meaning, Verses, Terms, Pointers, Questions. The header carries a **Prior-state posture** block (FRESH or RE-EVALUATION with counts) and a **Method reminder** quoting the §3 filter and §6.2 Step 3 grouping model. The Verses section, per OWNER term, carries full ESV verse text, target word form, span-match indicator, and any Prior classification state (with group_code, is_relevant, is_anchor, is_related, set_aside_reason, notes, delete_flagged). Existing `verse_context_group` rows — active and dissolved — are rendered in a per-term summary table.

The authoritative structural reference is `docs/prose-store-architecture.md` §5 (prose store stages) and the renderer source `scripts/build_session_a_prose.py`. The `.md` is deterministic — running the renderer twice on unchanged DB state produces byte-identical output (modulo the generation timestamp).

**Source-table mapping** — the fields rendered in the `.md` derive from the database tables listed in §0.1 using the same source-table mapping that populated the legacy batch JSON. Full mapping with derivation rules is in **Annexure C** for reference and for use when rebuilding the fallback batch JSON.

### 5.4 Output file

**Primary output:** `data/exports/session_a/wa-{nnn}-{word}-session_a-{date}.md` — the per-registry Session A `.md`. Regenerated on demand; same-day regenerations overwrite (deterministic content). Handed to the Claude AI classification session.

**Fallback output (split-registry / legacy only):** `data/exports/verse_context/wa-vcb-{batch_id}-extract-{date}.json`. Produced only when a session cannot be run from a single `.md` document. Schema in Annexure C.
...
On receiving the classification input (Session A `.md` for per-registry sessions, or legacy batch JSON for split-registry sessions):
...
3. Load and parse the classification input (Session A `.md` — primary — or batch JSON where directed by Claude Code's hand-off).
4. **Prior-state posture declaration — mandatory.** Using the counts in the input's "Prior-state posture" block (from the Session A `.md` About section, or derived equivalently from a batch JSON), state verbatim one of the two templates below.
    * **FRESH posture** (no prior records for any OWNER term in the input):
    ...
    * **RE-EVALUATION posture** (any prior records exist):
    ...
    If the counts `n / total / m / d` do not match the counts visible in the input document, stop and flag — the input may be mis-rendered.
5. State the startup summary (Annexure A template).
6. Note any terms marked `term_classification_complete: true` — review these but do not re-classify unless a revision is clearly warranted (see revision rule in Section 6.2 Step 2).
7. Note any terms with existing `verse_context` records but `term_classification_complete = false` — these are partially classified and must be flagged to researcher before continuing (see partial completion rule in Section 6.2 Step 2).
8. **Processing order:** Terms are processed in registry sequence, ascending by registry number. Within a registry, terms are processed in the order they appear in the input document. This order is fixed and does not require researcher confirmation at start-up.
...
This instruction document is long. When loading it alongside a large Session A `.md` (or, in the fallback path, a batch JSON) and an observations file, the combined context load is significant. Claude AI should read the instruction once at session start and not reload it unless a specific section is needed. For patch construction sessions, load only the observations file, the Session A `.md` (or fallback batch JSON), and Sections 7–7.6 of this instruction — not the full document.
...
* **Orphan-group validation** — for every pre-existing active `verse_context_group` for every term in the session's input, either (a) at least one `verse_context` operation in this patch leaves the group with ≥ 1 active verse (`is_relevant = 1`, `delete_flagged = 0`), or (b) an `update` operation on the group sets `delete_flagged = 1`. A pre-existing active group reaching zero active verses with no dissolve operation is a validation failure. See §6.2 Step 6 orphan-group check.
* \_patch\_summary counts match the actual operation counts

**Programmatic validation — required for large sessions (>50 terms):** When the patch is produced in a deferred patch construction session (see Section 6.4), validation must be performed programmatically against the input document before the patch file is written. Specifically:
...
**Claude Code expectation — Verse Context input preparation after a reset:** When Claude Code prepares Verse Context input (the Session A `.md`, or a fallback batch JSON) for a registry that previously had verse_context records and has since been reset, the preparation routine must:
...
## Annexure C — Legacy Batch JSON Schema (fallback path)

Retained for split-registry sessions where a single Session A `.md` is impractical and classification must be carried on a JSON extract instead. This is the schema and source-table mapping previously published as §5.3 of v2_8.

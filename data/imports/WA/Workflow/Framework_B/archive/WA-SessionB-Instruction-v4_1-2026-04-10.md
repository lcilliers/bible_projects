...
| `unique_term_count` / `shared_term_count` / `term_sharing_ratio` | Consistent with terms data |
| `strongs_list` | Cross-check every Strong's number and count against active term records. Flag any deleted term still listed. Flag any count that does not match the term's verse records. |
...
| `active_term_count` | Count terms where `delete_flagged = 0` |
| `owner_term_count` | Count terms where `term_owner_type = OWNER` |
| `xref_term_count` | Count terms where `term_owner_type = XREF` |
...
| Deleted terms | If `status = delete` in mti block and term is still referenced in `strongs_list` → flag |
...
Also check: does `root_gloss` in the `wa_term_root_family` records match the `root_gloss` shown in `correlations.root_families` for the same root code? If not, record the discrepancy.
...
Read `correlations.xref_sharing`. For each partner registry listed, the shared strongs terms are drawn from the active term inventory. Check:
- Do the shared terms include any terms where mti `status = delete`? If so, flag — these terms should not be generating xref signals after deletion is applied and the extract is regenerated.
- Is the shared_term_count consistent with the number of items in `shared_strongs`?
...
### strongs_list corrections required?
[ ] Yes — [list corrections]
[ ] No
...
- `strongs_list` corrections (removing deleted terms, correcting counts)
...
- `strongs_list` no longer contains deleted terms
...
The five passes are conducted in sequence. Each pass is completed before the next begins. Findings from each pass are written to the observations log before the pass closes.
...
**Purpose:** Establish the full, data-confirmed picture of every correlation signal this word produces, so that Section 5 of the word study can be accurately written.

This pass reads the correlation data that is now present in the clean extract following Stage 1 remediation.

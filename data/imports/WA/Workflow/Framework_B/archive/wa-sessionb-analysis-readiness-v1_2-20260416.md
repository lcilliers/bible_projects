...
Version 1.2 | 20260416 | Status: Active**

| **Document** | **Value** |
|---|---|
| Filename | wa-sessionb-analysis-readiness-v1_2-20260416.md |
| Supersedes | wa-sessionb-analysis-readiness-v1_1-20260416.md |
| Companion documents | wa-global-general-rules-v2_4-20260416.json │ wa-dimensionreview-instruction-v[current].md │ wa-versecontext-instruction-v[current].md │ wa-sessionb-cc-instructions-v[current].md │ wa-patch-specification-v[current].md |
...
Claude AI must confirm the global rules file has been loaded before beginning any work. State aloud: *"Global rules wa-global-general-rules-v2_4-20260416.json loaded."*
...
- Global rules file: `wa-global-general-rules-v2_2-20260415.json`
...
| Type (a) patch | `wa-[nnn]-preanalysis-v[n]-[YYYYMMDD].json` | Constructed at Step 1.4 | Data correction patch. Compiled from the patch accumulator section of the observations log. Filename per GR-FILE-001/007/009; internal `patch_id` field retains `PATCH-[YYYYMMDD]-[nnn]-PREANALYSIS-V[n]` for applicator compatibility. |
...
3. Name the patch file: `wa-[nnn]-preanalysis-v1-[YYYYMMDD].json` (per GR-FILE-001/007/009). The internal `patch_id` in `_patch_meta` follows the format `PATCH-[YYYYMMDD]-[nnn]-PREANALYSIS-V1` for applicator compatibility — do not confuse filename with patch_id.
...
**Format compliance check (per GR-DIR-006 — mandatory before submission):**

Before presenting the patch for researcher approval, verify all six structural compliance points against the patch specification. Record the check result in the observations log as `Patch format compliance check: [PASS / FAIL — detail if FAIL]`.

| Point | Requirement |
|---|---|
| 1 | Top-level structure: exactly three keys — `_patch_meta`, `operations`, `_patch_summary` |
| 2 | `_patch_meta` completeness: `patch_id`, `registry_id`, `word`, `produced_date`, `produced_by`, `patch_type`, `session_b_status` (non-null for PREANALYSIS), `description` |
| 3 | `patch_type` value: `PREANALYSIS` (controlled vocabulary per patch spec Appendix A.6) |
| 4 | `session_b_status` value: `"Pre-Analysis Complete"` (required for PREANALYSIS type) |
| 5 | Operations format: each operation has `op_id` in `OP-NNN` format; `operation` is a valid lowercase named type from patch spec Section 3; `match` and `set` dicts present where required |
| 6 | `_patch_summary`: present; `total_operations` matches the length of the `operations` array |

A patch failing any point must be corrected and re-checked before submission. Do not submit a non-compliant patch.
...
*wa-sessionb-analysis-readiness-v1_2-20260416.md*
*Framework B — Soul Word Analysis Programme*

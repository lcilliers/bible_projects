# WA-DimensionReview-Instruction-v1_1-2026-04-06

**Framework B — Soul Word Analysis Programme**
**Dimension Review Instruction — Claude AI and Claude Code**
**Version 1.1 | April 2026 | Status: Active governing instruction**

| **Document** | **Value** |
|---|---|
| Filename | WA-DimensionReview-Instruction-v1.1-2026-04-06.md |
| Supersedes | WA-DimensionReview-Instruction-v1.0-2026-04-06.md |
| Change note | v1.1 — Four substantive additions following researcher review of v1.0: (1) Section 4.5 added: Group description correction protocol — group descriptions found to be inadequate during quality review may be corrected via patch, and may trigger a Verse Context batch re-run where verse-level decisions are affected; (2) Section 8 rewritten: Claude Code protocol and boundaries stated explicitly, separating Claude Code responsibilities from Claude AI responsibilities throughout; (3) Section 5.4 added: Session B and D pointer capture — analytical observations arising during dimension review must be captured via `wa_session_b_findings` and `wa_session_research_flags` inserts in the dimension patch; (4) Section 9 added: Data extracts specification — all database extracts required to support Dimension Review work are defined in JSON format with tables and fields from the schema. |
...
| Scenario | Action |
|---|---|
| Correction is clear and does not affect verse assignments | Claude AI produces a group description correction patch (Section 7.4) — updates `verse_context_group.context_description` and records rationale in `verse_context_group.notes` |
| Correction requires reviewing which verses belong in the group | Claude AI produces the group description correction patch AND flags that verse-level review is needed; researcher decides whether to trigger a Verse Context batch re-run for the affected terms |
...
JSON patch for Claude Code. One patch per cluster session (or per registry for targeted corrections). Follows `patch_specification-v1.6`. Patch type: `DIMREVIEW` (new type — see Section 8.5).

**Patch meta:**
```json
{
  "_patch_meta": {
    "patch_id": "PATCH-{YYYYMMDD}-DIM-{cluster}-V{n}",
    "cluster": "{C##}",
...
### 7.4 Group Description Correction Patch

Separate patch file when Phase B triggers a group description correction. Patch type: `DIMREVIEW-GRPDESC`.

```json
{
  "_patch_meta": {
    "patch_id": "PATCH-{YYYYMMDD}-DIMGRPDESC-{registry_no}-V{n}",
    "registry_id": {registry_no},
    "word": "{word}",
    "produced_date": "{ISO date}",
    "produced_by": "WA-DimensionReview-Instruction-v1.1-2026-04-06",
...
|---|---|---|---|
| `DIMREVIEW` | WA-DimensionReview-Instruction-v1.1 | `null` | `wa_dimension_index` updates; `wa_session_b_findings` inserts; `wa_session_research_flags` inserts |
| `DIMREVIEW-GRPDESC` | WA-DimensionReview-Instruction-v1.1 | `null` | `verse_context_group` description updates; `wa_dimension_index` context_description sync |
...
| Group verification extract | `wa-dim-grpverify-{group_code}-{date}.json` | `wa-dim-grpverify-112-003-2026-04-06.json` |
| Existing pointers extract | `wa-dim-existing-pointers-{cluster}-{date}.json` | `wa-dim-existing-pointers-C01-2026-04-06.json` |
| Cluster review assessment | `wa-dim-cluster-review-{cluster}-v{n}-{date}.md` | `wa-dim-cluster-review-C01-v1-2026-04-06.md` |
| Dimension refinement log | `wa-dim-refinement-log-v{n}-{date}.md` | `wa-dim-refinement-log-v1.0-2026-04-06.md` |
| Dimension patch | `wa-dim-patch-{cluster}-v{n}-{date}.json` | `wa-dim-patch-C01-v1-2026-04-06.json` |
| Group description correction patch | `wa-dim-grpdesc-patch-{registry_no}-v{n}-{date}.json` | `wa-dim-grpdesc-patch-112-v1-2026-04-06.json` |
| Session log | `wa-dim-session-log-{scope}-v{n}-{date}.md` | `wa-dim-session-log-C01-v1-2026-04-06.md` |

---

*WA-DimensionReview-Instruction-v1.1-2026-04-06 | Supersedes v1.0-2026-04-06 | v1.1: Group description correction protocol (Section 4.5); Claude Code protocol and boundaries (Section 8); Session B/D pointer capture (Section 5.4); Data extracts specification (Section 9)*

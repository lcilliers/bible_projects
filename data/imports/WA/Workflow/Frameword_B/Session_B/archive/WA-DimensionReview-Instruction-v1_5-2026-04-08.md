# WA-DimensionReview-Instruction-v1_5-2026-04-08

**Framework B — Soul Word Analysis Programme**
**Dimension Review Instruction — Claude AI and Claude Code**
**Version 1.5 | April 2026 | Status: Active governing instruction**

| **Document** | **Value** |
|---|---|
| Filename | WA-DimensionReview-Instruction-v1.5-2026-04-08.md |
| Supersedes | WA-DimensionReview-Instruction-v1.4-2026-04-08.md |
| Change note | v1.5 — Methodological correction to Section 9.1 (Cluster Extract). Anchor verses and related verses — including verse reference and verse text — are now mandatory fields in the cluster extract. Previously the extract carried verse counts only, making Phase B quality assessment and Phase C dimension discernment blind to the actual verse evidence. This is incompatible with the verse-first principle (Section 1.1). Change: Section 9.1 field mapping extended with `anchor_verses` and `related_verses` arrays per group, drawn from `verse_context` joined to `wa_verse_records`. Claude Code must include these fields in every cluster extract from v1.5 onwards. The group verification extract (Section 9.2) remains available for targeted deep review but is no longer the only mechanism by which verse text reaches Claude AI during Phase B/C. No other sections changed. |
...
```json
{
  ...
  "record": {
    ...
    "session_b_instruction": "WA-DimensionReview-Instruction-v1.5-2026-04-08"
  },
...
```json
{
  ...
  "record": {
    ...
    "session_raised": "WA-DimensionReview-Instruction-v1.5-2026-04-08",
    "raised_date": "{ISO date}",
    "resolved": 0
  },
...
*Derived from data. Working vocabulary as of v1.5. Will be revised as additional data is reviewed.*
...
```json
{
  "_patch_meta": {
    ...
    "produced_by": "WA-DimensionReview-Instruction-v1.5-2026-04-08",
...
|---|---|---|---|
| `DIMREVIEW` | WA-DimensionReview-Instruction-v1.5 | `null` | `wa_dimension_index` updates (dimension + dominant_subject); `wa_session_b_findings` inserts; `wa_session_research_flags` inserts; `verse_context_group` description corrections (when arising within a live session) |
| `DIMREVIEW-GRPDESC` | WA-DimensionReview-Instruction-v1.5 | `null` | `verse_context_group` description updates; `wa_dimension_index` context_description sync — **out-of-session use only** |
...
```json
{
  "extract_meta": {
    ...
    "produced_by": "Claude Code — WA-DimensionReview-Instruction-v1.5",
    "governing_instruction": "WA-DimensionReview-Instruction-v1.5-2026-04-08",
    "row_count": 0
  },
  "cluster_registries": [ ... ],
  "groups": [
...
*WA-DimensionReview-Instruction-v1.5-2026-04-08 | Supersedes v1.4-2026-04-08 | v1.5: Methodological correction — anchor verses and related verses (reference + text) added as mandatory fields in the cluster extract (Section 9.1); verse-first principle now enforceable at Phase B and Phase C without a separate CC directive; group verification extract (Section 9.2) retained for targeted deep review | No other sections changed*

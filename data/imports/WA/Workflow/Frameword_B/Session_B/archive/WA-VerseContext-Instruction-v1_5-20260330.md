# WA-VerseContext-Instruction-v1_2-20260329

**Framework B — Soul Word Analysis Programme**
**Verse Context Instruction — Integrated Claude AI and Claude Code**
**Version 1.2 | March 2026 | Status: Active governing instruction**

| **Document** | **Value** |
|---|---|
| Filename | WA-VerseContext-Instruction-v1.5-20260330.md |
| Supersedes | WA-VerseContext-Instruction-v1.4-20260330.md |
| Change note | v1.4 — Section 5.2: SQL policy statement added — SQL construction is Claude Code's responsibility; Claude AI instruction provides criteria, field names, and derivation rules (G01/G08). Section 6.1: all-verses-fail error rule added — if all verses for a term fail the relevance filter, this is an error condition; stop and flag to researcher with term details and reasons (G05). Section 6.1: partial completion rule updated — partial completion (term_classification_complete = false with existing records) must be flagged to researcher with reasons; batch must be fixed and resubmitted, not continued (G02). Section 6.1: revision documentation rule updated — when revision is clearly warranted, document reasons in notes field and report to researcher in per-term summary (G03). Section 6.2 Step 3: grouping criterion specified — new group warranted when context is materially different, meaning an analysis of the verses would arrive at a differential conclusion about how the inner-being characteristic operates (G04). |
| Companion documents | WA-VerseContext-SetupInstruction-v1 │ WA-Reference-v5.3 │ WA-VerseContext-ImpactStudy-v3 │ WA-SessionB-DataPrep-Instruction-v5.4 |
...
| Outputs | Verse Context batch JSON — wa-vcb-{batch_id}-extract-{date}.json (Claude Code → Claude AI) │ Verse Context patch — wa-vcb-{batch_id}-patch-{date}.json (Claude AI → Claude Code) │ Re-exported full word JSON per registry (Claude Code → DataPrep) |
...
```json
{
  ...
  "produced_by": "Claude Code — WA-VerseContext-Instruction v1.2",
  "governing_instruction": "WA-VerseContext-Instruction-v1.2-20260329.md",
  "total_verse_count": 0,
  "total_term_count": 0,
...
```json
{
  "_patch_meta": {
    ...
    "batch_id": "VCB-{nnn}",
    "produced_date": "{yyyy-mm-dd}",
    "produced_by": "WA-VerseContext-Instruction-v1.2",
...
Registries in this batch: {n}
  ...
  Still In Progress: {n} — {list: nnn—word, reason if known}
...
```
Verse Context v1.2 startup complete.
Batch: {batch_id}
Governing instruction: WA-VerseContext-Instruction-v1.2-20260329.md
...
*WA-VerseContext-Instruction-v1.5-20260330 | Supersedes v1.4-20260330 | Section 11.5: REPAIR patch requirement; CC reset expectations for audit_word and verse context batch preparation; Section 13.3: double-check verification after Complete*

# WA-DimensionReview-Instruction-v2.1-2026-04-10

**Framework B — Soul Word Analysis Programme**
**Dimension Review Instruction — Claude AI and Claude Code**
**Version 2.1 | April 2026 | Status: Active governing instruction**

| **Document** | **Value** |
|---|---|
| Filename | WA-DimensionReview-Instruction-v2.1-2026-04-10.md |
| Supersedes | WA-DimensionReview-Instruction-v2.0-2026-04-10.md |
| Change note | v2.1 — Two additions. (1) Incremental session discipline mandated (Section 6.1, 6.2, 6.3, 6.4, 6.6): the C17 Dimension Review session (2026-04-10) demonstrated that large clusters cannot be processed reliably in a single session. The phase is now the minimum unit of work; Phase A, Phase B, and Phase C are each separate sessions; within a single registry, phases are also done separately; the observations log is written to disk and downloaded at the end of every session and is the sole persistent record carried into the next session; Claude AI must never attempt to complete an entire cluster or registry across all phases in one session; a new Section 6.6 Session Boundary Protocol defines the mandatory stop-and-download discipline. Section 6.1 updated. Section 6.2 file writing discipline updated. Section 6.3 session log updated. Section 6.4 startup protocol Step 8 added. (2) Analysis Complete exclusion rule added (Section 3.2, 4.1, 4.6, 5.1, 6.4 Step 6, 7.1, 7.2): any registry in a cluster extract with `session_b_status = "Analysis Complete"` has completed the full word analysis programme and must be excluded from all Phase B and Phase C processing. These registries are noted in Phase A for cluster coherence context but their groups receive no QA flags, no dimension assignments, and are not included in coverage verification counts or patch operations. A new `[EXCLUDED]` observations log entry format is defined. The exclusion is identified at startup (Section 6.4 Step 6) and recorded before Phase A begins. All instruction version strings updated to v2.1. |
...
Date: {date} | Instruction version: WA-DimensionReview-Instruction-v2.1-2026-04-10
...
*Derived from data. Working vocabulary as of v2.1. Will be revised as additional data is reviewed.*
...
- **At the end of every session, present the observations log file for researcher download before stopping.** The researcher must download the log before the next session begins. This is non-negotiable — the observations log is the sole persistent record carried across session boundaries. Analytical work not written to a downloaded log is at risk of being lost.
- **At the start of every session (except the very first), the researcher uploads the existing observations log.** Claude AI reads it in full before beginning any new analytical work. Claude AI does not re-derive or re-run any analysis already recorded in the log — it only appends.
...
- Produce a patch in the same session as analytical work — the patch session is always separate.
...
| Phase C — up to ~15 groups | 1 session |
| Phase C — 15+ groups | Split by term family; 1 session per ~15 groups |
| Patch construction | 1 session (reads completed observations log) |
...
Governing instruction: WA-DimensionReview-Instruction-v2.1-2026-04-10
...
dim_review_version: WA-DimensionReview-Instruction-v2.1-2026-04-10
...
```json
{
  "_patch_meta": {
    ...
    "produced_by": "WA-DimensionReview-Instruction-v2.1-2026-04-10",
...
**Registry Mode patch_id pattern:** When run as a Session B Stage 1 sub-process targeting a single registry, use:
```
PATCH-{YYYYMMDD}-DIMREVIEW-{cluster}-REG{nnn}-V{n}
```
Example: `PATCH-20260410-DIMREVIEW-C17-REG068-V1`

The cluster code in the patch_id reflects the cluster the target registry belongs to, not the scope of Phase C.
...
```json
{
  ...
  "description": "Registry {nnn} ({word}): Dimension Review complete under v2.1"
}
```
...
```json
{
  ...
  "record": {
    ...
    "instruction_version": "WA-DimensionReview-Instruction-v2.1-2026-04-10",
    "registry_count": {n},
    "group_count": {n},
    "anchored_count": {n},
...
|---|---|---|---|
| `DIMREVIEW` | WA-DimensionReview-Instruction-v2.1 | `null` | `wa_dimension_index` updates; `verse_context_group` corrections; `wa_session_b_findings` inserts; `wa_session_research_flags` inserts; `word_registry` stamp updates; `wa_dim_review_cluster_log` inserts |
| `DIMREVIEW-GRPDESC` | WA-DimensionReview-Instruction-v2.1 | `null` | Out-of-session group description corrections only |
...
**`produced_by`:** `"Claude Code — WA-DimensionReview-Instruction-v2.1-2026-04-10"`
...
| `dim_review_version` | `word_registry` | `WA-DimensionReview-Instruction-v2.1-2026-04-10` |

# WA-SessionB-ClaudeCode-Instructions-v3_2-20260330

> Claude Code consolidated instruction document for Framework B v5.
> Version 3.2 — 2026-03-30.
> Supersedes: WA-SessionB-ClaudeCode-Instructions-v3.1-20260330.md.
> Change note v3.2: Section 2.4 (audit_word) expanded — REPAIR patch verification requirement before re-run; STALE_TERM mechanism as the authoritative path for wa_term_inventory updates on re-run; CC expectation to verify REPAIR patch applied before proceeding. Section 15 added — REPAIR Patch Catalogue: all four cascade reset REPAIR patches specified with full field-level operations (STEP extraction re-run, audit_word re-run, Verse Context re-run, Analysis re-run). Section 16 added — Failure Patch Catalogue: failure patch structure and when to produce.
...
When Stage 2 Session B Analysis begins, produce pool analysis datasets on researcher instruction.

**Trigger:** All words in a pool have reached Pre-Analysis Complete (confirmed by pool readiness check in Section 9 of WA-SessionB-DataPrep-Instruction-v5.5).

**File naming:** `wa-pool-{pool_id}-analysis-{date}.json`

**Full JSON structure specification:** WA-SessionB-Analysis-Instruction-v5.5 Annexure B.

**Pool membership:** Pool membership for single-cluster pools is derived from word_registry.cluster_assignment. For multi-cluster pools (e.g. Pool 1 sub-pools), pool membership is defined in WA-Registry-Management-Guide-v5.6 Section 7a — the pool_id controlled vocabulary table lists every pool_id with its exact word membership. Claude Code must reference Section 7a directly to determine the complete list of registry_no values in the pool.

**Pre-construction check — mandatory before assembling the dataset:**
1. Determine all registry_no values in the pool (from cluster_assignment or RMG Section 7a as appropriate)
2. Query word_registry to verify verse_context_status = 'Complete' for all pool registries
3. If any registry in the pool has verse_context_status != 'Complete': halt. Report to researcher listing which registries are not Complete. Do not proceed with dataset construction until all are Complete.
4. If all Complete: proceed with construction.

**Source tables for construction:**

- Pool membership: `word_registry` (cluster_assignment + pool processing sequence from WA-Registry-Management-Guide-v5.5 Section 7)
...
**After construction:** provide to Claude AI for Session B Analysis (WA-SessionB-Analysis-Instruction-v5.3).
...
All pipeline re-runs require a REPAIR patch to be applied and confirmed before the re-run begins. Claude Code applies these patches via apply_session_patch.py. The patch is produced by Claude AI or researcher instruction. Full specification: WA-PipelineStatusReview-v2-20260330.
...
**After patch:** Re-run Session B Analysis (re-assemble pool analysis dataset if needed). Then re-run Extraction.
...
```json
{
  "_patch_meta": {
    ...
    "produced_by": "{governing instruction}",
    "patch_type": "REPAIR",
...
*WA-SessionB-ClaudeCode-Instructions-v3.2 | Updated 20260330 | Schema v3.8.0 | Supersedes v3.1-20260330 | Section 2.4: re-run requirements; Section 15: REPAIR patch catalogue; Section 16: failure patch*

# WA Programme-Stage Prose — 2026-04-23
...
**Section types seeded:** 50  ·  **Content sections populated:** 50
...
*Section id 15 · status `draft` · v2 · 465 words · author `claude_ai`*
...
_`prog_purp_mission`  ·  type id 36  ·  sort 2_
...
*Section id 2 · status `draft` · v1 · 178 words · author `claude_ai`*
...
_`prog_purp_scope`  ·  type id 37  ·  sort 3_
...
*Section id 3 · status `draft` · v1 · 200 words · author `claude_ai`*
...
_`prog_purp_this_inner_being_programme`  ·  type id 38  ·  sort 4_
...
*Section id 4 · status `draft` · v1 · 395 words · author `claude_ai`*
...
_`prog_purp_defining_inner_being`  ·  type id 39  ·  sort 5_
...
*Section id 5 · status `draft` · v1 · 470 words · author `claude_ai`*
...
_`prog_purp_science_and_bible`  ·  type id 40  ·  sort 6_
...
*Section id 6 · status `draft` · v1 · 535 words · author `claude_ai`*
...
_`prog_purp_expected_outcome`  ·  type id 41  ·  sort 7_
...
*Section id 7 · status `draft` · v1 · 530 words · author `claude_ai`*
...
_`prog_meth_overview`  ·  type id 42  ·  sort 8_
...
*Section id 8 · status `draft` · v1 · 534 words · author `claude_ai`*
...
_`prog_meth_registry`  ·  type id 43  ·  sort 9_
...
*Section id 9 · status `draft` · v1 · 548 words · author `claude_ai`*
...
_`prog_meth_flow`  ·  type id 44  ·  sort 10_
...
*Section id 10 · status `draft` · v1 · 694 words · author `claude_ai`*
...
_`prog_meth_science`  ·  type id 45  ·  sort 11_
...
*Section id 11 · status `draft` · v1 · 925 words · author `claude_ai`*
...
_`prog_meth_publishing`  ·  type id 46  ·  sort 12_
...
*Section id 12 · status `draft` · v1 · 395 words · author `claude_ai`*
...
_`prog_meth_principles`  ·  type id 47  ·  sort 13_
...
*Section id 13 · status `draft` · v1 · 591 words · author `claude_ai`*
...
_`prog_meth_constraints`  ·  type id 48  ·  sort 14_
...
*Section id 14 · status `draft` · v1 · 422 words · author `claude_ai`*
...
_`prog_disc_traceability`  ·  type id 49  ·  sort 15_
...
*Section id 16 · status `draft` · v1 · 673 words · author `claude_ai`*
...
_`prog_disc_two_ai`  ·  type id 50  ·  sort 16_
...
*Section id 17 · status `draft` · v1 · 619 words · author `claude_ai`*
...
The two agents interact only through two structured artefacts: **patches** and **directives**. Patches are Claude AI's instrument for changing the content of the database — adding a row, updating a flag, correcting a value, recording a finding. Directives are Claude AI's instrument for operations that fall outside the patch format — schema changes, queries that return data, extract production, structural operations on the database itself. Every database change begins as one or the other; no informal change takes effect.

The researcher sits at the gate between the two agents. Every patch and every directive is reviewed by the researcher before Claude Code applies it. Claude AI produces the patch or directive, states what it will do and what the confirmation output will be, and waits for explicit researcher approval. Claude Code proceeds on that approval and not before it. The review step is not advisory — it is the mechanism by which no analytical change enters the record without researcher judgement on it.

Claude AI works from versioned extracts, not from direct database access. At the start of a session and at any point the extract may have become stale, Claude AI confirms the version of the extract it is working from; an extract whose version is not confirmed is not worked on. Between turns, Claude AI does not assume that the database state held in the current extract is still current. Where an operation depends on state that may have changed, Claude AI requests a refresh from Claude Code before proceeding. Recent is not current.

The architecture defines what Claude AI cannot do alone and what Claude Code will not do at all. Claude AI cannot change the database without Claude Code, and cannot rely on memory of prior state in place of a fresh read. Claude Code will not produce an analytical view, will not decide what a verse means, and will not commit an analytical finding that has not come to it through a patch reviewed by the researcher. The division is the architecture, and the discipline is what keeps it intact across every session.

#### Session continuity and memory discipline

_`prog_disc_session_continuity`  ·  type id 51  ·  sort 17_
...
*Section id 18 · status `draft` · v1 · 879 words · author `claude_ai`*
...
_`prog_disc_tools`  ·  type id 52  ·  sort 18_
...
*Section id 19 · status `draft` · v1 · 1049 words · author `claude_ai`*

The programme runs on a small set of tools, each with a defined role. The list is short by design: the research operates on text evidence held in a structured record, interrogated by two AI agents under researcher direction. The tools are the instruments through which this work is performed.
...
**The instruction corpus** is the programme's process control. Each phase of the work — Session A, Verse Context, Dimension Review, Session B, Session C, Session D — has an authoritative instruction document that sets out what is done, in what order, to what standard. Programme-wide rules sit in the global rules document loaded at every session start. The registry-management guide governs how the registry is extended and maintained; the patch and directive specifications govern how database changes are authored. The instruction corpus is the mechanism by which process is controlled rather than re-negotiated each session; referring to it is how the programme keeps its discipline stable across months of work.
...
_`prog_disc_research_decisions`  ·  type id 53  ·  sort 19_
...
*Section id 20 · status `draft` · v1 · 926 words · author `claude_ai`*
...
_`prog_disc_scope_integrity`  ·  type id 54  ·  sort 20_
...
*Section id 21 · status `draft` · v1 · 758 words · author `claude_ai`*
...
_`prog_data_database`  ·  type id 55  ·  sort 21_
...
*Section id 22 · status `draft` · v1 · 458 words · author `claude_ai`*
...
_`prog_data_registry`  ·  type id 56  ·  sort 22_
...
*Section id 23 · status `draft` · v1 · 563 words · author `claude_ai`*
...
_`prog_data_terms`  ·  type id 57  ·  sort 23_
...
*Section id 24 · status `draft` · v1 · 939 words · author `claude_ai`*
...
_`prog_xref_architecture`  ·  type id 28  ·  sort 24_
...
*Section id 25 · status `draft` · v1 · 624 words · author `claude_ai`*
...
_`prog_data_verses`  ·  type id 58  ·  sort 25_
...
*Section id 26 · status `draft` · v1 · 829 words · author `claude_ai`*
...
_`prog_anchor_verse`  ·  type id 27  ·  sort 26_
...
*Section id 27 · status `draft` · v1 · 604 words · author `claude_ai`*
...
_`prog_data_dimensions`  ·  type id 59  ·  sort 27_
...
*Section id 28 · status `draft` · v1 · 829 words · author `claude_ai`*
...
_`prog_data_questions`  ·  type id 60  ·  sort 28_
...
*Section id 29 · status `draft` · v1 · 936 words · author `claude_ai`*
...
_`prog_data_synthesis`  ·  type id 61  ·  sort 29_
...
*Section id 30 · status `draft` · v1 · 712 words · author `claude_ai`*
...
_`prog_data_prose_store`  ·  type id 62  ·  sort 30_
...
*Section id 31 · status `draft` · v1 · 962 words · author `claude_ai`*
...
_`prog_validation_standard`  ·  type id 29  ·  sort 103_
...
*Section id 32 · status `draft` · v1 · 828 words · author `claude_ai`*
...
_`prog_delete_discipline`  ·  type id 30  ·  sort 104_

> Delete_flagged semantics; cascade rules; no physical deletes in automated flows; audit-trail retention.

*Section id 33 · status `draft` · v1 · 507 words · author `claude_ai`*
...
_`prog_field_authority`  ·  type id 31  ·  sort 105_
...
*Section id 34 · status `draft` · v1 · 941 words · author `claude_ai`*
...
_`prog_backup_discipline`  ·  type id 32  ·  sort 106_
...
*Section id 35 · status `draft` · v1 · 409 words · author `claude_ai`*
...
_`prog_patch_failure_protocol`  ·  type id 33  ·  sort 107_
...
*Section id 36 · status `draft` · v1 · 447 words · author `claude_ai`*
...
_`prog_instruction_override_protocol`  ·  type id 34  ·  sort 108_
...
*Section id 37 · status `draft` · v1 · 856 words · author `claude_ai`*
...
_`prog_doc_impl_alignment`  ·  type id 63  ·  sort 109_
...
*Section id 38 · status `draft` · v1 · 636 words · author `claude_ai`*
...
_`prog_instr_global_rules`  ·  type id 64  ·  sort 110_
...
*Section id 39 · status `draft` · v1 · 221 words · author `claude_ai`*
...
_`prog_instr_reference`  ·  type id 65  ·  sort 111_
...
*Section id 40 · status `draft` · v1 · 216 words · author `claude_ai`*
...
_`prog_instr_patches`  ·  type id 66  ·  sort 112_
...
*Section id 41 · status `draft` · v1 · 227 words · author `claude_ai`*
...
_`prog_instr_directives`  ·  type id 67  ·  sort 113_
...
*Section id 42 · status `draft` · v1 · 219 words · author `claude_ai`*
...
_`prog_instr_claudecode`  ·  type id 68  ·  sort 114_
...
*Section id 43 · status `draft` · v1 · 218 words · author `claude_ai`*
...
_`prog_instr_registry_guide`  ·  type id 69  ·  sort 115_
...
*Section id 44 · status `draft` · v1 · 204 words · author `claude_ai`*
...
_`prog_instr_session_a`  ·  type id 70  ·  sort 116_
...
*Section id 45 · status `draft` · v1 · 220 words · author `claude_ai`*
...
_`prog_instr_verse_context`  ·  type id 71  ·  sort 117_
...
*Section id 46 · status `draft` · v1 · 212 words · author `claude_ai`*
...
_`prog_instr_dimension_review`  ·  type id 72  ·  sort 118_
...
*Section id 47 · status `draft` · v1 · 212 words · author `claude_ai`*
...
_`prog_instr_session_b_readiness`  ·  type id 73  ·  sort 119_
...
*Section id 48 · status `draft` · v1 · 200 words · author `claude_ai`*
...
_`prog_instr_session_b_output`  ·  type id 74  ·  sort 120_
...
*Section id 49 · status `draft` · v1 · 216 words · author `claude_ai`*
...
_`prog_instr_session_c`  ·  type id 75  ·  sort 121_
...
*Section id 50 · status `draft` · v1 · 230 words · author `claude_ai`*
...
_`prog_instr_session_d`  ·  type id 76  ·  sort 122_
...
*Section id 51 · status `draft` · v1 · 221 words · author `claude_ai`*
...
---
*Generated 2026-04-23T13:22:38Z.*
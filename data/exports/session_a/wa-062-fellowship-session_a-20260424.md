# Session A — Registry 062 fellowship

**Generated:** 2026-04-24 06:05  
**Source:** `data/bible_research.db` (deterministic render, no analytics)  
**Governing instruction:** wa-versecontext-instruction [current]  
**Produced by:** `scripts/build_session_a_prose.py`

---

## About this document

This document is the Session A input for **Verse Context classification** of
registry 062 fellowship. It contains all STEP-sourced data for this
registry's OWNER terms and is self-sufficient for the VC classification task.

Read the VC instruction in full before beginning (see `wa-versecontext-instruction [current]`).
This document is the **data**; the instruction is the **method**. The `.md`
carries no analytical content from downstream stages (no Session B findings,
no dimensional placements, no Session C prose, no Session D synthesis) —
that content is a product of later stages and must not bleed back into VC.

### Prior-state posture

This registry has **19 active** prior
`verse_context` groups across its OWNER terms
(0 dissolved); 13
of 13 OWNER terms carry prior classifications.
**Approach this as a RE-EVALUATION** — every prior classification will be
reviewed under the current filter and grouping model; every pre-existing
active group must be retained (with verses), dissolved, or
documented-retained at term close. No silent pass-through. See VC
Instruction §6.1 (prior-state posture declaration) and §6.2 Step 6
(re-evaluation self-check + orphan-group check).

### Method reminder

> **Governing filter (VC Instruction §3).** For each verse, ask:
> *Does this verse, through the use of this term, say something about the
> inner being — the non-physical, internal states, capacities, and
> expressions that constitute a person's invisible life: how a person
> thinks, feels, chooses, relates, and orients themselves toward meaning,
> others, and God?*
> If yes → classify the contextual meaning. If no → set aside
> (`is_relevant = 0`, with a `set_aside_reason` from the controlled
> vocabulary: `no_inner_being` / `physical_only` / `spatial_only` /
> `wrong_face` / `other`).
>
> **Grouping model (VC Instruction §6.2 Step 3 — characteristic-perspective).**
> Groups are formed from the perspective of the inner-being characteristic
> the verse cluster is primarily about — not from the perspective of what
> the term does. For property terms especially, name the characteristic
> being served (seat / channel / expression / mechanism / obstacle /
> counterpart), not just the term's function. Reuse existing groups where
> they fit; create a new group only when the inner-being characteristic is
> materially different, or the same characteristic is engaged in a
> materially different way.
>
> **Anchor designation (VC Instruction §4).** 1–2 anchor verses per group,
> each making the contextual meaning evident without surrounding context.
> Every term must have at least one active anchor (Rule R4).

---

## 1. Word Summary

| Field | Value |
|---|---|
| Registry | 062 |
| Word | fellowship |
| Cluster | C17 |
| Phase 1 status | Complete |
| Phase 1 term count | 15 |
| Verse Context status | Complete |
| Session B status | Verse Context Reset |
| OWNER terms | 13 |
| XREF terms | 0 |
| Active OWNER verses (total) | 89 |

### Data-quality flag summary (informational, post-M29)

| Flag | Terms affected |
|---|---:|
| `NO_WORD_ANALYSIS` | 13 |
| `PROSE_ONLY_MEANING` | 9 |
| `VERSE_EVIDENCE_CONCENTRATED` | 10 |

---

## 2. Meaning

Per-OWNER-term lexical context, STEP-sourced and mechanically parsed. Read
this section before filtering verses — understand what the term means in
its lexical range before assessing each verse's inner-being engagement.


### G2842 · *koinōnia* · participation

- **Language:** Greek
- **mti_term_id:** 873
- **STEP gloss:** participation
- **Word-analysis gloss:** participation
- **Mounce short def:** fellowship, the close association between persons, emphasizing what is common between them; by extension: participation, sharing, contribution, gift, the outcome of such close relationships
- **Occurrence count:** 21

**Parsed senses:**

- `1` fellowship, the close association between persons, emphasizing what is common between them; by extension: participation, sharing, contribution, gift, the outcome of such close relationships 
fellowship, partnership, Acts 2:42; 2Cor. 6:14; 13:13; Gal. 2:9; Phil. 3:10; 1Jn. 1:3; participation, communion, 1Cor. 10:16; aid, relief, Heb. 13:16; contribution in aid, Rom. 15:26

**Related words:**

- `G2844` *koinōnos* — participant

**Root family:**

- `KOINŌN` (Greek) — participant — Backfilled 2026-04-09 from wa_term_related_words clustering

**Data-quality flags (informational):**

- `NO_WORD_ANALYSIS` — meaning field is null for G2842. STEP returned no word analysis block for this term.
- `PROSE_ONLY_MEANING` — Meaning for G2842 stored as single prose block. STEP medium_def contains no structured sense numbering. No further subdivision available.


### G2844 · *koinōnos* · participant

- **Language:** Greek
- **mti_term_id:** 5367
- **STEP gloss:** participant
- **Word-analysis gloss:** participant
- **Mounce short def:** partner, participant, one who joins in with another in some enterprise or activity, in business or ministry
- **Occurrence count:** 15

**Parsed senses:**

- `1` partner, participant, one who joins in with another in some enterprise or activity, in business or ministry 
a fellow, partner, companion, Mt. 23:30; Lk. 5:10; 1Cor. 10:18, 20; 2Cor. 8:23; Phlm. 17; Heb. 10:33; a sharer, partaker, 2Cor. 1:7; 1Pet. 5:1; 2Pet. 1:4*

**Related words:**

- `G2839G` *koinos* — common: unsanctified
- `G2839H` *koinos* — common: shared
- `G2841` *koinōneō* — to participate
- `G2842` *koinōnia* — participation
- `G2843` *koinōnikos* — generous
- `G4791` *sunkoinōnos* — sharer

**Root family:**

- `KOINŌN` (Greek) — participant — Backfilled 2026-04-09 from wa_term_related_words clustering

**Data-quality flags (informational):**

- `NO_WORD_ANALYSIS` — meaning field is null for G2844. STEP returned no word analysis block for this term.
- `PROSE_ONLY_MEANING` — Meaning for G2844 stored as single prose block. STEP medium_def contains no structured sense numbering. No further subdivision available.
- `VERSE_EVIDENCE_CONCENTRATED` — Low occurrence count: 15. Statistical patterns unreliable with fewer than 20 occurrences.


### H2250 · *chab.bu.rah* · wound

- **Language:** Hebrew
- **mti_term_id:** 7568
- **STEP gloss:** wound
- **Word-analysis gloss:** wound
- **Occurrence count:** 7

**Parsed senses:**

- `1` bruise, stripe, wound, blow
Aramaic equivalent: cha.vu.lah (חֲבוּלָא "crime" H2248)

**Related words:**

- `H2248` *cha.vu.la* — crime
- `H2249` *cha.vor* — Habor
- `H2266` *cha.var* — to unite
- `H2267` *che.ver* — spell
- `H2268G` *che.ver* — Heber
- `H2268H` *che.ver* — Heber
- `H2268I` *che.ver* — Heber
- `H2268J` *che.ver* — Heber
- `H2270` *cha.ver* — companion
- `H2271` *chab.bar* — associate
- `H2272` *cha.var.bu.rah* — spot
- `H2274` *chev.rah* — company
- `H2275A` *chev.ron* — Hebron
- `H2275B` *chev.ron* — Hebron
- `H2275H` *chev.ron* — Hebron [Valley]
- `H2275I` *chev.ron* — Hebron
- `H2276` *chev.ro.ni* — Hebronite
- `H2277` *chev.ri* — Heberite
- `H2278` *cha.ve.ret* — consort
- `H2279` *cho.ve.ret* — set
- `H4225` *mach.be.ret* — joining
- `H4226` *me.chab.be.rah* — clamp

**Data-quality flags (informational):**

- `NO_WORD_ANALYSIS` — meaning field is null for H2250. STEP returned no word analysis block for this term.
- `PROSE_ONLY_MEANING` — Meaning for H2250 stored as single prose block. STEP medium_def contains no structured sense numbering. No further subdivision available.
- `VERSE_EVIDENCE_CONCENTRATED` — Low occurrence count: 7. Statistical patterns unreliable with fewer than 20 occurrences.


### H2266 · *cha.var* · to unite

- **Language:** Hebrew
- **mti_term_id:** 7565
- **STEP gloss:** to unite
- **Word-analysis gloss:** to unite
- **Occurrence count:** 29

**Parsed senses:**

- `1` to unite, join, bind together, be joined, be coupled, be in league, heap up, have fellowship with, be compact, be a charmer
- `1a` (Qal)
- `1a1` to unite, be joined
- `1a2` to tie magic charms, charm
- `1b` (Piel)
- `1b1` to unite with, make an ally of
- `1b2` to unite, join, ally
- `1c` (Pual)
- `1c1` to be allied with, be united
- `1c2` to be joined together
- `1d` (Hiphil) to join together, pile up (words)
- `1e` (Hithpael) to join oneself to, make an alliance, league together

**Related words:**

- `H2249` *cha.vor* — Habor
- `H2250` *chab.bu.rah* — wound
- `H2267` *che.ver* — spell
- `H2268G` *che.ver* — Heber
- `H2268H` *che.ver* — Heber
- `H2268I` *che.ver* — Heber
- `H2268J` *che.ver* — Heber
- `H2269` *cha.var* — fellow
- `H2270` *cha.ver* — companion
- `H2271` *chab.bar* — associate
- `H2272` *cha.var.bu.rah* — spot
- `H2274` *chev.rah* — company
- `H2275A` *chev.ron* — Hebron
- `H2275B` *chev.ron* — Hebron
- `H2275H` *chev.ron* — Hebron [Valley]
- `H2275I` *chev.ron* — Hebron
- `H2276` *chev.ro.ni* — Hebronite
- `H2277` *chev.ri* — Heberite
- `H2278` *cha.ve.ret* — consort
- `H2279` *cho.ve.ret* — set
- `H4225` *mach.be.ret* — joining
- `H4226` *me.chab.be.rah* — clamp

**Data-quality flags (informational):**

- `NO_WORD_ANALYSIS` — meaning field is null for H2266. STEP returned no word analysis block for this term.


### H2267 · *che.ver* · spell

- **Language:** Hebrew
- **mti_term_id:** 7569
- **STEP gloss:** spell
- **Word-analysis gloss:** spell
- **Occurrence count:** 7

**Parsed senses:**

- `1` association, company, band
- `2` shared, association, society
- `3` a magician, charmer, spell

**Related words:**

- `H2249` *cha.vor* — Habor
- `H2250` *chab.bu.rah* — wound
- `H2266` *cha.var* — to unite
- `H2268G` *che.ver* — Heber
- `H2268H` *che.ver* — Heber
- `H2268I` *che.ver* — Heber
- `H2268J` *che.ver* — Heber
- `H2270` *cha.ver* — companion
- `H2271` *chab.bar* — associate
- `H2272` *cha.var.bu.rah* — spot
- `H2274` *chev.rah* — company
- `H2275A` *chev.ron* — Hebron
- `H2275B` *chev.ron* — Hebron
- `H2275H` *chev.ron* — Hebron [Valley]
- `H2275I` *chev.ron* — Hebron
- `H2276` *chev.ro.ni* — Hebronite
- `H2277` *chev.ri* — Heberite
- `H2278` *cha.ve.ret* — consort
- `H2279` *cho.ve.ret* — set
- `H4225` *mach.be.ret* — joining
- `H4226` *me.chab.be.rah* — clamp

**Data-quality flags (informational):**

- `NO_WORD_ANALYSIS` — meaning field is null for H2267. STEP returned no word analysis block for this term.
- `VERSE_EVIDENCE_CONCENTRATED` — Only 4 confirmed verse records for H2267. Threshold is 5.


### H2270 · *cha.ver* · companion

- **Language:** Hebrew
- **mti_term_id:** 7566
- **STEP gloss:** companion
- **Word-analysis gloss:** companion
- **Occurrence count:** 21

**Parsed senses:**

- `1` adj
1) united
n m
2) associate, fellow, worshippers
3) companion

**Related words:**

- `H2249` *cha.vor* — Habor
- `H2250` *chab.bu.rah* — wound
- `H2266` *cha.var* — to unite
- `H2267` *che.ver* — spell
- `H2268G` *che.ver* — Heber
- `H2268H` *che.ver* — Heber
- `H2268I` *che.ver* — Heber
- `H2268J` *che.ver* — Heber
- `H2269` *cha.var* — fellow
- `H2271` *chab.bar* — associate
- `H2272` *cha.var.bu.rah* — spot
- `H2273` *chav.rah* — associate
- `H2274` *chev.rah* — company
- `H2275A` *chev.ron* — Hebron
- `H2275B` *chev.ron* — Hebron
- `H2275H` *chev.ron* — Hebron [Valley]
- `H2275I` *chev.ron* — Hebron
- `H2276` *chev.ro.ni* — Hebronite
- `H2277` *chev.ri* — Heberite
- `H2278` *cha.ve.ret* — consort
- `H2279` *cho.ve.ret* — set
- `H4225` *mach.be.ret* — joining
- `H4226` *me.chab.be.rah* — clamp

**Data-quality flags (informational):**

- `NO_WORD_ANALYSIS` — meaning field is null for H2270. STEP returned no word analysis block for this term.
- `PROSE_ONLY_MEANING` — Meaning for H2270 stored as single prose block. STEP medium_def contains no structured sense numbering. No further subdivision available.


### H2271 · *chab.bar* · associate

- **Language:** Hebrew
- **mti_term_id:** 7572
- **STEP gloss:** associate
- **Word-analysis gloss:** associate
- **Occurrence count:** 1

**Parsed senses:**

- `1` associate, partner (in trade)

**Related words:**

- `H2249` *cha.vor* — Habor
- `H2250` *chab.bu.rah* — wound
- `H2266` *cha.var* — to unite
- `H2267` *che.ver* — spell
- `H2268G` *che.ver* — Heber
- `H2268H` *che.ver* — Heber
- `H2268I` *che.ver* — Heber
- `H2268J` *che.ver* — Heber
- `H2270` *cha.ver* — companion
- `H2272` *cha.var.bu.rah* — spot
- `H2274` *chev.rah* — company
- `H2275A` *chev.ron* — Hebron
- `H2275B` *chev.ron* — Hebron
- `H2275H` *chev.ron* — Hebron [Valley]
- `H2275I` *chev.ron* — Hebron
- `H2276` *chev.ro.ni* — Hebronite
- `H2277` *chev.ri* — Heberite
- `H2278` *cha.ve.ret* — consort
- `H2279` *cho.ve.ret* — set
- `H4225` *mach.be.ret* — joining
- `H4226` *me.chab.be.rah* — clamp

**Data-quality flags (informational):**

- `NO_WORD_ANALYSIS` — meaning field is null for H2271. STEP returned no word analysis block for this term.
- `PROSE_ONLY_MEANING` — Meaning for H2271 stored as single prose block. STEP medium_def contains no structured sense numbering. No further subdivision available.
- `VERSE_EVIDENCE_CONCENTRATED` — Only 1 confirmed verse records for H2271. Threshold is 5.


### H2272 · *cha.var.bu.rah* · spot

- **Language:** Hebrew
- **mti_term_id:** 7573
- **STEP gloss:** spot
- **Word-analysis gloss:** spot
- **Occurrence count:** 1

**Parsed senses:**

- `1` spots, stripe, mark

**Related words:**

- `H2249` *cha.vor* — Habor
- `H2250` *chab.bu.rah* — wound
- `H2266` *cha.var* — to unite
- `H2267` *che.ver* — spell
- `H2268G` *che.ver* — Heber
- `H2268H` *che.ver* — Heber
- `H2268I` *che.ver* — Heber
- `H2268J` *che.ver* — Heber
- `H2270` *cha.ver* — companion
- `H2271` *chab.bar* — associate
- `H2274` *chev.rah* — company
- `H2275A` *chev.ron* — Hebron
- `H2275B` *chev.ron* — Hebron
- `H2275H` *chev.ron* — Hebron [Valley]
- `H2275I` *chev.ron* — Hebron
- `H2276` *chev.ro.ni* — Hebronite
- `H2277` *chev.ri* — Heberite
- `H2278` *cha.ve.ret* — consort
- `H2279` *cho.ve.ret* — set
- `H4225` *mach.be.ret* — joining
- `H4226` *me.chab.be.rah* — clamp

**Data-quality flags (informational):**

- `NO_WORD_ANALYSIS` — meaning field is null for H2272. STEP returned no word analysis block for this term.
- `PROSE_ONLY_MEANING` — Meaning for H2272 stored as single prose block. STEP medium_def contains no structured sense numbering. No further subdivision available.
- `VERSE_EVIDENCE_CONCENTRATED` — Only 1 confirmed verse records for H2272. Threshold is 5.


### H2274 · *chev.rah* · company

- **Language:** Hebrew
- **mti_term_id:** 7574
- **STEP gloss:** company
- **Word-analysis gloss:** company
- **Occurrence count:** 1

**Parsed senses:**

- `1` company, association

**Related words:**

- `H2249` *cha.vor* — Habor
- `H2250` *chab.bu.rah* — wound
- `H2266` *cha.var* — to unite
- `H2267` *che.ver* — spell
- `H2268G` *che.ver* — Heber
- `H2268H` *che.ver* — Heber
- `H2268I` *che.ver* — Heber
- `H2268J` *che.ver* — Heber
- `H2270` *cha.ver* — companion
- `H2271` *chab.bar* — associate
- `H2272` *cha.var.bu.rah* — spot
- `H2275A` *chev.ron* — Hebron
- `H2275B` *chev.ron* — Hebron
- `H2275H` *chev.ron* — Hebron [Valley]
- `H2275I` *chev.ron* — Hebron
- `H2276` *chev.ro.ni* — Hebronite
- `H2277` *chev.ri* — Heberite
- `H2278` *cha.ve.ret* — consort
- `H2279` *cho.ve.ret* — set
- `H4225` *mach.be.ret* — joining
- `H4226` *me.chab.be.rah* — clamp

**Data-quality flags (informational):**

- `NO_WORD_ANALYSIS` — meaning field is null for H2274. STEP returned no word analysis block for this term.
- `PROSE_ONLY_MEANING` — Meaning for H2274 stored as single prose block. STEP medium_def contains no structured sense numbering. No further subdivision available.
- `VERSE_EVIDENCE_CONCENTRATED` — Only 1 confirmed verse records for H2274. Threshold is 5.


### H2278 · *cha.ve.ret* · consort

- **Language:** Hebrew
- **mti_term_id:** 7576
- **STEP gloss:** consort
- **Word-analysis gloss:** consort
- **Occurrence count:** 1

**Parsed senses:**

- `1` companion, wife, consort

**Related words:**

- `H2249` *cha.vor* — Habor
- `H2250` *chab.bu.rah* — wound
- `H2266` *cha.var* — to unite
- `H2267` *che.ver* — spell
- `H2268G` *che.ver* — Heber
- `H2268H` *che.ver* — Heber
- `H2268I` *che.ver* — Heber
- `H2268J` *che.ver* — Heber
- `H2270` *cha.ver* — companion
- `H2271` *chab.bar* — associate
- `H2272` *cha.var.bu.rah* — spot
- `H2274` *chev.rah* — company
- `H2275A` *chev.ron* — Hebron
- `H2275B` *chev.ron* — Hebron
- `H2275H` *chev.ron* — Hebron [Valley]
- `H2275I` *chev.ron* — Hebron
- `H2276` *chev.ro.ni* — Hebronite
- `H2277` *chev.ri* — Heberite
- `H2279` *cho.ve.ret* — set
- `H4225` *mach.be.ret* — joining
- `H4226` *me.chab.be.rah* — clamp

**Data-quality flags (informational):**

- `NO_WORD_ANALYSIS` — meaning field is null for H2278. STEP returned no word analysis block for this term.
- `PROSE_ONLY_MEANING` — Meaning for H2278 stored as single prose block. STEP medium_def contains no structured sense numbering. No further subdivision available.
- `VERSE_EVIDENCE_CONCENTRATED` — Only 1 confirmed verse records for H2278. Threshold is 5.


### H2279 · *cho.ve.ret* · set

- **Language:** Hebrew
- **mti_term_id:** 7570
- **STEP gloss:** set
- **Word-analysis gloss:** set
- **Occurrence count:** 4

**Parsed senses:**

- `1` junction, a thing joined
- `2` curtain pieces of the tabernacle

**Related words:**

- `H2249` *cha.vor* — Habor
- `H2250` *chab.bu.rah* — wound
- `H2266` *cha.var* — to unite
- `H2267` *che.ver* — spell
- `H2268G` *che.ver* — Heber
- `H2268H` *che.ver* — Heber
- `H2268I` *che.ver* — Heber
- `H2268J` *che.ver* — Heber
- `H2270` *cha.ver* — companion
- `H2271` *chab.bar* — associate
- `H2272` *cha.var.bu.rah* — spot
- `H2274` *chev.rah* — company
- `H2275A` *chev.ron* — Hebron
- `H2275B` *chev.ron* — Hebron
- `H2275H` *chev.ron* — Hebron [Valley]
- `H2275I` *chev.ron* — Hebron
- `H2276` *chev.ro.ni* — Hebronite
- `H2277` *chev.ri* — Heberite
- `H2278` *cha.ve.ret* — consort
- `H4225` *mach.be.ret* — joining
- `H4226` *me.chab.be.rah* — clamp

**Data-quality flags (informational):**

- `NO_WORD_ANALYSIS` — meaning field is null for H2279. STEP returned no word analysis block for this term.
- `VERSE_EVIDENCE_CONCENTRATED` — Only 3 confirmed verse records for H2279. Threshold is 5.


### H4225 · *mach.be.ret* · joining

- **Language:** Hebrew
- **mti_term_id:** 7567
- **STEP gloss:** joining
- **Word-analysis gloss:** joining
- **Occurrence count:** 8

**Parsed senses:**

- `1` thing joined, joint, seam, place of joining
- `1a` thing joined
- `1b` place of joining

**Related words:**

- `H2249` *cha.vor* — Habor
- `H2250` *chab.bu.rah* — wound
- `H2266` *cha.var* — to unite
- `H2267` *che.ver* — spell
- `H2268G` *che.ver* — Heber
- `H2268H` *che.ver* — Heber
- `H2268I` *che.ver* — Heber
- `H2268J` *che.ver* — Heber
- `H2270` *cha.ver* — companion
- `H2271` *chab.bar* — associate
- `H2272` *cha.var.bu.rah* — spot
- `H2274` *chev.rah* — company
- `H2275A` *chev.ron* — Hebron
- `H2275B` *chev.ron* — Hebron
- `H2275H` *chev.ron* — Hebron [Valley]
- `H2275I` *chev.ron* — Hebron
- `H2276` *chev.ro.ni* — Hebronite
- `H2277` *chev.ri* — Heberite
- `H2278` *cha.ve.ret* — consort
- `H2279` *cho.ve.ret* — set
- `H4226` *me.chab.be.rah* — clamp

**Data-quality flags (informational):**

- `NO_WORD_ANALYSIS` — meaning field is null for H4225. STEP returned no word analysis block for this term.
- `VERSE_EVIDENCE_CONCENTRATED` — Low occurrence count: 8. Statistical patterns unreliable with fewer than 20 occurrences.


### H4226 · *me.chab.be.rah* · clamp

- **Language:** Hebrew
- **mti_term_id:** 7571
- **STEP gloss:** clamp
- **Word-analysis gloss:** clamp
- **Occurrence count:** 2

**Parsed senses:**

- `1` binder, clamp, joint

**Related words:**

- `H2249` *cha.vor* — Habor
- `H2250` *chab.bu.rah* — wound
- `H2266` *cha.var* — to unite
- `H2267` *che.ver* — spell
- `H2268G` *che.ver* — Heber
- `H2268H` *che.ver* — Heber
- `H2268I` *che.ver* — Heber
- `H2268J` *che.ver* — Heber
- `H2270` *cha.ver* — companion
- `H2271` *chab.bar* — associate
- `H2272` *cha.var.bu.rah* — spot
- `H2274` *chev.rah* — company
- `H2275A` *chev.ron* — Hebron
- `H2275B` *chev.ron* — Hebron
- `H2275H` *chev.ron* — Hebron [Valley]
- `H2275I` *chev.ron* — Hebron
- `H2276` *chev.ro.ni* — Hebronite
- `H2277` *chev.ri* — Heberite
- `H2278` *cha.ve.ret* — consort
- `H2279` *cho.ve.ret* — set
- `H4225` *mach.be.ret* — joining

**Data-quality flags (informational):**

- `NO_WORD_ANALYSIS` — meaning field is null for H4226. STEP returned no word analysis block for this term.
- `PROSE_ONLY_MEANING` — Meaning for H4226 stored as single prose block. STEP medium_def contains no structured sense numbering. No further subdivision available.
- `VERSE_EVIDENCE_CONCENTRATED` — Only 2 confirmed verse records for H4226. Threshold is 5.


---

## 3. Verses

The complete verse corpus for each OWNER term. This is the object of
classification. Every active verse (`delete_flagged = 0`) must be assessed
against the governing filter. Any existing `verse_context` state is shown
per verse as **Prior classification** — review, and revise only where
clearly warranted (VC Instruction §6.2 Step 2).


### G2842 · *koinōnia* — 17 active verse(s)

- **mti_term_id:** 873
- **Language:** Greek
- **Gloss:** participation
- **Total verses:** 17 (17 active, 0 delete_flagged)
- **Prior verse_context groups:** 2 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `873-001` | Vertical fellowship with God — the inner-relational participation in divine nature, the Son, and the Spirit that constitutes the believer's communion with God | active |
| `873-002` | Horizontal fellowship among believers — the inner-relational bond of mutual belonging, shared commitment, and generous participation arising from shared life in Christ | active |

**Verses:**

**vid=162728** — `Act 2:42`
*target word in verse:* `fellowship`

> Act 2:42 And they devoted themselves to the apostles ’ teaching and the fellowship , to the breaking of bread and the prayers .

_Prior classification: **relevant** · related · group=`873-002`_


**vid=24296** — `Rom 15:26`
*target word in verse:* `contribution`

> Rom 15:26 For Macedonia and Achaia have been pleased to make some contribution for the poor among the saints at Jerusalem .

_Prior classification: *set aside — reason: `(not set)`*_


**vid=24288** — `1Cor 1:9`
*target word in verse:* `fellowship`

> 1Cor 1:9 God is faithful , by whom you were called into the fellowship of his Son , Jesus Christ our Lord .

_Prior classification: **relevant** · related · group=`873-001`_


**vid=24289** — `1Cor 10:16`
*target word in verse:* `participation in`

> 1Cor 10:16 The cup of blessing that we bless , is it not a participation in the blood of Christ ? The bread that we break , is it not a participation in the body of Christ ?

_Prior classification: **relevant** · related · group=`873-001`_


**vid=24291** — `2Cor 6:14`
*target word in verse:* `fellowship`

> 2Cor 6:14 Do not be unequally yoked with unbelievers . For what partnership has righteousness with lawlessness ? Or what fellowship has light with darkness ?

_Prior classification: **relevant** · **anchor** · group=`873-002`_


**vid=24292** — `2Cor 8:4`
*target word in verse:* `taking part`

> 2Cor 8:4 begging us earnestly for the favor of taking part in the relief of the saints —

_Prior classification: **relevant** · related · group=`873-002`_


**vid=24293** — `2Cor 9:13`
*target word in verse:* `contribution`

> 2Cor 9:13 By their approval of this service , they will glorify God because of your submission that comes from your confession of the gospel of Christ , and the generosity of your contribution for them and for all others,

_Prior classification: *set aside — reason: `(not set)`*_


**vid=24290** — `2Cor 13:14`
*target word in verse:* `fellowship`

> 2Cor 13:14 The grace of the Lord Jesus Christ and the love of God and the fellowship of the Holy Spirit be with you all .

_Prior classification: **relevant** · related · group=`873-001`_


**vid=24294** — `Gal 2:9`
*target word in verse:* `fellowship`

> Gal 2:9 and when James and Cephas and John , who seemed to be pillars , perceived the grace that was given to me , they gave the right hand of fellowship to Barnabas and me , that we should go to the Gentiles and they to the circumcised .

_Prior classification: **relevant** · related · group=`873-002`_


**vid=162729** — `Phili 1:5`
*target word in verse:* `partnership`

> Phili 1:5 because of your partnership in the gospel from the first day until now .

_Prior classification: **relevant** · related · group=`873-002`_


**vid=162730** — `Phili 2:1`
*target word in verse:* `participation`

> Phili 2:1 So if there is any encouragement in Christ , any comfort from love , any participation in the Spirit , any affection and sympathy ,

_Prior classification: **relevant** · related · group=`873-001`_


**vid=162731** — `Phili 3:10`
*target word in verse:* `share`

> Phili 3:10 that I may know him and the power of his resurrection , and may share his sufferings , becoming like him in his death ,

_Prior classification: **relevant** · **anchor** · group=`873-001`_


**vid=162732** — `Phile 6`
*target word in verse:* `sharing`

> Phile 6 and I pray that the sharing of your faith may become effective for the full knowledge of every good thing that is in us for the sake of Christ .

_Prior classification: **relevant** · related · group=`873-002`_


**vid=24295** — `Heb 13:16`
*target word in verse:* `share`

> Heb 13:16 Do not neglect to do good and to share what you have, for such sacrifices are pleasing to God .

_Prior classification: *set aside — reason: `(not set)`*_


**vid=162725** — `1Jo 1:3`
*target word in verse:* `fellowship`

> 1Jo 1:3 that which we have seen and heard we proclaim also to you , so that you too may have fellowship with us ; and indeed our fellowship is with the Father and with his Son Jesus Christ .

_Prior classification: **relevant** · **anchor** · group=`873-001`_


**vid=162726** — `1Jo 1:6`
*target word in verse:* `fellowship`

> 1Jo 1:6 If we say we have fellowship with him while we walk in darkness , we lie and do not practice the truth .

_Prior classification: **relevant** · related · group=`873-001`_


**vid=162727** — `1Jo 1:7`
*target word in verse:* `fellowship`

> 1Jo 1:7 But if we walk in the light , as he is in the light , we have fellowship with one another , and the blood of Jesus his Son cleanses us from all sin .

_Prior classification: **relevant** · related · group=`873-001`_



### G2844 · *koinōnos* — 10 active verse(s)

- **mti_term_id:** 5367
- **Language:** Greek
- **Gloss:** participant
- **Total verses:** 10 (10 active, 0 delete_flagged)
- **Prior verse_context groups:** 2 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `5367-001` | Inner-spiritual participation — the disposition of the person as sharer in divine nature, suffering, comfort, or cultic alignment through inner orientation | active |
| `5367-002` | Moral alignment through participation — the inner orientation that places a person in solidarity with righteousness or complicity with wickedness | active |

**Verses:**

**vid=162741** — `Mat 23:30`
*target word in verse:* `taken part`

> Mat 23:30 saying , ‘ If we had lived in the days of our fathers , we would not have taken part with them in shedding the blood of the prophets .’

_Prior classification: **relevant** · **anchor** · group=`5367-002`_


**vid=162740** — `Luk 5:10`
*target word in verse:* `partners`

> Luk 5:10 and so also were James and John , sons of Zebedee , who were partners with Simon . And Jesus said to Simon , “Do not be afraid ; from now on you will be catching men .”

_Prior classification: *set aside — reason: `(not set)`*_


**vid=162733** — `1Cor 10:18`
*target word in verse:* `participants`

> 1Cor 10:18 Consider the people of Israel : are not those who eat the sacrifices participants in the altar ?

_Prior classification: **relevant** · related · group=`5367-001`_


**vid=162734** — `1Cor 10:20`
*target word in verse:* `participants`

> 1Cor 10:20 No , I imply that what pagans sacrifice they offer to demons and not to God . I do not want you to be participants with demons .

_Prior classification: **relevant** · related · group=`5367-001`_


**vid=162736** — `2Cor 1:7`
*target word in verse:* `share`

> 2Cor 1:7 Our hope for you is unshaken , for we know that as you share in our sufferings , you will also share in our comfort .

_Prior classification: **relevant** · related · group=`5367-001`_


**vid=162737** — `2Cor 8:23`
*target word in verse:* `partner`

> 2Cor 8:23 As for Titus , he is my partner and fellow worker for your benefit. And as for our brothers , they are messengers of the churches , the glory of Christ .

_Prior classification: *set aside — reason: `(not set)`*_


**vid=162742** — `Phile 17`
*target word in verse:* `partner`

> Phile 17 So if you consider me your partner , receive him as you would receive me .

_Prior classification: **relevant** · related · group=`5367-002`_


**vid=162739** — `Heb 10:33`
*target word in verse:* `partners`

> Heb 10:33 sometimes being publicly exposed to reproach and affliction , and sometimes being partners with those so treated .

_Prior classification: **relevant** · related · group=`5367-002`_


**vid=162735** — `1Pe 5:1`
*target word in verse:* `partaker`

> 1Pe 5:1 So I exhort the elders among you , as a fellow elder and a witness of the sufferings of Christ , as well as a partaker in the glory that is going to be revealed :

_Prior classification: **relevant** · related · group=`5367-001`_


**vid=162738** — `2Pe 1:4`
*target word in verse:* `partakers`

> 2Pe 1:4 by which he has granted to us his precious and very great promises , so that through them you may become partakers of the divine nature , having escaped from the corruption that is in the world because of sinful desire .

_Prior classification: **relevant** · **anchor** · group=`5367-001`_



### H2250 · *chab.bu.rah* — 6 active verse(s)

- **mti_term_id:** 7568
- **Language:** Hebrew
- **Gloss:** wound
- **Total verses:** 6 (6 active, 0 delete_flagged)
- **Prior verse_context groups:** 2 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `7568-001` | disciplinary wound as instrument of inner moral purification | active |
| `7568-002` | redemptive wound producing inner healing and restoration | active |

**Verses:**

**vid=237770** — `Gen 4:23`
*target word in verse:* `striking me`

> Gen 4:23 Lamech said to his wives : “ Adah and Zillah , hear my voice ; you wives of Lamech , listen to what I say : I have killed a man for wounding me, a young man for striking me .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237769** — `Exo 21:25`
*target word in verse:* `stripe`

> Exo 21:25 burn for burn , wound for wound , stripe for stripe .

_Prior classification: *set aside — reason: `no_inner_being`*_


**vid=237774** — `Psa 38:5`
*target word in verse:* `wounds`

> Psa 38:5 My wounds stink and fester because of my foolishness ,

_Prior classification: **relevant** · related · group=`7568-001`_


**vid=237773** — `Pro 20:30`
*target word in verse:* `Blows`

> Pro 20:30 Blows that wound cleanse away evil ; strokes make clean the innermost parts .

_Prior classification: **relevant** · **anchor** · group=`7568-001`_


**vid=237771** — `Isa 1:6`
*target word in verse:* `bruises`

> Isa 1:6 From the sole of the foot even to the head , there is no soundness in it, but bruises and sores and raw wounds ; they are not pressed out or bound up or softened with oil .

_Prior classification: **relevant** · related · group=`7568-002`_


**vid=237772** — `Isa 53:5`
*target word in verse:* `wounds`

> Isa 53:5 But he was pierced for our transgressions ; he was crushed for our iniquities ; upon him was the chastisement that brought us peace , and with his wounds we are healed .

_Prior classification: **relevant** · **anchor** · group=`7568-002`_



### H2266 · *cha.var* — 25 active verse(s)

- **mti_term_id:** 7565
- **Language:** Hebrew
- **Gloss:** to unite
- **Total verses:** 25 (25 active, 0 delete_flagged)
- **Prior verse_context groups:** 4 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `7565-001` | idolatrous or wrongful relational alliance as volitional inner joining | active |
| `7565-002` | occult charming as forbidden inner-spiritual practice | active |
| `7565-003` | joining words as inner rhetorical expression | active |
| `7565-004` | communal participation as ground of inner hope | active |

**Verses:**

**vid=237745** — `Gen 14:3`
*target word in verse:* `joined`

> Gen 14:3 And all these joined forces in the Valley of Siddim ( that is, the Salt Sea ).

_Prior classification: *set aside — reason: `no_inner_being`*_


**vid=237734** — `Exo 26:3`
*target word in verse:* `coupled`

> Exo 26:3 Five curtains shall be coupled to one another , and the other five curtains shall be coupled to one another .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237735** — `Exo 26:6`
*target word in verse:* `couple`

> Exo 26:6 And you shall make fifty clasps of gold , and couple the curtains one to the other with the clasps , so that the tabernacle may be a single whole .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237736** — `Exo 26:9`
*target word in verse:* `couple`

> Exo 26:9 You shall couple five curtains by themselves, and six curtains by themselves, and the sixth curtain you shall double over at the front of the tent .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237733** — `Exo 26:11`
*target word in verse:* `couple`

> Exo 26:11 “You shall make fifty clasps of bronze , and put the clasps into the loops , and couple the tent together that it may be a single whole .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237737** — `Exo 28:7`
*target word in verse:* `attached`

> Exo 28:7 It shall have two shoulder pieces attached to its two edges , so that it may be joined together .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237738** — `Exo 36:10`
*target word in verse:* `coupled`

> Exo 36:10 He coupled five curtains to one another , and the other five curtains he coupled to one another .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237739** — `Exo 36:13`
*target word in verse:* `coupled`

> Exo 36:13 And he made fifty clasps of gold , and coupled the curtains one to the other with clasps . So the tabernacle was a single whole .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237740** — `Exo 36:16`
*target word in verse:* `coupled`

> Exo 36:16 He coupled five curtains by themselves , and six curtains by themselves .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237741** — `Exo 36:18`
*target word in verse:* `couple`

> Exo 36:18 And he made fifty clasps of bronze to couple the tent together that it might be a single whole .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237742** — `Exo 39:4`
*target word in verse:* `attaching`

> Exo 39:4 They made for the ephod attaching shoulder pieces , joined to it at its two edges .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237731** — `Deu 18:11`
*target word in verse:* `charmer`

> Deu 18:11 or a charmer or a medium or a necromancer or one who inquires of the dead ,

_Prior classification: **relevant** · related · group=`7565-002`_


**vid=237726** — `2Ch 20:35`
*target word in verse:* `joined`

> 2Ch 20:35 After this Jehoshaphat king of Judah joined with Ahaziah king of Israel , who acted wickedly .

_Prior classification: **relevant** · related · group=`7565-001`_


**vid=237727** — `2Ch 20:36`
*target word in verse:* `joined`

> 2Ch 20:36 He joined him in building ships to go to Tarshish , and they built the ships in Ezion-geber .

_Prior classification: **relevant** · related · group=`7565-001`_


**vid=237728** — `2Ch 20:37`
*target word in verse:* `joined`

> 2Ch 20:37 Then Eliezer the son of Dodavahu of Mareshah prophesied against Jehoshaphat , saying , “Because you have joined with Ahaziah , the Lord will destroy what you have made .” And the ships were wrecked and were not able to go to Tarshish .

_Prior classification: **relevant** · related · group=`7565-001`_


**vid=237747** — `Job 16:4`
*target word in verse:* `join`

> Job 16:4 I also could speak as you do, if you were in my place; I could join words together against you and shake my head at you.

_Prior classification: **relevant** · **anchor** · group=`7565-003`_


**vid=237749** — `Psa 58:5`
*target word in verse:* `enchanter`

> Psa 58:5 so that it does not hear the voice of charmers or of the cunning enchanter .

_Prior classification: **relevant** · **anchor** · group=`7565-002`_


**vid=237750** — `Psa 94:20`
*target word in verse:* `allied`

> Psa 94:20 Can wicked rulers be allied with you, those who frame injustice by statute ?

_Prior classification: **relevant** · related · group=`7565-001`_


**vid=237748** — `Psa 122:3`
*target word in verse:* `bound firmly`

> Psa 122:3 Jerusalem — built as a city that is bound firmly together ,

_Prior classification: *set aside — reason: `spatial_only`*_


**vid=237732** — `Ecc 9:4`
*target word in verse:* `joined`

> Ecc 9:4 But he who is joined with all the living has hope , for a living dog is better than a dead lion .

_Prior classification: **relevant** · **anchor** · group=`7565-004`_


**vid=237744** — `Eze 1:9`
*target word in verse:* `touched`

> Eze 1:9 their wings touched one another . Each one of them went straight forward , without turning as they went .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237743** — `Eze 1:11`
*target word in verse:* `touched`

> Eze 1:11 Such were their faces . And their wings were spread out above . Each creature had two wings, each of which touched the wing of another, while two covered their bodies .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237730** — `Dan 11:6`
*target word in verse:* `alliance`

> Dan 11:6 After some years they shall make an alliance , and the daughter of the king of the south shall come to the king of the north to make an agreement . But she shall not retain the strength of her arm , and he and his arm shall not endure , but she shall be given up , and her attendants , he who fathered her, and he who supported her in those times .

_Prior classification: *set aside — reason: `no_inner_being`*_


**vid=237729** — `Dan 11:23`
*target word in verse:* `alliance`

> Dan 11:23 And from the time that an alliance is made with him he shall act deceitfully , and he shall become strong with a small people .

_Prior classification: *set aside — reason: `no_inner_being`*_


**vid=237746** — `Hos 4:17`
*target word in verse:* `joined`

> Hos 4:17 Ephraim is joined to idols ; leave him alone .

_Prior classification: **relevant** · **anchor** · group=`7565-001`_



### H2267 · *che.ver* — 4 active verse(s)

- **mti_term_id:** 7569
- **Language:** Hebrew
- **Gloss:** spell
- **Total verses:** 4 (4 active, 0 delete_flagged)
- **Prior verse_context groups:** 2 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `7569-001` | occult charming as forbidden spiritual practice | active |
| `7569-002` | reliance on occult power as spiritual misdirection and arrogance | active |

**Verses:**

**vid=237781** — `Deu 18:11`
*target word in verse:* `charmer`

> Deu 18:11 or a charmer or a medium or a necromancer or one who inquires of the dead ,

_Prior classification: **relevant** · related · group=`7569-001`_


**vid=237784** — `Psa 58:5`
*target word in verse:* `enchanter`

> Psa 58:5 so that it does not hear the voice of charmers or of the cunning enchanter .

_Prior classification: **relevant** · **anchor** · group=`7569-001`_


**vid=237783** — `Isa 47:9`
*target word in verse:* `enchantments`

> Isa 47:9 These two things shall come to you in a moment , in one day ; the loss of children and widowhood shall come upon you in full measure , in spite of your many sorceries and the great power of your enchantments .

_Prior classification: **relevant** · **anchor** · group=`7569-002`_


**vid=237782** — `Isa 47:12`
*target word in verse:* `enchantments`

> Isa 47:12 Stand fast in your enchantments and your many sorceries , with which you have labored from your youth ; perhaps you may be able to succeed ; perhaps you may inspire terror .

_Prior classification: **relevant** · related · group=`7569-002`_



### H2270 · *cha.ver* — 11 active verse(s)

- **mti_term_id:** 7566
- **Language:** Hebrew
- **Gloss:** companion
- **Total verses:** 11 (11 active, 0 delete_flagged)
- **Prior verse_context groups:** 4 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `7566-001` | companionship as chosen moral and spiritual alignment | active |
| `7566-002` | companionship as moral alignment with wickedness | active |
| `7566-003` | companionship as mutual relational support | active |
| `7566-004` | divinely-willed communal restoration as inner-being oneness | active |

**Verses:**

**vid=237756** — `Judg 20:11`
*target word in verse:* `united`

> Judg 20:11 So all the men of Israel gathered against the city , united as one man .

_Prior classification: **relevant** · related · group=`7566-001`_


**vid=237759** — `Psa 45:7`
*target word in verse:* `companions`

> Psa 45:7 you have loved righteousness and hated wickedness . Therefore God , your God , has anointed you with the oil of gladness beyond your companions ;

_Prior classification: *set aside — reason: `wrong_face`*_


**vid=237758** — `Psa 119:63`
*target word in verse:* `companion`

> Psa 119:63 I am a companion of all who fear you, of those who keep your precepts .

_Prior classification: **relevant** · **anchor** · group=`7566-001`_


**vid=237757** — `Pro 28:24`
*target word in verse:* `companion`

> Pro 28:24 Whoever robs his father or his mother and says , “That is no transgression ,” is a companion to a man who destroys .

_Prior classification: **relevant** · related · group=`7566-002`_


**vid=237751** — `Ecc 4:10`
*target word in verse:* `fellow`

> Ecc 4:10 For if they fall , one will lift up his fellow . But woe to him who is alone when he falls and has not another to lift him up!

_Prior classification: **relevant** · **anchor** · group=`7566-003`_


**vid=237760** — `Song 1:7`
*target word in verse:* `companions`

> Song 1:7 Tell me, you whom my soul loves , where you pasture your flock , where you make it lie down at noon ; for why should I be like one who veils herself beside the flocks of your companions ?

_Prior classification: *set aside — reason: `wrong_face`*_


**vid=237761** — `Song 8:13`
*target word in verse:* `companions`

> Song 8:13 O you who dwell in the gardens , with companions listening for your voice ; let me hear it .

_Prior classification: *set aside — reason: `wrong_face`*_


**vid=237754** — `Isa 1:23`
*target word in verse:* `companions`

> Isa 1:23 Your princes are rebels and companions of thieves . Everyone loves a bribe and runs after gifts . They do not bring justice to the fatherless , and the widow’s cause does not come to them .

_Prior classification: **relevant** · **anchor** · group=`7566-002`_


**vid=237755** — `Isa 44:11`
*target word in verse:* `companions`

> Isa 44:11 Behold , all his companions shall be put to shame , and the craftsmen are only human . Let them all assemble , let them stand forth . They shall be terrified ; they shall be put to shame together .

_Prior classification: **relevant** · related · group=`7566-002`_


**vid=237752** — `Eze 37:16`
*target word in verse:* `with him`

> Eze 37:16 “ Son of man , take a stick and write on it, ‘ For Judah , and the people of Israel associated with him ’; then take another stick and write on it, ‘ For Joseph (the stick of Ephraim ) and all the house of Israel associated with him .’

_Prior classification: *set aside — reason: `no_inner_being`*_


**vid=237753** — `Eze 37:19`
*target word in verse:* `associated`

> Eze 37:19 say to them, Thus says the Lord God : Behold , I am about to take the stick of Joseph ( that is in the hand of Ephraim ) and the tribes of Israel associated with him. And I will join with it the stick of Judah , and make them one stick , that they may be one in my hand .

_Prior classification: **relevant** · **anchor** · group=`7566-004`_



### H2271 · *chab.bar* — 1 active verse(s)

- **mti_term_id:** 7572
- **Language:** Hebrew
- **Gloss:** associate
- **Total verses:** 1 (1 active, 0 delete_flagged)

**Verses:**

**vid=237790** — `Job 41:6`
*target word in verse:* `traders`

> Job 41:6 Will traders bargain over him? Will they divide him up among the merchants ?

_Prior classification: *set aside — reason: `no_inner_being`*_



### H2272 · *cha.var.bu.rah* — 1 active verse(s)

- **mti_term_id:** 7573
- **Language:** Hebrew
- **Gloss:** spot
- **Total verses:** 1 (1 active, 0 delete_flagged)
- **Prior verse_context groups:** 1 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `7573-001` | indelibly formed inner moral character depicted as fixed marking | active |

**Verses:**

**vid=237791** — `Jer 13:23`
*target word in verse:* `spots`

> Jer 13:23 Can the Ethiopian change his skin or the leopard his spots ? Then also you can do good who are accustomed to do evil .

_Prior classification: **relevant** · **anchor** · group=`7573-001`_



### H2274 · *chev.rah* — 1 active verse(s)

- **mti_term_id:** 7574
- **Language:** Hebrew
- **Gloss:** company
- **Total verses:** 1 (1 active, 0 delete_flagged)
- **Prior verse_context groups:** 1 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `7574-001` | keeping company with evildoers as moral alignment | active |

**Verses:**

**vid=237792** — `Job 34:8`
*target word in verse:* `company`

> Job 34:8 who travels in company with evildoers and walks with wicked men ?

_Prior classification: **relevant** · **anchor** · group=`7574-001`_



### H2278 · *cha.ve.ret* — 1 active verse(s)

- **mti_term_id:** 7576
- **Language:** Hebrew
- **Gloss:** consort
- **Total verses:** 1 (1 active, 0 delete_flagged)
- **Prior verse_context groups:** 1 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `7576-001` | covenantal companionship as the relational bond that faithlessness violates | active |

**Verses:**

**vid=237850** — `Mal 2:14`
*target word in verse:* `is your companion`

> Mal 2:14 But you say , “ Why does he not?” Because the Lord was witness between you and the wife of your youth , to whom you have been faithless , though she is your companion and your wife by covenant .

_Prior classification: **relevant** · **anchor** · group=`7576-001`_



### H2279 · *cho.ve.ret* — 3 active verse(s)

- **mti_term_id:** 7570
- **Language:** Hebrew
- **Gloss:** set
- **Total verses:** 3 (3 active, 0 delete_flagged)

**Verses:**

**vid=237786** — `Exo 26:4`
*target word in verse:* `set`

> Exo 26:4 And you shall make loops of blue on the edge of the outermost curtain in the first set . Likewise you shall make loops on the edge of the outermost curtain in the second set .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237785** — `Exo 26:10`
*target word in verse:* `set`

> Exo 26:10 You shall make fifty loops on the edge of the curtain that is outermost in one set , and fifty loops on the edge of the curtain that is outermost in the second set .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237787** — `Exo 36:17`
*target word in verse:* `connecting`

> Exo 36:17 And he made fifty loops on the edge of the outermost curtain of the one set , and fifty loops on the edge of the other connecting curtain .

_Prior classification: *set aside — reason: `physical_only`*_



### H4225 · *mach.be.ret* — 7 active verse(s)

- **mti_term_id:** 7567
- **Language:** Hebrew
- **Gloss:** joining
- **Total verses:** 7 (7 active, 0 delete_flagged)

**Verses:**

**vid=237762** — `Exo 26:4`
*target word in verse:* `set`

> Exo 26:4 And you shall make loops of blue on the edge of the outermost curtain in the first set . Likewise you shall make loops on the edge of the outermost curtain in the second set .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237763** — `Exo 26:5`
*target word in verse:* `set`

> Exo 26:5 Fifty loops you shall make on the one curtain , and fifty loops you shall make on the edge of the curtain that is in the second set ; the loops shall be opposite one another .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237764** — `Exo 28:27`
*target word in verse:* `seam`

> Exo 28:27 And you shall make two rings of gold , and attach them in front to the lower part of the two shoulder pieces of the ephod , at its seam above the skillfully woven band of the ephod .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237765** — `Exo 36:11`
*target word in verse:* `set`

> Exo 36:11 He made loops of blue on the edge of the outermost curtain of the first set . Likewise he made them on the edge of the outermost curtain of the second set .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237766** — `Exo 36:12`
*target word in verse:* `set`

> Exo 36:12 He made fifty loops on the one curtain , and he made fifty loops on the edge of the curtain that was in the second set . The loops were opposite one another .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237767** — `Exo 36:17`
*target word in verse:* `set`

> Exo 36:17 And he made fifty loops on the edge of the outermost curtain of the one set , and fifty loops on the edge of the other connecting curtain .

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237768** — `Exo 39:20`
*target word in verse:* `seam`

> Exo 39:20 And they made two rings of gold , and attached them in front to the lower part of the two shoulder pieces of the ephod , at its seam above the skillfully woven band of the ephod .

_Prior classification: *set aside — reason: `physical_only`*_



### H4226 · *me.chab.be.rah* — 2 active verse(s)

- **mti_term_id:** 7571
- **Language:** Hebrew
- **Gloss:** clamp
- **Total verses:** 2 (2 active, 0 delete_flagged)

**Verses:**

**vid=237788** — `1Ch 22:3`
*target word in verse:* `clamps`

> 1Ch 22:3 David also provided great quantities of iron for nails for the doors of the gates and for clamps , as well as bronze in quantities beyond weighing ,

_Prior classification: *set aside — reason: `physical_only`*_


**vid=237789** — `2Ch 34:11`
*target word in verse:* `binders`

> 2Ch 34:11 They gave it to the carpenters and the builders to buy quarried stone , and timber for binders and beams for the buildings that the kings of Judah had let go to ruin .

_Prior classification: *set aside — reason: `physical_only`*_



---

## 4. Terms

The full term landscape of this registry. OWNER terms are classified in
Verse Context; XREF terms are not — their classification is inherited
programme-wide from the OWNER's VC records (VC Instruction §0.2).


### OWNER terms

| mti_id | Strong's | Translit. | Gloss | Language | mti_status | Occurrence |
|---:|---|---|---|---|---|---:|
| 873 | `G2842` | koinōnia | participation | Greek | extracted | 21 |
| 5367 | `G2844` | koinōnos | participant | Greek | extracted | 15 |
| 7568 | `H2250` | chab.bu.rah | wound | Hebrew | extracted | 7 |
| 7565 | `H2266` | cha.var | to unite | Hebrew | extracted | 29 |
| 7569 | `H2267` | che.ver | spell | Hebrew | extracted | 7 |
| 7566 | `H2270` | cha.ver | companion | Hebrew | extracted | 21 |
| 7572 | `H2271` | chab.bar | associate | Hebrew | extracted | 1 |
| 7573 | `H2272` | cha.var.bu.rah | spot | Hebrew | extracted | 1 |
| 7574 | `H2274` | chev.rah | company | Hebrew | extracted | 1 |
| 7576 | `H2278` | cha.ve.ret | consort | Hebrew | extracted | 1 |
| 7570 | `H2279` | cho.ve.ret | set | Hebrew | extracted | 4 |
| 7567 | `H4225` | mach.be.ret | joining | Hebrew | extracted | 8 |
| 7571 | `H4226` | me.chab.be.rah | clamp | Hebrew | extracted | 2 |

### XREF terms

_None._

---

## 5. Pointers

Structural pointers — what other parts of the programme this registry
connects to. Informational only; not required for classification.
No Session D synthesis content here.


### Cross-registry links

_None recorded._

### Terms borrowed as XREF from other registries

_None._

---

## 6. Questions

Open questions from the programme's observation catalogue linked to the
data-quality flags present on this registry. These are for awareness —
classification work may surface answers; many will be addressed in
Session B. Do not try to answer them here.


| obs_id | code | triggering flag | question |
|---:|---|---|---|
| 199 | `Q-COV-05` | `VERSE_EVIDENCE_CONCENTRATED` | Was STEP's verse capture exhausted for this term? Was any pagination or filter step incomplete? |
| 200 | `Q-COV-06` | `VERSE_EVIDENCE_CONCENTRATED` | What do the few verses that exist establish about the term's inner-being contribution? |
| 201 | `Q-COV-07` | `VERSE_EVIDENCE_CONCENTRATED` | Does the narrow verse set concentrate in a particular literary genre, historical period, or speaker? |

---

## Footer — what happens next

When a Claude AI Verse Context session consumes this document, it produces a
`VERSECONTEXT` patch populating `verse_context_group` and `verse_context`
for each OWNER term. The patch is applied via
`scripts/apply_session_patch.py`; Claude Code then advances this registry's
`verse_context_status` to Complete (subject to XREF coverage rules per
VC Instruction §0.2) and a fresh Session A `.md` can be regenerated.

*End of Session A prose for registry 062 fellowship.*

# Session A — Registry 207 blindness (spiritual)

**Generated:** 2026-04-24 05:13  
**Source:** `data/bible_research.db` (deterministic render, no analytics)  
**Governing instruction:** wa-versecontext-instruction [current]  
**Produced by:** `scripts/build_session_a_prose.py`

---

## About this document

This document is the Session A input for **Verse Context classification** of
registry 207 blindness (spiritual). It contains all STEP-sourced data for this
registry's OWNER terms and is self-sufficient for the VC classification task.

Read the VC instruction in full before beginning (see `wa-versecontext-instruction [current]`).
This document is the **data**; the instruction is the **method**. The `.md`
carries no analytical content from downstream stages (no Session B findings,
no dimensional placements, no Session C prose, no Session D synthesis) —
that content is a product of later stages and must not bleed back into VC.

### Method reminder

> **Governing filter (VC Instruction §3).** For each verse, ask:
> *Does this verse, through the use of this term, say something about the
> inner being — the non-physical, internal states, capacities, and
> expressions that constitute a person's invisible life: how a person
> thinks, feels, chooses, relates, and orients themselves toward meaning,
> others, and God?*
> If yes → classify the contextual meaning. If no → set aside
> (`is_relevant = 0`, with a `set_aside_reason` from the controlled
> vocabulary: `no_inner_being` / `physical_only` / `spatial_only` /
> `wrong_face` / `other`).
>
> **Grouping model (VC Instruction §6.2 Step 3 — characteristic-perspective).**
> Groups are formed from the perspective of the inner-being characteristic
> the verse cluster is primarily about — not from the perspective of what
> the term does. For property terms especially, name the characteristic
> being served (seat / channel / expression / mechanism / obstacle /
> counterpart), not just the term's function. Reuse existing groups where
> they fit; create a new group only when the inner-being characteristic is
> materially different, or the same characteristic is engaged in a
> materially different way.
>
> **Anchor designation (VC Instruction §4).** 1–2 anchor verses per group,
> each making the contextual meaning evident without surrounding context.
> Every term must have at least one active anchor (Rule R4).

---

## 1. Word Summary

| Field | Value |
|---|---|
| Registry | 207 |
| Word | blindness (spiritual) |
| Cluster | C21 |
| Phase 1 status | Complete |
| Phase 1 term count | 9 |
| Verse Context status | Complete |
| Session B status | Verse Context Reset |
| OWNER terms | 7 |
| XREF terms | 4 |
| Active OWNER verses (total) | 103 |

---

## 2. Meaning

Per-OWNER-term lexical context, STEP-sourced and mechanically parsed. Read
this section before filtering verses — understand what the term means in
its lexical range before assessing each verse's inner-being engagement.


### G4652 · *skoteinos* · dark

- **Language:** Greek
- **mti_term_id:** 7435
- **STEP gloss:** dark
- **Word-analysis gloss:** dark
- **Mounce short def:** dark
- **Occurrence count:** 17

**Parsed senses:**

- `1` dark, Mt. 6:23; Lk. 11:34, 36*

**Related words:**

- `G4653` *skotia* — darkness
- `G4654` *skotizō* — to darken
- `G4655` *skotos* — darkness
- `G4656` *skotoō* — to darken

**Root family:**

- `SKOT` (Greek) — dark — Backfilled 2026-04-09 from wa_term_related_words clustering


### G4653 · *skotia* · darkness

- **Language:** Greek
- **mti_term_id:** 7433
- **STEP gloss:** darkness
- **Word-analysis gloss:** darkness
- **Mounce short def:** darkness, the dark
- **Occurrence count:** 20

**Parsed senses:**

- `1` darkness, Jn. 6:17; 20:1; privacy, Mt. 10:27; Lk. 12:3; metaphorically moral or spiritual darkness, the dark Jn. 1:5 (2x); 8:12; 12:35, 46; 1Jn. 1:5; 2:8, 9, 11*

**Related words:**

- `G4652` *skoteinos* — dark
- `G4653` *skotia* — darkness
- `G4654` *skotizō* — to darken
- `G4655` *skotos* — darkness
- `G4656` *skotoō* — to darken

**Root family:**

- `SKOT` (Greek) — dark — Backfilled 2026-04-09 from wa_term_related_words clustering


### G4654 · *skotizō* · to darken

- **Language:** Greek
- **mti_term_id:** 7434
- **STEP gloss:** to darken
- **Word-analysis gloss:** to darken
- **Mounce short def:** to darken
- **Occurrence count:** 15

**Parsed senses:**

- `1` to darken 
(pass.) to be or become dark, be darkened

**Related words:**

- `G4652` *skoteinos* — dark
- `G4653` *skotia* — darkness
- `G4655` *skotos* — darkness
- `G4656` *skotoō* — to darken

**Root family:**

- `SKOT` (Greek) — dark — Backfilled 2026-04-09 from wa_term_related_words clustering


### G4655 · *skotos* · darkness

- **Language:** Greek
- **mti_term_id:** 7432
- **STEP gloss:** darkness
- **Word-analysis gloss:** darkness
- **Mounce short def:** darkness, the dark
- **Occurrence count:** 131

**Parsed senses:**

- `1` darkness, the dark 
darkness, Mt. 27:45; Acts 2:20; gloom of punishment and misery, Mt. 8:12; 2Pet. 2:17; metaphorically moral or spiritual darkness, Mt. 4:16; Jn. 3:19; Eph. 5:11; a realm of moral darkness, Eph. 5:8; 6:12

**Related words:**

- `G4639` *skia* — shadow
- `G4652` *skoteinos* — dark
- `G4653` *skotia* — darkness
- `G4654` *skotizō* — to darken
- `G4656` *skotoō* — to darken

**Root family:**

- `SKOT` (Greek) — dark — Backfilled 2026-04-09 from wa_term_related_words clustering


### G4656 · *skotoō* · to darken

- **Language:** Greek
- **mti_term_id:** 1371
- **STEP gloss:** to darken
- **Word-analysis gloss:** to darken
- **Mounce short def:** (<i>passive</i>) to be or become darkeneddarken
- **Occurrence count:** 8

**Parsed senses:**

- `1` (passive) to be or become darkeneddarken 
to darken, shroud in darkness, Eph. 4:18; Rev. 9:2; 16:10

**Related words:**

- `G4652` *skoteinos* — dark
- `G4653` *skotia* — darkness
- `G4654` *skotizō* — to darken
- `G4655` *skotos* — darkness

**Root family:**

- `SKOT` (Greek) — dark — Backfilled 2026-04-09 from wa_term_related_words clustering


### G5185 · *tuflos* · blind

- **Language:** Greek
- **mti_term_id:** 1372
- **STEP gloss:** blind
- **Word-analysis gloss:** blind
- **Mounce short def:** blind; (n.) blind person
- **Occurrence count:** 81

**Parsed senses:**

- `1` blind; (n.) blind person 
blind, Mt. 9:27, 28; 11:5; 12:22; metaphorically mentally blind, Mt. 15:14; 23:16

**Related words:**

- `G5186` *tufloō* — to blind

**Root family:**

- `TUFL` (Greek) — blind — Backfilled 2026-04-09 from wa_term_related_words clustering


### G5186 · *tufloō* · to blind

- **Language:** Greek
- **mti_term_id:** 7438
- **STEP gloss:** to blind
- **Word-analysis gloss:** to blind
- **Mounce short def:** to cause blindness, deprive of sightblind
- **Occurrence count:** 4

**Parsed senses:**

- `1` to cause blindness, deprive of sightblind 
to blind, render blind;, met. Jn. 12:40; 2Cor. 4:4; 1Jn. 2:11*

**Related words:**

- `G5185` *tuflos* — blind

**Root family:**

- `TUFL` (Greek) — blind — Backfilled 2026-04-09 from wa_term_related_words clustering


---

## 3. Verses

The complete verse corpus for each OWNER term. This is the object of
classification. Every active verse (`delete_flagged = 0`) must be assessed
against the governing filter. Any existing `verse_context` state is shown
per verse as **Prior classification** — review, and revise only where
clearly warranted (VC Instruction §6.2 Step 2).


### G4652 · *skoteinos* — 3 active verse(s)

- **mti_term_id:** 7435
- **Language:** Greek
- **Gloss:** dark
- **Total verses:** 3 (3 active, 0 delete_flagged)
- **Prior verse_context groups:** 1 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `7435-001` | Term names the inner darkness that fills the person when their inner orientation is corrupted — the whole inner life darkened by a bad eye | active |

**Verses:**

**vid=231984** — `Mat 6:23`
*target word in verse:* `full of darkness`

> Mat 6:23 but if your eye is bad , your whole body will be full of darkness . If then the light in you is darkness , how great is the darkness !

_Prior classification: **relevant** · **anchor** · group=`7435-001`_


**vid=231982** — `Luk 11:34`
*target word in verse:* `darkness`

> Luk 11:34 Your eye is the lamp of your body . When your eye is healthy , your whole body is full of light , but when it is bad , your body is full of darkness .

_Prior classification: **relevant** · related · group=`7435-001`_


**vid=231983** — `Luk 11:36`
*target word in verse:* `dark`

> Luk 11:36 If then your whole body is full of light , having no part dark , it will be wholly bright , as when a lamp with its rays gives you light .”

_Prior classification: **relevant** · related · group=`7435-001`_



### G4653 · *skotia* — 12 active verse(s)

- **mti_term_id:** 7433
- **Language:** Greek
- **Gloss:** darkness
- **Total verses:** 12 (12 active, 0 delete_flagged)
- **Prior verse_context groups:** 2 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `7433-001` | Term names darkness as the inner condition of spiritual alienation — the state of the person who hates, does not know where they are going, and has not come to the light | active |
| `7433-002` | Term names the darkness of concealment — the inner life hidden in secret, which will be brought to light | active |

**Verses:**

**vid=231973** — `Mat 10:27`
*target word in verse:* `dark`

> Mat 10:27 What I tell you in the dark , say in the light , and what you hear whispered , proclaim on the housetops .

_Prior classification: **relevant** · related · group=`7433-002`_


**vid=231972** — `Luk 12:3`
*target word in verse:* `dark`

> Luk 12:3 Therefore whatever you have said in the dark shall be heard in the light , and what you have whispered in private rooms shall be proclaimed on the housetops .

_Prior classification: **relevant** · **anchor** · group=`7433-002`_


**vid=231966** — `Joh 1:5`
*target word in verse:* `darkness`

> Joh 1:5 The light shines in the darkness , and the darkness has not overcome it .

_Prior classification: **relevant** · related · group=`7433-001`_


**vid=231970** — `Joh 6:17`
*target word in verse:* `dark`

> Joh 6:17 got into a boat , and started across the sea to Capernaum . It was now dark , and Jesus had not yet come to them .

_Prior classification: *set aside — reason: `(not set)`*_


**vid=231971** — `Joh 8:12`
*target word in verse:* `darkness`

> Joh 8:12 Again Jesus spoke to them , saying , “ I am the light of the world . Whoever follows me will not walk in darkness , but will have the light of life .”

_Prior classification: **relevant** · related · group=`7433-001`_


**vid=231967** — `Joh 12:35`
*target word in verse:* `darkness`

> Joh 12:35 So Jesus said to them , “The light is among you for a little while longer . Walk while you have the light , lest darkness overtake you . The one who walks in the darkness does not know where he is going .

_Prior classification: **relevant** · related · group=`7433-001`_


**vid=231968** — `Joh 12:46`
*target word in verse:* `darkness`

> Joh 12:46 I have come into the world as light , so that whoever believes in me may not remain in darkness .

_Prior classification: **relevant** · related · group=`7433-001`_


**vid=231969** — `Joh 20:1`
*target word in verse:* `dark`

> Joh 20:1 Now on the first day of the week Mary Magdalene came to the tomb early , while it was still dark , and saw that the stone had been taken away from the tomb .

_Prior classification: *set aside — reason: `(not set)`*_


**vid=231962** — `1Jo 1:5`
*target word in verse:* `darkness`

> 1Jo 1:5 This is the message we have heard from him and proclaim to you , that God is light , and in him is no darkness at all .

_Prior classification: **relevant** · related · group=`7433-001`_


**vid=231964** — `1Jo 2:8`
*target word in verse:* `darkness`

> 1Jo 2:8 At the same time , it is a new commandment that I am writing to you , which is true in him and in you , because the darkness is passing away and the true light is already shining .

_Prior classification: **relevant** · related · group=`7433-001`_


**vid=231965** — `1Jo 2:9`
*target word in verse:* `darkness`

> 1Jo 2:9 Whoever says he is in the light and hates his brother is still in darkness .

_Prior classification: **relevant** · related · group=`7433-001`_


**vid=231963** — `1Jo 2:11`
*target word in verse:* `darkness`

> 1Jo 2:11 But whoever hates his brother is in the darkness and walks in the darkness , and does not know where he is going , because the darkness has blinded his eyes .

_Prior classification: **relevant** · **anchor** · group=`7433-001`_



### G4654 · *skotizō* — 8 active verse(s)

- **mti_term_id:** 7434
- **Language:** Greek
- **Gloss:** to darken
- **Total verses:** 8 (8 active, 0 delete_flagged)
- **Prior verse_context groups:** 1 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `7434-001` | Term describes the darkening of inner capacities — understanding, heart, and eyes — as the consequence of wilful rejection of God or as divine judicial act | active |

**Verses:**

**vid=231977** — `Mat 24:29`
*target word in verse:* `darkened`

> Mat 24:29 “ Immediately after the tribulation of those days the sun will be darkened , and the moon will not give its light , and the stars will fall from heaven , and the powers of the heavens will be shaken .

_Prior classification: *set aside — reason: `(not set)`*_


**vid=231976** — `Mar 13:24`
*target word in verse:* `darkened`

> Mar 13:24 “ But in those days , after that tribulation , the sun will be darkened , and the moon will not give its light ,

_Prior classification: *set aside — reason: `(not set)`*_


**vid=231975** — `Luk 23:45`
*target word in verse:* `light failed`

> Luk 23:45 while the sun’s light failed . And the curtain of the temple was torn in two .

_Prior classification: *set aside — reason: `(not set)`*_


**vid=231980** — `Rom 1:21`
*target word in verse:* `darkened`

> Rom 1:21 For although they knew God , they did not honor him as God or give thanks to him, but they became futile in their thinking , and their foolish hearts were darkened .

_Prior classification: **relevant** · related · group=`7434-001`_


**vid=231981** — `Rom 11:10`
*target word in verse:* `darkened`

> Rom 11:10 let their eyes be darkened so that they cannot see , and bend their backs forever .”

_Prior classification: **relevant** · related · group=`7434-001`_


**vid=231974** — `Eph 4:18`
*target word in verse:* `darkened`

> Eph 4:18 They are darkened in their understanding , alienated from the life of God because of the ignorance that is in them , due to their hardness of heart .

_Prior classification: **relevant** · **anchor** · group=`7434-001`_


**vid=231978** — `Rev 8:12`
*target word in verse:* `darkened`

> Rev 8:12 The fourth angel blew his trumpet, and a third of the sun was struck , and a third of the moon , and a third of the stars , so that a third of their light might be darkened , and a third of the day might be kept from shining , and likewise a third of the night .

_Prior classification: *set aside — reason: `(not set)`*_


**vid=231979** — `Rev 9:2`
*target word in verse:* `darkened`

> Rev 9:2 He opened the shaft of the bottomless pit , and from the shaft rose smoke like the smoke of a great furnace , and the sun and the air were darkened with the smoke from the shaft .

_Prior classification: *set aside — reason: `(not set)`*_



### G4655 · *skotos* — 30 active verse(s)

- **mti_term_id:** 7432
- **Language:** Greek
- **Gloss:** darkness
- **Total verses:** 30 (30 active, 0 delete_flagged)
- **Prior verse_context groups:** 4 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `7432-003` | Term names the darkness of the inner life — hidden purposes, corrupted inner light, self-deception about one's inner state | active |
| `7432-001` | Term names darkness as the inner-spiritual domain the person inhabits, is rescued from, or identifies with — their fundamental spiritual condition and identity | active |
| `7432-002` | Term names darkness as the moral domain of works and powers the person either participates in or is called to renounce | active |
| `7432-004` | Term names the outer darkness of eschatological judgment — the ultimate inner-being consequence of exclusion from God | active |

**Verses:**

**vid=231957** — `Mat 4:16`
*target word in verse:* `darkness`

> Mat 4:16 the people dwelling in darkness have seen a great light , and for those dwelling in the region and shadow of death , on them a light has dawned .”

_Prior classification: **relevant** · related · group=`7432-001`_


**vid=231958** — `Mat 6:23`
*target word in verse:* `darkness`

> Mat 6:23 but if your eye is bad , your whole body will be full of darkness . If then the light in you is darkness , how great is the darkness !

_Prior classification: **relevant** · related · group=`7432-003`_


**vid=231959** — `Mat 8:12`
*target word in verse:* `darkness`

> Mat 8:12 while the sons of the kingdom will be thrown into the outer darkness . In that place there will be weeping and gnashing of teeth .”

_Prior classification: **relevant** · related · group=`7432-004`_


**vid=231954** — `Mat 22:13`
*target word in verse:* `darkness`

> Mat 22:13 Then the king said to the attendants , ‘ Bind him hand and foot and cast him into the outer darkness . In that place there will be weeping and gnashing of teeth .’

_Prior classification: **relevant** · **anchor** · group=`7432-004`_


**vid=231955** — `Mat 25:30`
*target word in verse:* `darkness`

> Mat 25:30 And cast the worthless servant into the outer darkness . In that place there will be weeping and gnashing of teeth .’

_Prior classification: **relevant** · related · group=`7432-004`_


**vid=231956** — `Mat 27:45`
*target word in verse:* `darkness`

> Mat 27:45 Now from the sixth hour there was darkness over all the land until the ninth hour .

_Prior classification: *set aside — reason: `(not set)`*_


**vid=231953** — `Mar 15:33`
*target word in verse:* `darkness`

> Mar 15:33 And when the sixth hour had come , there was darkness over the whole land until the ninth hour .

_Prior classification: *set aside — reason: `(not set)`*_


**vid=231949** — `Luk 1:79`
*target word in verse:* `darkness`

> Luk 1:79 to give light to those who sit in darkness and in the shadow of death , to guide our feet into the way of peace .”

_Prior classification: **relevant** · related · group=`7432-001`_


**vid=231950** — `Luk 11:35`
*target word in verse:* `darkness`

> Luk 11:35 Therefore be careful lest the light in you be darkness .

_Prior classification: **relevant** · related · group=`7432-003`_


**vid=231951** — `Luk 22:53`
*target word in verse:* `darkness`

> Luk 22:53 When I was with you day after day in the temple , you did not lay hands on me . But this is your hour , and the power of darkness .”

_Prior classification: **relevant** · related · group=`7432-002`_


**vid=231952** — `Luk 23:44`
*target word in verse:* `darkness`

> Luk 23:44 It was now about the sixth hour , and there was darkness over the whole land until the ninth hour ,

_Prior classification: *set aside — reason: `(not set)`*_


**vid=231947** — `Joh 3:19`
*target word in verse:* `darkness`

> Joh 3:19 And this is the judgment : the light has come into the world , and people loved the darkness rather than the light because their works were evil .

_Prior classification: **relevant** · related · group=`7432-001`_


**vid=231941** — `Act 2:20`
*target word in verse:* `darkness`

> Act 2:20 the sun shall be turned to darkness and the moon to blood , before the day of the Lord comes , the great and magnificent day.

_Prior classification: *set aside — reason: `(not set)`*_


**vid=231940** — `Act 13:11`
*target word in verse:* `darkness`

> Act 13:11 And now , behold , the hand of the Lord is upon you , and you will be blind and unable to see the sun for a time .” Immediately mist and darkness fell upon him , and he went about seeking people to lead him by the hand .

_Prior classification: **relevant** · related · group=`7432-001`_


**vid=231942** — `Act 26:18`
*target word in verse:* `darkness`

> Act 26:18 to open their eyes , so that they may turn from darkness to light and from the power of Satan to God , that they may receive forgiveness of sins and a place among those who are sanctified by faith in me .’

_Prior classification: **relevant** · related · group=`7432-001`_


**vid=231961** — `Rom 2:19`
*target word in verse:* `darkness`

> Rom 2:19 and if you are sure that you yourself are a guide to the blind , a light to those who are in darkness ,

_Prior classification: **relevant** · related · group=`7432-001`_


**vid=231960** — `Rom 13:12`
*target word in verse:* `darkness`

> Rom 13:12 The night is far gone ; the day is at hand . So then let us cast off the works of darkness and put on the armor of light .

_Prior classification: **relevant** · **anchor** · group=`7432-002`_


**vid=231932** — `1Cor 4:5`
*target word in verse:* `darkness`

> 1Cor 4:5 Therefore do not pronounce judgment before the time , before the Lord comes , who will bring to light the things now hidden in darkness and will disclose the purposes of the heart . Then each one will receive his commendation from God .

_Prior classification: **relevant** · **anchor** · group=`7432-003`_


**vid=231937** — `2Cor 4:6`
*target word in verse:* `darkness`

> 2Cor 4:6 For God , who said , “Let light shine out of darkness ,” has shone in our hearts to give the light of the knowledge of the glory of God in the face of Jesus Christ .

_Prior classification: **relevant** · related · group=`7432-001`_


**vid=231938** — `2Cor 6:14`
*target word in verse:* `darkness`

> 2Cor 6:14 Do not be unequally yoked with unbelievers . For what partnership has righteousness with lawlessness ? Or what fellowship has light with darkness ?

_Prior classification: **relevant** · related · group=`7432-002`_


**vid=231945** — `Eph 5:8`
*target word in verse:* `darkness`

> Eph 5:8 for at one time you were darkness , but now you are light in the Lord . Walk as children of light

_Prior classification: **relevant** · **anchor** · group=`7432-001`_


**vid=231944** — `Eph 5:11`
*target word in verse:* `darkness`

> Eph 5:11 Take no part in the unfruitful works of darkness , but instead expose them.

_Prior classification: **relevant** · related · group=`7432-002`_


**vid=231946** — `Eph 6:12`
*target word in verse:* `darkness`

> Eph 6:12 For we do not wrestle against flesh and blood , but against the rulers , against the authorities , against the cosmic powers over this present darkness , against the spiritual forces of evil in the heavenly places .

_Prior classification: **relevant** · related · group=`7432-002`_


**vid=231943** — `Col 1:13`
*target word in verse:* `darkness`

> Col 1:13 He has delivered us from the domain of darkness and transferred us to the kingdom of his beloved Son ,

_Prior classification: **relevant** · related · group=`7432-001`_


**vid=231935** — `1Th 5:4`
*target word in verse:* `darkness`

> 1Th 5:4 But you are not in darkness , brothers , for that day to surprise you like a thief .

_Prior classification: **relevant** · related · group=`7432-001`_


**vid=231936** — `1Th 5:5`
*target word in verse:* `darkness`

> 1Th 5:5 For you are all children of light , children of the day . We are not of the night or of the darkness .

_Prior classification: **relevant** · related · group=`7432-001`_


**vid=231934** — `1Pe 2:9`
*target word in verse:* `darkness`

> 1Pe 2:9 But you are a chosen race , a royal priesthood , a holy nation , a people for his own possession , that you may proclaim the excellencies of him who called you out of darkness into his marvelous light .

_Prior classification: **relevant** · related · group=`7432-001`_


**vid=231939** — `2Pe 2:17`
*target word in verse:* `darkness`

> 2Pe 2:17 These are waterless springs and mists driven by a storm . For them the gloom of utter darkness has been reserved .

_Prior classification: **relevant** · related · group=`7432-004`_


**vid=231933** — `1Jo 1:6`
*target word in verse:* `darkness`

> 1Jo 1:6 If we say we have fellowship with him while we walk in darkness , we lie and do not practice the truth .

_Prior classification: **relevant** · related · group=`7432-001`_


**vid=231948** — `Jude 13`
*target word in verse:* `darkness`

> Jude 13 wild waves of the sea , casting up the foam of their own shame ; wandering stars , for whom the gloom of utter darkness has been reserved forever .

_Prior classification: **relevant** · related · group=`7432-004`_



### G4656 · *skotoō* — 1 active verse(s)

- **mti_term_id:** 1371
- **Language:** Greek
- **Gloss:** to darken
- **Total verses:** 1 (1 active, 0 delete_flagged)
- **Prior verse_context groups:** 1 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `1371-001` | Term describes the plunging of the person/domain into darkness as divine judgment, producing inner anguish | active |

**Verses:**

**vid=55299** — `Rev 16:10`
*target word in verse:* `darkness`

> Rev 16:10 The fifth angel poured out his bowl on the throne of the beast , and its kingdom was plunged into darkness . People gnawed their tongues in anguish

_Prior classification: **relevant** · **anchor** · group=`1371-001`_



### G5185 · *tuflos* — 46 active verse(s)

- **mti_term_id:** 1372
- **Language:** Greek
- **Gloss:** blind
- **Total verses:** 46 (46 active, 0 delete_flagged)
- **Prior verse_context groups:** 2 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `1372-001` | Term names spiritual blindness as the inner condition of the person who claims to see but does not — self-deception, hardness, and inability to perceive spiritual reality | active |
| `1372-002` | Term names physical blindness as the condition the Messiah proclaims he will heal — the restoration of perceptual capacity as part of Messianic mission, pointing to the recovery of spiritual sight | active |

**Verses:**

**vid=55342** — `Mat 9:27`
*target word in verse:* `blind`

> Mat 9:27 And as Jesus passed on from there , two blind men followed him , crying aloud , “Have mercy on us , Son of David .”

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55343** — `Mat 9:28`
*target word in verse:* `blind`

> Mat 9:28 When he entered the house , the blind men came to him , and Jesus said to them , “Do you believe that I am able to do this ?” They said to him , “ Yes , Lord .”

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55330** — `Mat 11:5`
*target word in verse:* `blind`

> Mat 11:5 the blind receive their sight and the lame walk , lepers are cleansed and the deaf hear , and the dead are raised up , and the poor have good news preached to them.

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55331** — `Mat 12:22`
*target word in verse:* `blind`

> Mat 12:22 Then a demon-oppressed man who was blind and mute was brought to him , and he healed him , so that the man spoke and saw .

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55332** — `Mat 15:14`
*target word in verse:* `blind`

> Mat 15:14 Let them alone ; they are blind guides . And if the blind lead the blind , both will fall into a pit .”

_Prior classification: **relevant** · related · group=`1372-001`_


**vid=55333** — `Mat 15:30`
*target word in verse:* `blind`

> Mat 15:30 And great crowds came to him , bringing with them the lame , the blind , the crippled , the mute , and many others , and they put them at his feet , and he healed them ,

_Prior classification: *set aside — reason: `(not set)`*_


**vid=55334** — `Mat 15:31`
*target word in verse:* `blind`

> Mat 15:31 so that the crowd wondered , when they saw the mute speaking , the crippled healthy , the lame walking , and the blind seeing . And they glorified the God of Israel .

_Prior classification: *set aside — reason: `(not set)`*_


**vid=55335** — `Mat 20:30`
*target word in verse:* `blind men`

> Mat 20:30 And behold , there were two blind men sitting by the roadside , and when they heard that Jesus was passing by , they cried out , “ Lord , have mercy on us , Son of David !”

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55336** — `Mat 21:14`
*target word in verse:* `blind`

> Mat 21:14 And the blind and the lame came to him in the temple , and he healed them .

_Prior classification: *set aside — reason: `(not set)`*_


**vid=55337** — `Mat 23:16`
*target word in verse:* `blind`

> Mat 23:16 “ Woe to you , blind guides , who say , ‘ If anyone swears by the temple , it is nothing , but if anyone swears by the gold of the temple , he is bound by his oath .’

_Prior classification: **relevant** · related · group=`1372-001`_


**vid=55338** — `Mat 23:17`
*target word in verse:* `blind`

> Mat 23:17 You blind fools ! For which is greater , the gold or the temple that has made the gold sacred ?

_Prior classification: **relevant** · related · group=`1372-001`_


**vid=55339** — `Mat 23:19`
*target word in verse:* `blind`

> Mat 23:19 You blind men! For which is greater , the gift or the altar that makes the gift sacred ?

_Prior classification: **relevant** · related · group=`1372-001`_


**vid=55340** — `Mat 23:24`
*target word in verse:* `blind`

> Mat 23:24 You blind guides , straining out a gnat and swallowing a camel !

_Prior classification: **relevant** · related · group=`1372-001`_


**vid=55341** — `Mat 23:26`
*target word in verse:* `blind`

> Mat 23:26 You blind Pharisee ! First clean the inside of the cup and the plate , that the outside also may be clean .

_Prior classification: **relevant** · related · group=`1372-001`_


**vid=55328** — `Mar 8:22`
*target word in verse:* `blind man`

> Mar 8:22 And they came to Bethsaida . And some people brought to him a blind man and begged him to touch him .

_Prior classification: *set aside — reason: `(not set)`*_


**vid=55329** — `Mar 8:23`
*target word in verse:* `blind man`

> Mar 8:23 And he took the blind man by the hand and led him out of the village , and when he had spit on his eyes and laid his hands on him , he asked him , “Do you see anything ?”

_Prior classification: *set aside — reason: `(not set)`*_


**vid=55325** — `Mar 10:46`
*target word in verse:* `blind`

> Mar 10:46 And they came to Jericho . And as he was leaving Jericho with his disciples and a great crowd , Bartimaeus , a blind beggar , the son of Timaeus , was sitting by the roadside .

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55326** — `Mar 10:49`
*target word in verse:* `blind man`

> Mar 10:49 And Jesus stopped and said , “ Call him .” And they called the blind man , saying to him , “Take heart . Get up ; he is calling you .”

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55327** — `Mar 10:51`
*target word in verse:* `blind man`

> Mar 10:51 And Jesus said to him , “ What do you want me to do for you ?” And the blind man said to him , “ Rabbi , let me recover my sight .”

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55321** — `Luk 4:18`
*target word in verse:* `blind`

> Luk 4:18 “The Spirit of the Lord is upon me , because he has anointed me to proclaim good news to the poor . He has sent me to proclaim liberty to the captives and recovering of sight to the blind , to set at liberty those who are oppressed ,

_Prior classification: **relevant** · **anchor** · group=`1372-002`_


**vid=55322** — `Luk 6:39`
*target word in verse:* `blind`

> Luk 6:39 He also told them a parable : “ Can a blind man lead a blind man? Will they not both fall into a pit ?

_Prior classification: **relevant** · related · group=`1372-001`_


**vid=55323** — `Luk 7:21`
*target word in verse:* `blind`

> Luk 7:21 In that hour he healed many people of diseases and plagues and evil spirits , and on many who were blind he bestowed sight .

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55324** — `Luk 7:22`
*target word in verse:* `blind`

> Luk 7:22 And he answered them , “ Go and tell John what you have seen and heard : the blind receive their sight , the lame walk , lepers are cleansed , and the deaf hear , the dead are raised up, the poor have good news preached to them.

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55318** — `Luk 14:13`
*target word in verse:* `blind`

> Luk 14:13 But when you give a feast , invite the poor , the crippled , the lame , the blind ,

_Prior classification: *set aside — reason: `(not set)`*_


**vid=55319** — `Luk 14:21`
*target word in verse:* `blind`

> Luk 14:21 So the servant came and reported these things to his master . Then the master of the house became angry and said to his servant , ‘ Go out quickly to the streets and lanes of the city , and bring in the poor and crippled and blind and lame .’

_Prior classification: *set aside — reason: `(not set)`*_


**vid=55320** — `Luk 18:35`
*target word in verse:* `blind man`

> Luk 18:35 As he drew near to Jericho , a blind man was sitting by the roadside begging .

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55304** — `Joh 5:3`
*target word in verse:* `blind`

> Joh 5:3 In these lay a multitude of invalids — blind , lame , and paralyzed .

_Prior classification: *set aside — reason: `(not set)`*_


**vid=55305** — `Joh 9:1`
*target word in verse:* `blind`

> Joh 9:1 As he passed by , he saw a man blind from birth .

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55310** — `Joh 9:2`
*target word in verse:* `blind`

> Joh 9:2 And his disciples asked him , “ Rabbi , who sinned , this man or his parents , that he was born blind ?”

_Prior classification: **relevant** · related · group=`1372-001`_


**vid=55306** — `Joh 9:13`
*target word in verse:* `blind`

> Joh 9:13 They brought to the Pharisees the man who had formerly been blind .

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55307** — `Joh 9:17`
*target word in verse:* `blind man`

> Joh 9:17 So they said again to the blind man , “ What do you say about him , since he has opened your eyes ?” He said , “He is a prophet .”

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55308** — `Joh 9:18`
*target word in verse:* `blind`

> Joh 9:18 The Jews did not believe that he had been blind and had received his sight , until they called the parents of the man who had received his sight

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55309** — `Joh 9:19`
*target word in verse:* `blind`

> Joh 9:19 and asked them , “ Is this your son , who you say was born blind ? How then does he now see ?”

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55311** — `Joh 9:20`
*target word in verse:* `blind`

> Joh 9:20 His parents answered , “We know that this is our son and that he was born blind .

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55312** — `Joh 9:24`
*target word in verse:* `blind`

> Joh 9:24 So for the second time they called the man who had been blind and said to him , “ Give glory to God . We know that this man is a sinner .”

_Prior classification: **relevant** · related · group=`1372-001`_


**vid=55313** — `Joh 9:25`
*target word in verse:* `blind`

> Joh 9:25 He answered , “ Whether he is a sinner I do not know . One thing I do know , that though I was blind , now I see .”

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55314** — `Joh 9:32`
*target word in verse:* `blind`

> Joh 9:32 Never since the world began has it been heard that anyone opened the eyes of a man born blind .

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55315** — `Joh 9:39`
*target word in verse:* `blind`

> Joh 9:39 Jesus said , “ For judgment I came into this world , that those who do not see may see , and those who see may become blind .”

_Prior classification: **relevant** · related · group=`1372-001`_


**vid=55316** — `Joh 9:40`
*target word in verse:* `blind`

> Joh 9:40 Some of the Pharisees near him heard these things , and said to him , “ Are we also blind ?”

_Prior classification: **relevant** · related · group=`1372-001`_


**vid=55317** — `Joh 9:41`
*target word in verse:* `blind`

> Joh 9:41 Jesus said to them , “ If you were blind , you would have no guilt ; but now that you say , ‘We see ,’ your guilt remains .

_Prior classification: **relevant** · **anchor** · group=`1372-001`_


**vid=55302** — `Joh 10:21`
*target word in verse:* `blind`

> Joh 10:21 Others said , “ These are not the words of one who is oppressed by a demon . Can a demon open the eyes of the blind ?”

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55303** — `Joh 11:37`
*target word in verse:* `blind`

> Joh 11:37 But some of them said , “ Could not he who opened the eyes of the blind man also have kept this man from dying ?”

_Prior classification: **relevant** · related · group=`1372-002`_


**vid=55301** — `Act 13:11`
*target word in verse:* `blind`

> Act 13:11 And now , behold , the hand of the Lord is upon you , and you will be blind and unable to see the sun for a time .” Immediately mist and darkness fell upon him , and he went about seeking people to lead him by the hand .

_Prior classification: **relevant** · related · group=`1372-001`_


**vid=55345** — `Rom 2:19`
*target word in verse:* `blind`

> Rom 2:19 and if you are sure that you yourself are a guide to the blind , a light to those who are in darkness ,

_Prior classification: **relevant** · related · group=`1372-001`_


**vid=55300** — `2Pe 1:9`
*target word in verse:* `blind`

> 2Pe 1:9 For whoever lacks these qualities is so nearsighted that he is blind , having forgotten that he was cleansed from his former sins .

_Prior classification: **relevant** · related · group=`1372-001`_


**vid=55344** — `Rev 3:17`
*target word in verse:* `blind`

> Rev 3:17 For you say , “I am rich , I have prospered , and I need nothing ,” not realizing that you are wretched , pitiable , poor , blind , and naked .

_Prior classification: **relevant** · related · group=`1372-001`_



### G5186 · *tufloō* — 3 active verse(s)

- **mti_term_id:** 7438
- **Language:** Greek
- **Gloss:** to blind
- **Total verses:** 3 (3 active, 0 delete_flagged)
- **Prior verse_context groups:** 1 active

**Existing verse_context groups for this term:**

| group_code | description | state |
|---|---|---|
| `7438-001` | Term describes the act of blinding the inner capacities — eyes and mind — whether through hatred, spiritual adversary, or divine judicial action | active |

**Verses:**

**vid=231995** — `Joh 12:40`
*target word in verse:* `blinded`

> Joh 12:40 “He has blinded their eyes and hardened their heart , lest they see with their eyes , and understand with their heart , and turn , and I would heal them .”

_Prior classification: **relevant** · related · group=`7438-001`_


**vid=231994** — `2Cor 4:4`
*target word in verse:* `blinded`

> 2Cor 4:4 In their case the god of this world has blinded the minds of the unbelievers , to keep them from seeing the light of the gospel of the glory of Christ , who is the image of God .

_Prior classification: **relevant** · **anchor** · group=`7438-001`_


**vid=231993** — `1Jo 2:11`
*target word in verse:* `blinded`

> 1Jo 2:11 But whoever hates his brother is in the darkness and walks in the darkness , and does not know where he is going , because the darkness has blinded his eyes .

_Prior classification: **relevant** · related · group=`7438-001`_



---

## 4. Terms

The full term landscape of this registry. OWNER terms are classified in
Verse Context; XREF terms are not — their classification is inherited
programme-wide from the OWNER's VC records (VC Instruction §0.2).


### OWNER terms

| mti_id | Strong's | Translit. | Gloss | Language | mti_status | Occurrence |
|---:|---|---|---|---|---|---:|
| 7435 | `G4652` | skoteinos | dark | Greek | extracted | 17 |
| 7433 | `G4653` | skotia | darkness | Greek | extracted | 20 |
| 7434 | `G4654` | skotizō | to darken | Greek | extracted | 15 |
| 7432 | `G4655` | skotos | darkness | Greek | extracted | 131 |
| 1371 | `G4656` | skotoō | to darken | Greek | extracted | 8 |
| 1372 | `G5185` | tuflos | blind | Greek | extracted | 81 |
| 7438 | `G5186` | tufloō | to blind | Greek | extracted | 4 |

### XREF terms

_These terms appear in this registry's term inventory but their primary
analytical home (OWNER) is in a different registry. Their verse records
are `delete_flagged = 1` here. Do not classify them in this session —
VC records for them (if any) are held at the OWNER._


| Strong's | Translit. | Gloss | Language | OWNER registry | OWNER word |
|---|---|---|---|---:|---|
| `G4456` | pōroō | to harden | Greek | — | to harden |
| `G4456` | pōroō | to harden | Greek | 74 | to harden |
| `G4457` | pōrōsis | hardening | Greek | — | hardening |
| `G4457` | pōrōsis | hardening | Greek | 74 | hardness |

---

## 5. Pointers

Structural pointers — what other parts of the programme this registry
connects to. Informational only; not required for classification.
No Session D synthesis content here.


### Cross-registry links

_None recorded._

### Terms borrowed as XREF from other registries

Each XREF term's OWNER registry is its primary analytical home.

- `G4456` *pōroō* — OWNER: registry — (to harden)
- `G4456` *pōroō* — OWNER: registry 74 (to harden)
- `G4457` *pōrōsis* — OWNER: registry — (hardening)
- `G4457` *pōrōsis* — OWNER: registry 74 (hardness)

---

## 6. Questions

Open questions from the programme's observation catalogue linked to the
data-quality flags present on this registry. These are for awareness —
classification work may surface answers; many will be addressed in
Session B. Do not try to answer them here.


_No open catalogue questions linked to this registry's flags._

---

## Footer — what happens next

When a Claude AI Verse Context session consumes this document, it produces a
`VERSECONTEXT` patch populating `verse_context_group` and `verse_context`
for each OWNER term. The patch is applied via
`scripts/apply_session_patch.py`; Claude Code then advances this registry's
`verse_context_status` to Complete (subject to XREF coverage rules per
VC Instruction §0.2) and a fresh Session A `.md` can be regenerated.

*End of Session A prose for registry 207 blindness (spiritual).*

...
Version 1_7 | 20260427 | Status: Active — Architecture v2 addendum applied (readiness is now a CC operation)**

| **Document** | **Value** |
|---|---|
| Filename | wa-sessionb-analysis-readiness-v1_7-20260427.md |
| Supersedes | wa-sessionb-analysis-readiness-v1_6-20260418.md (Architecture v2 — readiness is now mostly a CC operation) |
| Companion documents | wa-global-general-rules [current] │ wa-reference [current] │ wa-dimensionreview-instruction [current] │ wa-versecontext-instruction [current] │ wa-claudecode-instruction [current] │ wa-patch-instruction [current] │ wa-directive-instruction [current] │ db-capture-phase1-results-and-table-architecture-v1-20260427.md |
| Inputs (v1_7) | DB state at generation time (no JSON export needed; CC reads tables directly via the readiness generator) |
| Outputs (v1_7) | **Readiness `.md` (human-readable) + Readiness `.json` (machine-readable)**, plus Analytic Status `.md`+`.json` for revision sessions |
| Claude AI role (v1_7) | **Minimal under v2 — readiness is now a CC operation.** AI's role limited to receiving the readiness output and proceeding to Analysis Output |
| Claude Code role (v1_7) | **Owns the readiness phase end-to-end:** generate readiness `.md`+`.json`, validate field-level destinations, populate §N Open Items, run pre-flight integrity checks, raise `DATA_ANOMALY_*` findings as `wa_session_b_findings.status='open'` if data inconsistent |

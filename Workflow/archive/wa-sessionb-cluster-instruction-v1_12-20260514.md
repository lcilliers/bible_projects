# wa-sessionb-cluster-instruction-v1_12-20260514

> Framework B Soul Word Analysis Programme — Session B Cluster Analysis
> Version: v1_12 | Date: 20260514
...
> **Change note v1_11 → v1_12 (2026-05-14):** Three preventive additions from M39 first-run lessons. (1) §10.4 SCOPE Operation 2 bullet — explicit reminder that Phase 7 must populate **both** `verse_context.group_id` and `verse_context.cluster_subgroup_id` (Phase 7 directives have always specified this; M39's apply script omitted `cluster_subgroup_id` and the §13.8 validator caught it with 15 false-positive C1 gaps; reminder added so future apply scripts don't repeat the omission). (2) §A1 column-reference appendix extended: added **`wa_obs_question_catalogue`** (column is `question_code`, not `prompt_code` — caught in dir-005 authoring) and **`wa_dimension_index`** with the C-code legacy caveat (the `cluster_assignment` column still uses pre-2026-05-04 C01–C22 codes; join via `verse_context_group_id` to current M-cluster). (3) §11.8 marker catalogue — added `**[BOUNDARY — H1234 translit]**` per-term marker convention as the canonical pattern for §11.6 BOUNDARY characterisations (M39 used a single consolidated `**[BOUNDARY]**` block; per-term is the §11.6 intent). No analytical-rule changes.
> Full history: see §19.
...
*wa-sessionb-cluster-instruction-v1_12-20260514 | Active — authoritative instruction for Session B cluster analytics*
*Replaces (on finalisation): wa-sessionb-analysis-readiness, wa-sessionb-analysis-output, wa-dimensionreview-instruction, wa-versecontext-instruction, wa-registry-management-guide*

...
Version 1_3 | 20260427 | Status: Active — Prose-revision-on-review rule added (v2.7)**

| **Document** | **Value** |
|---|---|
| Filename | wa-sessionb-analysis-output-v1_3-20260427.md |
| Supersedes | wa-sessionb-analysis-output-v1_2-20260427.md (adds v2.7 prose-revision-on-review rule) |
| Companion documents | wa-global-general-rules [current] │ wa-reference [current] │ wa-claudecode-instruction [current] │ wa-patch-instruction [current] │ wa-directive-instruction [current] │ wa-sessionc-instruction [current] │ wa-sessionb-analysis-readiness [current] │ db-capture-phase1-results-and-table-architecture-v1-20260427.md |
| Inputs (v1_3) | **Readiness `.md` (data) + Analytic Status `.md` (revision input only) + master catalogue [embedded in readiness §L]** |
| Outputs (v1_3) | **Comprehensive obslog `.md` only.** CC parses obslog → DB writes (no patches). Side-effect: chapters in `prose_section`, findings + Q&A links + entity links + verse_context.analysis_note populated. **Revision sessions** must include superseded chapter blocks per v2.7 when prior prose is affected by the revision. |
...
| Prerequisite | Readiness `.md` produced by CC (replaces v1's "Stage 1 Completion Record" gate under v2) |
...
For initial analysis: **only the readiness `.md`** (data). It carries: §A registry overview · §B Stage 1 Completion Record (synthesised) · §C term inventory · §D lexical foundation · §E XREF · §F group landscape with dimensions · §G correlation signals · §H existing flags + findings · §I phase2 flags · §J verbatim verse text · §K legacy-VC notice · §L generic catalogue (147) + registry-specific · §M readiness verification · **§N Open Session B Items**.
...
For every universal catalogue question (147 in Sections 1-5), Stage 2b must produce one of:
...
- Session C prose rule: `wa-global-sessionc-prose-rule [current]` — load and read before writing any chapter
...
*wa-sessionb-analysis-output-v1_3-20260427.md*
*Framework B — Soul Word Analysis Programme*
*Supersedes wa-sessionb-analysis-output-v1_2-20260427.md (adds v2.7 prose-revision-on-review rule)*

# wa-sessionb-cluster-instruction-v1_0-draft-20260507

> Framework B Soul Word Analysis Programme — Session B Cluster Analysis
> Version: v1_0 (draft) | Date: 20260507
> Status: DRAFT — under researcher review for M05 first-run
> Researcher decisions baked in (2026-05-07): Phase 7 default = per-sub-group directive (§10); Phase 2 default = VCREVISE patch with AI-resolved IDs (§5); legacy `wa_session_research_flags` retained as appendix-tier with explicit handling rules (§12.1).
...
- wa-sessionc-instruction [current] — downstream consumer of completed cluster findings
...
- `wa-cluster-overview-v{N}-{date}.md` — cluster's place in the programme
...
**Purpose:** subdivide the cluster's terms into provisional sub-groups based on distinct characteristics. Apply the T1 framework definition: a distinct inner-being characteristic has identifiable constitutional location, engages a distinguishable set of inner faculties, produces recognisable impact, has a structural opposite, and can be distinguished by causal direction or directional object.

**Recognised tension** — the inner being is not made of separable bricks neatly assignable to sub-groups. It is made of compounds that morph and change character through the person's inner life. **The onion principle:** each analytical layer peels without discarding the whole. Sub-groupings are peels — not divisions.

**Inputs:**

- A fresh extract of the cluster dataset including all glosses for all terms
- `wa-cluster-{code}-detail-v{N}-{date}.md` — gloss list per term, root family info (extended in v2 of the grouped report)
- `wa-cluster-overview-v{N}-{date}.md` — cluster's named characteristic (e.g., M06 "Hate, Contempt and Hostility")
...
- Outcome codes used: **E** (evidenced — the verse evidence confirms this), **S** (silent — the verses do not address this prompt), **G** (gap — the question should be answerable but requires further investigation or CC database query).
- No theoretical elaboration beyond what the text shows.
- Silence recorded as silence, not glossed over.
- Each sub-group must have a detailed answer for each catalogue prompt.
- All observations rooted in the cluster's data — not from memory, not from AI training, not from extraneous knowledge.
...
- Every prompt in the catalogue was applied to each of the seven sub-groups (or BOUNDARY where the cluster has one).
...
**Do not rerun the entire analysis** to prepare the directive — the directive is built from the obslog + consolidated findings, not from a new pass.
...
| Comprehensive cluster | `_generate_cluster_comprehensive_v1_*.py` | 1, 2, 4 | Full per-term + verses + groups + findings + pointers; verses by sub-group (post-apply) |
...
| 3 | Every term is in a sub-group, BOUNDARY, or flagged for cluster reassignment |
| 4 | Every flagged term has a researcher-resolution path; sub-group counts match cluster term count |
| 5 | One summary per sub-group; each summary names verse anchors |
| 6 | Every non-set-aside verse is assigned to a group; every group has an anchor |
| 8 | Every prompt × sub-group cell has a finding, silent, or gap marker |
| 9 | Validation step complete; every gap has CC-query path or external-dependency note |
| 10 | All verification flags addressed; cluster-status flip authorised |
...
## 18. Open items for finalisation

This is a v1_0 draft. Three of the original five open items were resolved by researcher direction on 2026-05-07 and are now baked into the instruction body:

- ~~Phase 7 split~~ — **resolved.** Default is **per sub-group directive** (§10). Combined directive permitted only by explicit researcher direction.
- ~~Phase 2 routing~~ — **resolved.** Default is **VCREVISE patch** with AI-resolved `verse_record_id`s (§5). No new mechanism introduced.
- ~~Legacy `wa_session_research_flags`~~ — **resolved.** Retained as appendix-tier; cluster findings layer is additive. Full handling instructions in §12.1.

Remaining open items expected to be settled by the M05 first-run:

1. **Catalogue version pinning** — whether the Phase 9 directive should pin `catalogue_version` in `_patch_meta` or `notes`. Currently implicit via `wa_obs_question_catalogue` lookup at apply time. M05 will demonstrate whether explicit pinning is needed.
2. **Cluster status update routing** — whether the Phase 10 step 5 cluster-status flip deserves a standalone directive or rides on the corrections directive's outcome. M06 used a one-line UPDATE via Bash; instruction-grade practice prefers a directive. M05 will set the precedent.

When the M05 run completes, the resolution of these two items will produce v1_1.

---

*wa-sessionb-cluster-instruction-v1_0-draft-20260507 | DRAFT — under researcher review for M05 first-run*
...
*Cross-references: wa-directive-instruction [current] §11 (cluster-process directives), §10 (schema enablement); wa-patch-instruction [current] §3 (patch types), §15 (input version gate)*

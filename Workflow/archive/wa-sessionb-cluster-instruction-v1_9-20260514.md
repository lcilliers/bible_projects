# wa-sessionb-cluster-instruction-v1_9-20260514

> Framework B Soul Word Analysis Programme — Session B Cluster Analysis
> Version: v1_9 | Date: 20260514
...
> **Change note v1_8 → v1_9 (2026-05-14):** Authoring discipline — package directives, automate status transitions. (1) New §2.5 **Directive packaging discipline**: every directive bundles its phase's full operation set; standalone "ceremony" directives (status-only, single-field write) are forbidden; splitting requires a stated failure-isolation reason. (2) New §2.6 **Status transition discipline**: all `cluster.status` transitions are operations within the relevant phase's directive — never a standalone directive. CC processes the transition automatically as part of the directive's operation succession. (3) §7.7 rewritten — single packaged subgroup-assign directive with Operation A (sub-group create + assign) · Operation B (cluster_code rebind, if applicable) · Operation C (status `Data - In Progress` → `Analysis - In Progress`); no standalone term-rebind or status-bump directives. (4) §9.5 output document sections renamed §A–§E → §1–§5 — resolves the letter-collision with the Pass A / B / C labels in §9.3. Cross-refs updated in §10.2 and §10.4. (5) §10.3 (new) **Pre-assessment and segmentation declaration**: Phase 7 begins with a data-volume pre-assessment written to the obslog declaring the processing segments (cluster-wide / per-sub-group / within-sub-group / combined); the obslog records a segment-close summary at the end of each segment before the next begins. (6) §13.9 closure simplified — the `Analysis Completed` transition is an operation within the verification-corrections directive; no standalone status-complete directive. (7) §17 Phase 7 pre-check gains the pre-assessment requirement.
> Full history: see §19.
...
**Operation A — `cluster_subgroup` create + `mti_terms.cluster_subgroup_id` assign.** Create each sub-group row; assign each term's `mti_terms.cluster_subgroup_id`.
...
- **COMPLETION CONFIRMATION** — `SELECT subgroup_code, COUNT(*) FROM mti_terms mt JOIN cluster_subgroup cs ON cs.id=mt.cluster_subgroup_id WHERE mt.cluster_code='{code}' GROUP BY subgroup_code` matches expected counts; `SELECT status FROM cluster WHERE cluster_code='{code}'` returns `'Analysis - In Progress'`; if Operation B fired, a count query confirms the rebind volume.
...
*wa-sessionb-cluster-instruction-v1_9-20260514 | Active — authoritative instruction for Session B cluster analytics*
*Replaces (on finalisation): wa-sessionb-analysis-readiness, wa-sessionb-analysis-output, wa-dimensionreview-instruction, wa-versecontext-instruction, wa-registry-management-guide*

...
Claude AI must confirm the global rules file has been loaded before beginning any work in this session per GR-LOAD-001.
...
One Markdown file per word per version. All filenames lowercase per GR-FILE-007. Previous versions are never overwritten per GR-FILE-004.

| Version | Filename pattern | Example |
|---|---|---|
| v1 — initial | `wa-[nnn]-[word]-word-study-v1-[date].md` | `wa-068-grace-word-study-v1-20260409.md` |
| v2 — Session B update | `wa-[nnn]-[word]-word-study-v2-[date].md` | `wa-068-grace-word-study-v2-20260410.md` |
| v3 — Session D update | `wa-[nnn]-[word]-word-study-v3-[date].md` | `wa-068-grace-word-study-v3-[date].md` |

Dates in filenames use compact format YYYYMMDD per GR-FILE-009.
...
| `terms` — owner terms only, `gloss`, `god_as_subject`, `somatic_link`, `phase2_flags` fields only | Note each owner term's gloss, whether God is the primary actor, whether a somatic link is flagged, and what phase 2 flags are present. Do not read Mounce definitions, BDB senses, or related words lists at this stage. Note: `god_as_subject` and `somatic_link` are Phase 1 working fields (GR-DATA-005) — read them for orientation but treat them as indicative rather than authoritative. The authoritative somatic and god-as-subject records are in `mti_term_flags`. |
...
- Let what the data says come through — per GR-PROG-008, do not impose a pre-existing theological framework
...
**What to take into account:**
- The `somatic_link` field on each term indicates whether it has a documented bodily dimension — where set, this should be reflected in the term's description. Note: this is a Phase 1 working field per GR-DATA-005; treat it as indicative and verify against verse evidence in Section 3.
...
**Constraint:** Per GR-PROG-009: every connection listed must be traceable to something in the data. If a connection is inferential, say so explicitly. Inferential connections may not be presented as confirmed.
...
**Accuracy over accessibility.** Per GR-PROC-002: if the data does not support a statement, do not make it in order to produce a cleaner narrative. A gap honestly acknowledged is better than a gap filled with inference.

**Emergence over imposition.** Per GR-PROG-008: the character of each word must emerge from what the data says. Do not impose a pre-existing theological framework onto the material.

**Inferential is not confirmed.** Per GR-PROG-009: where a connection or claim is not supported by the data, label it explicitly as inferential.
...
**v1_5 (20260418):** GR-REF-002 sweep. (1) Instruction version banner: `v1.4 | 20260414` → `v1_5 | 20260418`. (2) Governing Rules section: stale `wa-global-general-rules-v2-20260414.json` → `[current]`; stale State-aloud example replaced with GR-LOAD-001 reference. (3) No other body content changed — this document has no operational cross-references to sibling instructions beyond the governing rules. Note on FLAG-001: Session C instruction remains flagged as "under construction" pending completion of Analysis Readiness and Analysis Output instructions; this reference sweep does not change that status.

v1.4 (20260414): Global rules integration. (1) Governing Rules header added — mandatory load of wa-global-general-rules-v2-20260414.json before session start. (2) Output File table: filename examples updated to compact date format per GR-FILE-009; "All filenames lowercase / do not overwrite" prose replaced with references to GR-FILE-007 and GR-FILE-004. (3) Pass 1 terms read: GR-DATA-005 caution added to somatic_link and god_as_subject field references — these are Phase 1 working fields, treat as indicative. (4) Section 4: GR-DATA-005 caution added to somatic_link reference. (5) Section 5 constraint: replaced with reference to GR-PROG-009. (6) Section 1 constraint: replaced with reference to GR-PROG-008. (7) Quality Principles: "Emergence over imposition" and "Accuracy over accessibility" replaced with references to GR-PROG-008 and GR-PROC-002; GR-PROG-009 added as a named principle.

v1.3 (20260411): Five additions. (1) XREF anchor verse ownership rule formalised. (2) Two-pass JSON reading procedure replacing the flat reading list. (3) Output filename convention updated. (4) Document lifecycle section added. (5) Completion note template updated.

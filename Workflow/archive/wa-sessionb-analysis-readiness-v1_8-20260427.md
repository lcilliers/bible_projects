...
Version 1_8 | 20260427 | Status: Active — Phase A prose precondition + paths post-restructure**

| **Document** | **Value** |
|---|---|
| Filename | wa-sessionb-analysis-readiness-v1_8-20260427.md |
| Supersedes | wa-sessionb-analysis-readiness-v1_7-20260427.md (adds Phase A prose precondition + post-restructure paths + explicit output destination) |
...
| Inputs (v1_8) | DB state at generation time, **including `prose_section` rows under `source_stage='session_a'` (the Phase A prose; see §v2.R0)**. No JSON export needed — CC reads tables directly via the readiness generator. |
| Outputs (v1_8) | **Readiness `.md` (human-readable) + Readiness `.json` (machine-readable)**, paired in `Sessions/Session_B/07_Analysis_Readiness_Status/`. Plus Analytic Status `.md`+`.json` for revision sessions (same folder). |
| Claude AI role (v1_8) | **Minimal under v2 — readiness is now a CC operation.** AI's role limited to receiving the readiness output and proceeding to Analysis Output |
| Claude Code role (v1_8) | **Owns the readiness phase end-to-end:** verify Phase A prose precondition (§v2.R0); generate readiness `.md`+`.json`; validate field-level destinations; populate §N Open Items; run pre-flight integrity checks; raise `DATA_ANOMALY_*` findings as `wa_session_b_findings.status='open'` if data inconsistent |
| Handoff to | wa-sessionb-analysis-output [current] — AI session begins on readiness `.md` (and analytic status `.md` if revision) |
...
*wa-sessionb-analysis-readiness-v1_8-20260427.md*
*Framework B — Soul Word Analysis Programme*
*Supersedes wa-sessionb-analysis-readiness-v1_7-20260427.md (adds Phase A prose precondition + post-restructure paths)*

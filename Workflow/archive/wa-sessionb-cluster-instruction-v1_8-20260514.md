...
> **Change note v1_7 → v1_8 (2026-05-14):** Streamlining + consistency pass. (1) Frontmatter de-duplicated — only the latest change note is in the header; full version history in §19. (2) §5 numbering gap closed (§5.4 → §5.2; no §5.2/§5.3 stub remained from the v1_6 reorganisation). (3) §5.1 heading historical annotation removed. (4) §17 Phase 4 row gains the `cluster.status='Analysis - In Progress'` post-check defined in §7.7. (5) §17 Phase 7 row gains the `vcg_term` backfill post-check defined in §10.4. (6) §11.6 BOUNDARY characterisation no longer hard-codes `T1.2.1` — softened to "the T1.2.x structural-role prompt (currently T1.2.1)". (7) §7.4 defines `OQ-NNN` on first use. No rule-meaning changes.
> Full history: see §19.
...
| **7 Mapping apply** | Every sub-group has Phase 6 mapping document with all five sections; all §C decisions explicit | COMPLETION CONFIRMATION queries match expected; application report written; **`vcg_term` backfill check** — every NEW VCG has ≥1 `vcg_term` row per `mti_term_id` whose verses belong to it (per §10.4) |
...
*wa-sessionb-cluster-instruction-v1_8-20260514 | Active — authoritative instruction for Session B cluster analytics*
*Replaces (on finalisation): wa-sessionb-analysis-readiness, wa-sessionb-analysis-output, wa-dimensionreview-instruction, wa-versecontext-instruction, wa-registry-management-guide*

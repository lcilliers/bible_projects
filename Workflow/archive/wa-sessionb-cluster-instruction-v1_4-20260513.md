# wa-sessionb-cluster-instruction-v1_4-20260513

> Framework B Soul Word Analysis Programme — Session B Cluster Analysis
> Version: v1_4 | Date: 20260513
...
### 4.1 Status-init directive (required at session open, before any analytical work)

When the comprehensive report is generated for a cluster currently at `cluster.status = 'Not started'`, AI's **first** action is to author a small status-init directive that transitions the cluster to `Data - In Progress`. This is not optional — preparing the first consolidated data and registering the analytical session as active are tied together.

- **Filename:** `wa-cluster-{code}-dir-001-status-init-v1-{date}.md`
- **Pattern:** cluster-process directive per `wa-directive-instruction [current]` (compact form — single UPDATE).
- **Five elements:**
  - **MOTIVATION** — cite session open; first consolidated data prepared; this is dir-001 for the cluster.
  - **SCOPE** — `UPDATE cluster SET status='Data - In Progress', last_updated_date=? WHERE cluster_code='{code}' AND status='Not started'`.
  - **OUTCOME REQUIRED** — one row updated; `cluster.status = 'Data - In Progress'`.
  - **COMPLETION CONFIRMATION** — `SELECT cluster_code, status, last_updated_date FROM cluster WHERE cluster_code='{code}'` reports `Data - In Progress`.
- **When this directive is NOT needed:** if the cluster's current status is already `Data - In Progress` (re-entry into Phase 1 mid-stream). Skip the directive; record in obslog that status was already `Data - In Progress` at session open.

The status-init directive is applied before Phase 1 process step 1 below. The obslog records the transition.
...
- Status-init directive applied (§4.1) — `cluster.status = 'Data - In Progress'`.
...
- `cluster.status = 'Data - In Progress'` (verified by the status-init directive's COMPLETION CONFIRMATION).
...
**DB writes:** One — the status-init directive (§4.1). The data reading itself is read-only.
...
**DB writes — VCREVISE patch:**

Phase 2 uses the standard VC patch path per `wa-patch-instruction [current]` §3, §15.

- AI runs an ID-resolver query (or asks CC for a one-shot lookup) mapping each (book, chapter, verse_num, mti_term_id) to its `verse_record_id` before drafting the patch.
- `_patch_meta.patch_type = "VCREVISE"`; `_patch_meta.terms_covered` lists every mti_term_id touched; `_patch_meta.input_versions` per `wa-patch-instruction` §15.6.4-bis.
- Filename: `wa-cluster-{code}-patch-vcrevise-utreview-v1-{date}.json`
- Operations:
...
|---|---|---|
| 2 | _(patch — VCREVISE)_ | `WA-{code}-UT-verse-review-v1-{date}.md` |
...
|---|---|---|
| **1 Comprehension** | Comprehensive report regenerated; obslog created; cluster status is `Not started` or `Data - In Progress` | Status-init directive applied (if status was `Not started`); `cluster.status='Data - In Progress'`; overview note answers all five count metrics |
...
**v1_4 (2026-05-13) — Active.** Two refinements: (1) §4 Phase 1 — cluster status transition `Not started` → `Data - In Progress` is now an explicit required step at session open, authored as `dir-001-status-init` before any analytical work. Previously hand-wavy. (2) §11 Phase 8 — per-cluster science extract document (`Workflow/Sciences/wa-{cluster_code_lower}-{name}-scienceextract-v{V}-{date}.md`) is a mandatory input for the catalogue pass; T7 prompts cannot be answered substantively without the scientific framing context. Added to §11.1 inputs, §11.2 pre-check, §14 reports map, and §17 pre/post controls table.
...
*wa-sessionb-cluster-instruction-v1_4-20260513 | Active — authoritative instruction for Session B cluster analytics*
*Replaces (on finalisation): wa-sessionb-analysis-readiness, wa-sessionb-analysis-output, wa-dimensionreview-instruction, wa-versecontext-instruction, wa-registry-management-guide*

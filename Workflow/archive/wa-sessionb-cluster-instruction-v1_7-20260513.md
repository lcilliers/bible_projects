# wa-sessionb-cluster-instruction-v1_7-20260513

> Framework B Soul Word Analysis Programme — Session B Cluster Analysis
> Version: v1_7 | Date: 20260513
> Status: **Active — authoritative instruction for Session B cluster analytics**
> **Change note v1_6 → v1_7 (2026-05-13):** Parser-safety tightening from M20 first-run lessons. (1) §11.8 — Phase 8 output style now **mandates one scope marker per line**: each `**[X]**`, `**[X — Label]**`, `**[A, B, C]**`, `**[CLUSTER]**` marker must be the first non-whitespace token on its line. Multiple scope markers on the same line (inline) are forbidden — they trip line-based parsers and produce ambiguous ownership. (2) §11.8 — **every prompt must have at least one explicit scope marker**. The shorthand "no-scope" pattern (where a prompt's body is plain prose with no `**[...]**` marker, implying cluster-wide coverage) is forbidden. If the finding is uniform across all sub-groups, use a single `**[CLUSTER]**` or `**[CLUSTER — all sub-groups]**` marker. (3) §11.8 — **one block per (prompt × scope)**. If the same scope appears twice for the same prompt, the parser will collapse by concatenation as a fallback, but the author should write one consolidated block per scope per prompt. (4) §11.8 — clarified inline outcome-code conventions and added an explicit **end-of-block marker** spec: `---` horizontal rule between prompts (preferred), or the next `**T#.#.#**` header. (5) §10.4 — Phase 7 directive content now explicitly requires **`vcg_term` INSERT** rows for every NEW VCG (post-M47 m:n schema). The M20 first-run revealed that apply scripts skipping `vcg_term` inserts produce VCGs invisible to the grouped report.
...
*wa-sessionb-cluster-instruction-v1_7-20260513 | Active — authoritative instruction for Session B cluster analytics*
*Replaces (on finalisation): wa-sessionb-analysis-readiness, wa-sessionb-analysis-output, wa-dimensionreview-instruction, wa-versecontext-instruction, wa-registry-management-guide*

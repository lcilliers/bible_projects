# wa-sessionb-cluster-instruction-v1_11-20260514

> Framework B Soul Word Analysis Programme — Session B Cluster Analysis
> Version: v1_11 | Date: 20260514
...
> **Change note v1_10 → v1_11 (2026-05-14):** Schema consistency follow-up to v1_10. (1) §7.7 SCOPE bullet corrected: "every `cluster_subgroup` row to be created with code, label, description" → "with `subgroup_code`, `label`, `core_description`" — eliminates an internal inconsistency with the Operation A wording on the line above. (2) New §A1 **Cluster-process tables — column reference** appendix added: a compact six-table column listing for `cluster`, `cluster_subgroup`, `mti_term_subgroup`, `verse_context_group`, `vcg_term`, `verse_context`. Authoring AI must consult this when drafting Phase 4 or Phase 7 directives. Driven by M39 first-run findings where AI confused `cluster_subgroup.core_description` (Phase 4) with `verse_context_group.context_description` (Phase 7) — analogy-by-name was the root cause. The appendix gives authoritative column names for the tables Phase 4 and Phase 7 touch.
> Full history: see §19.
...
*wa-sessionb-cluster-instruction-v1_11-20260514 | Active — authoritative instruction for Session B cluster analytics*
*Replaces (on finalisation): wa-sessionb-analysis-readiness, wa-sessionb-analysis-output, wa-dimensionreview-instruction, wa-versecontext-instruction, wa-registry-management-guide*

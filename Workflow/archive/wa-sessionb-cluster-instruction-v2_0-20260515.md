# wa-sessionb-cluster-instruction-v2_0-20260515

> Framework B Soul Word Analysis Programme — Session B Cluster Analysis
> Version: v2_0 | Date: 20260515
...
> Supersedes: wa-sessionb-cluster-instruction-v1_13-20260514 (archived to `Workflow/archive/`)
...
| Disposition | Meaning |
|---|---|
| `RESOLVED-BY-CATALOGUE` | the finding is already captured in one of the new cluster_finding rows (T0–T7) — name the cluster_finding id(s) |
| `FOLD-INTO-PROMPT` | the finding adds new evidence to an existing cluster_finding — name the target prompt + scope |
| `NEW-CLUSTER-FINDING` | the finding is real new evidence that doesn't fit any existing prompt — name a target T-code |
| `SUPERSEDED` | the finding was authored under the pre-cluster-pivot lens and is no longer relevant — name the replacing cluster_finding |
...
- Operations:
  - **Op A:** UPDATE `wa_session_b_findings.status` per disposition — `resolved` / `folded` / `promoted` / `superseded` / `routed_session_d` / `open` (RESEARCHER-DECISION); append `resolution_note` citing the cluster_finding or rationale.
  - **Op B:** UPDATE `wa_session_research_flags` similarly for SD/SB flags.
...
- Every inherited finding has a status not in `(open, unresolved-equivalents)` OR has an explicit `routed_session_d` flag OR is a researcher-decision item awaiting review.
...
CC's structural-and-full-text loader (`scripts/_apply_{code}_findings_record_*.py` modelled on M46's loader):

1. Build T-code → obs_id map from `wa_obs_question_catalogue`.
2. Parse each part for `**T#.#.#**` headers and scope markers.
3. Group (prompt, scope) cells; merge multi-body groups (inline + Phase-10 fold) per UNIQUE constraint `(obs_id, cluster_code, cluster_subgroup_id, version)`.
4. Resolve final status per group (gap > silent > finding / cluster_synthesis).
5. INSERT one `cluster_finding` row per group.

### 14.3 Directive

- Filename: `wa-cluster-{code}-dir-{seq}-findings-record-v1-{date}.md`
- One operation: bulk INSERT of cluster_finding rows.

...
**Filename:** wa-global-flags-v1_4-20260418.md
**Version:** v1_4
**Date:** 2026-04-18
**Status:** Active — supersedes wa-global-flags-v1_3-20260418.md
...
- **Resolution:** Executed in session 2026-04-18 per researcher authorisation, ahead of the originally-sequenced FLAG-013 dependency. Ten instruction documents edited and version-incremented:
...
Cumulative: ~130+ operational references migrated to `[current]`; ~25 provenance references correctly preserved (Supersedes, produced_by fields, historical change notes). Session record: `wa-global-ref-consistency-obslog-v1-20260418.md` (entries O-009 through O-018). All outputs dual-written per GR-FILE-008 and presented per GR-CAD-001.
...
### FLAG-015 — Registry management guide not visible in Project Files

- **Status:** Open
- **Raised:** 2026-04-18
- **Description:** The researcher reported in the 2026-04-18 session that `wa-registry-management-guide` had been uploaded to Project Files. Directory scans of `/mnt/project/` and `/mnt/user-data/uploads/` by Claude AI showed no matching file. The document is referenced by multiple instruction documents and appears in WA-Reference §1.4 as a standard instruction document. Its absence means: (1) the file itself could not be version-bumped in the FLAG-012 sweep; (2) cross-references to it in other documents were migrated to `[current]` as usual, which will resolve correctly once the file becomes visible.
- **Action needed:** (1) Confirm the upload status — whether the document is now available, still pending, or resident outside Project Files. (2) If the document is present, perform the equivalent FLAG-012 sweep against it (minor version bump + operational refs → `[current]`). (3) If the document has been retired, update WA-Reference §1.4 to reflect its retirement and adjust §12 Patch Index accordingly (CLUSTERING patch governing document needs redirection).
- **Gate:** No pipeline operations blocked. CLUSTERING patch production would be affected if that governing document is needed.

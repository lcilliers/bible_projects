# wa-sessionb-cluster-instruction-v2_9-20260526

> Framework B Soul Word Analysis Programme — Session B Cluster Analysis
> Version: v2_9 | Date: 20260526
...
> Supersedes: wa-sessionb-cluster-instruction-v2_8-20260519 (archived to `Workflow/archive/`)
> Governed by: wa-global-general-rules [current]
>
> **Change note v2_8 → v2_9 (2026-05-26):** Phase 8 scaled down. The researcher comparison report and researcher gate (v2_0 → v2_8 §11.3/§11.4) are removed — the new VCG corpus (Phase 7) is structurally complete and the old (inherited) VCGs are no longer referenced by any is_relevant verse, so the dissolution is a silent CC mechanical soft-delete. §11 retains: identification of inherited VCG ids → soft-delete Op A (`verse_context_group`) + Op B (`vcg_term`) → post-checks. Researcher review of Phase 5+7 outputs already covered the structural decision; no separate Phase 8 sign-off needed. Triggered by researcher direction 2026-05-26: *"we no longer need to compare the old VCGs with the new. I am happy with silently marking the old VCGs as redundant, and only work with the new VCGs. Scale down the phase 8 processes."* §16 (lifecycle) and §21 (pre/post controls) updated accordingly. No schema changes.
>
...
- Dissolution of old VCGs (Phase 8) is a silent CC mechanical operation (v2_9); AI is not asked to compare old-vs-new and there is no researcher gate. No-op when there are no inherited VCGs.
...
Phase 7 does not compare against inherited VCGs. Inherited VCGs are silently soft-deleted by CC at Phase 8 (v2_9 — no comparison report, no researcher gate); AI is not involved.
...
## 11. Phase 8 — Silent dissolution of inherited VCGs (CC)

**Purpose:** soft-delete the inherited (pre-cluster-pivot) `verse_context_group` rows and their `vcg_term` links, now that every is_relevant verse has been re-routed to a new (Phase 7) VCG. **No comparison report. No researcher gate.** The researcher's review of Phase 5+7 covered the structural decision; Phase 8 is a silent mechanical soft-delete.

**Owner:** CC. No AI involvement. Fires immediately after Phase 7 apply commits.

**No-op clusters:** for clusters constituted from scratch (no inherited VCG corpus), Phase 8 is a recorded no-op. Common cases: post-split clusters (e.g. M10c — constituted 2026-05-22, 0 inherited VCGs). The obslog records "Phase 8 — no-op (no inherited VCGs)" and the cluster proceeds to Phase 8.5 / Phase 8.7.

### 11.1 Inputs

- Post-Phase-7 DB state (every is_relevant vc row points at a new VCG).
- The set of VCG ids created by the Phase 7 directive (recorded in the Phase 7 obslog).
...
If the result is empty: record no-op in the obslog and skip to Phase 8.5.

### 11.3 Unrouted guard (pre-check)

Before soft-deleting, verify no is_relevant verse still points at an inherited VCG (i.e. `group_id` ∈ inherited set). If any do (`UNROUTED`), Phase 7 routing is incomplete — STOP and return to Phase 7. Phase 8 does not proceed until every is_relevant verse references a new VCG.
...
WHERE mt.cluster_code = '{code}'
  AND vc.is_relevant = 1
  AND COALESCE(vc.delete_flagged, 0) = 0
  AND vc.group_id IN (<inherited VCG ids>);
-- expected: 0
```

### 11.4 Dissolution directive

- Filename: `wa-cluster-{code}-dir-{seq}-vcg-dissolve-v1-{date}.md`
- Operations (single transaction):
  - **Op A:** UPDATE `verse_context_group SET delete_flagged=1, notes='Soft-deleted at Phase 8 ({directive id}) — superseded by Phase 7 VCG set'` WHERE `id IN (<inherited VCG ids>)` AND `delete_flagged=0`.
...
- Every inherited VCG has `delete_flagged=1`.
...
**Obslog:** record inherited VCG count dissolved, or "no-op" if zero. No comparison report.
...
| Old VCG soft-delete | **Directive (Phase 8)** | Silent CC mechanical soft-delete; no comparison report; no researcher gate (v2_9). No-op when inherited VCG count = 0. |
...
| **8 Old VCG dissolution** *(v2_9 silent)* | Phase 7 directive applied; §11.3 unrouted-guard passes (no is_relevant verse references inherited VCGs) | Inherited VCGs soft-deleted (or no-op if 0 inherited) |
...
5. Old VCG dissolution is CC-mechanical and silent (v2_9): inherited VCGs are soft-deleted after Phase 7 commits, with no comparison report and no researcher gate. AI is not asked to reconcile.
...
**v2_9 (2026-05-26)** — Phase 8 scaled down to silent CC soft-delete. §11.3 (comparison report) and §11.4 (researcher gate) removed; §11 retains identification + unrouted-guard + Op A/B soft-delete + post-checks. §16 routing-table entry and §21 pre/post-controls row for Phase 8 updated. No-op recording added for clusters with 0 inherited VCGs (post-split clusters — M10c precedent). Triggered by researcher direction post-M10c Phase 7: the new VCG corpus is structurally complete by Phase 7 commit; the old VCGs are no longer referenced by any is_relevant verse; researcher review at Phase 5 + 7 already covers the structural decision; no separate Phase 8 sign-off needed. No schema changes; no section renumbering.

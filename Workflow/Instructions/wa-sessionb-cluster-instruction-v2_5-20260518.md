# wa-sessionb-cluster-instruction-v2_5-20260518

> Framework B Soul Word Analysis Programme — Session B Cluster Analysis
> Version: v2_5 | Date: 20260518
...
> Supersedes: wa-sessionb-cluster-instruction-v2_4-20260517 (archived to `Workflow/archive/`)
...
Under v2_5, Phase 9 fires against a **structurally-final corpus** — every BOUNDARY decision has already been resolved at Phase 8.5 (§11A). Findings therefore describe the cluster the researcher intends, not a parked queue.

### 12.1 Inputs

- Post-Phase-8.5 grouped report — regenerated to reflect the new VCG structure plus any Phase 8.5 sub-group/VCG promotions.
- Per-cluster science extract: `Workflow/Sciences/wa-{code_lower}-{name}-scienceextract-v{V}-{date}.md` — **mandatory** input for T7 prompts. See §19 for the report map.
- Cluster instruction (this document).
- The obslog through Phase 8.5.
...
*(Under v2_5 the BOUNDARY-scope marker `**[BOUNDARY — H1234 translit]**` should not appear, since Phase 8.5 resolved BOUNDARY before findings authored. If it appears, Phase 8.5 was skipped or incomplete; return to §11A.)*
...
- No `[BOUNDARY ...]` markers present (Phase 8.5 should have eliminated them).

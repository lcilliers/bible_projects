...
|Claude AI role|Reads programme-prose start-up sections; acknowledges per-term `md_version` (A-03 gate); declares prior-state posture (FRESH or RE-EVALUATION — narrative hint only post-v3_5); verse reading; relevance filtering; contextual grouping; anchor designation; **per-verse `insert` vs `update` decision based on the header's "Existing verse_context rows" signal + the Verses section per-verse state (v3_5 A-06)**; per-term re-evaluation self-check + orphan-group check; produces up to four patches per §7.9 with per-term `input_versions` map|
|Claude Code role|Per-term Session A `.md` rendering from database (primary) — header now carries `Existing verse_context rows: {n} active (of {m} verses)` per v3_5 §4; per-registry hybrid render on demand; applies the four VC session patches (VCNEW, VCREVISE, VCSBFLAGS, VCSDPOINTERS — and legacy VERSECONTEXT); group_code resolution; XREF status propagation; **enforces A-03 version gate** (rejects stale mismatches); **enforces A-06 rowcount gate** (rejects UPDATE ops matching 0 rows via `_exec_update_strict`, rolls transaction back, emits REPAIR-FAILURE); **per-term `mti_terms.vc_status = 'vc_completed'` write on successful VCNEW/VCREVISE apply**; **bumps `mti_terms.md_version` on same**; **derived `word_registry.verse_context_status` aggregation from term states**; orphan-group validation; integrity validation; completion reporting at term and registry scope; Session A `.md` re-render on status change|
|Interaction model|Per-term Session A `.md` rendered from current DB state (with `md_version` + existing-row count stamped) → programme-prose + instruction loaded; classifier acknowledges `md_version` and declares per-term prior-state posture → Claude AI classifies per-verse, choosing `insert` vs `update` per the header's row-presence signal (session scope = 1..N terms at researcher discretion) → per-term self-check + orphan-group check on re-evaluation terms → produces up to four patches (VCNEW / VCREVISE / VCSBFLAGS / VCSDPOINTERS) carrying `_patch_meta.terms_covered` + `input_versions` where applicable → Claude Code applies each in order, enforces version and rowcount gates, flips `mti_terms.vc_status` to `vc_completed` per term, bumps `md_version`, derives registry-level `verse_context_status` from the aggregate → DataPrep gate opens when the DB state reaches Complete (§13.3)|
...
**Operation type — per-verse rule (v3_5 A-06):** Whether a set-aside (or any verse_context row) is emitted as `insert` or `update` is determined per verse:

* **No active `verse_context` row exists** for `(mti_term_id, verse_record_id)` → emit `insert`
* **Active row exists** (classified anchor / related / prior set-aside) and the classification is being revised → emit `update`

The per-term Session A `.md` header declares `Existing verse_context rows: {n} active (of {m} verses)` and the Verses section shows each verse's prior state. Use those signals to pick the operation type. Term-level FRESH / REFRESH posture (§6.1) is narrative framing; it does NOT control operation type.

### 3.2b No row is a meaningful state

Under the skeleton-free model (M39 applied 2026-04-24 deleted 22,333 strict NULL-skeleton rows — A-07), absence of a `verse_context` row for `(mti_term_id, verse_record_id)` means **the verse has not been analysed for this term**. It is NOT equivalent to "set-aside with no reason."

Implications:

* Coverage metrics count classified rows (`is_relevant=1`) + explicit set-aside rows (`set_aside_reason IS NOT NULL`) only. Rows without either are counted as **pending**, not covered.
* A term with no inner-being engagement in any verse still produces rows — one explicit set-aside per verse with `set_aside_reason` populated. That is the only way the work is recorded in the DB.
* A classifier that skips a verse entirely leaves it pending, not set-aside. The per-term self-check (§6.2 Step 6) rejects any term with un-accounted verses.

### 3.2c Cluster-treatment discipline (added v3_10)

Some terms have corpora containing homogeneous blocks of verses where the term's use is uniform — the same syntactic role, the same semantic content, and the same filter outcome — across many verses. Examples include the 45 sacrificial-formula uses of H8549G *ta.mim* ("a male without blemish") in the Pentateuchal sacrificial code, or the ~30 Pauline epistolary thanksgiving openings of G2168 *eucharisteō* ("I thank my God always for you"). For such homogeneous blocks, **cluster-level filter reasoning is permitted under bounded discipline**.

**The discipline:**

1. **Read every verse in the cluster.** Cluster-treatment is permitted only after individual reading confirms the homogeneity. The reading itself is non-skippable; what is permitted is the consolidated reasoning, not the consolidated reading.
2. **Document the homogeneity finding before applying it.** State plainly: which verses are in the cluster (vid + ref list), what they share (syntactic role of the term, semantic content, filter direction), and the shared reasoning. The documentation must be such that a reviewer reading the obslog can, by spot-checking any verse against the cluster's stated pattern, verify that the verse genuinely fits.
3. **Each verse receives its own `verse_context` row.** Cluster-treatment governs the *reasoning*, not the row count. Every verse in the cluster gets an insert row with the same `set_aside_reason` (or the same `group_id`) and the same or substantively-identical `notes` text. The DB record is per-verse; the analytical work is per-cluster.
4. **Any verse that departs from the cluster pattern receives individual treatment.** If reading a verse reveals it does not fit the cluster's syntactic role or semantic content (e.g. it is figurative where the rest are literal, or it engages an inner-being facet the rest do not), pull it out of the cluster and treat it on its own merits with its own reasoning.

**When cluster-treatment is appropriate:**

* Sacrificial / cultic formulae where the term qualifies a physical animal (e.g. *ta.mim* "without blemish")
* Epistolary or liturgical formulae where the term has a fixed routinised use (e.g. Pauline thanksgiving openings)
* Legal-categorical lists where the term names a category alongside others (e.g. *zonah* "prostitute" listed with widow / divorced woman in priestly marriage law)
* Idiomatic merism constructions where the term is part of a stock phrase ("good or bad" = "anything at all")

**When cluster-treatment is NOT appropriate:**

* Wisdom or prophetic uses of the term, where each verse engages a distinct rhetorical or analytical point
* Verses where the term carries inner-being weight that varies verse to verse
* Heterogeneous narrative occurrences, even if many verses involve the same translation surface
* Any block where individual reading has not confirmed homogeneity

The §6.2 Step 6 self-check counts verses individually regardless of whether cluster-treatment was applied to their reasoning. Cluster-treatment is a documentation efficiency, not a coverage shortcut.
...
**Primary output:** `Sessions/Session_A/terms/wa-{NNN}-{word}-{strongs}-session_a-{date}.md` — the per-term Session A `.md`. Regenerated on demand per term; same-day regenerations overwrite (deterministic content). Handed to the Claude AI classification session as the classifier's input for that term.
...
**Legacy batch JSON (deprecated):** `Sessions/Session_B/01_Verse_Context_Process_input/wa-vcb-{batch_id}-extract-{date}.json`. Retained in Annexure D for historical reference; not produced under v3_2.
...
5. **Prior-state posture declaration — mandatory, once per term.** For each per-term `.md` in the session, read its "Prior-state posture (this term)" block in the About section and state verbatim one of the two templates below.
    ...
    **Posture is narrative framing only (v3_5 A-06).** FRESH / RE-EVALUATION informs the classifier about how much prior context to read and whether orphan-group discipline applies. It does **not** control the `insert` vs `update` operation type in the patch. Operation type is always per-verse, driven by row presence at `(mti_term_id, verse_record_id)` — see §3.2 and §3.2b. A FRESH term may emit only `insert` ops (common); a RE-EVALUATION term may emit a mix of `insert` (new set-asides on verses with no prior row) and `update` (revisions to existing classifications).
...
**Halt resolution discipline (added v3_10).** The halt is a *stop-and-investigate* surfacing mechanism, not a *stop-and-walk-away*. When the researcher gives instruction to investigate the previously-pending verses, the work is to complete what the prior session left undone — not to defer it again, not to bury it behind a flag, and not to assume the absence of a row is itself a meaningful classification. Specifically:

* **Every previously-pending verse is read.** No skipping, no sampling, no extrapolation from a sample to the rest.
* **Every previously-pending verse receives an explicit `verse_context` row.** Either `is_relevant=1` with a group assignment, or `is_relevant=0` with a controlled-vocabulary `set_aside_reason` (`no_inner_being` / `physical_only` / `spatial_only` / `wrong_face` / `other`). Implicit-set-aside (no row) is the original prior-session error and must not be repeated.
* **The full corpus is now visible — use the visibility.** The previously-pending verses may surface (a) sub-patterns the existing groups do not cover, calling for new groups; (b) inconsistencies in the prior classifications, calling for revision; (c) homogeneous blocks suitable for cluster-treatment per §3.2c. The classifier is responsible for noticing and acting on these as part of the halt-resolution work.
* **The halted term re-enters the patch flow.** Once the work is complete, the term's patch routing is determined per §6.3 Step 3 (NEW-ONLY / REVISE-ONLY / MIXED / NO-CHANGE) on the same basis as any other term. Halt-resolution is not a separate patch class — it is normal classification work on a term whose prior state was incomplete.

This discipline applies whether the halt-resolution happens in the same session as the original halt (researcher gives instruction immediately) or in a follow-on session (researcher schedules the resolution). The instruction's intent is that no halted term remains halted indefinitely; halt resolution is the mechanism that closes the surface-and-investigate loop.
...
**Step 3 — Classify each term for patch routing.** Before producing any patch, for each term in the session declare its patch-contribution class in the observations file using exactly one of the following:

| Per-term outcome | What the session produced for this term | Which patch(es) carry the term |
| --- | --- | --- |
| **NEW-ONLY** | First-time classifications only (FRESH term); no pre-existing rows touched | VCNEW (`terms_covered` + `input_versions` include the term; inserts for all its verses) |
| **REVISE-ONLY** | Revisions to pre-existing classifications only (RE-EVALUATION term with ≥1 change) | VCREVISE (same) |
| **MIXED** | Both first-time inserts AND revisions to pre-existing rows (RE-EVALUATION term with partial coverage + revisions) | VCNEW for the inserts AND VCREVISE for the updates — both patches declare the term in `terms_covered`; applicator writes `vc_status = 'vc_completed'` on the first successful patch; the second finds it already complete and re-runs the version gate against the bumped `md_version` |
| **NO-CHANGE** | RE-EVALUATION term; full review confirms all existing classifications correct; no operations of any kind | **Empty-ops VCREVISE** — term declared in `terms_covered` + `input_versions`; no `operations` for this term (see §7.9.3b) |

State aloud for each term: *"Term {strongs} mti_id={n}: patch-contribution class = {NEW-ONLY | REVISE-ONLY | MIXED | NO-CHANGE}. Declared in terms_covered of: {VCNEW | VCREVISE | both}."*

**Mixed-session MIXED-term interaction with the version gate:** A MIXED term whose VCNEW applies first will have its `md_version` bumped (v1 → v2). The follow-on VCREVISE, produced from the same pre-session `.md`, still carries `input_versions[{term_id}] = 1` and WILL be rejected by the A-03 version gate. To avoid this: for MIXED terms in a session, produce **one** VCNEW+VCREVISE pair such that the VCNEW is submitted and applied first, and the VCREVISE is regenerated against the bumped version before submission — or, simpler, bundle the term's inserts and updates into a single legacy **VERSECONTEXT** patch (§7.9.4 back-compat), where both classes land atomically under one `input_versions` check and one `md_version` bump. The legacy combined pattern is acceptable for MIXED terms; for NEW-ONLY and REVISE-ONLY the split four-patch model is preferred.

**Step 4 — Produce the session's patch set per §7.9 (up to four patches: VCNEW, VCREVISE, VCSBFLAGS, VCSDPOINTERS; plus optional legacy VERSECONTEXT for MIXED terms per Step 3 above). Any patch class whose operation list would be empty is not produced, with one exception: VCREVISE IS produced with `operations: []` when the session contains any NO-CHANGE term (empty-ops pattern per §7.9.3b). The empty-ops VCREVISE is the authoritative record of "these terms were reviewed and confirmed unchanged" and is required to advance their `vc_status` to `vc_completed`.**
...
### 6.5.0 In-session resolution before flagging (added v3_10)

A flag is a request for researcher judgement on a question the classifier cannot answer by reading. Findings that the classifier *can* resolve by reading must be resolved by reading in the same session — not deferred to the flags register, not parked for "future consideration", not filed as an observation for another day.

**The discipline:**

1. **A finding raised by reading is investigated by reading.** If a verse, a comparison between verses, or the now-visible full corpus surfaces a question the classifier can answer by re-reading the relevant material, the answer is produced in-session. Examples: "Does this verse fit existing group X or warrant new group Y?" — answer by reading the verse against the group descriptions and the other group members. "Are the prior classifications of related verses consistent with this new finding?" — answer by reading those verses. "Is this verse genuinely homogeneous with the cluster, or does it depart?" — answer by re-reading the verse on its own merits.
2. **A flag is for findings that genuinely require researcher judgement.** Reserve flagging for: (a) decisions that change the term's analytical scope in a way the researcher should sanction (e.g. creating a new group with broad implications); (b) decisions that touch programme-level vocabulary or boundaries (e.g. whether a new dimension or set-aside reason is warranted); (c) findings that affect other terms or registries beyond the classifier's immediate scope; (d) factual ambiguities that only the researcher can resolve.
3. **Document the resolution in the obslog.** When a finding is resolved by reading, record the question, the reading-investigation undertaken, and the answer reached. The obslog is the audit trail showing the classifier did the work rather than deferring it.

**What this rules out:**

* Filing observations like "this question may be worth reviewing in a future session" when the question can be investigated now.
* Recording a finding as a "soft observation" (OBS-VC-{nnn}) when the finding could be resolved by reading the corpus the classifier already has.
* Treating the flags register as a parking lot for unresolved-but-resolvable questions.

**What this preserves:**

* The deferred-flag protocol for genuine researcher-judgement questions (§6.5.1 onwards) is unchanged.
* The mid-session no-stop discipline is unchanged: classification proceeds; investigation happens at term-close or end-of-session, not mid-classification.
...
```json
{
  ...
  "description": "Revised context description for group 142-001: {reason}"
}
```

**Match-shape rule (v3_5 correction 2026-04-24):** Match `verse_context_group` updates on `(mti_term_id, group_code)` — both fields are present in the per-term Session A `.md` (the header's `mti_term_id` line and the Existing-groups table's `group_code` column). Do NOT match on the integer `id` field — it is a database internal not visible to the classifier. Claude Code resolves `(mti_term_id, group_code)` to the integer id at apply time via `verse_context_group` lookup. The applicator rejects any UPDATE whose resolved match returns 0 rows (A-06 hardening).
...
```json
{
  ...
  "description": "{Book Ch:V} — reclassified: promoted to anchor, group 142-001"
}
```

**Match-shape rule (v3_5 correction 2026-04-24):** Match `verse_context` updates on `(mti_term_id, verse_record_id)` — both fields are present in the per-term Session A `.md` (the header's `mti_term_id` and each verse's `verse_record_id` in the Verses section). Do NOT match on the integer `id` field of `verse_context` — it is a database internal not visible to the classifier. The pair `(mti_term_id, verse_record_id)` is unique in the `verse_context` table (DB invariant). Applicator rejects UPDATE matching 0 rows (A-06 hardening).
...
**Note on group\_id references:** In `set` clauses, `group_id` may be supplied as a group\_code string (e.g. `"142-001"`) — Claude Code resolves it to the integer id via `verse_context_group` lookup (same-patch inserts via `_group_code_map`; prior-patch groups via DB lookup on group\_code). Supplying the integer id directly also works but requires the classifier to know it; the string form keeps the classifier tied to `.md`-visible values only.
...
```text
  ...
  ROWCOUNT GATE (A-06) — silent-loss hardening:
    Every UPDATE on verse_context and verse_context_group is routed through
    _exec_update_strict, which raises ApplicatorError when cursor.rowcount == 0.
    On error the whole transaction rolls back and a REPAIR-FAILURE patch is
    produced. This protects against the VC-7 class of bug where a VCREVISE
    update targets a row that no longer exists (e.g. deleted by M39 cleanup,
    or a mis-keyed verse_record_id). The correct remedy is to declare the
    op as `insert` (when the row genuinely does not exist) or to correct
    the match clause — NEVER to loosen the applicator.
...
| **VCREVISE** | (a) Changes to existing VC data — OR no-change confirmation | Either: any revision is made to existing classifications (group description edits, dissolves, verse reassignments, anchor promotions/demotions); OR the re-evaluation confirms no changes are needed (empty-ops pattern — see §7.9.3b) | `verse_context_group` + `verse_context` (updates, including `delete_flagged = 1`); or no DB writes except `mti_terms.vc_status` + `md_version` |
...
At the end of the session (§6.3 Steps 1–3), the classifier partitions the observations file into the four patches and produces each as a separate JSON file. Any patch whose operation list would be empty is not produced — **with one exception**: a VCREVISE whose operation list is empty IS produced when the session contains any RE-EVALUATION term whose review concluded "no changes needed" (see §7.9.3b). Such a patch advances `mti_terms.vc_status` to `vc_completed` and bumps `md_version` via `terms_covered` alone.

#### 7.9.3b No-change re-evaluation — the empty-ops VCREVISE pattern

A re-evaluation term (RE-EVALUATION posture per §6.1) reviewed under v3_5 may legitimately conclude that **no changes are needed** — all prior classifications, groups, anchors, and set-asides are correct and remain unchanged. This is a meaningful classification outcome, not a non-result: the classifier has done the work, the term has been verified under the current filter and grouping model, and the researcher expects the per-term `vc_status` to advance.

**The empty-ops VCREVISE pattern:**

```json
{
  "_patch_meta": {
    "patch_id": "PATCH-{YYYYMMDD}-{reg}-VCREVISE-V1",
    "registry_id": 96,
    "word": "{word}",
    ...
    "patch_type": "VCREVISE",
    "session_b_status": null,
    "terms_covered": [{mti_term_id}, ...],
    "input_versions": {"{mti_term_id}": {n}, ...},
    "governing_instruction": "wa-versecontext-instruction-v3_5-20260424.md",
    "description": "Re-evaluation of {word} — no changes recommended. All verses and groups confirmed correct under v3_5. {brief 1-sentence summary of what was reviewed and the basis for the no-change finding.}"
  },
  "operations": []
}
```

**Applicator behaviour for empty-ops VCREVISE:**

- A-03 version gate runs normally. Stale `input_versions` → reject.
- Operations loop is a no-op (zero ops).
- Version bump + `vc_status = 'vc_completed'` write fires for every `mti_term_id` in `terms_covered` (same as a populated VCREVISE).
- Registry aggregation runs; affected registries advance to Complete if all OWNER + XREF-via-OWNER terms are at vc_completed.

**Do NOT produce a cosmetic-notes-stamp VCREVISE** (a pre-v3_5 workaround that stamped "Re-evaluated YYYY-MM-DD" into the `notes` field of each group just to have an operation). This is:

- noise in `notes` (accumulates over multiple sessions);
- misleading provenance (an `update` op that isn't a real update);
- wasted classifier effort.

**When to use which pattern:**

| Session outcome for a RE-EVALUATION term | Pattern |
| --- | --- |
| Any classification revised, group dissolved, anchor changed, set-aside added, etc. | Populated VCREVISE (one `update` per change) |
| Full review; no changes needed to any classification or group | Empty-ops VCREVISE (declares `terms_covered` + `input_versions`; operations: []) |
| Mixed session: some terms have changes, others are no-change | One VCREVISE per session — include `update` ops for the changed terms, **and include the no-change terms in `terms_covered` + `input_versions`** so they advance to vc_completed. Ops are the union of the per-term updates. |

**Provenance note:** the patch `description` field is the authoritative record of the "reviewed, no changes" finding. It should state clearly what was reviewed, how many verses/groups, and the basis for the no-change conclusion. Researchers auditing the programme later read the patch description — not the operations — to understand what happened in a no-change session.
...
(The legacy combined `wa-vcb-{batch_id}-patch-versecontext-v1-{date}.json` is retained for backwards compatibility with pre-v3_3 sessions and is preferred for MIXED-pattern terms (§6.3 Step 3). **VERSECONTEXT is fully under v2_6+ contracts when used: it requires `_patch_meta.terms_covered` + `_patch_meta.input_versions` and runs the A-03 version gate identically to VCNEW/VCREVISE.** The applicator dispatches all three patch types through the same `_apply_versecontext_term_updates` helper. Patch instruction [current] §15.7 documents the contracts authoritatively.)
...
The classifier's end-of-session check must verify, for the session's four patches (or five, if a legacy VERSECONTEXT is produced for MIXED terms):
...
3. Every orphan-group disposition noted in §6.2 Step 6 is reflected in VCREVISE or VERSECONTEXT (as a dissolve update) — or documented as retained with rationale.
4. **(v3_9) `_patch_meta.terms_covered` + `_patch_meta.input_versions` are present on VCNEW, VCREVISE, AND VERSECONTEXT.** All three patch types run the A-03 version gate identically (applicator dispatch: `if patch_type in ("VERSECONTEXT", "VCNEW", "VCREVISE")`). Every term in `terms_covered` must have an `input_versions[term_id]` entry equal to the `md_version` in the per-term `.md` the classifier worked from. Stale → applicator rejects.
5. `_patch_meta.batch_id` is identical across all session patches (proving they belong to one session). Optional under A-04.
6. The union of `terms_covered` across VCNEW + VCREVISE + VERSECONTEXT matches the terms the session classified.
7. **(v3_5 A-06) Operation type matches row presence.** For every `verse_context` op: if the header's Existing-vc-rows signal indicates no active row for the verse_record_id, the op MUST be `insert`; if it indicates an active row, the op MUST be `update`. A mismatch will be rejected by the applicator's rowcount gate at apply. Classifier must verify before submission. Same rule applies to `verse_context_group` ops (insert for new group_code; update for existing group_code).
8. **(v3_5 A-08) Every term in the session appears in exactly one VCNEW or VCREVISE `terms_covered` entry** (or both, for MIXED terms per §6.3 Step 3). Specifically:
    * **NEW-ONLY** terms → in VCNEW `terms_covered` only.
    * **REVISE-ONLY** terms → in VCREVISE `terms_covered` only; at least one `update` op in VCREVISE references the term.
    * **MIXED** terms → in both VCNEW and VCREVISE `terms_covered` (or in a single legacy VERSECONTEXT patch).
    * **NO-CHANGE** terms → in VCREVISE `terms_covered` with NO operations referencing the term. An empty-ops VCREVISE is produced when every RE-EVALUATION term in the session is NO-CHANGE. A mixed-content VCREVISE includes NO-CHANGE terms in `terms_covered` alongside REVISE-ONLY terms; the applicator advances `vc_status` for every term in `terms_covered` regardless of whether it had ops.
    * **No term is silently dropped.** A session that classified a term without emitting it in any `terms_covered` list is a self-check failure; that term's `vc_status` will not advance and the session's work on it is lost. If the classifier truly meant to skip a term (e.g. researcher aborted mid-term), state this explicitly in the session log and exclude it from `terms_covered` with a written rationale.

No observation is left without a corresponding operation. The obslog and the patch set must be consistent. No reviewed term is silently dropped from the session's patch set.
...
{
  "\_patch\_meta": {
    "patch\_id": "PATCH-{YYYYMMDD}-VCGROUP-{mti\_term\_id}-{group\_code}-V1",
    "produced\_date": "{yyyy-mm-dd}",
    "produced\_by": "{governing\_instruction — e.g. wa-versecontext-instruction-v3\_5-20260424.md}",
...
Use `update` operation matching on `(mti_term_id, verse_record_id)`. Include notes explaining the reason for reclassification. Maintain anchor integrity.
...
*wa-versecontext-instruction-v3_10-20260425 | Supersedes wa-versecontext-instruction-v3_9-20260425 | v3_10: Halt-resolution discipline; cluster-treatment discipline; in-session resolution discipline. (1) New §3.2c — cluster-treatment is permitted only after individual reading establishes homogeneity; per-verse rows always; departures get individual treatment. (2) §6.2 Step 2 partial-completion rule strengthened — halt is "stop and investigate", not "stop and exclude"; on researcher instruction every previously-pending verse receives an explicit `verse_context` row. (3) New §6.5.0 — findings the classifier can resolve by reading must be resolved by reading in-session before being filed as flags. Origin: VCB-013 halt-resolution work on H2896A, H8549G, H2181 (198 previously-pending verses). Guidance-only update; no schema or applicator change.*

# wa-claudecode-instruction-v4_5-20260514

> Framework B Soul Word Analysis Programme — Claude Code Operating Guide
> Version: v4_5 | Date: 20260514
> Supersedes: wa-claudecode-instruction-v4_4-20260428.md (records the 2026-05-04 cluster-model pivot; marks the Architecture v2 per-word obslog→DB pipeline as legacy / 6 reset-registries only; adds cluster-operations pointers to `wa-sessionb-cluster-instruction [current]` and `wa-sessionc-cluster-overview [current]`)
...
- **Cluster-model CC operations** (directive execution, comprehensive/grouped report generation, cluster_finding writes, supersede/archival of cluster instructions) — see §Cluster-model context below
- **Session B Architecture v2 — readiness output generation, obslog parser, Phase 2 writer, analytic status generator, anomaly detection** (per Architecture v2 addendum below; **status as of 2026-05-14: legacy — applies only to the 6 reset-registries still in the per-word pipeline**)
...
- Cluster Session B analytical flow (10 phases, VCG reconciliation, Phase 8 catalogue, BOUNDARY discipline) → wa-sessionb-cluster-instruction [current]
- Cluster Session C publication (assembler, shared style, per-chapter prose instructions) → wa-sessionc-cluster-overview [current]
...
## Cluster-model context (effective 2026-05-04)

The programme's active analytical model is **Session B cluster analysis**. CC's role in this model is:

- **Directive execution** — every cluster-process change to the database goes via a `wa-directive-instruction [current]` directive. CC validates the five required elements, applies under transactional control, returns the specified confirmation.
- **Comprehensive report generation** — `scripts/_generate_cluster_comprehensive_v1_*.py --m-cluster {code}` produces the Phase 1/2 reading dataset. Idempotent; performs the inline `Not started` → `Data - In Progress` status transition (cluster-instruction v1_5+).
- **Grouped report generation** — `scripts/_generate_cluster_grouped_v1_*.py` joins via post-M47 `vcg_term` m:n schema; used as Phase 5/6/8 input.
- **Phase 9 cluster_finding writes** — directive-driven inserts into the `cluster_finding` table, one row per (cluster, scope, prompt) with parsed outcome codes (E/S/G/M/U/N/I plus FOLLOWUP/QUESTION/REFINE).
- **Session C publication assembly** — `scripts/_generate_cluster_session_c_inputs_v2_*.py` and `scripts/_assemble_cluster_publication_v1_*.py` produce the publishable artefact from per-chapter drafts.
- **Validation** — `scripts/_validate_cluster_completion_v1_*.py` (C1 VC-coverage gaps; C2 stale vc_status sync) gates Phase 10 closure.
- **Cluster-instruction supersede/archival** — when a cluster-instruction version bump lands, the prior version moves to `Workflow/archive/` with the v{N} suffix preserved (GR-FILE-003).

**Active cluster-process documents (use `[current]` token):**

- `wa-sessionb-cluster-instruction [current]` — 10-phase analytical flow
- `wa-sessionc-cluster-overview [current]` — publication pipeline entry point
- `wa-sessionc-cluster-style-method [current]` — shared style/method
- `wa-sessionc-cluster-ch{1..7}-instruction [current]` + `wa-sessionc-cluster-appendices-instruction [current]` — per-chapter prose authoring

**Cluster completion state (snapshot at v4_5 issue date 2026-05-14):** 5 of ~47 clusters at `cluster.status = 'Analysis Completed'` — M05, M06, M15 (Wisdom, published ~34k-word PDF 2026-05-12), M20 (Doubt/Despair/Anxiety, closed 2026-05-13), M26. 42 clusters not yet started — researcher's choice for sequence.

**Per-word Session B (Architecture v2) — legacy status:** the obslog→DB capture pipeline documented in the Architecture v2 addendum below (v2.CC1–CC10) is operational only for the **6 reset-registries** (023 compassion, 062 fellowship, 064 forgiveness, 068 grace, 103 love, 111 mercy) carried over from the pre-pivot pipeline. New per-word analysis is not expected — these registries are expected to be folded into their respective clusters rather than re-run through per-word. Architecture v2 documentation is preserved as the authoritative reference for the legacy path until those registries close.

---

## Architecture v2 — addendum (effective 2026-04-27; **legacy as of 2026-05-04 — 6 reset-registries only**)
...
| Change | Section |
|---|---|
| Recorded the **2026-05-04 cluster-model pivot**. New top-level §Cluster-model context section added between Document scope and the Architecture v2 addendum, documenting CC's role in cluster ops (directive execution, comprehensive/grouped report generation, cluster_finding writes, publication assembly, validation, supersede/archival) and listing the active cluster-process documents | New §Cluster-model context |
| Marked the **Architecture v2 per-word obslog→DB pipeline (v2.CC1–CC10) as legacy**: addendum header retitled "*legacy as of 2026-05-04 — 6 reset-registries only*"; Document scope updated with the legacy caveat. Architecture v2 content is preserved verbatim — it remains authoritative for the 6 reset-registries (023 compassion, 062 fellowship, 064 forgiveness, 068 grace, 103 love, 111 mercy) | Frontmatter, §Document scope, §Architecture v2 header |
| Added cluster-instruction pointers to the "NOT authoritative for" pointer list: `wa-sessionb-cluster-instruction [current]`, `wa-sessionc-cluster-overview [current]` | §Document scope |
| Cluster completion snapshot (5 of ~47 clusters at `Analysis Completed`: M05, M06, M15, M20, M26) | §Cluster-model context |

...
Version 1_10 | 20260501 | Status: Active — §L embeds v2 second-tier catalogue (T0–T7); legacy session_b_status acceptance clarified**

| **Document** | **Value** |
|---|---|
| Filename | wa-sessionb-analysis-readiness-v1_10-20260501.md |
| Supersedes | wa-sessionb-analysis-readiness-v1_9-20260427.md (post catalogue v2 migration: §L now embeds the 189 v2 prompts grouped by tier→component→prompt; clarifies that `session_b_status='Verse Context Reset'` is legacy-acceptable on entry to Stage 1) |
| Companion documents | wa-global-rules-all [current] │ wa-reference [current] │ **wa-sessiona-prose-instruction [current]** │ wa-dimensionreview-instruction [current] │ wa-versecontext-instruction [current] │ wa-claudecode-instruction [current] │ wa-patch-instruction [current] │ wa-directive-instruction [current] │ db-capture-phase1-results-and-table-architecture-v1-20260427.md |
| Inputs (v1_9) | DB state at generation time, **including `prose_section` rows under `source_stage='session_a'` (the Phase A prose; see §v2.R0)**. No JSON export needed — CC reads tables directly via the readiness generator. |
| Outputs (v1_9) | **Readiness `.md` + `.json`** + **Validation report `.md` + `.json`** (15-check verdict, see §v2.R8), all paired in `Sessions/Session_B/07_Analysis_Readiness_Status/`. Plus Analytic Status `.md`+`.json` for revision sessions (same folder). |
| Claude AI role (v1_9) | **Minimal under v2 — readiness is now a CC operation.** AI's role limited to receiving the readiness output and proceeding to Analysis Output |
| Claude Code role (v1_9) | **Owns the readiness phase end-to-end:** verify Phase A prose precondition (§v2.R0); generate readiness `.md`+`.json`; validate field-level destinations; populate §N Open Items; run pre-flight integrity checks; raise `DATA_ANOMALY_*` findings as `wa_session_b_findings.status='open'`; **run the 15-check validation (§v2.R8); produce validation report; gate handoff on READY verdict** |
| Handoff to | wa-sessionb-analysis-output [current] — **only on READY verdict** (validation report has 0 FAIL). AI session begins on readiness `.md` (and analytic status `.md` if revision) |

---

## Architecture v2 — addendum (effective 2026-04-27)

**Source-of-truth:** [research/investigations/db-capture-phase1-results-and-table-architecture-v1-20260427.md](../../research/investigations/db-capture-phase1-results-and-table-architecture-v1-20260427.md) Parts 10-12.

The instruction body below describes the v1 patch-driven readiness flow with AI-led data audit. **Under v2, the readiness phase is owned by CC and the audit happens through the readiness `.md`/`.json` generation and validation pipeline.**

### v2.R0 Precondition — Phase A prose captured to DB

Before the readiness sweep can produce a fully-populated readiness output, **Phase A (Session A) prose must already be captured in the `prose_section` table** for the registry under `source_stage='session_a'` (six rows, codes `sa_s1_d1` through `sa_s1_d6`).

The readiness generator composes registry-orientation, lexical-foundation, and term-inventory panels from these prose rows. If they are absent, those panels render thin or empty and the readiness output fails its own §M verification check.

**Production of Phase A prose is governed by [wa-sessiona-prose-instruction [current]](wa-sessiona-prose-instruction-v1_0-20260427.md).** The flow is:

```text
1. python scripts/generate_session_a_extract.py --registry=N
   → produces .md in Sessions/Session_A/Data_Prose/
   → produces PROSE patch in Sessions/Patches/

2. python scripts/apply_session_patch.py <patch-file>
   → 6 prose_section rows inserted (or session_a_replace on regenerate)
   → patch auto-archived to archive/patches/

3. THEN this readiness sweep can run.
```

**CC pre-flight check (mandatory at readiness generation):** before producing the readiness output, CC verifies:
...
If this returns < 6, CC must:

1. Halt the readiness generation.
2. Surface the gap explicitly: "Phase A prose missing for R{NNN} — n of 6 sa_s1_d* rows present. Run `generate_session_a_extract.py --registry={N}` first per wa-sessiona-prose-instruction [current]."
3. Raise a `DATA_ANOMALY_PHASE_A_PROSE_MISSING` finding in `wa_session_b_findings` for the next session's §N.

This precondition was added 2026-04-27 after a scramble during R030 contrition's first readiness preparation, where Phase A prose absence was not surfaced as a blocking gap.

### v2.R1 Readiness phase ownership

**CC owns:**

- Reading current DB state for the registry
- Producing `wa-{NNN}-{word}-readiness-output-v{n}-{date}.md` and `.json` (paired)
- Validating field-level destinations: every data field has a downstream destination in the analytical pipeline (Q&A / SD pointer / chapter / observation / verse_context.analysis_note / catalogue / status)
- Populating §N Open Session B Items by querying `wa_session_b_findings WHERE status='open' AND registry_id=?`
- Pre-flight integrity checks: anchor count consistency, dimension assignment coherence, term-status integrity, group description vs dimension drift
- For revision sessions: also producing the Analytic Status `.md`/`.json`

**AI's role under v2:**

- Acknowledge the readiness output as input
- Proceed to Analysis Output (Stage 2a/2b/2c per the analysis-output instruction)

The "Stage 1 Completion Record" of v1_6 is now generated as **Section B** of the readiness `.md` — synthesised from DB state at generation time. There is no separate AI-led readiness session under v2.

### v2.R2 Readiness `.md` structure

The output is structurally specified by the readiness generator (`scripts/_pilot_build_readiness_output_v2_*.py`). 14 sections (A through N + M):

| § | Content |
|---|---|
| A | Registry overview (status fields, inference_note, word_synopsis, description) |
| B | Stage 1 Completion Record (synthesised seven-domain pass status from DB) |
| C | OWNER term inventory |
| D | Lexical foundation (meaning_parse, root family, related, LSJ where present) |
| E | XREF terms |
| F | Verse context groups landscape (with dimensions from `wa_dimension_index`) |
| G | Correlation signals (computed: XREF sharing, verse co-occurrence, shared anchors) |
| H | Existing SD pointers + session_b_findings |
| I | Thin-evidence phase2 flags |
| J | Anchor verse material (verbatim verse text, classifications, anchor markers) |
| K | Legacy-VC notice (per vc-corrective-strategy v2 §4) |
| L | Stage 2b reference: **second-tier catalogue v2 (T0–T7) — 189 active prompts across 56 components in 8 tiers, embedded as JSON grouped tier→component→prompt** + registry-specific extensions where present. (Catalogue version `v2-2026-04-29`. Old v1 generic-section questions are retained as `status='redundant_v1'` for provenance and are NOT rendered.) |
| **N** | **Open Session B items — must resolve this session (4 outcome paths)** |
| M | Readiness verification (gates and quick checks) |

### v2.R3 CC anomaly detection — output of pre-flight validation

When CC's pre-flight integrity checks surface an anomaly, CC writes a row to `wa_session_b_findings`:

```text
finding_type: DATA_ANOMALY_ANCHOR_UNCITED  /  _DIMENSION_DRIFT  /
              _ANSWER_UNGROUNDED  /  _EMPTY_TERM  /  _ORPHAN_ANALYSIS
status:       open
session_b_instruction:  wa-sessionb-analysis-readiness-v1_7
```

This open finding then appears in §N of the next readiness output. AI addresses it via one of the four resolution paths (per analysis-output instruction §v2.3).

### v2.R4 Idempotency

CC's readiness generator is deterministic — same DB state → byte-identical `.md`/`.json` modulo the generation timestamp. The generator can be re-run any time the DB state changes, and the new output strictly reflects the new state.

### v2.R5 Stage retirement / continuity

The body of this document below — particularly the Step 1 data audit, Step 2 patch accumulation, Step 3 directive production, Stage 1 Completion Record discipline, integrity rules SB-1..SB-25 — describes the **discipline** of what comprehensive readiness review entails. **Under v2 these checks are mechanised in CC's readiness generator and validation pipeline.** The body remains authoritative on **what is checked**; the **how** is the generator + validators.

### v2.R6 Operational invocation (post-2026-04-27 restructure)

**Authoritative scripts:**

- Readiness output (initial + revision): `scripts/_pilot_build_readiness_output_v2_20260426.py`
- Analytic status (revision sessions only): `scripts/_pilot_build_analytic_status_v1_20260427.py`

**Invocation:**

```bash
# Initial analysis (first time analysing this registry)
python scripts/_pilot_build_readiness_output_v2_20260426.py --registry N

# Revision session (registry analysed before; produces both readiness + analytic status)
python scripts/_pilot_build_readiness_output_v2_20260426.py --registry N
python scripts/_pilot_build_analytic_status_v1_20260427.py --registry N
```

**Output destinations** (all paired `.md` + `.json`):

| Artefact | Destination | Filename pattern |
|---|---|---|
| Readiness output | `Sessions/Session_B/07_Analysis_Readiness_Status/` | `wa-{NNN}-{word}-readiness-output-v{n}-{date}.{md,json}` |
| Analytic status (revision) | `Sessions/Session_B/07_Analysis_Readiness_Status/` | `wa-{NNN}-{word}-analytic-status-v{n}-{date}.{md,json}` |

**The obslog parser/writer outputs (manifest, validation, capture report) live in a different folder** — `Sessions/Session_B/09_Analysis_output_logs/words/`. Do not co-mingle: readiness is for stage **07** (pre-analysis preparation); the obslog pipeline is for stage **09** (post-analysis capture).

### v2.R7 Pre-flight gate sequence (CC's readiness generator)

In order, before any output is written:

1. **Phase A prose check** (per §v2.R0 above) — 6 prose_section rows under `source_stage='session_a'` for the registry. Halt + raise anomaly if missing.
2. **Status gate** — `word_registry.phase1_status='Complete'` AND `word_registry.verse_context_status='Complete'`. (For words flagged "In Progress" on VC: emit a warning but continue, since partial readiness output is still useful for review.)
3. **Engine audit** — `word_registry.last_automation_run='AUDITED'`. Warn if not.
4. **Schema version** — `schema_version` table reports 3.17.0 or later (M40-M43 applied). Halt if lower.
5. **Integrity pre-flight** — anchor count consistency, dimension assignment coherence, term-status integrity, group description vs dimension drift. Each anomaly written as an open `wa_session_b_findings` row of the appropriate `DATA_ANOMALY_*` type (see §v2.R3) and surfaced in §N of the readiness output.
6. **Readiness generation** — produce `.md`+`.json` to `Sessions/Session_B/07_Analysis_Readiness_Status/`.
7. **§M self-verification** — the readiness output's own §M (Readiness Verification) confirms each section rendered with non-empty content. Empty sections produce inline warnings the AI sees at session start.

### v2.R8 Validation checklist (15 checks; READY/BLOCKED gate)

After the readiness output is produced, CC runs a **fixed 15-check validation** and produces a paired report (`.md` + `.json`) in `Sessions/Session_B/07_Analysis_Readiness_Status/`. The validation script is `scripts/_pilot_validate_readiness_v1_20260427.py`.

```bash
python scripts/_pilot_validate_readiness_v1_20260427.py --registry N
```

**Output filenames:** `wa-{NNN}-{word}-readiness-validation-v1-{YYYYMMDD}.{md,json}`

**The 15 checks:**

| ID | Check | Verdict policy |
|---|---|---|
| C01 | Schema version at 3.17.0 (M40-M43 applied) | FAIL if lower |
...
| C06 | Phase A prose captured (6 sa_s1_d* rows) | FAIL if < 6 |
| C07 | Readiness output `.md` + `.json` present | FAIL if missing |
| C08 | OWNER terms non-empty, all with ≥1 verse | FAIL if no terms; WARN if any zero-verse term |
| C09 | All VC groups have a dimension assignment | FAIL if any group lacks a dimension |
| C10 | All VC groups have ≥1 anchor verse | WARN if any group lacks an anchor |
| C11 | No group with set-aside ratio > 90% | WARN if any (signals possible VC drift) |
| C12 | Legacy-VC caveat (per-term `vc_status`) | WARN if any term still `not_done` (informational) |
| C13 | Dimension confidence (any 'queried'?) | WARN if any 'queried'; WARN if no 'confirmed' assignments |
| C14 | No open `DATA_ANOMALY_*` findings | FAIL if any open |
| C15 | Researcher fields (`inference_note`, `word_synopsis`) | WARN if either absent (informational) |

**Verdict semantics:**

- **PASS** — check satisfied; no action required.
- **WARN** — informational; surfaces in §N of the readiness output for AI to address during Stage 2 where it materially affects findings. Does not block handoff.
- **FAIL** — blocking. The registry cannot proceed to Session B Stage 2 until the failure is resolved.

**Overall verdict:**

- **READY** — `FAIL = 0`. Registry cleared for handoff to `wa-sessionb-analysis-output [current]`.
- **BLOCKED** — `FAIL ≥ 1`. Resolve failures and re-run validation.

**Handoff gate:** the Analysis Readiness phase is closed only when validation reports `READY`. The validation `.md` + `.json` are the artefacts that record this gate-passing — they are the audit trail for the handoff. The validation report is itself an input AI reads at session start (alongside the readiness output) so AI knows which WARNs to track during Stage 2a.

**Idempotency:** validation reads-only against the DB and the filesystem; safe to re-run any time.

**Rationale for these 15 checks (vs other possible checks):**

- C01–C05 are the registry-level gates — non-negotiable preconditions.
- C06 is the new Phase A prose gate (per §v2.R0).
- C07 ensures the readiness sweep itself produced its outputs.
- C08–C13 are content-level health checks on the data the analytical session will read.
- C14 ensures CC's own pre-flight integrity work didn't surface unresolved anomalies.
- C15 is informational — researcher-authored fields, not produced by any pipeline.

The list is open for extension as new failure modes surface; extend by adding a `check_*` function and a row in the `CHECKS` list in the validation script, then bump this instruction.
...
**v1_10 (2026-05-01):** Catalogue v2 alignment + status-gate clarification. (1) Banner version + filename + Supersedes refreshed. (2) §v2.R2 row L revised: §L now embeds the v2 second-tier catalogue (T0–T7) — 189 active prompts grouped tier→component→prompt — replacing the retired v1 generic-section format. The v1 questions remain in `wa_obs_question_catalogue` as `status='redundant_v1'` for provenance. (3) §S3 / cross-field gate clarification (Step S3 status table + Stage 1 entry table): retains the existing wording — `Pre-Analysis Complete` (preferred) or `Verse Context Reset` (legacy acceptable) on entry; only `Analysis Complete` / `Session B Complete` / `NULL` trigger researcher confirmation. The hard gates remain `verse_context_status='Complete'` AND `dim_review_status='Complete'`. (4) Rationale: AI sessions reading recent readiness outputs were misinterpreting `Verse Context Reset` as a stop condition; the instruction has always permitted it, but the wording is now reinforced inline. (5) Authoritative readiness builder: `scripts/_pilot_build_readiness_output_v2_20260426.py` updated to query `tier IS NOT NULL AND status='active'` and render §L grouped by tier→component→prompt; output filename version bumped to `v6`; meta carries `catalogue_version='v2-2026-04-29'`. First application: R034 covenant, R099 kindness, R117 peace (all READY: PASS=11, WARN=4, FAIL=0).

**v1_9 (2026-04-27):** Validation checklist gate added before handoff. (1) Banner version + filename + Supersedes refreshed. (2) Outputs row: validation report `.md`+`.json` added; Handoff row: explicit READY-verdict gate. (3) Claude Code role row: 15-check validation + READY/BLOCKED verdict + handoff gating added. (4) New **§v2.R8** Validation checklist: documents the 15 checks (C01–C15), PASS/WARN/FAIL verdict policy per check, READY/BLOCKED overall verdict semantics, handoff gate, idempotency note, and rationale for the check list. (5) Authoritative validation script: `scripts/_pilot_validate_readiness_v1_20260427.py`. First application: R030 contrition (PASS=12, WARN=3, FAIL=0 → READY).

**v1_8 (2026-04-27):** Phase A prose precondition + paths post-restructure. (1) Banner version + filename + Supersedes refreshed. (2) Companion documents row: `wa-global-rules-all [current]` (post-restructure name) and **wa-sessiona-prose-instruction [current]** added (new precondition reference). (3) Inputs row: clarifies that `prose_section.source_stage='session_a'` rows are an input, with cross-ref to §v2.R0. (4) Outputs row: explicit destination `Sessions/Session_B/07_Analysis_Readiness_Status/`. (5) Claude Code role row: adds Phase A prose precondition verification. (6) New **§v2.R0** added before §v2.R1: documents the Phase A prose precondition, the SQL pre-flight check, and the halt+anomaly behaviour if missing. New `DATA_ANOMALY_PHASE_A_PROSE_MISSING` finding type introduced (informational; carry to next session's §N). (7) New **§v2.R6** added: operational invocation — authoritative scripts, command-line invocations, output destination table, and the explicit separation of stage-07 readiness vs stage-09 obslog folders. (8) New **§v2.R7** added: pre-flight gate sequence — 7-step ordered gate (Phase A prose, status, audit, schema, integrity pre-flight, generation, §M self-verification). (9) Source-of-truth path corrected from `../../../research/` to `../../research/` (post-restructure depth from Workflow/Instructions/ is two levels). Body retained.

**v1_7 (2026-04-27):** Architecture v2 addendum applied. (1) Version, filename, Supersedes refreshed. (2) Inputs row: DB state directly (no JSON export). (3) Outputs row: readiness `.md`+`.json` + analytic status `.md`+`.json` for revisions. (4) Claude AI role: minimised — readiness is now a CC operation. (5) Claude Code role: owns the readiness phase end-to-end. (6) New "Architecture v2 — addendum" section documents readiness ownership (v2.R1), `.md` structure (v2.R2), CC anomaly detection (v2.R3), idempotency (v2.R4), stage continuity (v2.R5). Body retained as authoritative on **discipline of checks**; mechanism is now the generator + validators.
...
*wa-sessionb-analysis-readiness-v1_10-20260501.md*
*Framework B — Soul Word Analysis Programme*
*Supersedes wa-sessionb-analysis-readiness-v1_9-20260427.md (post catalogue v2 migration: §L embeds T0–T7 catalogue; legacy session_b_status acceptance reinforced)*
*Paired with wa-sessionb-analysis-output [current] (Stage 2)*

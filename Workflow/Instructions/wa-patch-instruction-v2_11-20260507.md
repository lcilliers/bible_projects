# wa-patch-instruction-v2_11-20260507

> Framework B Soul Word Analysis Programme — Patch Preparation and Execution
> Version: v2_11 | Date: 20260507
> Supersedes: wa-patch-instruction-v2_10-20260427.md (cluster-process routing addendum)
>
> **v2_11 (20260507) — Cluster-process routing addendum.** New §1.5 routing table and a small note in §1.2: cluster-process work (sub-group assignment, cluster reassignment, group-verse mapping apply, cluster findings recording, schema enablement for cluster tables) flows through **directives**, not patches — see `wa-directive-instruction [current]` §11. Patches remain authoritative for VC family, REPAIR, DIMREVIEW, and word-level metadata writes. The introduction of the cluster as the primary analytical scope (per the term-anchor reset 2026-05-04 and the M06 cluster pass 2026-05-06) does not change the patch-vs-directive principle — it adds a class of work that lands in the directive lane. v2_11 makes the routing explicit so AI does not draft patch shapes for cluster_finding / cluster_subgroup / verse_context_group cluster-scoped operations. No semantic change for existing patch types; doc-only clarification.
>
> **v2_10 (20260427) — Architecture v2 addendum: Session B writes via the obslog-to-DB capture pipeline, not via patches.** Source-of-truth: [research/investigations/db-capture-phase1-results-and-table-architecture-v1-20260427.md](../../../research/investigations/db-capture-phase1-results-and-table-architecture-v1-20260427.md) Parts 10-12.
>
> Under v2:
>
> - **Session B no longer produces patches.** AI submits a comprehensive obslog `.md`; CC's Phase 2 writer (`scripts/_pilot_capture_obslog_to_db_v1_*.py`) parses and writes to the DB. The patch types previously used for Session B (SESSIONB, SESSIONB_FINDINGS, SESSIONB-COMPLETE, PROSE for chapters, CATALOGUE_POPULATION, DIMREVIEW where it intersects Session B) are deprecated for the Session B flow.
> - **The instruction-gap items CAT-B-001, CAT-C-001, CAT-D-001, CAT-E-001 (logged in the goodness obslog v2 backwards-coverage check) are resolved by the v2 routing change.** Catalogue links, chapter writes, catalogue population, and review-note maintenance are all handled by CC's writer — no patch shapes needed for these.
> - **What stays in the patch path:**
>   - **VERSECONTEXT family** (VCNEW, VCREVISE, VCSBFLAGS, VCSDPOINTERS, VERSECONTEXT, VCGROUP, VCVERSE) — Verse Context work continues to use patches per `wa-versecontext-instruction [current]`.
>   - **REPAIR** — surgical data-correction patches.
>   - **DIMREVIEW** for dimension-index corrections; manual application per §11.6 remains.
> - **Schema migrations M40-M43** (live as of 2026-04-27): `verse_context.analysis_note` column · `wa_prose_section_citations` table · `wa_obs_question_catalogue.review_note` column · `wa_finding_catalogue_links.finding_id` NULL allowed. These support the writer pipeline and have no patch-instruction implications.
>
> The body of the document below describes patch shapes and discipline that **remain authoritative for VC patches, REPAIR, and DIMREVIEW**. Session-B-specific patch shapes (where they exist in §3, §11, §15) are now **historical reference** — the writer flow replaces them.
>
...
### 1.5 Decision table — cluster-process work (new in v2_11)

Cluster-process work (sub-group structure, group-verse mapping, cluster findings) flows through **directives**, not patches. The routing is fixed — AI must not draft patch JSON for these operations.

| Change | Method | Document / section |
|---|---|---|
| Create / modify `cluster_subgroup` rows; assign `mti_terms.cluster_subgroup_id` | Directive (cluster-process) | wa-directive-instruction [current] §11.4 |
| Reassign a term's `mti_terms.cluster_code` (cross-cluster move) | Directive (cluster-process) | wa-directive-instruction [current] §11.6 |
| Apply a sub-group's group-verse mapping document — `verse_context_group` UPDATE/INSERT, `verse_context` UPSERT (incl. dual rows), anchor `is_anchor=1` toggles | Directive (cluster-process) | wa-directive-instruction [current] §11.4 |
| Write `cluster_finding` rows from a catalogue-pass consolidated findings document | Directive (cluster-process) | wa-directive-instruction [current] §11.5 |
| Resolve `cluster_finding` gap rows via DB query | Directive (cluster-process, lite) | wa-directive-instruction [current] §11.2 / §11.6 |
| BOUNDARY characterisation rows in `cluster_finding` | Directive (cluster-process) | wa-directive-instruction [current] §11.5 |
| Add a column / relax a constraint on `cluster`, `cluster_subgroup`, or `cluster_finding` | Directive (schema enablement) | wa-directive-instruction [current] §10 |
| Update `cluster.status` (e.g. `Data - In Progress` → `Analysis - In Progress` → `Analysis Completed`) | Directive (typically rides the closing directive's outcome) | wa-directive-instruction [current] §11 |
| Verse-status corrections at cluster scope (UT → confirmed/borderline/set-aside, **with** verse_record_id resolution required) | Directive (cluster-process or VC remediation) | wa-directive-instruction [current] §11.2; VC family remains for known-IDs writes |
| Verse-context status corrections **with all IDs known in advance** (VCNEW, VCREVISE, etc.) | Patch (VC family) | This document §3, §11 |

**Why directive, not patch, for cluster-process work:**

Cluster-process operations require CC inspection of the database to resolve identifiers (mti_term_id from Strong's, group_id from group_code, verse_record_id from book/chap/verse, cluster_subgroup_id from subgroup_code). The source artefact is structured-markdown produced by Claude AI's catalogue or mapping pass — CC parses it and applies. This is the directive's design space (per `wa-directive-instruction [current]` §1.1 and §11.1).

**Cluster-process directives never carry data operations that belong in a patch.** A mapping-apply directive does not write `mti_terms.evidential_status` (that is patch territory). A findings-record directive does not write `wa_session_b_findings` (that table is registry-scope, not cluster-scope; see `wa-sessionb-cluster-instruction [current]` §15).
...
Location: `Sessions/Session_D/session_d/`
...
Default output: `Workflow/reference/wa-global-rules-extract-{YYYYMMDD}.json` + `.md`. Obsolete rules and addenda are excluded by default.
...
5. **Regenerate the extract** — `python scripts/build_programme_prose_extract.py --all-formats` produces JSON + MD + DOCX in `Workflow/reference/` + `outputs/docx/`.
...
*wa-patch-instruction-v2_11-20260507 | Supersedes wa-patch-instruction-v2_10-20260427 | v2_11 adds §1.5 cluster-process routing table — cluster_subgroup / cluster_finding / verse_context_group cluster-scoped operations route to directives per wa-directive-instruction [current] §11. Doc-only clarification; no semantic change for existing patch types.*

*Historical: wa-patch-instruction-v2_6-20260424 | Supersedes wa-patch-instruction-v2_5-20260424 | v2_6 adds A-03 version-gate support: `_patch_meta.input_versions` (map {mti_term_id: md_version}) required for VCNEW and VCREVISE; applicator rejects stale mismatches and bumps md_version on apply. `_patch_meta.batch_id` reclassified as optional (A-04). §15.2 / §15.3 / §15.4 / §15.5 / §15.6 updated. A-02: vc_status values now reference 'vc_completed' (was 'complete'; 'approved' dropped).*

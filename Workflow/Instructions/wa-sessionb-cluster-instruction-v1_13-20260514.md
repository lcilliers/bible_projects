# wa-sessionb-cluster-instruction-v1_13-20260514

> Framework B Soul Word Analysis Programme — Session B Cluster Analysis
> Version: v1_13 | Date: 20260514
...
> **Change note v1_12 → v1_13 (2026-05-14):** §5 Phase 2 — **provisional-anchor convention**. New §5.1.1 documents the rule: when a VCNEW patch introduces the first relevant verses for a term that has no existing anchor, the first `is_relevant=1` operation for that term is marked `is_anchor=1` as a provisional anchor (to satisfy the applicator's R4 integrity check). Phase 6 reconciliation re-evaluates per VCG. Per-classification-rules table extended with the `is_anchor` column and a dedicated row for the "first per term, no existing anchor" case. CC-side enforcement clause added: CC must apply the promotion before dry-run, especially for API-generated batches where AI returns all is_anchor=0 by default. Established 2026-05-14 during M46's API-driven UT review of 197 verses / 16 terms / 12 new terms requiring provisional anchors. No analytical-rule changes.
> Full history: see §19.
...
| Phase 2 outcome | `is_relevant` | `is_anchor` | `set_aside_reason` | `group_id` |
|---|---:|---:|---|---|
| **Set aside** | 0 | 0 | populated (one-line reason) | `null` |
| **Confirmed relevant** | 1 | 0 (default) — see provisional-anchor rule below | omit (or null) | `null` (Phase 7 will assign) |
| **Confirmed relevant — first per term, no existing anchor** | 1 | **1** (provisional anchor) | omit | `null` |
| **Borderline** | _(do not write — held in obslog only for researcher decision)_ | — | — | — |

#### 5.1.1 Provisional-anchor rule (R4 satisfaction at Phase 2)

The applicator's R4 integrity check requires every term with any `is_relevant=1` row to have at least one `is_anchor=1` row. Phase 2 does not normally designate anchors (that is Phase 6's job during VCG reconciliation), but a VCNEW patch that introduces the first relevant verses for a term must satisfy R4 on apply. Discipline:

1. **For each term in the patch**, determine whether it already has an active anchor row in `verse_context` (`is_anchor=1`, `delete_flagged=0`).
2. **If no existing anchor**, mark the **first** `is_relevant=1` operation for that term with `is_anchor=1`. This is a **provisional** anchor — its role is to satisfy R4 at apply time, not to claim that verse as the cluster-canonical anchor.
3. **If only set-asides** are produced for a term (no relevant verses), R4 does not require an anchor — leave all `is_anchor=0`.
4. **Phase 6** reconciliation re-evaluates anchors per VCG and may move the anchor designation to a different verse. The provisional anchor is a placeholder, not a finding.

**CC-side enforcement:** when CC builds or post-processes a Phase 2 VCNEW patch (especially for API-generated batches where AI returns all classifications with `is_anchor=0` by default), CC must apply the provisional-anchor promotion before the patch is dry-run. Single-line scan: per term, if no existing anchor, set `is_anchor=1` on the first relevant op for that term.

This convention was established 2026-05-14 during M46's API-driven UT review (197 verses across 16 terms; 12 new terms required provisional anchors). It applies to all future Phase 2 patches.
...
- **SCOPE** — Operations A/B/C as above; every Strong's with its sub-group destination; every `cluster_subgroup` row to be created with `subgroup_code`, `label`, `core_description` (see §A1 for the actual column list); any cluster_code rebinds; the status transition.
...
- Five-element form:
  ...
  - **SCOPE** — segment coverage (sub-group(s) or split-portion), term set, operations:
    ...
    - `verse_context` UPSERT — set `group_id`, **`cluster_subgroup_id`** (mandatory — both fields must be populated; an apply script that sets only `group_id` and leaves `cluster_subgroup_id` NULL will trip the §13.8 validator's C1 check), `is_anchor`, `is_relevant`; include dual-assignment second rows. The `cluster_subgroup_id` value is the same `cluster_subgroup.id` carrying the term's `mti_term_subgroup` placement — there is no separate lookup.
...
| `**[BOUNDARY — H1234 translit]**` | **per-term** structural characterisation for one BOUNDARY term (Phase 8 only; canonical form per §11.6 — one block per BOUNDARY term) | `finding` |
| `**[BOUNDARY]**` | aggregate BOUNDARY characterisation covering all BOUNDARY terms in one block (legacy / fallback — use the per-term form above when there are multiple BOUNDARY terms with distinct structural roles) | `finding` |
...
The six tables touched by Phase 4 and Phase 7 directives. **Author Phase 4 and Phase 7 directives by consulting this list, not by analogy from another table.** The M39 first-run produced two distinct schema errors traceable to analogy-by-name: AI assumed `verse_context_group` shared `cluster_subgroup`'s column names, but they differ. Use the actual column names below.

### `cluster`

`cluster_code` (PK) · `description` · `gloss` (not null) · `source` · `bucket` · `status` · `version` · `last_updated_date` · `short_name`

### `cluster_subgroup` (Phase 4 — Operation A creates these)

`id` (PK) · `cluster_code` (not null) · **`subgroup_code`** (not null — e.g. `M39-A`, `M39-BOUNDARY`) · `label` (not null) · **`core_description`** · `sort_order` · `status` · `version` · `source` · `notes` · `delete_flagged` · `created_at` · `last_updated_date`

> **Note.** BOUNDARY is identified by `subgroup_code` naming convention (e.g. `M39-BOUNDARY`). There is no `is_boundary` column.

### `mti_term_subgroup` (Phase 4 — Operation A populates the term↔sub-group join)
...
> **Note.** There is no `mti_terms.cluster_subgroup_id` column. Term placement lives in this join table.

### `verse_context_group` (Phase 6 designs, Phase 7 — Operation 1 INSERTs/UPDATEs)

`id` (PK) · **`group_code`** (not null — note: `group_code`, not `subgroup_code`) · **`context_description`** (not null — note: `context_description`, not `core_description`) · `notes` · `delete_flagged` · `vertical_pass_flag`

> **Note — the two confusing column names.** `cluster_subgroup` uses `core_description`; `verse_context_group` uses `context_description`. The naming differs because they describe different things (a sub-group's analytical scope versus a VCG's verse-shared context). Do not assume by analogy.

### `vcg_term` (Phase 7 — required for every NEW VCG, post-M47)
...
### `verse_context` (Phase 2 inserts, Phase 7 — Operation 2 sets `group_id`)

`id` (PK) · `verse_record_id` (not null) · `mti_term_id` (not null) · `group_id` · `cluster_subgroup_id` · `is_anchor` (not null) · `is_relevant` (not null) · `is_related` (not null) · `notes` · `delete_flagged` · `vertical_pass_flag` · `set_aside_reason` · `analysis_note`

### `cluster_finding` (Phase 9 — findings record)

`id` (PK) · `obs_id` (not null) · `cluster_code` (not null) · `cluster_subgroup_id` · `finding_status` (not null — `finding` / `cluster_synthesis` / `silent` / `gap`) · `finding_text` · `source_file` · `version` · `notes` · `delete_flagged` · `created_at` · `last_updated_date`

> **Unique constraint:** `(obs_id, cluster_code, cluster_subgroup_id, version)`. Multiple rows per (prompt × scope) are permitted with different `version` values, but the cleaner default is one row per cell; APPEND additional content to `finding_text` rather than INSERTing parallel rows.

### `wa_obs_question_catalogue` (Phase 8 / Phase 9 — prompt resolution)

`obs_id` (PK alias on `id`) · **`question_code`** (the T#.#.# code — note: `question_code`, not `prompt_code`) · `section` · `source_word` · `source_registry_no` · `question_text` · `pattern_type` · `scope` · `status` · `deleted` · `date_added` · `catalogue_version` · `review_note` · `tier` · `component_code` · `component_title` · `prompt_seq`

> **Note — column name.** Resolve a prompt to its `obs_id` with `SELECT obs_id FROM wa_obs_question_catalogue WHERE question_code='T#.#.#' AND COALESCE(deleted,0)=0`. The column is `question_code`. The directive-text variant `prompt_code` does not exist.

### `wa_dimension_index` (Phase 9 — dimensional-sharing analytics)

`id` (PK) · `verse_context_group_id` (FK to `verse_context_group.id`) · `owning_registry_no` · **`cluster_assignment`** (TEXT — **legacy: uses C01–C22 codes from the pre-2026-05-04 cluster taxonomy; do NOT join M-cluster codes directly to this column**) · `dimension` · `anchor_count` · `related_count` · `set_aside_count` · `total_verse_count` · `delete_flagged` · `dimension_confidence` · `manual_override` · `notes` · `last_modified` · `dominant_subject`

> **Note — legacy cluster codes.** `cluster_assignment` was populated under the pre-pivot C-cluster schema. Zero rows exist for any M-cluster code. To use dimensional data under the current M-cluster taxonomy, **join via `verse_context_group_id` → `verse_context_group.id`, then to current cluster via `verse_context → mti_terms.cluster_code`**. Dimensions attach to VCGs and remain analytically valid across the cluster migration.

**Schema verification:** run `python -c "import sqlite3; c=sqlite3.connect('database/bible_research.db'); [print(r['name']) for r in c.execute('PRAGMA table_info({table})')]"` against `database/bible_research.db` if uncertainty arises mid-authoring. The DB is the source of truth; this appendix is a current-state snapshot.

---

## 19. Change history

**v1_13 (2026-05-14) — Active.** §5 Phase 2 — provisional-anchor convention. New §5.1.1 documents the rule: when a VCNEW patch introduces the first relevant verses for a term with no existing anchor, the first `is_relevant=1` operation for that term is marked `is_anchor=1` as a provisional anchor to satisfy the applicator's R4 integrity check (which requires every term with relevant verses to have ≥1 anchor). Phase 6 VCG reconciliation re-evaluates anchors per VCG and may move designation. Per-classification-rules table extended with `is_anchor` column and a dedicated row for the "first per term, no existing anchor" case. CC-side enforcement: when CC builds or post-processes a Phase 2 VCNEW patch (especially API-generated batches where AI returns all is_anchor=0 by default), CC must apply the promotion before dry-run. The convention was established 2026-05-14 during M46's API-driven UT review (197 verses / 16 terms / 12 new terms required provisional anchors after the apply rejected the patch on R4). Reference: [scripts/_apply_m46_ut_review_via_api_20260514.py](../../scripts/_apply_m46_ut_review_via_api_20260514.py), [scripts/archive/_tmp_fix_anchors_in_patch.py](../../scripts/archive/_tmp_fix_anchors_in_patch.py). No analytical-rule changes.

**v1_12 (2026-05-14) — Superseded by v1_13.** Preventive additions from M39 first-run lessons. No analytical-rule changes. (1) §10.4 SCOPE Operation 2 bullet now explicitly states that Phase 7 must populate **both** `verse_context.group_id` and `verse_context.cluster_subgroup_id` — the directives have always specified both, but the M39 dir-002/003 apply script omitted `cluster_subgroup_id` and the §13.8 validator subsequently reported 15 false-positive C1 gaps; CC-level backfill of 739 rows resolved it. Reminder added so future apply scripts cover both fields. (2) §A1 column-reference appendix extended with **`wa_obs_question_catalogue`** (the `question_code` column — directive-text variants like `prompt_code` do not exist) and **`wa_dimension_index`** (with the C-code legacy caveat: join via `verse_context_group_id` for current M-cluster dimensional analytics, not via the legacy `cluster_assignment` column). Also added a unique-constraint note on `cluster_finding`. (3) §11.8 marker catalogue — added the per-term `**[BOUNDARY — H1234 translit]**` marker as the canonical Phase 8 BOUNDARY characterisation form, per §11.6 ("one structural characterisation note per BOUNDARY term"). The aggregate `**[BOUNDARY]**` form retained as legacy/fallback. Reference sources: [Sessions/Session_Clusters/M39/WA-M39-verification-corrections-applied-v1-20260514.md](../../Sessions/Session_Clusters/M39/WA-M39-verification-corrections-applied-v1-20260514.md), [Sessions/Session_Clusters/M39/WA-M39-phase10-input-pack-v1-20260514.md](../../Sessions/Session_Clusters/M39/WA-M39-phase10-input-pack-v1-20260514.md).

**v1_11 (2026-05-14) — Superseded by v1_12.** Schema consistency follow-up to v1_10. (1) §7.7 SCOPE bullet's "code, label, description" corrected to "`subgroup_code`, `label`, `core_description`" — eliminates internal inconsistency with the Operation A wording that already uses `core_description`. (2) New **§A1 Cluster-process tables — column reference** appendix added: authoritative column lists for `cluster`, `cluster_subgroup`, `mti_term_subgroup`, `verse_context_group`, `vcg_term`, `verse_context`, `cluster_finding`. Includes an explicit note on the `cluster_subgroup.core_description` vs `verse_context_group.context_description` distinction — the analogy-by-name confusion observed in M39 dir-002/003 authoring. Authoring AI must consult §A1 when drafting Phase 4 or Phase 7 directives. (3) v1_9 archived (was left in `Workflow/Instructions/` alongside v1_10 — GR-FILE-003 cleanup). No analytical-rule changes.

**v1_10 (2026-05-14) — Superseded by v1_11.** Phase 4 directive schema corrections from M39 first-run. §7.7 Operation A wording corrected: (1) term assignment uses `mti_term_subgroup` join-table INSERT, not `mti_terms.cluster_subgroup_id` UPDATE; (2) BOUNDARY is a `subgroup_code` naming convention, not an `is_boundary` column; (3) sub-group description field is `core_description`. §7.7 COMPLETION CONFIRMATION query corrected to join via `mti_term_subgroup`. Frontmatter change note updated. No other changes.
...
*wa-sessionb-cluster-instruction-v1_13-20260514 | Active — authoritative instruction for Session B cluster analytics*
*Replaces (on finalisation): wa-sessionb-analysis-readiness, wa-sessionb-analysis-output, wa-dimensionreview-instruction, wa-versecontext-instruction, wa-registry-management-guide*

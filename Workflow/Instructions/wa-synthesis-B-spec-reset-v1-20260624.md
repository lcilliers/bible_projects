...
- **The DISCARD↔INCLUDE balance is itself under calibration, in situ** *(researcher, 2026-06-25).* We are discovering unknowns; the line between a *rare-but-real* inner-being dynamic (→ include, singleton rule) and an *artifact* (→ discard, validity gate) is **not known in advance**. So it is **not set by rule** — every **discard**, and every **borderline include**, is **flagged and logged**, and the line is **calibrated from the accumulating cases** in review cycles. **Asymmetry while immature: bias toward surfacing/keeping over discarding** — a wrongly-*kept* candidate is visible and correctable later; a wrongly-*discarded* one is invisible and lost. (This is the parent tension under the singleton rule, the validity gate, and D3.)
...
- **D3 — Assembly model — IN EVALUATION (resolved in situ, see §X).** Provisional default: **(i) recognise-then-attach** — recognise a movement from verses rich enough to show its shape, then **attach** sparser single-edge verses to the *named* movement by their D1 signal (each verse preserved; nothing force-merged; consistent with the singleton rule). **(ii) fragment-stitch** — reconstruct a movement never wholly present in one verse — is used only when the movement is genuinely *inherently distributed* and with a join key that is an explicit **D1 signal, never the lemma**. **Every (ii) use and every borderline attach is FLAGGED as a D3-occurrence and logged** (the model, the join key, forced-or-safe, what's at stake), so the (i)/(ii) balance is **calibrated from real cases**, not pre-decided. A sparse verse **joins** a movement; it is **never deleted for being sparse.**
...
## §X — Decisions IN EVALUATION (resolved in situ, not abstractly)

1. **D3 assembly model — FLAGGED IN-EVALUATION (researcher, 2026-06-25).** We don't yet know the balance, so we **surface it rather than decide it**. Provisional default: **(i) recognise-then-attach.** **Every use of (ii) fragment-stitch, and every borderline attach, is flagged as a D3-OCCURRENCE** and logged (in the discovery register's D3 log), recording: the model used · the **join key** · whether the join felt **forced or safe** · what was at stake (what would be lost under the other model). The (i)/(ii) line is then **calibrated from the accumulated occurrences** in review cycles and settled empirically — *evaluated in situ.* Standing constraint while in evaluation: **(ii) never uses the lemma as the join key**, and (per the discard↔include balance, §1) **bias toward attaching/keeping over stitching-or-dropping** when unsure. *(Cluster formation §2a and the B+D pointers §5a are established practice — settled.)*

...
- _2026-06-07:_ **Researcher markup received** (`wa-archive-decisions-for-guidance-v1-20260607.md`). Executed:
  **Sciences/** — relocated the **44 per-cluster `scienceextract` files → their `Sessions-v2/{CODE}-{Name}/` cluster folders** (git mv); prose drafts + obslog kept in Sciences/. **Tiers/** — archived `wa-tpopb-findings-catalogue-extract` + `wa-tpopc-btarget-flags-extract`; kept `wa-tier-framework-definitions-v1_2` + `wa-tier-questions-extract-v1`. **schema/** — generated fresh `database-schema-v3.28.0-20260607.json`; archived superseded `v3.17.0` snapshot. **Instructions/** — archived the 2 non-governing pre-2026-05-14 items (`database-schema-v3.14.0`, `programme-prose-extract`); **all governing [current] instructions kept regardless of date** (date rule applied only to non-governing working docs). **Clusters/** — archived **all 12** current files; generated fresh `wa-cluster-overview-20260607.md` from the live DB. **methodology/, programme_analysis/, the April bulk set** — researcher already archived (verified). **Kept:** global_rules, registry, Program_reports, programme_prose.
  **Held / flagged:** `Corpus_prose/` "bring up to date" — narrative regen, not a mechanical move; left in place pending the source/method (do not archive the only corpus prose without a replacement). `wa-Session-A-Instruction-v8-final.docx` + the `.docx` records — **kept**, currency to confirm.

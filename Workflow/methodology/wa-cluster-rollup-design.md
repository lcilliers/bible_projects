...
- **(b) reroute verses/terms to other clusters, or set aside** — the **term is the unit of movement**
  (`feedback_term_is_the_unit_of_movement`); it carries its verses/VCGs. **Set-aside is layered (D3):**
  **verses at L1**, **BOUNDARY terms at the sub-group stage (L2)**, **terms rerouted to other clusters at the
  VCG stage (L3)**.
...
- **(e) reuse vs re-create (C2 consistency rule)** — when re-deriving an item (meaning / VCG / observation)
  that already exists: if the **content is identical → reuse it**; if it **differs → (1) surface the *why* to
  the researcher** (the root cause of the difference must be understood), then **(2) create new + soft-delete
  the old**.
...
- **Does:** partition the cluster's relevant verses/terms into **provisional sub-groups** from the L1
  ...
  characteristic.* Terms that **cannot yet be placed are marked BOUNDARY (undecided)** — the placement is
  deferred, **to be resolved by the end of sub-group processing** (E5).
...
- **Gate — post-L2 completeness:** **every in-focus item is allocated to a sub-group OR explicitly marked
  cluster-level OR marked BOUNDARY** (or held for routing/set-aside). No item unaccounted.
...
- **Does:** the **aggregate question** — *what does this VCG's verses jointly say that no single verse
  captures?*
- **Updates:** the VCG-emergent observation at the **VCG-group level** (E6) — keyed to
  `verse_context_group` (`context_description` and/or a new VCG-group observation field), **not**
  `cluster_observation` (which is cluster-level).
...
- **Gate — VCG-completion cross-check (the closing completeness check):**
  ...
  3. **No orphaned VCGs or sub-groups** — every VCG sits in a sub-group and is observed; every sub-group has a
     characteristic and a cluster-account home.
  4. **All BOUNDARY terms resolved (E5)** — every term placed, rerouted to another cluster, or set aside;
     **the audit must reveal any orphaned term** in the cluster.
  5. **Zero orphans overall** — every verse / flag / pointer / prior observation/finding accounted for.
...
- **L8b · study-level cross-cluster reconciliation (once all clusters complete).**
  ...
  - **Does:** a **single processing pass over all cluster-level findings** to **resolve additional
    ...
    cross-cluster groupings** (foundations §d); interdependent clusters finalise together. **The science
    layer is introduced here (F1)** — correlate with the per-cluster science extracts and **develop
    science-derived findings** at this cross-cluster stage.
  - **Updates:** cross-cluster grouping/finding structures **C**; finding adjustments **U**; science-derived
    findings **C** 〔schema TBD〕.
...
(4) **every open question is a finding** (E7) — recorded as a finding flagged **`needs_research`** (a new
`cluster_finding` column) marking it for further research.
...
- **Retire `vertical_pass_flag`** (E4) — a legacy operational flag from the superseded "vertical pass"
  experiment (VCB-031, 2026-04-08); soft-retire under V3_2. `is_related` and the UT-verse classification are
  **retained**.

---

## Ownership & cost (D1 / D2)

- **Governance:** the whole process is **run by CC with researcher input**, **referring select operations to
  AI** (via **API or chat** — whichever gives the best result). L1 mechanical = CC; L1-select + L3–L5 deep
  read = AI; findings write = CC; gates = researcher.
- **Cost:** **submit a costing for any API operation > \$2**; the researcher monitors cumulative cost
  otherwise (a large re-process of already-done work is expected).
...
### Resolved this pass (2026-06-07)
- **B3 [L8b] — RESOLVED.** A cross-cluster finding seeded into Y is **consumed when Y is processed**; if Y is
  **already complete**, it is consumed in the **final L8b review** of all clusters.
- **C1 [L3] — RESOLVED (surface-to-researcher).** When clarifying a term's sense needs corpus outside the
  unit, **surface it to the researcher** and deal with the *why* **when it happens** — not pre-solved in
  theory.
- **C2 [L3] — RESOLVED (reuse rule).** If an item's content is **identical** → **reuse** it. If it
  **differs** → **(a) surface the *why* to the researcher** (the root cause of the difference must be
  understood), then **(b) create new + soft-delete the old**.
- **D1 [all] — RESOLVED.** The whole process is **governed by CC with researcher input**, **referring select
  operations to AI** (via API **or** chat — whichever gives the best result).
- **D2 [L3–L5] — RESOLVED.** **Submit a costing for any API operation > $2**; the researcher monitors
  cumulative cost otherwise (a re-process of much already-done work is expected).
- **D3 [layered] — RESOLVED (set-aside is layered).** Items are set aside at the level where the decision is
  made: **verses at L1**; **BOUNDARY terms at the sub-group stage (L2)**; **terms rerouted to other clusters
  at the VCG stage (L3)**.
- **E1 [L2] — RESOLVED.** Yes — a fresh cluster's L2 forms provisional sub-groups from the L1
  similarity/keyword groups (~80% no issue); **if CC struggles, hand the data to AI in chat** to help.
- **E2 [all] — RESOLVED.** The **V3_2 instruction can re-run any cluster and respects what is already done**;
  the reason for a re-run will be clear and **testable against the instruction before executing**.
- **E3 [L3/L5] — RESOLVED.** **Anchor verses per VCG are retained and important** — they are the **only
  reference that survives into the findings** (the citation seed).
- **E5 [L2] — RESOLVED.** **BOUNDARY = undecided terms.** They **first surface when the sub-groups are
  formed (L2)** and **must be completely resolved by the time all sub-groups are processed**; the
  **end-of-sub-group-stage audit must reveal any orphaned terms** in the cluster.

### Still open
- **E4 [L1] — PARTIAL.** `is_related` is **still relevant and must be maintained**; **UT** verses are
  history and by definition drawn into L1 (**retained**). **`vertical_pass_flag` → RETIRE** (legacy flag from
  the superseded "vertical pass" experiment, VCB-031 2026-04-08; researcher does not identify with it) —
  confirm retirement under V3_2.
- **E6 [L4] — RESOLVED.** The VCG-emergent observation lives at a **new VCG-group level** (keyed to
  `verse_context_group`), **not** at cluster level (`cluster_observation`). *(Schema: a VCG-group-level
  observation — V3_2 schema pass.)*
- **E7 [L3] — RESOLVED.** **Every open question must be a finding.** Open questions are recorded **as
  findings**, with a **new `needs_research` column** on `cluster_finding` marking findings that need further
  research. *(Schema change.)*
- **E8 [L1] — RESOLVED.** Similarity is **rolled up into the STEP process** (the STEP sense + STEP-capture
  keywords) — it is *evident* there; **no separate similarity field**.
- **E9 [L1/L3] — OPEN (needs info).** `analysis_note` (AI / L3 rich) vs a new terse `step_meaning_applied`
...
### F. Adjacent layers
- **F1 [L8] — RESOLVED.** The **science layer enters at the cross-cluster finding stage (L8)** — science is
  introduced there and **findings are developed from it** (`feedback_phase9_science_extract_required`).
- **F2 [after L8a] — RESOLVED.** The **publication input-folder generation / readiness is after L8a** (once a
  cluster's outbound cross-cluster findings are generated). Findings render downstream into essays/guides
  (Session C cluster publication).

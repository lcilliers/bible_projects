...
**Version 5_10 | 20260418 | Reference guide — not an operational instruction**

| **Document** | **Value** |
| --- | --- |
| Filename | wa-registry-management-guide-v5_10-20260418.md |
| Supersedes | wa-registry-management-guide-v5_9-20260414.md |
...
| Not covered | Transactional operations — see wa-versecontext-instruction [current], wa-sessionb-analysis-readiness [current], and wa-sessionb-analysis-output [current] |
...
- Registry field definitions and meanings (`word_registry` table fields)
- Registry status lifecycle (`session_b_status`, `verse_context_status`)
- OWNER / XREF term distinction and pure XREF registry pattern
- Cluster assignments and cluster processing sequence
- Registry-level audit integrity rules and verse-accounting queries
- Terminology reference for registry concepts

This document is NOT authoritative for (pointers, not copies):
- Schema field definitions and controlled vocabulary → `wa-reference [current]`
- File naming rules (full catalogue) → GR-FILE-001 through GR-FILE-009; `wa-reference [current] §1`
- Patch format → `wa-patch-instruction [current]`
- Directive format → `wa-directive-instruction [current]`
- CC operational routines → `wa-claudecode-instruction [current]`
- Verse Context operational procedure → `wa-versecontext-instruction [current]`
- Session B analytical procedure → `wa-sessionb-analysis-readiness [current]` and `wa-sessionb-analysis-output [current]`
- Dimension Review procedure → `wa-dimensionreview-instruction [current]`
- Programme-wide binding rules → `wa-global-general-rules [current]`
- Open issues and flags → `wa-global-flags [current]`

Per GR-REF-001 Discipline 1 (pointer, not copy): content owned by those documents is not re-stated here.

---

## Change Control — v5_10

| **Change** | **Detail** |
| --- | --- |
| Reference alignment per GR-REF-002 | Operational cross-references migrated to `[current]` token: header "Not covered" line; §Governing Rules block; §2 `wa-reference` pointer; §5.2 processing pointer; §8 `wa-reference` pointer. Provenance references retained with specific version. |
| Stale / retired cross-references corrected | `WA-VerseContext-Instruction-v2.6-20260414` → `wa-versecontext-instruction [current]`. `WA-SessionB-Instruction-v4.7` and `v4.8` references retired (per FLAG-003) and redirected to the two successor documents: `wa-sessionb-analysis-readiness [current]` and `wa-sessionb-analysis-output [current]`. |
| Dangling rule ID removed | `GR-DATA-008` citations removed at §Governing Rules body and at §Change Control v5_9 row. Rule does not exist in `wa-global-general-rules-v2_11-20260418.json` rules array or any addendum. The substantive guidance (engine-derived Phase 1 fields are stale — use live queries) is retained inline; the guide is the authoritative source for field-level meaning. |
| Document Scope section added per GR-REF-001 Discipline 5 | New "Document scope" block after header, listing what this guide is authoritative for and what it points to. |
| Missing rule citations added | GR-DATA-001 (active terms filter) cited at §6a.3 and §6c.1 where SQL queries use the filter. GR-FILE-001 through GR-FILE-009 pointer added at §9 File Naming Reference intro with pointer to `wa-reference [current] §1` for full catalogue. |
| Filename format | Filename migrated to underscore version format per GR-FILE-003 v3_0. |
| Governing rules pointer | "Governed by" row added to document header table. |

**Change Control — v5.9**

| **Change** | **Detail** |
| --- | --- |
| Header | Governing Rules block added — mandatory load of global rules. Filename updated to wa- prefix and compact date format per GR-FILE-009. Not covered references updated to v2.6 and v4.8. |
| Section 2 | Engine-derived Phase 1 fields note added. |

**Change Control — v5.8**

| **Change** | **Detail** |
| --- | --- |
| Section 5.2 | Pool-based processing sequence retired. Replaced with individual-word, cluster-order processing model reflecting actual practice since April 2026. |
| Section 7 | Pool ID Controlled Vocabulary retired. Section now documents retired status and directs to Section 5.2. |
| Header | Not covered field: DataPrep-Instruction removed (retired); SessionB-Instruction-v4.7 added. VerseContext updated to v2.5. |
| Sections 3.3, 6.2, 6.8 | DataPrep gate references updated: verse_context_status = Complete is the Session B gate. Ready for Analysis status noted as legacy/unused. |

---

## Governing Rules

This guide is governed by **wa-global-general-rules [current]**.

Claude AI must confirm the global rules file has been loaded before using this guide in any session. Rules stated in the global file are not repeated here; rule IDs are cited where relevant.

Note on engine-derived fields: `phase1_term_count`, `phase1_verse_count`, `unique_term_count`, `shared_term_count`, and `term_sharing_ratio` reflect Phase 1 state only and are not updated by later pipeline stages. Use live queries against `wa_term_inventory` for current counts.
...
| dimensions | Multi-value semantic category field (formerly source_category, renamed M17). Comma-delimited. Populated at Session B extraction. Valid values in `wa-reference [current] §4.3`. |
...
| **Field** | **Definition and source query** |
| --- | --- |
...
> ⚠ **`Ready for Analysis` is only reachable when `verse_context_status = Complete`. Session B must not begin unless both conditions are met.**
...
### 6.2 Words Ready for Session B
```sql
SELECT no, word, cluster_assignment
FROM word_registry
WHERE verse_context_status = 'Complete'
  AND (session_b_status IS NULL OR session_b_status NOT IN ('Analysis Complete', 'Session B Complete'))
...
-- Registries where Verse Context is complete and Session B can begin
SELECT no, word, cluster_assignment
FROM word_registry
WHERE verse_context_status = 'Complete'
...
### 6.9 Cluster-Order Processing Readiness (Stage 2)
```sql
...
Run after every registry merge, restructuring event, or any operation that re-links terms between registries. The active-terms filter (`AND mt.status IN ('extracted', 'extracted_thin')`) is mandatory per **GR-DATA-001**.
...
The `live_verse_count` field in the registry overview export counts **all** active verses in a registry's files regardless of `mti_terms.status`. This is deliberately broad — it reflects total data presence. For audit and programme state purposes, two narrower queries are more useful. Both use the active-terms filter required by **GR-DATA-001**.

### 6c.1 VCB-scope verse count (per registry)

Counts only verses whose terms are in scope for VCB classification — `extracted` or `extracted_thin` status only (per **GR-DATA-001**). This is the verse set that VCB is expected to have classified.
...
- Stage 2 Session B progress by cluster
...
## 7a. Pool ID Controlled Vocabulary — Retired

**The pool ID controlled vocabulary and pool analysis dataset architecture are retired as of April 2026.**

Pool IDs, pool analysis dataset files (`wa-pool-{pool_id}-analysis-{date}.json`), and pool-based Session B sessions are no longer used. Session B is performed on individual registries — see Section 5.2 for the current processing model.

The semantic groupings from the March 2026 programme analysis remain useful as reference data for understanding cross-registry relationships. This information is accessible in each registry's complete extract through `correlations.xref_sharing`.
...
| Anchor verse | The programme's canonical reference verse for a specific contextual meaning group of a term. Serves as Session B Analysis input and programme citation. Every term must have at least one. See `wa-reference [current] §16`. |
...
| Pool | A set of registries connected through shared XREF terms at or above a defined threshold. The 8 pools identified in the March 2026 programme analysis define the Stage 2 processing sequence. Not a database table — a planning tool. Retired as an active architecture (see §7a). |
| Pool/cluster batch | A set of words analysed simultaneously in Session B because they share XREF terms. The shared terms' verse context is already classified — cross-word relationships are visible in a single analysis session. Retired (see §7a). |
| Pool analysis dataset | The JSON input for Session B Analysis. Contains all words in the pool batch, their OWNER term anchor verses grouped by contextual meaning, and XREF term profiles from OWNER registries. File naming: wa-pool-{pool_id}-analysis-{date}.json. Retired (see §7a). |
| Session B | The verse-grounded word analysis phase. Operates on individual registries in cluster order — see §5.2. |
...
| Verse Context Reset | A `session_b_status` value indicating prior Session B work exists but is superseded. Registry must reprocess through Verse Context and cluster-order Session B. |
| Cluster | Organisational grouping for Session B processing. 22 clusters, C01–C22. Not an analytical classification. |
| Patch | JSON file of database operations submitted to Claude Code. See `wa-patch-instruction [current]` for format and workflow. |
...
File naming is governed by **GR-FILE-001 through GR-FILE-009** in the global rules. See `wa-reference [current] §1` for the full pattern catalogue across file types. The table below is a quick reference for registry-facing file types.
...
| pool-analysis | Pool analysis dataset — wa-pool-{pool_id}-analysis-{date}.json (retired — see §7a) |

---

*wa-registry-management-guide-v5_9 | 20260414 | Governing Rules block added; engine-derived Phase 1 fields note cross-referenced; filename prefix and date format normalised*

*WA-Registry-Management-Guide-v5.8 | 20260412 | Supersedes v5.7-20260406 | Sections 5.2, 7 updated: pool architecture retired; DataPrep references removed*

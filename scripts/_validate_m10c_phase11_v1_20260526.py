"""M10c Phase 11 validation — covers the §15.2 Phase 12 pre-flight checks too.

Outputs a markdown report at
  Sessions/Session_Clusters/M10c/wa-cluster-M10c-phase11-validation-v1-20260522.md

Per v2_6 §14 + §15.2:
  1. Row counts (945 expected: 4 chars × 189 + 189 synthesis)
...
CLUSTER = 'M10c'
OUT = Path(f'Sessions/Session_Clusters/M10c/wa-cluster-M10c-phase11-validation-v1-20260526.md')

VERSE_REF_RE = re.compile(
    r'\b(?:[1-3] ?)?[A-Z][a-z]{1,3}\.?\s?\d+[:.]\d+(?:-\d+)?\b'
)
VCG_REF_RE = re.compile(r'\bVCG-\d+', re.IGNORECASE)
SUBGROUP_REF_RE = re.compile(r'\bM10c-[A-Z]\b')
...
CHAR_REF_RE = re.compile(r'\b(?:Char|Characteristic)\s+[1-4]\b')
...
# C1: VC-coverage gaps - every wa_verse_term_link for an M10c OWNER copy has a matching verse_context.
...
# R4 anchor check - every OWNER M10c term has at least one is_relevant verse (proxy for anchor)
...
# BOUNDARY_DECISION_PENDING — narrow to M10c's own Strong's set (registry-level
# query picks up false-positive flags from other clusters' closures on shared
# registries, e.g. M03 flags on registry 151 sorrow that don't belong to M10c)
m10b_strongs = [r[0] for r in cur.execute(
...
ph = ','.join('?' * len(reg_ids))
ph_strongs = ",".join("?" * len(m10b_strongs))
boundary_pending = cur.execute(
    f"SELECT COUNT(*) FROM wa_session_research_flags "
    f"WHERE strongs_reference IN ({ph_strongs}) "
    f"AND flag_code='BOUNDARY_DECISION_PENDING' "
...
lines.append(f"# M10c Phase 11 — Validation report")
lines.append(f"")
...
lines.append("")
lines.append(f"Rows with `cluster_subgroup_id` or `vcg_scope` populated under M10c: **{legacy_marker_rows}**")
lines.append("")
...
lines.append(check_line('BOUNDARY_DECISION_PENDING flags (unresolved on M10c Strong\'s)', boundary_pending,
                       note='(M10c had 0 BOUNDARY verdicts at Phase 3; shared-registry flags from M03 closure are not M10c\'s responsibility — see Phase 8.5 obslog)'))
lines.append(check_line(f'{CLUSTER}-BOUNDARY sub-group active verses (is_relevant=1)', boundary_active_verses))
lines.append("")
lines.append("---")
lines.append("")

# Section 8 — Phase 10 fold operation (no-op for M10c)
lines.append("## 8. Phase 10 fold operation status")
lines.append("")
lines.append("M10c is a fresh post-split cluster (created 2026-05-22 from the M10 split); no Phase 10 reconciliation was run because no inherited Session B findings (`wa_session_b_findings`) target M10c's terms. The fold operation under §14.5 has no inherited findings to fold.")
lines.append("")
...
if not fails:
    lines.append("**M10c is ready for Phase 12 closure.**")
else:
    lines.append("**M10c NOT yet ready for Phase 12.** Address the failed checks first.")
lines.append("")

"""M07 Phase 11 validation — covers the §15.2 Phase 12 pre-flight checks too.

Outputs a markdown report at
  Sessions/Session_Clusters/M07/WA-M07-phase11-validation-v1-20260519.md
...
CLUSTER = 'M07'
OUT = Path(f'Sessions/Session_Clusters/M07/WA-M07-phase11-validation-v1-20260520.md')

VERSE_REF_RE = re.compile(
    r'\b(?:[1-3] ?)?[A-Z][a-z]{1,3}\.?\s?\d+[:.]\d+(?:-\d+)?\b'
)
VCG_REF_RE = re.compile(r'\bVCG-\d+', re.IGNORECASE)
SUBGROUP_REF_RE = re.compile(r'\bM07-[A-Z]\b')
...
# C1: VC-coverage gaps - every wa_verse_term_link for an M07 OWNER copy has a matching verse_context.
...
# R4 anchor check - every OWNER M07 term has at least one is_relevant verse (proxy for anchor)
...
lines.append(f"# M07 Phase 11 — Validation report")
lines.append(f"")
...
lines.append("")
lines.append(f"Rows with `cluster_subgroup_id` or `vcg_scope` populated under M07: **{legacy_marker_rows}**")
lines.append("")
...
lines.append(check_line('BOUNDARY_DECISION_PENDING flags (unresolved)', boundary_pending,
                       note='(Note: any unresolved flags on M07-contributing registries are M01/M03 residue or out-of-scope per Phase 10 closure record)'))
lines.append(check_line(f'{CLUSTER}-BOUNDARY sub-group active verses (is_relevant=1)', boundary_active_verses))
lines.append("")
lines.append("---")
lines.append("")

# Section 8 — Phase 10 fold operation (no-op for M07)
lines.append("## 8. Phase 10 fold operation status")
lines.append("")
lines.append("Per Phase 10 closure record (`WA-M07-phase10-closure-record-v1-20260520.md`), zero `RESOLVED-BY-CATALOGUE` dispositions were assigned. The fold operation under §14.5 has no inherited findings to fold.")
lines.append("")
...
if not fails:
    lines.append("**M07 is ready for Phase 12 closure.**")
else:
    lines.append("**M07 NOT yet ready for Phase 12.** Address the failed checks first.")
lines.append("")

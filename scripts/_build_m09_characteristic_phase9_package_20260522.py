"""Build a per-characteristic Phase 9 AI package for M09.
...
Usage:
    python scripts/_build_m04_characteristic_phase9_package_20260522.py --char-seq 1
    python scripts/_build_m04_characteristic_phase9_package_20260522.py --char-seq 2
    ...

Outputs (per characteristic):
    Sessions/Session_Clusters/M09/wa-cluster-M09-phase9-char{N}-{short_name}-brief-v1-{date}.md
    Sessions/Session_Clusters/M09/wa-cluster-M09-phase9-char{N}-{short_name}-input-v1-{date}.md
"""
...
def main():
    ...
    ap.add_argument("--char-seq", type=int, required=True, help="Characteristic sequence number (1-6 for M09)")
    args = ap.parse_args()
    ...
    char = conn.execute(
        "SELECT * FROM characteristic WHERE cluster_code='M09' AND char_seq=? AND COALESCE(delete_flagged,0)=0",
        (args.char_seq,),
    ).fetchone()
    if not char:
        raise SystemExit(f"No M09 characteristic with seq={args.char_seq}")
    ...
    cluster = conn.execute(
        "SELECT description, gloss FROM cluster WHERE cluster_code='M09'"
    ).fetchone()

    # ===== File paths =====
    BRIEF = Path(f"Sessions/Session_Clusters/M09/wa-cluster-M09-phase9-char{args.char_seq}-{safe_short}-brief-v1-{TODAY}.md")
    INPUT = Path(f"Sessions/Session_Clusters/M09/wa-cluster-M09-phase9-char{args.char_seq}-{safe_short}-input-v1-{TODAY}.md")

    total_verses = sum(sg["verse_count"] or 0 for sg in subgroups)

    # ===== Brief =====
    B = []
    B.append(f"# M09 Phase 9 — Characteristic {args.char_seq} ({short}) — brief")
    B.append("")
    ...
    B.append(f"| 2 | **Structural input** — `Sessions/Session_Clusters/M09/{INPUT.name}` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts + carry-forward observations |")
    ...
    B.append("## Context")
    B.append("")
    B.append(f"M09 has 6 inner-being characteristics. You are processing **Characteristic {args.char_seq} ({short})**. Per researcher direction 2026-05-18 (carried forward to M09 under v2_8 §8.0):")
    B.append("")
    ...
    B.append("| 3 | **Citation discipline** | Every E finding cites at least one verse-reference or VCG code (e.g. `Pro 16:5`, `M09-A1-VCG-01`) | ALERT — name the uncited E prompts; do NOT proceed |")
    ...
    B.append("5. **No sub-group-scope findings.** All findings at characteristic scope. Where evidence differs by sub-group within the characteristic (e.g. CHAR-1 spans M09-A1/A2/A3/A4), the finding text names the sub-group(s) inline.")
    ...
    B.append("")
    B.append(f"Target filename: `Sessions/Session_Clusters/M09/wa-cluster-M09-phase9-char{args.char_seq}-{safe_short}-findings-v1-{TODAY}.md`")
    B.append("")
    ...
    B.append("")
    B.append("```markdown")
    B.append(f"# M09 Phase 9 — Characteristic {args.char_seq} ({short}) — findings")
    B.append("")
    ...
    B.append(f"2. Ping CC: \"M09 Char {args.char_seq} ({short}) Phase 9 findings ready\"")
    ...
    # ===== Structural input =====
    S = []
    S.append(f"# M09 Phase 9 — Characteristic {args.char_seq} ({short}) — structural input")
    S.append("")
    ...
    S.append(f"| 2 | **This document** — `Sessions/Session_Clusters/M09/{INPUT.name}` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts |")

"""M09 Phase 7 — apply VCG design.

Per v2_8 §10.5. Per directive WA-M09-dir-003-vcg-create-v1-20260520.md.
...
- Op A: INSERT verse_context_group rows (25 VCGs).
       group_code = provisional code from JSON (e.g. M09-A1-VCG-01).
...
- Op D: Clear prior M09 is_anchor=1, then set is_anchor=1 on the 25 new anchors.
"""
...
JSON_PATH = REPO / 'Sessions' / 'Session_Clusters' / 'M09' / 'wa-cluster-M09-vcg-creation-v1-20260522.json'
NOW = datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')
CLUSTER = 'M09'

data = json.loads(JSON_PATH.read_text(encoding='utf-8'))

# M09 has no duplicate orphans (M07-style); no Op 0
DUPLICATES_TO_SOFTDELETE: list[int] = []
# higher vc_id of each pair
...
sg_order = ['M09-A','M09-B','M09-C','M09-D','M09-E','M09-F','M09-G','M09-H']
specs = []
...
""", (CLUSTER,)).fetchone()[0]
print(f'  Prior is_anchor=1 rows on M09 verses: {prior_anchors} (will be cleared by Op D)')

# Op 0 pre-check: skipped (M09 has no duplicate rum vc rows; M07-style cleanup not needed)
print(f'  Op 0 pre-check: skipped — M09 has no duplicate vc rows requiring soft-delete')
...
try:
    ...
    for spec in specs:
        ...
        for vc_id in spec['verses']:
            ...
            if mti_id is None:
                raise RuntimeError(f'vc_id={vc_id} not in M09 is_relevant set (unexpected)')
    ...
    expected_updates = 109 - len(softdelete_set)
...
print(f'  New M09 VCGs in DB: {n_new_vcgs}')

expected_routed = 109 - len(softdelete_set)
n_routed = cur.execute("""
    ...
    WHERE mt.cluster_code=? AND vc.is_relevant=1
      ...
      AND vcg.group_code LIKE 'M09-%-VCG-%'
""", (CLUSTER,)).fetchone()[0]
print(f'  is_relevant verses routed to new M09 VCGs: {n_routed} (expected {expected_routed})')
...
n_a = cur.execute("""
    ...
    WHERE mt.cluster_code=? AND vc.is_anchor=1 AND COALESCE(vc.delete_flagged,0)=0
""", (CLUSTER,)).fetchone()[0]
print(f'  is_anchor=1 on M09 verses: {n_a} (expected {len(specs)})')
...
rows = cur.execute("""
    ...
    WHERE vcg.group_code LIKE 'M09-%-VCG-%' AND COALESCE(vcg.delete_flagged,0)=0

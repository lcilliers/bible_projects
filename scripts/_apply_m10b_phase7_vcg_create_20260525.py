"""M10b Phase 7 — apply VCG design.

Per v2_8 §10.5. Per directive wa-cluster-M10b-dir-002-phase7-vcg-create-v1-20260525.md.

Source: Sessions/Session_Clusters/M10b/files phase 7/wa-cluster-M10b-vcg-creation-v1-20260525.json
        (validated; 6 sub-groups, 36 VCGs, 515 verses)

Operations:
- Op A: INSERT 36 verse_context_group rows (one per VCG). group_code = JSON's
...
- Op C: UPDATE verse_context.group_id for every is_relevant verse in the cluster
        to point at its new VCG (515 updates).
- Op D: Clear any prior is_anchor=1 on M10b verses, then set is_anchor=1 on the
        36 new anchor verses.

Modelled on scripts/_apply_m10_phase7_vcg_create_20260523.py.
"""
...
JSON_PATH = REPO / "Sessions" / "Session_Clusters" / "M10b" / "files phase 7" / "wa-cluster-M10b-vcg-creation-v1-20260525.json"
...
def main() -> int:
    ...
    print(f"=== M10b Phase 7 apply — mode={'LIVE' if not dry else 'DRY-RUN'} ===")

    if not dry:
        ts = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
        backup = REPO / "backups" / f"bible_research_backup_{ts}_M10b-phase7-vcg-create.db"
    ...
    # Flatten VCG specs in sub-group order
    specs: list[dict] = []
    ...
    """, (CLUSTER,)).fetchone()[0]
    print(f"  Prior is_anchor=1 rows on M10b verses: {prior_anchors} (will be cleared by Op D)")
    ...
    assert not bad, f"{len(bad)} JSON vc_ids not in M10b is_relevant set (first 5: {bad[:5]})"
    print(f"  All {len(all_json_vc)} JSON vc_ids are M10b is_relevant rows")
    ...
    n_new_vcgs = cur.execute(
        ...
        "WHERE group_code LIKE 'M10b-%-VCG-%' AND COALESCE(delete_flagged,0)=0"
    ).fetchone()[0]
    print(f"  New M10b VCGs in DB: {n_new_vcgs}")
    ...
    """, (CLUSTER,)).fetchone()[0]
    print(f"  is_relevant verses routed to new M10b VCGs: {n_routed} (expected {EXPECTED_VERSES})")
    ...
    n_a = cur.execute("""
        ...
        WHERE mt.cluster_code=? AND vc.is_anchor=1 AND COALESCE(vc.delete_flagged,0)=0
    """, (CLUSTER,)).fetchone()[0]
    print(f"  is_anchor=1 on M10b verses: {n_a} (expected {len(specs)})")

    # R4 anchor gate per term
    print()
    ...
    rows = cur.execute("""
        ...
        WHERE vcg.group_code LIKE 'M10b-%-VCG-%' AND COALESCE(vcg.delete_flagged,0)=0

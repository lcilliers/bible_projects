"""Build the M10c Phase 9 cluster-synthesis (7th) session package.
...
Outputs:
  Sessions/Session_Clusters/M10c/wa-cluster-M10c-phase9-cluster-synthesis-brief-v1-{date}.md
  Sessions/Session_Clusters/M10c/wa-cluster-M10c-phase9-cluster-synthesis-input-v1-{date}.md
"""
...
BRIEF = Path(f"Sessions/Session_Clusters/M10c/wa-cluster-M10c-phase9-cluster-synthesis-brief-v1-{TODAY}.md")
INPUT = Path(f"Sessions/Session_Clusters/M10c/wa-cluster-M10c-phase9-cluster-synthesis-input-v1-{TODAY}.md")
...
def main():
    ...
    # Sanity: every q_code should have one entry per M10c characteristic
    EXPECTED_CHAR_COUNT = len(chars)
    ...
    # =====
    B = []
    B.append(f"# {CLUSTER} Phase 9 — Cluster Synthesis ({len(chars) + 1}th session) — brief")
    B.append("")
    B.append(f"**Cluster:** {CLUSTER} — {cluster['description']}")
    B.append(f"**Session:** {len(chars) + 1} (cluster-scope synthesis — final Phase 9 session)")
    ...
    B.append(f"| 2 | **Structural input** — `Sessions/Session_Clusters/M10c/{INPUT.name}` | Per-prompt synthesis matrix: for each of the 189 prompts, all {len(chars)} characteristics' findings stacked side-by-side (verbatim from `cluster_finding`); plus open observations |")
    ...
    B.append(f"| 4 | **Science extract** — `{SCIENCE}` (Section 4 only) | Programme-curated scientific lens for T7.3. **Note:** the doc was authored pre-split and covers M10/M10b/M10c siblings; for M10c only Section 4 (Defilement, impurity, contamination) is in scope. Sections 1 (Guilt) and 2 (Transgression) belong to M10; Section 3 (Evil/Wickedness) belongs to M10b. |")
    ...
    B.append("")
    B.append(f"After the 189-row block, write a free-form prose appendix capturing the emergent themes that surface across the {len(chars)} characteristics — themes that the 189-prompt structure does not fully capture. Suggested themes (drawn from M10c's Phase 3 verdicts, Phase 7 polysemy notes, and characteristic-mapping carry-forwards):")
    B.append("")
    B.append("- **The wicked-righteous structural opposite (M26).** Most M10c characteristics define themselves by explicit contrast with the righteous — across Job, Psalms, Proverbs, Sermon on the Mount, Romans. How does the M10c cluster collectively position M26 (Righteousness) as its structural foil? The T6 cross-characteristic findings will have surfaced this; consolidate.")
    ...
    B.append("- **M10c ↔ M27 boundary (idolatry / cosmic-evil / concrete-evil-thing).** M27 is a sibling cluster carrying idolatrous-object + ruin/desolation + cosmic-evil-agent register. M10c carries the *moral-character* side of the same triple-noun. CHAR-4 (idolatrous abomination), CHAR-2's cosmic-evil sub-corpus (M10c-B-VCG-01, INTEGRATION_NOTE #81), and CHAR-1's forensic-verdict sub-corpus (M10c-A-VCG-01, INTEGRATION_NOTE #80) all sit on this seam. Synthesise the cross-cluster pattern.")
    ...
    B.append("")
    B.append(f"1. Save the output as `Sessions/Session_Clusters/M10c/wa-cluster-M10c-phase9-cluster-synthesis-findings-v1-{TODAY}.md`.")

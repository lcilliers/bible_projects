"""Build a per-characteristic Phase 9 AI package for M10b.
...
Usage:
    python scripts/_build_m10b_characteristic_phase9_package_20260525.py --char-seq 1
    python scripts/_build_m10_characteristic_phase9_package_20260523.py --char-seq 2
    ...

Outputs (per characteristic):
    Sessions/Session_Clusters/M10b/wa-cluster-M10b-phase9-char{N}-{short_name}-brief-v1-{date}.md
    Sessions/Session_Clusters/M10b/wa-cluster-M10b-phase9-char{N}-{short_name}-input-v1-{date}.md
"""
...
def main():
    ...
    ap.add_argument("--char-seq", type=int, required=True, help="Characteristic sequence number (1-6 for M10b)")
    args = ap.parse_args()
    ...
    char = conn.execute(
        "SELECT * FROM characteristic WHERE cluster_code='M10b' AND char_seq=? AND COALESCE(delete_flagged,0)=0",
        (args.char_seq,),
    ).fetchone()
    if not char:
        raise SystemExit(f"No M10b characteristic with seq={args.char_seq}")
    ...
    cluster = conn.execute(
        "SELECT description, gloss FROM cluster WHERE cluster_code='M10b'"
    ).fetchone()

    # ===== File paths =====
    BRIEF = Path(f"Sessions/Session_Clusters/M10b/wa-cluster-M10b-phase9-char{args.char_seq}-{safe_short}-brief-v1-{TODAY}.md")
    INPUT = Path(f"Sessions/Session_Clusters/M10b/wa-cluster-M10b-phase9-char{args.char_seq}-{safe_short}-input-v1-{TODAY}.md")

    total_verses = sum(sg["verse_count"] or 0 for sg in subgroups)

    # ===== Brief =====
    B = []
    B.append(f"# M10b Phase 9 — Characteristic {args.char_seq} ({short}) — brief")
    B.append("")
    B.append(f"**Cluster:** M10b — {cluster['description']}")
    ...
    B.append(f"| 2 | **Structural input** — `Sessions/Session_Clusters/M10b/{INPUT.name}` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts + carry-forward observations |")
    ...
    B.append("| 4 | **Science extract** — `Workflow/Sciences/wa-m10-guilt-scienceextract-v1_0-20260513.md` (Section 3 only) | Programme-curated scientific lens for T7.3. **Note:** the doc was authored pre-split and covers M10/M10b/M10c siblings; for M10b only Section 3 (Evil/Wickedness) is in scope. Sections 1–2 (Guilt + Transgression) belong to M10. |")
    ...
    B.append("## Context")
    B.append("")
    B.append(f"M10b has 6 inner-being characteristics. You are processing **Characteristic {args.char_seq} ({short})**. Per researcher direction 2026-05-18 (carried forward to M10b under v2_8 §8.0):")
    B.append("")
    ...
    B.append("| 3 | **Citation discipline** | Every E finding cites at least one verse-reference or VCG code (e.g. `Pro 16:5`, `M10b-A-VCG-01`) | ALERT — name the uncited E prompts; do NOT proceed |")
    ...
    B.append("")
    B.append(f"Target filename: `Sessions/Session_Clusters/M10b/wa-cluster-M10b-phase9-char{args.char_seq}-{safe_short}-findings-v1-{TODAY}.md`")
    B.append("")
    ...
    B.append("")
    B.append("```markdown")
    B.append(f"# M10b Phase 9 — Characteristic {args.char_seq} ({short}) — findings")
    B.append("")
    ...
    B.append(f"2. Ping CC: \"M10b Char {args.char_seq} ({short}) Phase 9 findings ready\"")
    ...
    # ===== Structural input =====
    S = []
    S.append(f"# M10b Phase 9 — Characteristic {args.char_seq} ({short}) — structural input")
    S.append("")
    ...
    S.append("|---|---|---|")
    S.append(f"| 1 | **Brief (read first)** — `Sessions/Session_Clusters/M10b/{BRIEF.name}` | Primary task instructions |")
    S.append(f"| 2 | **This document** — `Sessions/Session_Clusters/M10b/{INPUT.name}` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts |")

"""_apply_m15_dir011_m15b_vcg_v1_20260511.py — DB-modifying.

Apply DIR-20260511-M15-011: M15-B VCG cleanup.

Operations (single transaction, foreign_keys=ON):

  Phase 0 — Backup
  Phase 1 — Create 7 new VCGs (M15-B-VCG01..07) + vcg_term links
  Phase 2 — Assign 307 verses to new VCGs + mark 7 anchors
  Phase 3 — Apply set-aside to 7 verses (derived by exclusion;
             reason 'no_inner_being_phenomenon')
  (No reroutes for M15-B.)

Source: Sessions/Session_Clusters/M15/m15-vcg-db-staging-M15B-v1-20260511.json
"""
...
DIRECTIVE_ID = "DIR-20260511-M15-011"
...
def main() -> int:
    ...
    print("=" * 72)
    print(f"{DIRECTIVE_ID} apply (M15-B VCG cleanup)")
    ...
    M15B_ID = sg_ids.get("M15-B")
    if not M15B_ID:
        print(f"  [ERR] missing M15-B sub-group")
        ok = False
    else:
        print(f"  [ok] M15-B sub-group id={M15B_ID}")
    ...
    for v in setaside_block["verses"]:
        all_targets.append((v["vr_id"], v["mti_term_id"]))
    print(f"  [info] total targets: {len(all_targets)} "
    ...
    for b in vcg_blocks:
        ...
        if not any(v["vr_id"] == a["vr_id"] for v in b["verses"]):
            # Anchor might be listed separately; check by walking the input
            # file context. Allow if anchor exists in any block's source.
            print(f"  [WARN] VCG {b['code']} anchor vr_id {a['vr_id']} "
                  f"not in its verses list — will be UPDATEd by vr_id regardless")
    ...
    if args.live:
        b = take_backup("m15_dir011_m15b_vcg")
        print(f"Backup: {b}")
        print()
    ...
    try:
        ...
        for block in vcg_blocks:
            ...
            for mti_id in block["mti_term_ids"]:
                cur.execute(
                    ...
                    "VALUES (?, ?, ?, 0, ?, ?)",
                    (rowid, mti_id,
                     f"{DIRECTIVE_ID}: M15-B VCG cleanup", ts, ts),
                )
                vcg_term_inserts += 1
        counts["phase1_vcgs_created"] = len(new_vcg_id_by_code)
        counts["phase1_vcg_term_inserts"] = vcg_term_inserts
        print(f"  Created {len(new_vcg_id_by_code)} VCGs, {vcg_term_inserts} vcg_term rows")
        ...
        for block in vcg_blocks:
            ...
            # Include the anchor in the verses if it's not in the verses list
            ...
            for vr_id in verse_vr_ids:
                ...
                rc = cur.execute(
                    ...
                    (vcg_id, M15B_ID, is_anchor, vr_id),
                ).rowcount
                ph2 += rc
        ...
        # Phase 3 — Set-asides
        # Also update cluster_subgroup_id so all 7 set-asides land in M15-B.
        # 2 of the 7 are currently in other sub-groups (M15-C, BOUNDARY) in
        # DB but the by-subgroup view (the source of the user's M15-B
        # input) treats them as M15-B verses — those 2 are moved as part
        # of being set aside.
        print()
        print("Phase 3 — Apply set-aside to derived verses (and move to M15-B)")
        print("-" * 72)
        ph3 = 0
        ph3_moved_from_other_sg = 0
        for v in setaside_block["verses"]:
            # Check pre-state to count moves
            pre = cur.execute(
            ...
            if pre and pre[0] != M15B_ID:
                ph3_moved_from_other_sg += 1
            rc = cur.execute(
                "UPDATE verse_context "
                "   SET group_id = NULL, "
                "       cluster_subgroup_id = ?, "
                ...
                " WHERE verse_record_id = ? "
                "   AND COALESCE(delete_flagged,0) = 0",
                (M15B_ID, setaside_block["reason_code"], v["vr_id"]),
            ).rowcount
            ph3 += rc
        counts["phase3_setasides"] = ph3
        counts["phase3_moved_from_other_sg"] = ph3_moved_from_other_sg
        print(f"  Set aside {ph3} verse_context rows "
              f"({ph3_moved_from_other_sg} moved into M15-B from other sg)")
        ...
        """, (M15B_ID, setaside_block["reason_code"])).fetchone()[0]
        print(f"  M15-B set-asides (post): {n_sa}")

"""Apply M02 Phase 8 — soft-delete 68 inherited VCGs + 68 vcg_term rows.

Directive: wa-cluster-M02-dir-004-vcg-dissolve-v1-20260516
...
Pre-conditions (verified by precheck):
  ...
  - 0 active is_relevant vc rows still pointing to inherited M02 VCGs
  - 68 inherited M02 VCGs (id < 3762, vcg_term-linked to M02 terms, active)

Operations (single transaction):
  Op A — Soft-delete 68 vcg_term rows
  Op B — Soft-delete 68 verse_context_group rows + append directive audit note

Trust basis (per researcher direction, see Workflow/methodology/WA-blind-verification-methodology-v1-20260516.md):
The M01 blind verification (3 tests, 272 findings) validated AI's Phase 7 internal
coherence. M02 used identical methodology, so the dissolution comparison report is
skipped per researcher direction. Structural integrity (R4, H2, H3) was verified at
Phase 7 apply.
"""
...
def precheck(conn) -> list[int]:
    ...
    log(f"  Inherited M02 VCGs to dissolve: {len(inherited)}")

    ids_str = ",".join(str(i) for i in inherited)
    r = conn.execute(f"""
        SELECT COUNT(*) AS n FROM verse_context
        WHERE COALESCE(delete_flagged,0)=0 AND is_relevant=1 AND group_id IN ({ids_str})
    """).fetchone()
    if r["n"] != 0:
        raise RuntimeError(f"{r['n']} active is_relevant vc rows still point to inherited VCGs")
...
def apply(conn, inherited: list[int], dry_run: bool) -> dict:
    ids_str = ",".join(str(i) for i in inherited)
    audit = f" | {DIRECTIVE} dissolved (superseded by DIR-20260516-010 new VCGs)"
...
def healthcheck(conn, inherited: list[int]) -> None:
    ...
    log(f"  D1 active vcg_term pointing to dissolved: {r['n']} ({'✓' if r['n']==0 else '✗'})")
    assert r["n"] == 0
    ...
    log(f"  D2 active inherited M02 VCGs (id < {NEW_VCG_MIN_ID}): {r['n']} ({'✓' if r['n']==0 else '✗'})")
    assert r["n"] == 0
    ...
    """, (CLUSTER,)).fetchone()
    log(f"  D3 active is_relevant vc with group_id NULL: {r['n']} ({'✓' if r['n']==0 else '✗'})")
    assert r["n"] == 0
    ...
    r = conn.execute("""
        ...
        JOIN mti_terms mt ON mt.id = vc.mti_term_id
        WHERE mt.cluster_code=? AND COALESCE(vc.delete_flagged,0)=0 AND vc.is_relevant=1 AND vcg.delete_flagged=1
    """, (CLUSTER,)).fetchone()
    log(f"  D4 active is_relevant vc pointing to delete_flagged VCG: {r['n']} ({'✓' if r['n']==0 else '✗'})")
    assert r["n"] == 0
    ...
    """, (CLUSTER,)).fetchone()
    log(f"  D5 active M02 VCGs reachable via vcg_term: {r['n']} (expected 26)")
    assert r["n"] == 26
...
def main():
    ...
    try:
        ...
        log(f"\nDONE — vcg_term={result['vcg_term_soft_deleted']} · vcg={result['vcg_soft_deleted']}")

"""Apply Phase 10 inherited-finding reconciliation for M01.

Directive: wa-cluster-M01-dir-005-inherited-findings-reconcile-v1-20260516
Governing: wa-sessionb-cluster-instruction-v2_1-20260516 §13

Inputs:
  - Sessions/Session_Clusters/M01/WA-M01-inherited-findings-reconciliation-v2-20260516.json
...
Health checks (post-apply):
  F1: wa_session_b_findings row count for M01 contributor registries unchanged
...
V2_JSON = REPO / "Sessions" / "Session_Clusters" / "M01" / "WA-M01-inherited-findings-reconciliation-v2-20260516.json"
DIRECTIVE = "DIR-20260516-005"
CLUSTER = "M01"
...
def precheck(conn, v2: dict) -> None:
    ...
    if n != 24:
        raise RuntimeError(f"v2 has {n} rows; expected 24")
...
def healthcheck(conn) -> None:
    ...
    log(f"  F1 wa_session_b_findings rows for M01 contributors: {r['n']} (informational)")
    ...
    log(f"  F4 distinct sbf status values for our 13 rows: {distinct_statuses}")
    # M01-specific: SUPERSEDED row (srf.17) lives in wa_session_research_flags, not sbf, so
    # the sbf set will only show resolved_qa / routed_cluster / routed_sd.
    expected = {"resolved_qa", "routed_cluster", "routed_sd"}

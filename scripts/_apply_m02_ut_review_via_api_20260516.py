"""M02 Phase 1 UT review via Claude API.

Classifies the 370 UT verses across M02's 47 terms with UT load.
Produces a VCNEW patch + a borderline/decision log for researcher review.

Modeled on scripts/_apply_m01_ut_review_via_api_20260515.py.
...
PATCH_PATH = OUT_DIR / f"wa-cluster-M02-patch-vcnew-utreview-api-v1-{DATE}.json"
LOG_PATH = OUT_DIR / f"WA-M02-UT-verse-review-api-v1-{DATE}.md"
RAW_PATH = OUT_DIR / f"WA-M02-UT-api-raw-responses-{DATE}.json"


SYSTEM_PROMPT = """You are Claude AI performing Phase 1 (UT verse review) of the Session B cluster analysis for cluster M02 (Anger, Wrath and Indignation).

CLUSTER M02 — DOMAIN

M02's characteristic-bearing scope: the inner-being engagement of anger and its kindred passions — the inner heat, fury, indignation, jealousy, provocation, dispute, and rage that arise within the person (and within God) under offense, injustice, betrayal, or threat to honor. The cluster names how the inner being is roused, kindled, fired, vexed, and provoked — and the moral evaluation that follows. It includes:

  - Burning anger and inner heat (cha.rah, cha.ron, che.mah, zal.a.phah, orgē) — the kindling of the inner being toward fury.
  - Wrath and indignation (qa.tsaph, qe.tsaph, za.am, aganaktēsis, orgē as enacted judgment) — settled or eruptive righteous and unrighteous wrath.
  - Provocation and being provoked (ka.as, ma.rar, tsur, paroxunō, parorgizō, erethizō) — the rousing of anger in another (often human against God, or God against humans).
  - Jealousy and zeal (qa.na, qin.ah, qan.na, qan.no) — the protective fury of God for his glory and people, and human possessive jealousy.
  - Strife, quarrel, dispute (riv, mats.tsah, mats.tsut, me.ri.vah, eris, eritheia, antilogia, filoneikos, logomachia, thumomacheō) — inner-being driven contention, the social-relational outworking of anger.
  - Vexation and rage (ka.a.s, za.aph, za.eph, re.gaz, na.phal-angry, zid) — the inner agitation that erupts into hostility.
  - Quick-temperedness (orgilos) — character disposition toward easy anger.
  - The structural opposite: patience, slow-to-anger (e.g. ʾerek apayim), forbearance, meekness, gentle answer turning away wrath.

T1 FRAMEWORK — what a characteristic-bearing inner-being verse must show

A verse evidences M02's characteristic when its content names one or more of:
  1. Constitutional location — heart/mind/soul/nose/spirit experiencing anger, fury, indignation, jealousy, vexation, or related rousing.
  2. Inner faculties engaged — the will being inflamed; the perception that injury, injustice, or honor has been violated; the move from offense to retaliation or restraint.
  3. Inner impact — anger's effect on the person (kindles judgment, drives action, hardens, leads to sin, motivates intervention, divine zeal for righteousness).
  4. Structural opposite — anger vs patience/slow-to-anger; fury vs forbearance; jealousy vs trust; strife vs peace.
  5. Direction or object — anger of God against sin; God's jealousy for his people; human anger against neighbor / brother / oppressor / God; anger as righteous indignation vs sinful rage.

PHASE 1 RUBRIC — three classifications

You assign each verse exactly ONE classification:

  **relevant** — the verse evidences the term's inner-being content within M02's register.
      Examples: Exo 32:19 (Moses' anger burned hot — anger kindled and erupted in action) for cha.rah.
      Examples: Rom 2:8 (those who do not obey the truth — God's wrath and indignation upon them) for orgē.
      Examples: Num 25:11 (Phinehas was zealous with my zeal — God's jealousy/zeal in human agency) for qin.ah.

  **set_aside** — the term appears in this verse but does NOT carry inner-being content
      within the M02 register. The verse uses the term in an unrelated sense:
        - literal-physical context with no inner-being engagement (e.g. "the burning [cha.ron]
          coals of the altar" used purely physically with no anger metaphor);
        - figurative use outside the anger register (e.g. "qin.ah" used for envy in a non-anger
          sense, or "tsur" used as a noun meaning "rock/cliff");
        - surface occurrence in a list, genealogy, or place-name with no inner content
          (homonyms — some Strong's entries cover multiple lexical senses, e.g. "tsur" as
          "rock" vs "to provoke/besiege");
...
When classification is "set_aside", the set_aside_reason field must:
  ...
  - Name the verse's actual sense of the term (e.g. "tsur used as 'rock' physically, not 'provoke/besiege'").
  - End with: " — outside M02 inner-being scope."
"""
...
def build_vcnew_patch(conn, by_term: list[dict], classifications: dict[int, list[dict]]) -> dict:
    ...
    for term in by_term:
        ...
        for v in term["verses"]:
            ...
            if cls == "relevant":
                ...
                reason = r.get("set_aside_reason") or "outside M02 inner-being scope"
                ops.append({
                    "op_id": f"OP-{seq:03d}",
                    "table": "verse_context",
                    "operation": "insert",
    ...
    return {
        "_patch_meta": {
            "patch_type": "VCNEW",
            "patch_id": f"wa-cluster-M02-patch-vcnew-utreview-api-v1-{DATE}",
            "cluster_code": CLUSTER,
            "session_ref": "M02 Phase 1 UT review via Claude API",
...
def write_log(by_term, classifications, patch, log_path):
    lines = [
        f"# WA-M02-UT-verse-review-api-v1-{DATE}",
        "",
        f"Phase 1 UT review for cluster M02 — Claude API classification of {sum(len(t['verses']) for t in by_term)} UT verses across {len(by_term)} terms.",
        "",
        "## Per-term counts",
        "",
...
def main():
    ...
    print(f"M02 UT load: {len(by_term)} terms · {n_verses} verses")
    for t in by_term:

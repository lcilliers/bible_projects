"""Apply Phase 12 — M03 cluster closure.

Directive: wa-cluster-M03-dir-007-closure-v1-20260517 (DIR-20260517-003)
...
Pre-flight checks:
  ...
  P1 — cluster_finding rows exist for M03 v1-20260517 (Phase 11 applied)
...
Operations (single transaction):
  Op A — BOUNDARY exit via flagged-for-decision: INSERT one wa_session_research_flags
         row per BOUNDARY term with flag_code='BOUNDARY_DECISION_PENDING'.
...
DIRECTIVE = "DIR-20260517-003"
CLUSTER = "M03"
VERSION = "v1-20260517"
BOUNDARY_SG_ID = 80  # M03-BOUNDARY
EXPECTED_BOUNDARY_COUNT = 28
PART4_FILE = "WA-M03-consolidated-findings-v1-20260517-part4-T5-T7.md"
...
def precheck(conn) -> dict:
    ...
    assert r["n"] >= 189 and r["prompts"] == 189
    ...
    bnd = conn.execute("""
        ...
        WHERE mts.cluster_subgroup_id=? AND COALESCE(mts.delete_flagged,0)=0
          AND COALESCE(mt.delete_flagged,0)=0
          AND mts.placement_note LIKE '%[boundary]%'
        ORDER BY mt.strongs_number
    """, (BOUNDARY_SG_ID,)).fetchall()
    log(f"  B1 BOUNDARY terms: {len(bnd)} (expected {EXPECTED_BOUNDARY_COUNT})")
    if len(bnd) != EXPECTED_BOUNDARY_COUNT:
        raise RuntimeError(
            f"BOUNDARY count mismatch: got {len(bnd)}, expected {EXPECTED_BOUNDARY_COUNT}"
        )
...
def apply(conn, dry_run: bool, boundary_terms: list[dict]) -> dict:
    ...
    log(f"\n{'='*60}\nOp A — BOUNDARY exit via flagged-for-decision\n{'='*60}")
    log(f"  Flagging {len(boundary_terms)} BOUNDARY terms with BOUNDARY_DECISION_PENDING")
    a_count = 0
    for b in boundary_terms:
        flag_label = f"M03-BOUNDARY-{b['strongs_number']}"
        description = (
            f"M03 closure ({DIRECTIVE}): BOUNDARY term {b['strongs_number']} "
            ...
            f"promote-to-M03-subgroup / reassign-to-other-cluster / "
            f"retain-BOUNDARY-with-extended-rationale. "
    ...
    log(f"\n{'='*60}\nOp B — Cluster status transition\n{'='*60}")
    if not dry_run:
...
def healthcheck(conn) -> None:
    log(f"\n{'='*60}\nPost-closure health checks\n{'='*60}")
    r = conn.execute("SELECT status FROM cluster WHERE cluster_code=?", (CLUSTER,)).fetchone()
    log(f"  Z1 cluster.status: {r['status']} "
        f"({'✓' if r['status']=='Analysis Completed' else '✗'})")
...
def main():
    ...
    try:
        ...
        log(f"\nDONE — {'simulated' if dry_run else 'applied'}: "
            f"boundary flags={result['boundary_flags_inserted']}")

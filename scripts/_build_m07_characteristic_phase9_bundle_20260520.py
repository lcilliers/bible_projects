...
def fetch_characteristic(conn, char_seq: int):
    row = conn.execute(
        "SELECT * FROM characteristic WHERE cluster_code='M07' AND char_seq=? "
        "AND COALESCE(delete_flagged,0)=0",
        (char_seq,),
    ).fetchone()
    if not row:
        raise SystemExit(f"No M07 characteristic with seq={char_seq}")
    return row
...
def main():
    ...
    cluster = conn.execute("SELECT description FROM cluster WHERE cluster_code='M07'").fetchone()
    ...
    # ===== Brief =====
    B = []
    B.append(f"# M07 Phase 9 — Bundle ({bundle_tag}) — brief")
    B.append("")
    ...
    B.append(f"| 2 | **Structural input** — `Sessions/Session_Clusters/M07/{INPUT.name}` | Per-characteristic data blocks + shared 189-prompt catalogue + carry-forward observations |")
    ...
    B.append("2. Ping CC: \"M07 Char N (<name>) Phase 9 findings ready\" — once per batch.")
    ...
    # ===== Structural input =====
    S = []
    S.append(f"# M07 Phase 9 — Bundle ({bundle_tag}) — structural input")
    S.append("")
    ...
    S.append(f"| 2 | **This document** — `Sessions/Session_Clusters/M07/{INPUT.name}` | Per-characteristic data blocks + 189-prompt catalogue |")

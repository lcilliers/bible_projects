"""Build a per-characteristic Phase 9 AI package for M10c.
...
Usage:
    python scripts/_build_m10c_characteristic_phase9_package_20260526.py --char-seq 1
    python scripts/_build_m10c_characteristic_phase9_package_20260526.py --char-seq 2
    ...

Outputs (per characteristic):
    Sessions/Session_Clusters/M10c/wa-cluster-M10c-phase9-char{N}-{short_name}-brief-v1-{date}.md
    Sessions/Session_Clusters/M10c/wa-cluster-M10c-phase9-char{N}-{short_name}-input-v1-{date}.md
"""
...
def main():
    ...
    ap.add_argument("--char-seq", type=int, required=True, help="Characteristic sequence number (1-4 for M10c)")
    args = ap.parse_args()
    ...
    char = conn.execute(
        "SELECT * FROM characteristic WHERE cluster_code='M10c' AND char_seq=? AND COALESCE(delete_flagged,0)=0",
        (args.char_seq,),
    ).fetchone()
    if not char:
        raise SystemExit(f"No M10c characteristic with seq={args.char_seq}")

    short = char["short_name"]
    # Sanitise for filename: replace spaces and em-dashes with hyphen, then collapse repeats
    import re as _re
    safe_short = short.replace(" ", "-").replace("/", "-").replace("—", "-").replace("–", "-")
    safe_short = _re.sub(r"-+", "-", safe_short).strip("-")
    ...
    cluster = conn.execute(
        "SELECT description, gloss FROM cluster WHERE cluster_code='M10c'"
    ).fetchone()

    # ===== File paths =====
    BRIEF = Path(f"Sessions/Session_Clusters/M10c/wa-cluster-M10c-phase9-char{args.char_seq}-{safe_short}-brief-v1-{TODAY}.md")
    INPUT = Path(f"Sessions/Session_Clusters/M10c/wa-cluster-M10c-phase9-char{args.char_seq}-{safe_short}-input-v1-{TODAY}.md")

    total_verses = sum(sg["verse_count"] or 0 for sg in subgroups)

    # ===== Brief =====
    B = []
    B.append(f"# M10c Phase 9 — Characteristic {args.char_seq} ({short}) — brief")
    B.append("")
    B.append(f"**Cluster:** M10c — {cluster['description']}")
    ...
    B.append("|---|---|---|")
    B.append(f"| 1 | **This brief** — `Sessions/Session_Clusters/M10c/{BRIEF.name}` | Primary task instructions |")
    B.append(f"| 2 | **Structural input** — `Sessions/Session_Clusters/M10c/{INPUT.name}` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts + carry-forward observations |")
    B.append("| 3 | **Governing instruction** — `Workflow/Instructions/wa-sessionb-cluster-instruction-v2_9-20260526.md` | §12 Phase 9 disciplines; §12.4 parser-safe form |")
    B.append("| 4 | **Science extract** — `Workflow/Sciences/wa-m10-guilt-scienceextract-v1_0-20260513.md` (Section 4 only) | Programme-curated scientific lens for T7.3. **Note:** the doc was authored pre-split and covers M10/M10b/M10c siblings; for M10c only Section 4 (Defilement, impurity, contamination) is in scope. Sections 1 (Guilt) and 2 (Transgression) belong to M10; Section 3 (Evil/Wickedness) belongs to M10b. |")
    ...
    B.append("## Context")
    B.append("")
    B.append(f"M10c has 4 inner-being characteristics. You are processing **Characteristic {args.char_seq} ({short})**. Per researcher direction 2026-05-18 (carried forward to M10c under v2_9 §8.0):")
    B.append("")
    ...
    B.append("")
    B.append("Cluster-scope synthesis (across all 4 characteristics) happens in a separate session at the end. **Don't author cluster-scope findings here.**")
    B.append("")
    B.append("---")
    B.append("")
    ...
    B.append("")
    B.append("Output format per prompt (one block per prompt; parser-safe form per v2_9 §12.4):")
    B.append("")
    B.append("```")
    ...
    B.append("| 3 | **Citation discipline** | Every E finding cites at least one verse-reference or VCG code (e.g. `Lev 11:24`, `M10c-A-VCG-07`) | ALERT — name the uncited E prompts; do NOT proceed |")
    ...
    B.append("---")
    B.append("")
    B.append("## Discipline (per v2_9 §12)")
    B.append("")
    ...
    B.append("4. **Fluency is not a quality signal** (v2_9 §2.4). Plausible-sounding text without specific citations is rejected.")
    ...
    B.append("6. **No cluster-scope findings.** Cluster synthesis runs after all 4 characteristics finish.")
    ...
    B.append("")
    B.append(f"Target filename: `Sessions/Session_Clusters/M10c/wa-cluster-M10c-phase9-char{args.char_seq}-{safe_short}-findings-v1-{TODAY}.md`")
    B.append("")
    ...
    B.append("")
    B.append("```markdown")
    B.append(f"# M10c Phase 9 — Characteristic {args.char_seq} ({short}) — findings")
    B.append("")
    ...
    B.append(f"2. Ping CC: \"M10c Char {args.char_seq} ({short}) Phase 9 findings ready\"")
    ...
    # ===== Structural input =====
    S = []
    S.append(f"# M10c Phase 9 — Characteristic {args.char_seq} ({short}) — structural input")
    S.append("")
    ...
    S.append("|---|---|---|")
    S.append(f"| 1 | **Brief (read first)** — `Sessions/Session_Clusters/M10c/{BRIEF.name}` | Primary task instructions |")
    S.append(f"| 2 | **This document** — `Sessions/Session_Clusters/M10c/{INPUT.name}` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts |")
    S.append("| 3 | Governing instruction — `Workflow/Instructions/wa-sessionb-cluster-instruction-v2_9-20260526.md` | §12 Phase 9 disciplines |")
    S.append("")
    S.append("---")
    S.append("")

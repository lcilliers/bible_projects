...
Output filenames:
    Sessions/Session_Clusters/M09/wa-cluster-M09-phase9-bundle-char{N1+N2+...}-brief-v1-{date}.md
    Sessions/Session_Clusters/M09/wa-cluster-M09-phase9-bundle-char{N1+N2+...}-input-v1-{date}.md

The AI is expected to produce ONE findings file per characteristic
(named wa-cluster-M09-phase9-char{N}-{short}-findings-v1-{date}.md). The
existing loader (`_apply_phase9_characteristic_findings_20260522.py`)
...
def fetch_characteristic(conn, char_seq: int):
    row = conn.execute(
        "SELECT * FROM characteristic WHERE cluster_code='M09' AND char_seq=? "
        "AND COALESCE(delete_flagged,0)=0",
        (char_seq,),
    ).fetchone()
    if not row:
        raise SystemExit(f"No M09 characteristic with seq={char_seq}")
    return row
...
def main():
    ...
    cluster = conn.execute("SELECT description FROM cluster WHERE cluster_code='M09'").fetchone()
    ...
    BRIEF = Path(f"Sessions/Session_Clusters/M09/wa-cluster-M09-phase9-bundle-{bundle_tag}-brief-v1-{TODAY}.md")
    INPUT = Path(f"Sessions/Session_Clusters/M09/wa-cluster-M09-phase9-bundle-{bundle_tag}-input-v1-{TODAY}.md")

    total_verses_all = sum(c["total_verses"] for c in chars_data)

    # ===== Brief =====
    B = []
    B.append(f"# M09 Phase 9 — Bundle ({bundle_tag}) — brief")
    B.append("")
    B.append(f"**Cluster:** M09 — {cluster['description']}")
    ...
    B.append("|---|---|---|")
    B.append(f"| 1 | **This brief** — `Sessions/Session_Clusters/M09/{BRIEF.name}` | Primary task instructions |")
    B.append(f"| 2 | **Structural input** — `Sessions/Session_Clusters/M09/{INPUT.name}` | Per-characteristic data blocks + shared 189-prompt catalogue + carry-forward observations |")
    ...
    B.append("| 4 | **Science extract** — `Workflow/Sciences/wa-m09-humility-scienceextract-v1_0-20260513.md` | Programme-curated scientific lens for T7.3 (human science framework) prompts — ensures consistent framing across clusters and reviewers |")
    ...
    B.append("")
    B.append("**Known prior failure modes:** (a) M07 bundle (2026-05-20) — the AI tried to hold all 4 characteristics × 189 prompts in working memory and fell over twice (mixed evidence across characteristics, lost CHAR-N markers). (b) M09 CHAR-1 (2026-05-21) — the AI tried to process all 189 prompts in one pass and drifted (carried prior-tier reasoning into later tiers, performed analysis between tiers, substituted fluency for citation). **Do not repeat either pattern.**")
    B.append("")
    ...
    B.append("")
    B.append("Each batch's 189 prompts split across 8 tiers. Tier sizes: **T0=12, T1=24, T2=31, T3=33, T4=24, T5=21, T6=24, T7=20.** Tiers chain in one batch when self-evaluation passes; ALERT and stop only when self-evaluation surfaces an issue.")
    B.append("")
    ...
    B.append("3. **Run the per-tier sequence (T0 → T7) with self-evaluation gates** — see next subsection.")
    B.append("4. **After T7's self-evaluation passes — final Self-check** — append the Self-check block for this batch.")
    B.append("5. **STOP at batch boundary** — do not begin Batch N+1 in the same response. CC validates Batch N's file before you start Batch N+1.")
    B.append("")
    ...
    B.append("5. **SELF-EVALUATE the tier** — run the self-evaluation gate (see below).")
    B.append("6. **DECIDE: proceed or ALERT** —")
    B.append("   - If self-evaluation PASS: announce *\"Batch N · Tier T{N} PASS — {n}/{n} prompts, [CHAR-N] markers verified. Proceeding to Tier T{N+1}.\"* Then immediately begin Tier T{N+1} in the SAME response.")
    B.append("   - If self-evaluation FAIL or surfaces a researcher-review item: announce *\"Batch N · Tier T{N} ALERT — {issue}. Pausing for researcher review.\"* Then STOP. Wait for next prompt.")
    B.append("")
    B.append("**Between tiers, working memory must reset.** Each tier re-reads §3 fresh with its own analytical lens. The chaining is procedural (same response); the analytical context is not.")
    B.append("")
    B.append("**Between batches, working memory must reset.** Don't carry verse-citation patterns or finding language from Batch N into Batch N+1. The next batch starts fresh from §3.{next-letter}.")
    B.append("")
    ...
    B.append("| 2 | **Scope marker** | Every `[CHAR-N]` marker in this tier matches the current batch's characteristic | ALERT — name the wrong-marker prompts |")
    B.append("| 3 | **Citation discipline** | Every E finding cites at least one verse-ref or VCG code (e.g. `Pro 16:5`, `M09-C-VCG-01`) | ALERT — name the uncited E prompts |")
    B.append("| 4 | **No cross-tier mixing** | This tier's findings do not reference findings from a later tier | ALERT — name the cross-tier references |")
    B.append("| 5 | **Distinct evidence per prompt** | No two prompts share an identical 80-character opening | ALERT — name the shared-opening prompts |")
    B.append("| 6 | **Verse-corpus validity** | Verse references look like they belong to this characteristic's evidence corpus (cross-check from §3.{letter}); flag any cited verse that is not in §3.{letter} | ALERT — name the phantom-ref prompts |")
    B.append("| 7 | **No cross-batch mixing** | This tier's findings do not cite evidence from another characteristic's §3 block | ALERT — name the cross-batch references |")
    B.append("")
    B.append("Document the self-evaluation result inline at the end of each tier section — one sentence per check, PASS or FAIL. If everything passes, the next tier follows immediately in the same response.")
    B.append("")
    B.append("### What NOT to do (each is a drift signal)")
    B.append("")
    B.append("- Authoring prompts from a tier you have not announced as OPEN")
    B.append("- Holding multiple tiers' worth of unwritten findings in your response (the file write is per-tier, even though tiers chain in one response)")
    B.append("- Skipping prompts \"to come back later\" — every prompt in a tier is answered before its self-evaluation")
    B.append("- Inter-tier or inter-batch analysis while a tier/batch is in flight — cross-tier patterns belong in the batch Self-check; cross-batch patterns belong in the cluster-synthesis session that runs after all batches finish")
    B.append("- Substituting fluency for citation — every E must name verses / VCGs from §3.{letter}")
    B.append("- Bulk-classifying prompts from a sample — each prompt is its own analytical pass")
    B.append("- Beginning Batch N+1 before Batch N's findings file is written and CC has validated")
    ...
    B.append("")
    B.append("If during a tier you notice any of the drift signals above:")
    B.append("")
    ...
    for idx, c in enumerate(chars_data, start=1):
        ...
        out_name = f"wa-cluster-M09-phase9-char{c['seq']}-{c['safe_short']}-findings-v1-{TODAY}.md"
        letter = chr(ord("A") + idx - 1)
        B.append(f"| {idx} | Char {c['seq']} — {c['short']} | {sg_codes} | ~{c['total_verses']} | §3.{letter} | `Sessions/Session_Clusters/M09/{out_name}` |")
    B.append("")
    ...
    B.append("```")
    B.append("")
    B.append("**Scope marker** is `**[CHAR-N]**` where **N is the characteristic number for that batch** (e.g., `**[CHAR-2]**` for the Submission batch, `**[CHAR-3]**` for the Boasting batch). CC's loader uses this to set characteristic_id on each row, so it MUST match the current batch's characteristic — and never reference a characteristic that isn't this bundle's set (M09 has only CHAR-1..CHAR-5; CHAR-1 is a separate session, not in this bundle).")
    B.append("")
    ...
    B.append("7. **No cluster-scope findings.** Cluster synthesis runs after all 6 characteristics finish.")
    ...
    B.append("")
    B.append("1. The batch's findings file is built incrementally by tier: T0 created the file with the header + T0 section; T1..T7 each appended their section. Tiers chained automatically when their self-evaluations passed.")
    B.append("2. **After T7's self-evaluation passes** — append the final batch Self-check block — counts of E/S/G, carry-forward observations addressed, unexpected patterns, per-tier self-eval summary.")
    B.append("3. **Announce batch complete** — *\"Batch N complete: <filename>, 189/189 prompts, [CHAR-N] markers verified, all per-tier evaluations PASS, Self-check posted.\"*")
    B.append("4. **STOP at batch boundary**. Do not begin Batch N+1 in the same response. Wait for the next prompt.")
    ...
    B.append("")
    B.append("**Cadence note:** One batch's 189 prompts may fit in one long response (if all per-tier self-evals pass and you chain straight through) or in 2-4 responses (if you split at tier boundaries to keep response length manageable). Either is fine. The hard constraints are: (a) per-tier self-evaluation gate; (b) per-tier file append; (c) no cross-batch chaining without CC validation. Bundle total: typically 4-8 responses for the authoring + 4 batch-Self-checks.")
    B.append("")
    B.append("---")
    B.append("")
    B.append("*End of brief. Load the structural input (#2) and begin **Batch 1 · Tier T0** in your first response. Chain tiers when self-evaluation passes; ALERT only on issues; STOP at batch boundaries.*")

    BRIEF.write_text("\n".join(B), encoding="utf-8")

    # ===== Structural input =====
    S = []
    S.append(f"# M09 Phase 9 — Bundle ({bundle_tag}) — structural input")
    S.append("")
    ...
    S.append("|---|---|---|")
    S.append(f"| 1 | **Brief (read first)** — `Sessions/Session_Clusters/M09/{BRIEF.name}` | Primary task instructions |")
    S.append(f"| 2 | **This document** — `Sessions/Session_Clusters/M09/{INPUT.name}` | Per-characteristic data blocks + 189-prompt catalogue |")

"""Build the M07 Phase 9 cluster-synthesis (8th) session package.
...
def main():
    ...
    B.append("")
    B.append("This is the final Phase 9 session for M07. The six characteristic-scope batches (Shame as experienced inner state, Humiliation as enforced abasement, Dishonour as relational worth-denial, Shamefulness as moral-evaluative judgment, Shame produced by contempt and rejection, Innocence as structural counter to shame) are complete; each authored 189 findings citing specific verses. This session looks ACROSS those six sets to author the cluster-scope findings — patterns, divergences, and shared anchors that emerge only when the six are compared.")
    B.append("")
    ...
    B.append(f"| 2 | **Structural input** — `Sessions/Session_Clusters/M07/{INPUT.name}` | Per-prompt synthesis matrix: for each of the 189 prompts, the 6 characteristics' findings stacked side-by-side (verbatim from `cluster_finding`); plus confirmed observations |")
    ...
    B.append("")
    B.append(f"For the M07 cluster as a whole, two output forms are required (per researcher direction):")
    B.append("")
    B.append("### Output A — 189 cluster-scope findings (parser-safe rows)")
    B.append("")
    B.append("For each of the 189 catalogue prompts, author ONE cluster-scope finding examining what surfaces when the 6 characteristics' findings are compared. The cluster-scope finding answers: *what does the M07 cluster — across its 6 characteristics — reveal about this question?* It is the comparative-integrative layer above the per-characteristic answers.")
    B.append("")
    ...
    B.append("")
    B.append("After the 189-row block, write a free-form prose appendix capturing the emergent themes that surface across the six characteristics — themes that the 189-prompt structure does not fully capture. Suggested themes (drawn from the M07 carry-forward observations + analytical hints surfaced through Phase 3–8.5):")
    B.append("")
    B.append("- The **CHAR-1 volume-split** across M07-A (unjust wound) / M07-B (moral consequence) / M07-C (divine encounter) — what coherent characteristic identity holds the three registers together, and where do they genuinely diverge?")
    B.append("- The **M06 ↔ M07 contempt-shame relational pair** — CHAR-5 holds the shame-recipient face; M06 holds the contempt-projection source. The cross-cluster dynamic in detail.")
    B.append("- The **M12 ↔ M07 innocence-purity polarity** — CHAR-6's innocence-as-shield-against-shame; the inner moral state that protects against guilt-shame.")
    B.append("- **OT vs NT vocabulary arcs** across the cluster — where does each characteristic's vocabulary tradition concentrate?")
    B.append("- **Vertical vs horizontal shame** — shame before God (M07-C) vs shame between humans (much of M07-A, M07-B, M07-D, M07-E, M07-G); what shifts across the registers?")
    B.append("- **Shame removed vs shame imposed** — the divine reversal of shame (e.g. Isa 49:23, 1Pe 2:6) vs God's putting-to-shame (Psa 25:3, 1Cor 1:27); when is each operative?")
    B.append("- **Christological convergence points** — Heb 12:2 (\"despising the shame\"); 1Pe 2:6 (\"will not be put to shame\"); Phili 3:19 (inverted glory-in-shame); Isa 50:7 (servant set face like flint against shame); Rom 9:33 (cornerstone faith vs disgrace).")
    ...
    B.append("")
    B.append("Suggested length: 2,000–4,000 words for the appendix. Use sub-headings per theme. Cite the per-characteristic findings (e.g. \"Char 5 T6.1.1's finding that contempt produces shame across 11 exoutheneō verses\") rather than re-citing verses already cited in the structural input.")
    B.append("")
    B.append("---")
    B.append("")
    B.append("## Discipline")
    ...
    B.append("2. **Cite by characteristic.** The cluster-scope finding names which characteristics contribute what — e.g., \"Shame-as-experienced (Char 1) and Humiliation-as-enforced (Char 2) co-occur in many verses but with different agencies — Char 1 names the inner state; Char 2 names the enforcing mechanism.\"")
    ...
    B.append("**Scope:** Cluster M07 (Shame, Disgrace and Humiliation) — across 6 characteristics")
    ...
    S.append("")
    S.append("M07 (Shame, Disgrace and Humiliation) holds 6 inner-being characteristics:")
    S.append("")
    ...
    S.append("")
    S.append("For each of the 189 prompts (T0.1.1 … T7.3.4), the 6 characteristics' findings are stacked verbatim below. Read all 6 before authoring the cluster-scope row in your output.")
    S.append("")

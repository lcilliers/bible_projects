...
def main():
    ...
    B.append("3. **Fluency is not a quality signal** (v2_8 §2.4). Plausible-sounding text without specific citations is rejected.")
    B.append("4. **No sub-group-scope findings.** All findings at characteristic scope. Where evidence differs by sub-group within the characteristic (e.g. CHAR-1 spans M08-A/B/C), the finding text names the sub-group(s) inline (e.g., \"primarily evidenced in M08-A; M08-B's moral-consequence register sharpens the pattern...\").")
    B.append("5. **No cluster-scope findings.** Cluster synthesis runs after all 5 characteristics finish.")
    ...
    B.append("")
    B.append(f"1. Drop the output in `Sessions/Session_Clusters/M08/wa-cluster-M08-phase9-char{args.char_seq}-{safe_short}-findings-v1-20260521.md`")

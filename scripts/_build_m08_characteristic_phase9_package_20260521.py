"""Build a per-characteristic Phase 9 AI package for M08.
...
Usage:
    python scripts/_build_m04_characteristic_phase9_package_20260521.py --char-seq 1
    python scripts/_build_m04_characteristic_phase9_package_20260521.py --char-seq 2
    ...

Outputs (per characteristic):
    Sessions/Session_Clusters/M08/wa-cluster-M08-phase9-char{N}-{short_name}-brief-v1-{date}.md
    Sessions/Session_Clusters/M08/wa-cluster-M08-phase9-char{N}-{short_name}-input-v1-{date}.md
"""
...
def main():
    ...
    ap.add_argument("--char-seq", type=int, required=True, help="Characteristic sequence number (1-6 for M08)")
    args = ap.parse_args()
    ...
    char = conn.execute(
        "SELECT * FROM characteristic WHERE cluster_code='M08' AND char_seq=? AND COALESCE(delete_flagged,0)=0",
        (args.char_seq,),
    ).fetchone()
    if not char:
        raise SystemExit(f"No M08 characteristic with seq={args.char_seq}")
    ...
    cluster = conn.execute(
        "SELECT description, gloss FROM cluster WHERE cluster_code='M08'"
    ).fetchone()

    # ===== File paths =====
    BRIEF = Path(f"Sessions/Session_Clusters/M08/wa-cluster-M08-phase9-char{args.char_seq}-{safe_short}-brief-v1-{TODAY}.md")
    INPUT = Path(f"Sessions/Session_Clusters/M08/wa-cluster-M08-phase9-char{args.char_seq}-{safe_short}-input-v1-{TODAY}.md")

    total_verses = sum(sg["verse_count"] or 0 for sg in subgroups)

    # ===== Brief =====
    B = []
    B.append(f"# M08 Phase 9 — Characteristic {args.char_seq} ({short}) — brief")
    B.append("")
    ...
    B.append(f"| 2 | **Structural input** — `Sessions/Session_Clusters/M08/{INPUT.name}` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts + carry-forward observations |")
    ...
    B.append("## Context")
    B.append("")
    B.append(f"M08 has 5 inner-being characteristics. You are processing **Characteristic {args.char_seq} ({short})**. Per researcher direction 2026-05-18 (carried forward to M08 under v2_8 §8.0):")
    B.append("")
    ...
    B.append("")
    B.append("Cluster-scope synthesis (across all 5 characteristics) happens in a separate session at the end. **Don't author cluster-scope findings here.**")
    B.append("")
    B.append("---")
    B.append("")
    ...
    B.append("5. **CONFIRM WRITTEN** — announce: *\"Tier T{N} written: {file_path}, {n}/{n} prompts, [CHAR-{seq}] markers verified.\"*")
    B.append("6. **STOP** — do not begin Tier T{N+1} in the same response. Wait for the next prompt from CC or the user.")
    B.append("")
    B.append("**Between tiers, your working memory must reset.** Do not carry T0's reasoning or finding patterns into T1's prompts. T1 starts fresh from the verse evidence, with T1's analytical lens.")
    B.append("")
    B.append("### What NOT to do (each is a drift signal)")
    B.append("")
    B.append("- Authoring prompts from a tier you have not announced as OPEN")
    B.append("- Holding multiple tiers' worth of unwritten findings in your response")
    B.append("- Skipping prompts \"to come back later\" — every prompt in a tier is answered before the tier is written")
    B.append("- Cross-tier analysis or summary while a tier is in flight (e.g. *\"now that I see T2, I notice T1 should have said…\"*) — finish T2, write, STOP. Cross-tier patterns belong in the self-check at the very end.")
    ...
    B.append("")
    B.append("### Recovery if you find yourself drifting mid-tier")
    B.append("")
    B.append("If at any point during a tier you notice you have:")
    B.append("")
    B.append("- Begun a second tier in the same response,")
    B.append("- Carried analysis from another tier into this one,")
    B.append("- Skipped prompts in the current tier,")
    B.append("- Bulk-classified prompts without per-prompt evidence,")
    B.append("")
    B.append("then: **STOP** the current response, announce the issue, abandon the half-written tier, and restart that tier from scratch (re-read §3, re-author from the first prompt). It is faster to restart a tier cleanly than to patch a contaminated one.")
    B.append("")
    B.append("---")
    B.append("")
    ...
    B.append("5. **No sub-group-scope findings.** All findings at characteristic scope. Where evidence differs by sub-group within the characteristic (e.g. CHAR-1 spans M08-A1/A2/A3/A4), the finding text names the sub-group(s) inline.")
    B.append("6. **No cluster-scope findings.** Cluster synthesis runs after all 5 characteristics finish.")
    ...
    B.append("")
    B.append(f"Target filename: `Sessions/Session_Clusters/M08/wa-cluster-M08-phase9-char{args.char_seq}-{safe_short}-findings-v1-{TODAY}.md`")
    B.append("")
    B.append("The file is built tier-by-tier. **Tier 0 (T0) creates the file with the header + T0 section. Each subsequent tier appends its `## T{N} — …` section.** At the end, after T7, post a separate Self-check response.")
    B.append("")
    B.append("### Tier 0 (first response) — file header + T0 section")
    B.append("")
    B.append("```markdown")
    B.append(f"# M08 Phase 9 — Characteristic {args.char_seq} ({short}) — findings")
    B.append("")
    ...
    B.append("```")
    B.append("")
    B.append("End with: *\"Tier T0 written: …filename… 12/12 prompts, [CHAR-{seq}] markers verified.\"* Then STOP.")
    B.append("")
    B.append("### Tiers T1..T7 (each in its own response) — APPEND")
    B.append("")
    B.append("On the next prompt, do T1: re-read §3 fresh, answer T1's 24 prompts, and emit ONLY the new `## T1 — Inner Being Faculty in Operation (24 prompts)` section as appendable markdown. The file already has the header — do not re-emit it. Update the tier-checklist line if you can.")
    B.append("")
    B.append("Repeat for T2..T7.")
    B.append("")
    B.append("### After T7 — Self-check (separate response)")
    B.append("")
    B.append("```markdown")
    B.append("## Self-check")
    B.append("")
    ...
    B.append("")
    B.append(f"1. Confirm the findings file is on disk with all 8 tier sections + the Self-check block appended at the end.")
    B.append(f"2. Ping CC: \"M08 Char {args.char_seq} ({short}) Phase 9 findings ready\"")
    B.append("3. CC parses, validates evidence-grounding + completeness, applies to cluster_finding with characteristic_id set.")
    B.append(f"4. Move to next characteristic (Char {args.char_seq + 1 if args.char_seq < 5 else '(cluster synthesis)'}).")
    B.append("")
    B.append("---")
    B.append("")
    B.append("*End of brief. Load the structural input (#2) and begin **Tier T0 only** in your first response.*")

    BRIEF.write_text("\n".join(B), encoding="utf-8")

    # ===== Structural input =====
    S = []
    S.append(f"# M08 Phase 9 — Characteristic {args.char_seq} ({short}) — structural input")
    S.append("")
    ...
    S.append(f"| 2 | **This document** — `Sessions/Session_Clusters/M08/{INPUT.name}` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts |")

...
Output filenames:
    Sessions/Session_Clusters/M04/WA-M04-phase9-bundle-char{N1+N2+...}-brief-v1-{date}.md
    Sessions/Session_Clusters/M04/WA-M04-phase9-bundle-char{N1+N2+...}-input-v1-{date}.md

The AI is expected to produce ONE findings file per characteristic
(named WA-M04-phase9-char{N}-{short}-findings-v1-{date}.md). The
...
def fetch_characteristic(conn, char_seq: int):
    row = conn.execute(
        "SELECT * FROM characteristic WHERE cluster_code='M04' AND char_seq=? "
        "AND COALESCE(delete_flagged,0)=0",
        (char_seq,),
    ).fetchone()
    if not row:
        raise SystemExit(f"No M04 characteristic with seq={char_seq}")
    return row
...
def main():
    ...
    cluster = conn.execute("SELECT description FROM cluster WHERE cluster_code='M04'").fetchone()
    ...
    BRIEF = Path(f"Sessions/Session_Clusters/M04/WA-M04-phase9-bundle-{bundle_tag}-brief-v1-{TODAY}.md")
    INPUT = Path(f"Sessions/Session_Clusters/M04/WA-M04-phase9-bundle-{bundle_tag}-input-v1-{TODAY}.md")

    total_verses_all = sum(c["total_verses"] for c in chars_data)

    # ===== Brief =====
    B = []
    B.append(f"# M04 Phase 9 — Bundle ({bundle_tag}) — brief")
    B.append("")
    ...
    B.append(f"| 2 | **Structural input** — `Sessions/Session_Clusters/M04/{INPUT.name}` | Per-characteristic data blocks + shared 189-prompt catalogue + carry-forward observations |")
    ...
    B.append("| 4 | **Science extract** — `Workflow/Sciences/wa-m04-joy-scienceextract-v1_0-20260513.md` | Programme-curated scientific lens for T7.3 (human science framework) prompts — ensures consistent framing across clusters and reviewers |")
    ...
    B.append("---")
    B.append("")
    B.append("## How to work this bundle — separate batches")
    B.append("")
    B.append(f"You will do **{len(chars_data)} separate 189-prompt batches**, one per characteristic, in the order listed below. Each batch is fully bounded to its own characteristic — do not reuse another characteristic's evidence within a batch.")
    B.append("")
    B.append("| Batch | Characteristic | Sub-groups | Verses | Output file (you produce) |")
    B.append("|---|---|---|---:|---|")
    for c in chars_data:
        sg_codes = ", ".join(sg["subgroup_code"] for sg in c["subgroups"])
        out_name = f"WA-M04-phase9-char{c['seq']}-{c['safe_short']}-findings-v1-{TODAY}.md"
        B.append(f"| {c['seq']} | {c['short']} | {sg_codes} | ~{c['total_verses']} | `Sessions/Session_Clusters/M04/{out_name}` |")
    B.append("")
    ...
    B.append("")
    B.append("Per prompt, one block; parser-safe form per v2_5 §12.4:")
    B.append("")
    B.append("```")
    ...
    B.append("```")
    B.append("")
    B.append("**Scope marker** is `**[CHAR-N]**` where **N is the characteristic number for that batch** (e.g., `**[CHAR-3]**` for the Gladness batch, `**[CHAR-7]**` for the Suffering-Joy batch). CC's loader uses this to set characteristic_id on each row, so it MUST match the current batch's characteristic.")
    B.append("")
    ...
    B.append("")
    B.append("1. **Read every verse-meaning in the structural input for the current batch.** No sampling. The Pass A meanings condense each verse's inner-being content — read them all for the batch's characteristic.")
    ...
    B.append("4. **No sub-group-scope findings.** All findings at characteristic scope. Where evidence differs by sub-group within a characteristic, the finding text names the sub-group(s) inline.")
    B.append("5. **No cross-characteristic mixing.** Evidence from another characteristic's block is OFF-LIMITS for the current batch. If a cross-characteristic observation is significant, note it in the Self-check for that batch — do NOT embed cross-batch findings.")
    B.append("6. **No cluster-scope findings.** Cluster synthesis runs after all 7 characteristics finish.")
    B.append("7. **Self-check before submitting each batch** — confirm 189 prompts have rows; confirm every E names evidence; confirm the `[CHAR-N]` marker matches the batch.")
    B.append("")
    B.append("---")
    B.append("")
    ...
    B.append("---")
    B.append("")
    B.append("## After you finish each batch")
    B.append("")
    B.append("1. Save the batch's findings to its own file (filename in the table above).")
    B.append("2. Ping CC: \"M04 Char N (<name>) Phase 9 findings ready\" — once per batch.")
    B.append("3. CC parses, validates, applies that file to cluster_finding with the right characteristic_id.")
    B.append("4. Begin the next batch.")
    B.append("")
    B.append("If a batch is too large to fit in a single response, split it into segments by tier-pair (T0+T1, T2+T3, T4+T5, T6+T7) — same convention used for Char 2.")
    B.append("")
    B.append("---")
    B.append("")
    B.append("*End of brief. Load the structural input (#2) and begin Batch 1.*")

    BRIEF.write_text("\n".join(B), encoding="utf-8")

    # ===== Structural input =====
    S = []
    S.append(f"# M04 Phase 9 — Bundle ({bundle_tag}) — structural input")
    S.append("")
    ...
    S.append(f"| 2 | **This document** — `Sessions/Session_Clusters/M04/{INPUT.name}` | Per-characteristic data blocks + 189-prompt catalogue |")

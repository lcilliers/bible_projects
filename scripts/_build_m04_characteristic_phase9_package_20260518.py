"""Build a per-characteristic Phase 9 AI package for M04.
...
Outputs (per characteristic):
    Sessions/Session_Clusters/M04/WA-M04-phase9-char{N}-{short_name}-brief-v1-{date}.md
    Sessions/Session_Clusters/M04/WA-M04-phase9-char{N}-{short_name}-input-v1-{date}.md
"""
...
def main():
    ...
    ap.add_argument("--char-seq", type=int, required=True, help="Characteristic sequence number (1-7 for M04)")
    args = ap.parse_args()
    ...
    char = conn.execute(
        "SELECT * FROM characteristic WHERE cluster_code='M04' AND char_seq=? AND COALESCE(delete_flagged,0)=0",
        (args.char_seq,),
    ).fetchone()
    if not char:
        raise SystemExit(f"No M04 characteristic with seq={args.char_seq}")
    ...
    cluster = conn.execute(
        "SELECT description, gloss FROM cluster WHERE cluster_code='M04'"
    ).fetchone()

    # ===== File paths =====
    BRIEF = Path(f"Sessions/Session_Clusters/M04/WA-M04-phase9-char{args.char_seq}-{safe_short}-brief-v1-{TODAY}.md")
    INPUT = Path(f"Sessions/Session_Clusters/M04/WA-M04-phase9-char{args.char_seq}-{safe_short}-input-v1-{TODAY}.md")

    total_verses = sum(sg["verse_count"] or 0 for sg in subgroups)

    # ===== Brief =====
    B = []
    B.append(f"# M04 Phase 9 — Characteristic {args.char_seq} ({short}) — brief")
    B.append("")
    ...
    B.append(f"**Task date:** 2026-05-18")
    ...
    B.append(f"| 2 | **Structural input** — `Sessions/Session_Clusters/M04/{INPUT.name}` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts + carry-forward observations |")
    ...
    B.append("| 4 | **Tier catalogue** — `Workflow/Tiers/wa-obs-catalogue-tiered-v{N}-{date}.md` (latest) | Full prompt catalogue T0–T7 (verses are also reproduced in §3 of the structural input for self-containment) |")
    ...
    B.append("## Context")
    B.append("")
    B.append(f"M04 has 7 inner-being characteristics. You are processing **Characteristic {args.char_seq} ({short})**. Per researcher direction 2026-05-18:")
    B.append("")
    ...
    B.append("4. **No sub-group-scope findings.** All findings at characteristic scope. Where evidence differs by sub-group, the finding text names the sub-group(s) inline (e.g., \"primarily evidenced in M04-A; with M04-G's affective register confirming...\").")
    B.append("5. **No cluster-scope findings.** Cluster synthesis runs after all 7 characteristics finish.")
    ...
    B.append("")
    B.append("```markdown")
    B.append(f"# M04 Phase 9 — Characteristic {args.char_seq} ({short}) — findings")
    B.append("")
    B.append("**Date:** 2026-05-18")
    ...
    B.append("")
    B.append(f"1. Drop the output in `Sessions/Session_Clusters/M04/WA-M04-phase9-char{args.char_seq}-{safe_short}-findings-v1-20260518.md`")
    B.append(f"2. Ping CC: \"M04 Char {args.char_seq} ({short}) Phase 9 findings ready\"")
    ...
    # ===== Structural input =====
    S = []
    S.append(f"# M04 Phase 9 — Characteristic {args.char_seq} ({short}) — structural input")
    S.append("")
    S.append(f"**Date:** 2026-05-18")
    S.append("")
    S.append("---")
    S.append("")
    ...
    S.append(f"| 2 | **This document** — `Sessions/Session_Clusters/M04/{INPUT.name}` | Characteristic definition + sub-groups + VCGs + verses + 189 prompts |")

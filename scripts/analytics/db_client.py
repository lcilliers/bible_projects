...
Provides:
  ...
  - init_schema_from_file()   — run Workflow/schema/create_tables.sql
...
Configuration (read from .env):
  DB_PATH   — path to the .db file, relative to project root
              default: database/bible_research.db
...
# db_client.py lives at scripts/analytics/db_client.py; project root is two levels up.
ROOT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
load_dotenv(os.path.join(ROOT_DIR, ".env"))

_DEFAULT_SCHEMA = os.path.join(ROOT_DIR, "Workflow", "schema", "create_tables.sql")
...
def get_connection(db_path: Optional[str] = None) -> sqlite3.Connection:
    ...
    db_path : str, optional
        ...
        variable, then to ``database/bible_research.db`` relative to the project root.
    ...
    """
    if db_path is None:
        db_path = os.getenv("DB_PATH", os.path.join(ROOT_DIR, "database", "bible_research.db"))
...
def init_schema_from_file(
    ...
    schema_path : str, optional
        ...
        ``Workflow/schema/create_tables.sql`` relative to the project root.
    """

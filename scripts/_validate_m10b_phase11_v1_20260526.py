"""M10b Phase 11 validation — covers the §15.2 Phase 12 pre-flight checks too.

Outputs a markdown report at
  Sessions/Session_Clusters/M10b/wa-cluster-M10b-phase11-validation-v1-20260522.md
...
CLUSTER = 'M10b'
OUT = Path(f'Sessions/Session_Clusters/M10b/wa-cluster-M10b-phase11-validation-v1-20260526.md')

VERSE_REF_RE = re.compile(
    r'\b(?:[1-3] ?)?[A-Z][a-z]{1,3}\.?\s?\d+[:.]\d+(?:-\d+)?\b'
)
VCG_REF_RE = re.compile(r'\bVCG-\d+', re.IGNORECASE)
SUBGROUP_REF_RE = re.compile(r'\bM10b-[A-Z]\b')
...
# C1: VC-coverage gaps - every wa_verse_term_link for an M10b OWNER copy has a matching verse_context.
...
# R4 anchor check - every OWNER M10b term has at least one is_relevant verse (proxy for anchor)
...
# BOUNDARY_DECISION_PENDING — narrow to M10b's own Strong's set (registry-level
# query picks up false-positive flags from other clusters' closures on shared
# registries, e.g. M03 flags on registry 151 sorrow that don't belong to M10b)
...
lines.append(f"# M10b Phase 11 — Validation report")
lines.append(f"")
...
lines.append("")
lines.append(f"Rows with `cluster_subgroup_id` or `vcg_scope` populated under M10b: **{legacy_marker_rows}**")
lines.append("")
...
lines.append(check_line('BOUNDARY_DECISION_PENDING flags (unresolved on M10b Strong\'s)', boundary_pending,
                       note='(M10b had 0 BOUNDARY verdicts at Phase 3; shared-registry flags from M03 closure are not M10b\'s responsibility — see Phase 8.5 obslog)'))
lines.append(check_line(f'{CLUSTER}-BOUNDARY sub-group active verses (is_relevant=1)', boundary_active_verses))
lines.append("")
lines.append("---")
lines.append("")

# Section 8 — Phase 10 fold operation (no-op for M10b)
lines.append("## 8. Phase 10 fold operation status")
lines.append("")
lines.append("M10b is a fresh post-split cluster (created 2026-05-22 from the M10 split); no Phase 10 reconciliation was run because no inherited Session B findings (`wa_session_b_findings`) target M10b's terms. The fold operation under §14.5 has no inherited findings to fold.")
lines.append("")
...
if not fails:
    lines.append("**M10b is ready for Phase 12 closure.**")
else:
    lines.append("**M10b NOT yet ready for Phase 12.** Address the failed checks first.")
lines.append("")

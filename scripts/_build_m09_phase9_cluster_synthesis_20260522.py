"""Build the M09 Phase 9 cluster-synthesis (8th) session package.
...
Outputs:
  Sessions/Session_Clusters/M09/wa-cluster-M09-phase9-cluster-synthesis-brief-v1-{date}.md
  Sessions/Session_Clusters/M09/wa-cluster-M09-phase9-cluster-synthesis-input-v1-{date}.md
"""
...
SCIENCE = "Workflow/Sciences/wa-m09-humility-scienceextract-v1_0-20260513.md"

BRIEF = Path(f"Sessions/Session_Clusters/M09/wa-cluster-M09-phase9-cluster-synthesis-brief-v1-{TODAY}.md")
INPUT = Path(f"Sessions/Session_Clusters/M09/wa-cluster-M09-phase9-cluster-synthesis-input-v1-{TODAY}.md")
...
def main():
    ...
    # Sanity: every q_code should have 5 entries (one per M09 characteristic)
    ...
    B.append(f"| 2 | **Structural input** — `Sessions/Session_Clusters/M09/{INPUT.name}` | Per-prompt synthesis matrix: for each of the 189 prompts, the 6 characteristics' findings stacked side-by-side (verbatim from `cluster_finding`); plus confirmed observations |")
    ...
    B.append("")
    B.append(f"For the M09 cluster as a whole, two output forms are required (per researcher direction):")
    B.append("")
    ...
    B.append("")
    B.append("After the 189-row block, write a free-form prose appendix capturing the emergent themes that surface across the five characteristics — themes that the 189-prompt structure does not fully capture. Suggested themes (drawn from the M09 carry-forward observations + analytical hints surfaced through Phase 3–8.5):")
    B.append("")
    B.append("- The **CHAR-1 volume-split** across M09-A (unjust wound) / M09-B (moral consequence) / M09-C (divine encounter) — what coherent characteristic identity holds the three registers together, and where do they genuinely diverge?")
    B.append("- The **M06 ↔ M09 contempt-pride relational pair** — CHAR-5 holds the pride-recipient face; M06 holds the contempt-projection source. The cross-cluster dynamic in detail.")
    B.append("- The **M12 ↔ M09 innocence-purity polarity** — CHAR-6's innocence-as-shield-against-pride; the inner moral state that protects against guilt-pride.")
    B.append("- **OT vs NT vocabulary arcs** across the cluster — where does each characteristic's vocabulary tradition concentrate?")
    B.append("- **Vertical vs horizontal pride** — pride before God (M09-C) vs pride between humans (much of M09-A, M09-B, M09-D, M09-E, M09-G); what shifts across the registers?")
    ...
    B.append("**Scope:** Cluster M09 (Humility, Meekness and Submission) — across 6 characteristics")
    ...
    B.append("")
    B.append(f"1. Save the output as `Sessions/Session_Clusters/M09/wa-cluster-M09-phase9-cluster-synthesis-findings-v1-{TODAY}.md`.")
    B.append(f"2. If segmenting by tier-pair: filename pattern `wa-cluster-M09-phase9-cluster-synthesis-findings-seg{{N}}-T#T#-v1-{TODAY}.md` in a sub-folder.")
    B.append("3. Ping CC: \"M09 cluster synthesis Phase 9 findings ready\".")
    ...
    S.append("")
    S.append("M09 (Humility, Meekness and Submission) holds 6 inner-being characteristics:")
    S.append("")

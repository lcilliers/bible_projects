"""Build the M04 Phase 9 cluster-synthesis (8th) session package.

Per researcher direction:
- Source structural input: per-prompt synthesis matrix - for each of the
  189 prompts (T0.1.1...T7.3.4), the 7 characteristics' findings are
...
Outputs:
  Sessions/Session_Clusters/M04/WA-M04-phase9-cluster-synthesis-brief-v1-{date}.md
  Sessions/Session_Clusters/M04/WA-M04-phase9-cluster-synthesis-input-v1-{date}.md
"""
...
SCIENCE = "Workflow/Sciences/wa-m04-joy-scienceextract-v1_0-20260513.md"

BRIEF = Path(f"Sessions/Session_Clusters/M04/WA-M04-phase9-cluster-synthesis-brief-v1-{TODAY}.md")
INPUT = Path(f"Sessions/Session_Clusters/M04/WA-M04-phase9-cluster-synthesis-input-v1-{TODAY}.md")
...
def main():
    ...
    # Pull all cluster_finding rows for the 7 characteristics, joined to the
    ...
    # Sanity: every q_code should have 7 entries (one per characteristic)
    bad = {qc: len(lst) for qc, lst in by_qcode.items() if len(lst) != 7}
    if bad:
        print(f"WARNING: {len(bad)} prompts do not have 7 findings. Sample: {dict(list(bad.items())[:3])}")
    ...
    B.append("")
    B.append("This is the final Phase 9 session for M04. The seven characteristic-scope batches (Exultation, Joy, Gladness, Delight, Pleasure, Wonder, Suffering-Joy) are complete; each authored 189 findings citing specific verses. This session looks ACROSS those seven sets to author the cluster-scope findings — patterns, divergences, and shared anchors that emerge only when the seven are compared.")
    B.append("")
    ...
    B.append(f"| 2 | **Structural input** — `Sessions/Session_Clusters/M04/{INPUT.name}` | Per-prompt synthesis matrix: for each of the 189 prompts, the 7 characteristics' findings stacked side-by-side (verbatim from `cluster_finding`); plus confirmed observations |")
    ...
    B.append("")
    B.append(f"For the M04 cluster as a whole, two output forms are required (per researcher direction):")
    B.append("")
    B.append("### Output A — 189 cluster-scope findings (parser-safe rows)")
    B.append("")
    B.append("For each of the 189 catalogue prompts, author ONE cluster-scope finding examining what surfaces when the 7 characteristics' findings are compared. The cluster-scope finding answers: *what does the M04 cluster — across its 7 characteristics — reveal about this question?* It is the comparative-integrative layer above the per-characteristic answers.")
    B.append("")
    ...
    B.append("- **E** — evidenced; the 7 characteristics' findings provide enough comparative evidence to author a cluster-scope answer")
    ...
    B.append("- **G** — gap; the cluster-level pattern would require evidence the 7 characteristics' findings don't supply")
    B.append("")
    B.append("### Output B — Synthesis prose appendix (free-form)")
    B.append("")
    B.append("After the 189-row block, write a free-form prose appendix capturing the emergent themes that surface across the seven characteristics — themes that the 189-prompt structure does not fully capture. Examples of themes the per-characteristic self-checks already flagged:")
    B.append("")
    B.append("- The **Joy/Gladness boundary** within M04-B and M04-C (confirmed in Char 2's run as one of the most significant analytical findings of M04 Phase 9)")
    B.append("- The **SPLIT_SUBGROUP** dynamic in M04-E — eschatological Joy (sa.s.von) vs Suffering-Joy paradox (agalliao)")
    B.append("- **OT vs NT vocabulary arcs** across the cluster — where does each characteristic's vocabulary tradition concentrate?")
    B.append("- **Spirit-constituted vs cultivated characteristics** — which require divine enabling (Suffering-Joy's explicit Spirit-location) vs which are constitutive human faculty (Pleasure's embodied modality)?")
    B.append("- **Christological convergence points** — Isa 9:6 Wonderful Counsellor; Psa 118:23 rejected stone; Isa 53:10 cha.phets atonement-Delight; Heb 12:2 \"joy set before him\"")
    ...
    B.append("")
    B.append("Suggested length: 2,000–4,000 words for the appendix. Use sub-headings per theme. Cite the per-characteristic findings (e.g. \"Char 2 T6.1.1's finding that Gladness co-occurs with Joy in 8+ formulaic pairings\") rather than re-citing verses already cited in the structural input.")
    B.append("")
    B.append("---")
    B.append("")
    B.append("## Discipline")
    B.append("")
    B.append("1. **Per-prompt comparative reading.** For each T#.#.# block in §3 of the structural input, read the 7 characteristics' findings stacked together before authoring the cluster-scope row. The test for a good cluster-scope answer is: *what does seeing the 7 side-by-side reveal that no single characteristic's answer reveals?*")
    B.append("2. **Cite by characteristic.** The cluster-scope finding names which characteristics contribute what — e.g., \"Joy and Gladness pair formulaically (Char 2 T6.1.1; Char 3 T6.1.1); Delight pairs more often with Wonder than with Joy (Char 4 T6.1.1; Char 6 T6.1.1).\"")
    ...
    B.append("**Scope:** Cluster M04 (Joy, Gladness and Delight) — across 7 characteristics")
    ...
    B.append("")
    B.append(f"1. Save the output as `Sessions/Session_Clusters/M04/WA-M04-phase9-cluster-synthesis-findings-v1-{TODAY}.md`.")
    B.append(f"2. If segmenting by tier-pair: filename pattern `WA-M04-phase9-cluster-synthesis-findings-seg{{N}}-T#T#-v1-{TODAY}.md` in a sub-folder.")
    B.append("3. Ping CC: \"M04 cluster synthesis Phase 9 findings ready\".")
    ...
    S.append("")
    S.append("M04 (Joy, Gladness and Delight) holds 7 inner-being characteristics:")
    S.append("")
    ...
    S.append("")
    S.append("For each of the 189 prompts (T0.1.1 … T7.3.4), the 7 characteristics' findings are stacked verbatim below. Read all 7 before authoring the cluster-scope row in your output.")
    S.append("")

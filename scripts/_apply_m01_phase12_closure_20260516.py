"""Apply Phase 12 — M01 cluster closure.

Directive: wa-cluster-M01-dir-007-closure-v1-20260516
...
Pre-flight checks:
  ...
  P1 — cluster_finding rows exist for M01 v1-20260516 (Phase 11 applied)
...
DIRECTIVE = "DIR-20260516-007"
CLUSTER = "M01"
VERSION = "v1-20260516"
BOUNDARY_SG_ID = 65  # M01-BOUNDARY (from earlier inspection)

# Default characterisation source (where researcher can find each term's Phase 9 notes)
PART4_FILE = "WA-M01-consolidated-findings-v1-20260516-part4-T5-T7.md"
...
def apply(conn, dry_run: bool, boundary_terms: list[dict]) -> dict:
    ...
    for b in boundary_terms:
        flag_label = f"M01-BOUNDARY-{b['strongs_number']}"
        description = (
            f"M01 closure (DIR-20260516-007): BOUNDARY term {b['strongs_number']} "
            ...
            f"promote-to-M01-subgroup / reassign-to-other-cluster / retain-BOUNDARY-with-extended-rationale. "
...
def healthcheck(conn) -> None:
    ...
    r = conn.execute("""
        ...
        WHERE flag_code='BOUNDARY_DECISION_PENDING' AND resolved=0
          AND description LIKE '%M01 closure (DIR-20260516-007)%'
    """).fetchone()
    ...
    r = conn.execute("""
        ...
        WHERE cluster_code='M01' AND status IN ('extracted','extracted_thin')
    ...
    r = conn.execute("""
        ...
        WHERE mt.cluster_code='M01' AND COALESCE(vcg.delete_flagged,0)=0
          AND COALESCE(vt.delete_flagged,0)=0
    """).fetchone()
    log(f"  Active VCGs: {r['n_vcgs']}")
    r = conn.execute("""
        SELECT COUNT(*) AS n FROM cluster_finding
        WHERE cluster_code='M01' AND version='v1-20260516'
    """).fetchone()

...
print("\nRemaining active terms for soul:")
terms = conn.execute(
    "SELECT strongs_number, step_search_gloss, occurrence_count FROM wa_term_inventory WHERE file_id=? AND delete_flagged=0 ORDER BY strongs_number",
    (FILE_ID,)
).fetchall()
for t in terms:
    print(f"  {t['strongs_number']:<12}  {t['step_search_gloss']:<30}  occur={t['occurrence_count']}")
conn.close()

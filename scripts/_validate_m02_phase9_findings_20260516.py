...
PHASE9_DIR = M02  # files already at cluster root
CATALOGUE = REPO / "Workflow" / "Tiers" / "wa-obs-catalogue-tiered-v2_1-20260513.md"

PARTS = [
    ("part1", PHASE9_DIR / "WA-M02-consolidated-findings-v1-20260516-part1.md", ["T0", "T1"]),
    ("part2", PHASE9_DIR / "WA-M02-consolidated-findings-v1-20260516-part2-T2.md", ["T2"]),
    ("part3", PHASE9_DIR / "WA-M02-consolidated-findings-v1-20260516-part3-T3-T4.md", ["T3", "T4"]),
    ("part4", PHASE9_DIR / "WA-M02-consolidated-findings-v1-20260516-part4-T5-T7.md", ["T5", "T6", "T7"]),
]
...
VCG_CODE_RE = re.compile(r"\bM02-[A-Z]+-VCG-\d+\b|\b[A-G]-VCG-\d+\b")
...
def parse_scope(raw: str) -> list[str]:
    ...
    or v2_2 VCG-level markers like '[E-VCG-02]' / '[E-VCG-02/03/04/05]'."""
    s = raw.strip()
    ...
    for p in parts:
        ...
        if re.fullmatch(r"[A-G]", p):
            ...
            # v2_2 VCG-level (single or slash-multi) — recognize as letter scope
            out.append(p[0])  # the letter
        else:
...
def render_report(catalogue_codes: set[str], part_results: list[dict]) -> str:
    lines: list[str] = []
    lines.append("# M02 Phase 9 consolidated findings — validation report")
    lines.append("")
...
def main():
    ...
    out_path = M02 / "WA-M02-phase9-findings-validation-v1-20260516.md"

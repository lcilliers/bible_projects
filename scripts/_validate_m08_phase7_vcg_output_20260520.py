...
- M08-C VCGs reflect M22 cross-register flag (informational check)
"""
...
JSON_PATH = Path('Sessions/Session_Clusters/M08/files phase 7/WA-M08-vcg-creation-v1-20260520.json')
CLUSTER = 'M08'
...
print(f'DB M08 is_relevant vc count: {len(db_vcs)} (expected 296)')
print()
...
# Informational: check M08-C VCGs for M22 flag indication
print()
print('=== Cross-register flag check (informational) ===')
m08c_descs = [s['description'].lower() for s in all_vcg_specs if s['subgroup_code'] == 'M08-C']
has_m22 = any('m22' in d or 'god-directed' in d or 'praise' in d or 'glorying' in d for d in m08c_descs)
print(f'  M08-C VCGs: {len(m08c_descs)}; M22/God-directed/glorying mention present: {has_m22}')
m08e_descs = [s['description'].lower() for s in all_vcg_specs if s['subgroup_code'] == 'M08-E']
has_m23 = any('m23' in d or 'strength' in d or 'power' in d or 'authority' in d or 'dominion' in d for d in m08e_descs)
print(f'  M08-E VCGs: {len(m08e_descs)}; M23/strength/power/authority mention present: {has_m23}')

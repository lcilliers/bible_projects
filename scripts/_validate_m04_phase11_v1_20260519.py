"""M04 Phase 11 validation — covers the §15.2 Phase 12 pre-flight checks too.

Outputs a markdown report at
  Sessions/Session_Clusters/M04/WA-M04-phase11-validation-v1-20260519.md

Per v2_6 §14 + §15.2:
  1. Row counts (1,512 expected: 7 chars × 189 + 189 synthesis)
...
CLUSTER = 'M04'
OUT = Path(f'Sessions/Session_Clusters/M04/WA-M04-phase11-validation-v1-20260519.md')

VERSE_REF_RE = re.compile(
    r'\b(?:[1-3] ?)?[A-Z][a-z]{1,3}\.?\s?\d+[:.]\d+(?:-\d+)?\b'
)
VCG_REF_RE = re.compile(r'\bVCG-\d+', re.IGNORECASE)
SUBGROUP_REF_RE = re.compile(r'\bM04-[A-Z]\b')
...
# C1: VC-coverage gaps - every wa_verse_term_link for an M04 OWNER copy has a matching verse_context.
...
# R4 anchor check - every OWNER M04 term has at least one is_relevant verse (proxy for anchor)
...
lines.append(f"# M04 Phase 11 — Validation report")
lines.append(f"")
...
lines.append("")
lines.append(f"Rows with `cluster_subgroup_id` or `vcg_scope` populated under M04: **{legacy_marker_rows}**")
lines.append("")
...
lines.append(check_line('BOUNDARY_DECISION_PENDING flags (unresolved)', boundary_pending,
                       note='(Note: 9 unresolved flags exist on M04-contributing registries but cite M01/M03 closure — out of M04 scope per Phase 10 closure record §6.2)'))
lines.append(check_line(f'{CLUSTER}-BOUNDARY sub-group active verses (is_relevant=1)', boundary_active_verses))
lines.append("")
lines.append("---")
lines.append("")

# Section 8 — Phase 10 fold operation (no-op for M04)
lines.append("## 8. Phase 10 fold operation status")
lines.append("")
lines.append("Per Phase 10 closure record (`WA-M04-phase10-closure-record-v1-20260519.md`), zero `RESOLVED-BY-CATALOGUE` dispositions were assigned. The fold operation under §14.5 has no inherited findings to fold.")
lines.append("")
...
if not fails:
    lines.append("**M04 is ready for Phase 12 closure.**")
else:
    lines.append("**M04 NOT yet ready for Phase 12.** Address the failed checks first.")
lines.append("")

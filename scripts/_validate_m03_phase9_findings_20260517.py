"""Validate M03 Phase 9 consolidated findings against catalogue + parser-safe form.

Adapted from _validate_m02_phase9_findings_20260516.py.

Checks:
  1. Every catalogue prompt (189 in v2.1) appears in one of the 4 parts.
  2. Every prompt has ≥1 scope marker (sub-group, VCG-level, BOUNDARY, or CLUSTER).
  3. Scope markers are on their own line and use canonical form.
...
Output: a markdown validation report at
  Sessions/Session_Clusters/M03/WA-M03-phase9-findings-validation-v1-20260517.md
"""
...
PHASE9_DIR = M03 / "files phase 9"
CATALOGUE = REPO / "Workflow" / "Tiers" / "wa-obs-catalogue-tiered-v2_1-20260513.md"

PARTS = [
    ("part1", PHASE9_DIR / "WA-M03-consolidated-findings-v1-20260517-part1.md", ["T0", "T1"]),
    ("part2", PHASE9_DIR / "WA-M03-consolidated-findings-v1-20260517-part2-T2.md", ["T2"]),
    ("part3", PHASE9_DIR / "WA-M03-consolidated-findings-v1-20260517-part3-T3-T4.md", ["T3", "T4"]),
    ("part4", PHASE9_DIR / "WA-M03-consolidated-findings-v1-20260517-part4-T5-T7.md", ["T5", "T6", "T7"]),
]

# M03 AI used inline bold form `**T0.1.1**` (not `### T0.1.1`).
# Match either heading-style or bold-inline form at line start.
...
# Match both full form (M03-A-VCG-01) and short form (A-VCG-01)
VCG_CODE_RE = re.compile(r"\bM03-[A-Z]+-VCG-\d+\b|\b[A-G]-VCG-\d+\b")


def parse_catalogue() -> set[str]:
    text = CATALOGUE.read_text(encoding="utf-8")
    return set(PROMPT_CODE_IN_CATALOGUE.findall(text))


def parse_scope(raw: str) -> list[str]:
    """Parse '[A]' / '[A, B, C]' / '[CLUSTER]' / '[BOUNDARY - H1234 translit]' /
    '[A-VCG-02]' / '[A-VCG-02/03]'."""
    s = raw.strip()
    ...
    for p in parts:
        if re.fullmatch(r"[A-G]", p):
            ...
            out.append(p)
        elif re.fullmatch(r"[A-G]-VCG-\d+(?:/\d+)*", p):
            out.append(p[0])  # the sub-group letter
        elif re.fullmatch(r"M03-[A-Z]+-VCG-\d+(?:/\d+)*", p):
            # full form
...
def parse_part(path: Path) -> dict:
    ...
    # M03 AI uses `**T0.1.1** — text` form (bold inline at line start, not headings).
    # Match either form: `### T0.1.1 ...` or `**T0.1.1** ...`.
    prompt_matches = list(re.finditer(
        r"^(?:### |\*\*)(T\d+\.\d+\.\d+)(?:\*\*)?[ —\-].*$",
        text, re.MULTILINE
    ))
    prompts: list[dict] = []
    for i, m in enumerate(prompt_matches):
        ...
        for j, sm in enumerate(scope_lines):
            ...
            block_start = sm.end()
            block_end = (scope_lines[j+1].start() if j+1 < len(scope_lines)
                         else (end_sep.start() if end_sep else len(body)))
...
def render_report(catalogue_codes: set[str], part_results: list[dict]) -> str:
    lines: list[str] = []
    lines.append("# M03 Phase 9 consolidated findings — validation report")
    lines.append("")
    ...
    e_no_citation: list[tuple[str,str,str]] = []
    ...
    if not e_no_citation:
        ...
        lines.append(f"Total: **{len(e_no_citation)}** E entries lack a parseable verse reference (`Book C:V`) or VCG code (e.g. `M03-A-VCG-01`).")
        lines.append("")
...
def main():
    ...
    out_path = M03 / "WA-M03-phase9-findings-validation-v1-20260517.md"

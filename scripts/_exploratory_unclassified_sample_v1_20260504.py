"""_exploratory_unclassified_sample_v1_20260504.py — read-only.

Picks N unclassified (verse_record, mti_term) pairs from the current DB and
builds the verse-context-records JSON ready for the meaning-router.

Selection criteria:
  - wa_verse_records active (delete_flagged=0)
  - wa_term_inventory active, term_owner_type='OWNER'
  - mti_terms active, status IN ('extracted','extracted_thin')
  - Strong's POS is content (noun / verb / adjective per the POS lookup)
  - No verse_context row exists for this (verse_record_id, mti_term_id)
  - Stratified across registries to give variety

Output:
  - outputs/markdown/unclassified-sample-{N}-pairs-{date}.json
    (matches the verse-context-records JSON schema; ready for the router)
"""
...
def get_term_meta(conn, strong: str) -> dict | None:
    r = conn.execute("""
...
def get_term_extract(conn, strong: str) -> dict | None:
...
def decode_morph(code: str, language: str) -> dict:
    """Lightweight morph decode — POS only."""
    body = code.split(":", 1)[-1]
    if language == "Hebrew":
        parts = body.split("/")
...
def select_unclassified_pairs(conn, n: int, pos_lookup: dict) -> list[tuple[int, int]]:
    """Returns n stratified (verse_record_id, mti_term_id) pairs."""
    content_strongs = [s for s, p in pos_lookup.items() if p in CONTENT_POS]
    quoted = ",".join(["'" + s + "'" for s in content_strongs])
    rows = conn.execute(f"""
    ...
    for r in rows:
        by_reg[r["reg_no"]].append((r["vr_id"], r["mti_id"]))

    rng = random.Random(20260504)
    for k in by_reg:
        rng.shuffle(by_reg[k])

    target_per_reg = max(1, n // len(by_reg))
    selected = []
    reg_keys = sorted(by_reg.keys())
    rng.shuffle(reg_keys)
    ...
    while len(selected) < n:
        ...
        for k in reg_keys:
            if not by_reg[k]:
                continue
            if sum(1 for s in selected if s[2] == k) >= target_per_reg + 5:
                continue
            vr_id, mti_id = by_reg[k].pop()
            selected.append((vr_id, mti_id, k))
            progress = True
            if len(selected) >= n:
                break
        if not progress:
    ...
    return [(s[0], s[1]) for s in selected[:n]]


def build_record(conn, vr_id: int, mti_id: int,
    ...
    # Cache verse spans by reference (multiple terms can share a verse)
    if ref not in cache:
        all_spans = []
...
def main() -> int:
    ...
    out_path = args.out or os.path.join(
        "outputs", "markdown",
        f"unclassified-sample-{args.n}-pairs-{today_compact()}.json"
    )
    ...
    print(f"Selecting {args.n} unclassified (verse, term) pairs...")
    pairs = select_unclassified_pairs(conn, args.n, pos_lookup)
    ...
    payload = {
        "meta": {
            ...
            "selection_criteria": "unclassified content-POS pairs, stratified across registries",

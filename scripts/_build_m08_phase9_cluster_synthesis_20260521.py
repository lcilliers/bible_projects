"""Build the M08 Phase 9 cluster-synthesis (8th) session package.
...
Outputs:
  Sessions/Session_Clusters/M08/wa-cluster-M08-phase9-cluster-synthesis-brief-v1-{date}.md
  Sessions/Session_Clusters/M08/wa-cluster-M08-phase9-cluster-synthesis-input-v1-{date}.md
"""
...
SCIENCE = "Workflow/Sciences/wa-m08-pride-scienceextract-v1_0-20260513.md"

BRIEF = Path(f"Sessions/Session_Clusters/M08/wa-cluster-M08-phase9-cluster-synthesis-brief-v1-{TODAY}.md")
INPUT = Path(f"Sessions/Session_Clusters/M08/wa-cluster-M08-phase9-cluster-synthesis-input-v1-{TODAY}.md")
...
def main():
    ...
    B.append(f"| 2 | **Structural input** — `Sessions/Session_Clusters/M08/{INPUT.name}` | Per-prompt synthesis matrix: for each of the 189 prompts, the 6 characteristics' findings stacked side-by-side (verbatim from `cluster_finding`); plus confirmed observations |")
    ...
    B.append("")
    B.append(f"For the M08 cluster as a whole, two output forms are required (per researcher direction):")
    B.append("")
    ...
    B.append("")
    B.append("After the 189-row block, write a free-form prose appendix capturing the emergent themes that surface across the six characteristics — themes that the 189-prompt structure does not fully capture. Suggested themes (drawn from the M08 carry-forward observations + analytical hints surfaced through Phase 3–8.5):")
    B.append("")
    B.append("- The **CHAR-1 volume-split** across M08-A (unjust wound) / M08-B (moral consequence) / M08-C (divine encounter) — what coherent characteristic identity holds the three registers together, and where do they genuinely diverge?")
    B.append("- The **M06 ↔ M08 contempt-pride relational pair** — CHAR-5 holds the pride-recipient face; M06 holds the contempt-projection source. The cross-cluster dynamic in detail.")
    B.append("- The **M12 ↔ M08 innocence-purity polarity** — CHAR-6's innocence-as-shield-against-pride; the inner moral state that protects against guilt-pride.")
    B.append("- **OT vs NT vocabulary arcs** across the cluster — where does each characteristic's vocabulary tradition concentrate?")
    B.append("- **Vertical vs horizontal pride** — pride before God (M08-C) vs pride between humans (much of M08-A, M08-B, M08-D, M08-E, M08-G); what shifts across the registers?")
    ...
    B.append("2. **Cite by characteristic.** The cluster-scope finding names which characteristics contribute what — e.g., \"Pride-as-experienced (Char 1) and Humiliation-as-enforced (Char 2) co-occur in many verses but with different agencies — Char 1 names the inner state; Char 2 names the enforcing mechanism.\"")
    ...
    B.append("**Scope:** Cluster M08 (Pride, Arrogance and Boasting) — across 6 characteristics")
    ...
    B.append("")
    B.append(f"1. Save the output as `Sessions/Session_Clusters/M08/wa-cluster-M08-phase9-cluster-synthesis-findings-v1-{TODAY}.md`.")
    B.append(f"2. If segmenting by tier-pair: filename pattern `wa-cluster-M08-phase9-cluster-synthesis-findings-seg{{N}}-T#T#-v1-{TODAY}.md` in a sub-folder.")
    B.append("3. Ping CC: \"M08 cluster synthesis Phase 9 findings ready\".")
    ...
    S.append("")
    S.append("M08 (Pride, Arrogance and Boasting) holds 5 inner-being characteristics:")
    S.append("")
